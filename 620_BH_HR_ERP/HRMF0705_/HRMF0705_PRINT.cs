...
namespace HRMF0705
{
    public partial class HRMF0705_PRINT : Office2007Form
    {
        ...
        private void HRMF0705_PRINT_Load(object sender, EventArgs e)
        {
            ...
            iedPRINT_PAGE.EditValue = 0; //프린트할 페이지
            ...
            iedPRINT_COUNT.EditValue = mPrint_Count;       //매수  
            ...
            if (Convert.ToInt32(iedCERT_TYPE_ID.EditValue) == 4299)
            {
        ...
        private void XLPrinting1()
        {
            ...
            int vCountRow = gridWITHHOLDING_TAX.RowCount; //gridWITHHOLDING_TAX 그리드의 총 행수
            if (vCountRow < 1)
            {
            ...
            try
            {
                ...
                decimal vPrintPage = iedPRINT_PAGE.NumberValue;
                ...
                if (EARNER_YN.CheckBoxString == "Y")
                {
                    ...
                    {
                        vPrintType = "소득자 보관용";
                        vPageNumber = xlPrinting.WriteMain(gridWITHHOLDING_TAX, gridSUPPORT_FAMILY, ida_PRINT_SAVING_INFO_2, ida_PRINT_SAVING_INFO_3, ida_PRINT_SAVING_INFO_4, ida_PRINT_SAVING_INFO_5, vPrintDate, vPrintType, mOutChoice, vPrintPage);
                        vPageNumberSUM = vPageNumberSUM + vPageNumber;
                        vPageNumber = 0;
                    }
                }
            ...
            vMessageText = string.Format("Printing End [Total : {0}] ---> {1}", vPageNumberSUM, vTimeSpan.ToString());
            isAppInterfaceAdv1.AppInterface.OnAppMessageEvent(vMessageText);
            System.Windows.Forms.Application.DoEvents();
        ...
        private void XLPrinting2()
        {
            ...
            try
            {
                ...
                //-------------------------------------------------------------------------------------
                xlPrinting.OpenFileNameExcel = "HRMF0705_002.xls";
                //-------------------------------------------------------------------------------------
        ...
        private void XLPrinting3()
        {
            ...
            try
            {
                ...
                //-------------------------------------------------------------------------------------
                xlPrinting.OpenFileNameExcel = "HRMF0705_003.xls";
                //-------------------------------------------------------------------------------------
        ...
        private void btnPRINT_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            else if (Convert.ToInt32(iedCERT_TYPE_CODE.EditValue) == 22) // 근로소득영수증
            {
                ...
                {
                    MessageBox.Show(isMessageAdapter1.ReturnText("출력 용도를 선택해주세요."), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    EARNER_YN.Focus();
                    return;
                }
            ...
            else if (Convert.ToInt32(iedCERT_TYPE_CODE.EditValue) == 20) //갑종근로소득에대한소득세원천징수증명서
            {
                if (iedPERSON_ID.EditValue == null)
                {// 성명 입력
                    MessageBoxAdv.Show(isMessageAdapter1.ReturnText("FCM_10037", "&&VALUE:=1인 출력만 가능하므로 '성명'"), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        {
            if (iedCERT_TYPE_CODE.EditValue.ToString() == "1".ToString())
            {
