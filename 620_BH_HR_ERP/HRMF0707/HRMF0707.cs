...
namespace HRMF0707
{
    public partial class HRMF0707 : Office2007Form
    {
        ...
        private string EXPORT_VALIDATE()
        {
            ...
            if (iConv.ISNull(VAT_NUMBER.EditValue) == string.Empty)
            {
                MessageBoxAdv.Show(string.Format("{0}은(는)은 필수입니다. 확인하세요", Get_Edit_Prompt(VAT_NUMBER)), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return vRETURN;
            }
            if (iConv.ISNull(TAX_OFFICE_CODE.EditValue) == string.Empty)
            {
                MessageBoxAdv.Show(string.Format("{0}은(는)은 필수입니다. 확인하세요", Get_Edit_Prompt(TAX_OFFICE_CODE)), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return vRETURN;
            }
        ...
        private void ExportTXT(String pFILE_TYPE, ISDataAdapter pData)
        {
            ...
            vFileName = String.Format("{0}{1}", vFIX_STRING, iConv.ISNull(VAT_NUMBER.EditValue).Replace("-", "").Substring(0, 7));
            saveFileDialog1.Title = "Save File";
            saveFileDialog1.FileName = vFileName;
            saveFileDialog1.DefaultExt = String.Format(".{0}", iConv.ISNull(VAT_NUMBER.EditValue).Replace("-", "").Substring(7, 3));
            //System.IO.DirectoryInfo vSaveFolder = new System.IO.DirectoryInfo(System.Environment.GetFolderPath(Environment.SpecialFolder.Desktop));
            System.IO.DirectoryInfo vSaveFolder = new System.IO.DirectoryInfo(vFilePath);
            saveFileDialog1.InitialDirectory = vSaveFolder.FullName;
            saveFileDialog1.Filter = String.Format("Text Files (*.{0})|*.{0}", iConv.ISNull(VAT_NUMBER.EditValue).Replace("-", "").Substring(7, 3));
        ...
        private void ExportTXT_BAK(String pFILE_TYPE, ISDataAdapter pData)
        {
            object vFIX_STRING = null;
            int vCountRow = pData.OraSelectData.Rows.Count;
            if (vCountRow < 1)
            ...
            saveFileDialog1.Title = "Save File";
            saveFileDialog1.FileName = String.Format("{0}{1}", vFIX_STRING, iConv.ISNull(VAT_NUMBER.EditValue).Replace("-", "").Substring(0, 7));
            saveFileDialog1.DefaultExt = String.Format(".{0}", iConv.ISNull(VAT_NUMBER.EditValue).Replace("-", "").Substring(7, 3));
            System.IO.DirectoryInfo vSaveFolder = new System.IO.DirectoryInfo(System.Environment.GetFolderPath(Environment.SpecialFolder.Desktop));
            saveFileDialog1.InitialDirectory = vSaveFolder.FullName;
            saveFileDialog1.Filter = String.Format("Text Files (*.{0})|*.{0}", iConv.ISNull(VAT_NUMBER.EditValue).Replace("-", "").Substring(7, 3));
        ...
        private void BTN_YEAR_ADJUST_FILE_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            ExportTXT("ADJUST", IDA_YEAR_ADJUST_FILE);
        }
        ...
        private void BTN_MEDIC_FILE_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            ExportTXT("MEDICAL", IDA_MEDICAL_FILE);
        }
        ...
        private void BTN_DONATION_FILE_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            ...
            ExportTXT("DONATION", IDA_DONATION_FILE);
        }
