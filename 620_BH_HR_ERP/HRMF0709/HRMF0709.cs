...
namespace HRMF0709
{
    public partial class HRMF0709 : Office2007Form
    {
        ...
        private void isAppInterfaceAdv1_AppMainButtonClick(ISAppButtonEvents e)
        {
            if (this.IsActive)
            {
                if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Search)
                {
                    ...
                    if (iString.ISNull(W_SUBMIT_DATE_FR.EditValue) == string.Empty)
                    {
                        MessageBoxAdv.Show(isMessageAdapter1.ReturnText("FCM_10010"), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        W_SUBMIT_DATE_FR.Focus();
                        return;
                    }
                    if (iString.ISNull(W_SUBMIT_DATE_TO.EditValue) == string.Empty)
                    {
                        MessageBoxAdv.Show(isMessageAdapter1.ReturnText("FCM_10011"), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        W_SUBMIT_DATE_TO.Focus();
                        return;
                    }
        ...
        private void HRMF0709_Load(object sender, EventArgs e)
        {
            ...
            W_SUBMIT_DATE_FR.EditValue = iDate.ISMonth_1st(string.Format("{0}-01", iDate.ISYear(DateTime.Today)));
            W_SUBMIT_DATE_TO.EditValue = iDate.ISMonth_Last(DateTime.Today);
        }
