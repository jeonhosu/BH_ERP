...
namespace HRMF0711
{
    public partial class HRMF0711 : Office2007Form
    {
        ...
        private void idaMEDICAL_INFO_PreRowUpdate(ISPreRowUpdateEventArgs e)
        {
            ...
            {
                MessageBoxAdv.Show("상호는 필수입니다. 확인하세요", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if (iString.ISNull(e.Row["CORP_TAX_REG_NO"]) == string.Empty)
            {
                MessageBoxAdv.Show("사업자번호는 필수입니다. 확인하세요", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if (iString.ISDecimaltoZero(e.Row["ETC_COUNT"],0) == 0)
            {
                MessageBoxAdv.Show("지급건수는 0보다 커야 합니다. 확인하세요", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if (iString.ISDecimaltoZero(e.Row["ETC_AMT"], 0) == 0)
            {
