...
namespace FCMF0884
{
    public class XLPrinting
    {
        ...
        private int mIncrementCopyMAX = 53;  //복사되어질 행의 범위
        ...
        private void SetArray1(InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid, out int[] pGDColumn, out int[] pXLColumn)
        {
            ...
            pGDColumn[0] = pGrid.GetColumnToIndex("PRINT_EXPORT_DATE"); //수출일자
            pGDColumn[1] = pGrid.GetColumnToIndex("PURCHASE_NO");       //매입번호
            pGDColumn[2] = pGrid.GetColumnToIndex("ITEM_NM");           //품명
            pGDColumn[3] = pGrid.GetColumnToIndex("SUPPLY_AMT");        //공급가액 - 수출실적 상위
            pGDColumn[4] = pGrid.GetColumnToIndex("REMARKS");           //비고
            pGDColumn[5] = pGrid.GetColumnToIndex("EXPORT_OUTPUT");     //수출실적 - 수출실적 하위


            pXLColumn[0] = 2;   //수출일자
            pXLColumn[1] = 9;   //매입번호
            pXLColumn[2] = 17;   //품명
            pXLColumn[3] = 31;  //공급가액 - 수출실적 상위
            pXLColumn[4] = 42;  //비고
            pXLColumn[5] = 31;  //수출실적 - 수출실적 하위
        }
        ...
        {
            int vPrintingLineEND = mCopyLineSUM - 13; //1~53: mCopyLineSUM=54에서 내용이 출력되는 행이 41 까지 이므로, 13을 빼면 된다
