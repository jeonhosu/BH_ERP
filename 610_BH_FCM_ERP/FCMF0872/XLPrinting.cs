...
namespace FCMF0872
{
    public class XLPrinting
    {
        ...
        private int mIncrementCopyMAX = 26;  //복사되어질 행의 범위
        ...
        private void SetArray1(InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid, out int[] pGDColumn, out int[] pXLColumn)
        {
            ...
            pGDColumn[0] = pGrid.GetColumnToIndex("ADDRESS"); //동
            pGDColumn[1] = pGrid.GetColumnToIndex("VAT_GROUND_YN"); //지상/지하 여부
            pGDColumn[2] = pGrid.GetColumnToIndex("PRINT_BLD_FLOOR"); //층
            pGDColumn[3] = pGrid.GetColumnToIndex("PRINT_ROOM"); //호
            pGDColumn[4] = pGrid.GetColumnToIndex("LEND_AREA"); //임대면적
            pGDColumn[5] = pGrid.GetColumnToIndex("CORP_NAME"); //상호
            pGDColumn[6] = pGrid.GetColumnToIndex("VAT_NUMBER"); //사업자등록번호
            pGDColumn[7] = pGrid.GetColumnToIndex("IN_DATE"); //입주일
            pGDColumn[8] = pGrid.GetColumnToIndex("MODIFY_DATE"); //갱신일
            pGDColumn[9] = pGrid.GetColumnToIndex("OUT_DATE"); //퇴거일
            pGDColumn[10] = pGrid.GetColumnToIndex("DEPOSIT"); //보증금
            pGDColumn[11] = pGrid.GetColumnToIndex("MM_FEE"); //월임대료
            pGDColumn[12] = pGrid.GetColumnToIndex("RENT_SUM"); //합계
            pGDColumn[13] = pGrid.GetColumnToIndex("DEEMED_RENT"); //보증금이자
            pGDColumn[14] = pGrid.GetColumnToIndex("TAX_MM_FEE"); //월임대료(계)


            pXLColumn[0] = 2; //동
            pXLColumn[1] = 10; //층(사용안함 --전호수).
            pXLColumn[2] = 11; //층
            pXLColumn[3] = 14; //호
            pXLColumn[4] = 16; //임대면적
            pXLColumn[5] = 18; //상호
            pXLColumn[6] = 23; //사업자등록번호
            pXLColumn[7] = 28; //입주일
            pXLColumn[8] = 32; //갱신일
            pXLColumn[9] = 36; //퇴거일
            pXLColumn[10] = 40; //보증금
            pXLColumn[11] = 45; //월임대료
            pXLColumn[12] = 49; //합계
            pXLColumn[13] = 54; //보증금이자
            pXLColumn[14] = 57; //월임대료(계)
        }
        ...
        private void XLHeader(InfoSummit.Win.ControlAdv.ISDataAdapter p_adapter_PRINT_BLD_AMT_SPEC)
        {
            ...
            try
            {
                ...
                if (vCountRow > 0)
                {
                    ...
                    if (IsConvert == true)
                    {
                        vConvertString = string.Format("(기간 : {0} )", vConvertString);
                        mPrinting.XLSetCell(vXLine, vXLColumn, vConvertString);
                    }
                    else
                    {
