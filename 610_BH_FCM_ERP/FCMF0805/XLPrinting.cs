...
namespace FCMF0805
{
    public class XLPrinting
    {
        ...
        private int mCopy_EndRow = 61;
        private int mPrintingLastRow = 54;  //최종 인쇄 라인.

        private int mCurrentRow = 30;       //현재 인쇄되는 row 위치.
        private int mDefaultPageRow = 11;    //페이지 skip후 적용되는 기본 PageCount 기본값.
        private int mCurrentRowAdd = 17;     //페이지 skip후 기본적으로 증가하는 현재 row 값.
        ...
        {// 헤더 인쇄.
            ...
            int vIDX_BUSINESS_TYPE = pGrid.GetColumnToIndex("BUSINESS_TYPE_S");
            ...
            { // 원본을 복사해서 타겟 에 복사해 넣음.(
                ...
                for (int i = 0; i < pGrid.RowCount; i++)
                {
                    ...
                    if ("P" == iString.ISNull(pGrid.GetCellValue(i, vIDX_BUSINESS_TYPE)))
                    {
                        vXLine = 23;
                    }
                    else if ("C" == iString.ISNull(pGrid.GetCellValue(i, vIDX_BUSINESS_TYPE)))
                    {
                        vXLine = 21;
                    }
                    else if ("T" == iString.ISNull(pGrid.GetCellValue(i, vIDX_BUSINESS_TYPE)))
                    {
                        vXLine = 19;
                    }
        ...
        {// 페이지수를 원본쉬트 복사하기 전에 원본쉬트에 기록하고 쉬트를 복사한다.
            ...
            int vXLRow = 56; //엑셀에 내용이 표시되는 행 번호
            int vXLCol = 40;
