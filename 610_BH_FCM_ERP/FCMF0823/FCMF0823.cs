...
namespace FCMF0823
{
    public partial class FCMF0823 : Office2007Form
    {
        ...
        private void Set_Default_Value()
        {
            ...
            PERIOD_YEAR.EditValue = iDate.ISYear(vGetDateTime);
        ...
        private void SEARCH_DB()
        {
            ...
            if (iString.ISNull(VAT_MNG_SERIAL.EditValue) == string.Empty)
            {
                MessageBoxAdv.Show(isMessageAdapter1.ReturnText("FCM_10487"), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                VAT_REPORT_NM.Focus();
                return;
            }
            if (iString.ISNull(DEAL_DATE_FR.EditValue) == string.Empty)
            {
                MessageBoxAdv.Show(isMessageAdapter1.ReturnText("FCM_10010"), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                DEAL_DATE_FR.Focus();
                return;
            }
            if (iString.ISNull(DEAL_DATE_TO.EditValue) == string.Empty)
            {
                MessageBoxAdv.Show(isMessageAdapter1.ReturnText("FCM_10011"), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                DEAL_DATE_FR.Focus();
                return;
            }
            if (Convert.ToDateTime(DEAL_DATE_FR.EditValue) > Convert.ToDateTime(DEAL_DATE_TO.EditValue))
            {
        ...
        {// pOutChoice : 출력구분.
            ...
            {// 폼에 있는 항목들중 기본적으로 출력해야 하는 값.
                ...
                // open해야 할 파일명 지정.
                //-------------------------------------------------------------------------------------
                xlPrinting.OpenFileNameExcel = "FCMF0823_001.xls";
                //-------------------------------------------------------------------------------------
                // 파일 오픈.
                //-------------------------------------------------------------------------------------
                ...
                {
                    // 헤더 인쇄.
                    IDA_PRINT_ZERO_RATE_DOCUMENT.Fill();
                    if (IDA_PRINT_ZERO_RATE_DOCUMENT.SelectRows.Count > 0)
                    {
                        xlPrinting.HeaderWrite(IDA_PRINT_ZERO_RATE_DOCUMENT);
                    }
                    // 실제 인쇄
