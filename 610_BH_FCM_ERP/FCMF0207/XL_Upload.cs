    ...
    public class XL_Upload
    {
        ...
        public bool LoadXL(InfoSummit.Win.ControlAdv.ISDataAdapter pAdapter, int pStartRow)
        {
            ...
            try
            {
                ...
                {
                    pAdapter.AddUnder();
                    vType = pAdapter.CurrentRow.Table.Columns["SLIP_DATE"].DataType; ;       //해당 셀의 데이터형식.  
                    if (vType.Name == "DateTime")
                    {
                        vObject = mExcel_Upload.XLGetCell(vRow, 1);  //기표일자.
                        vSLIP_DATE = ConvertDate(vObject);
                    }
                    else 
                    {
                        vSLIP_DATE = string.Empty;
                        pAdapter.Delete();
                    }
                    if (iString.ISNull(vSLIP_DATE) != string.Empty)  //기표일자가 있을 경우만 처리.
                    {                        
