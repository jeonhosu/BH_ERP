    ...
    public partial class APPF0030 : Office2007Form
    {
        ...
        private void NavigatorMenuShow(int pMenuID, ISTreeView.TreeNode pMenuNode)
        {
            ...
            foreach (DataRow row1 in vRows1)
            {
                ...
                //[Tree_Node_Expand]
                //------------------------------
                if (row1["MENU_NAME"].ToString() == "Sub_JHS")
                {
                    pMenuNode.Expand();
                    vCountMenu = 0;
                }
                ...
                foreach (DataRow row2 in vRows2)
                {
                    ...
                    ////[Child Node Select]
                    ////------------------------------------------------------------
                    //object vObject = row2["ASSEMBLY_INFO_ID"];
                    //if (vObject != System.DBNull.Value)
                    //{
                    //    bool isNull = string.IsNullOrEmpty(vObject.ToString());
                    //    if (isNull != true)
                    //    {
                    //        int vAssemblyID = int.Parse(vObject.ToString());
                    //        if (vAssemblyID == 606)
                    //        {
                    //            isTreeView1.SelectedNode = vMenuEntryNode;
                    //            mCountMenu = vCountMenu;
                    //        }
                    //    }
                    //}
                    ////------------------------------------------------------------
        ...
        private void ProcessStart(string pPath, ISTreeView.TreeNode pSelectedNode)
        {
            try
            {
                ...
                if (vFileInfo.Exists == true)
                {
                    ...
                    if (vType != null)
                    {
                        if (vNameSpace == "FCMF0204" || vNameSpace == "FCMF0205")
                        {
                            object[] vParam = new object[3];
