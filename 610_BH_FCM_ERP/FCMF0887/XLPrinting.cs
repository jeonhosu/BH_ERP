...
namespace FCMF0887
{
    public class XLPrinting
    {
        ...
        private int mPrintingLineSTART_SOURCE1 = 11;  //내국신용장 구매확인서에 의한 공급실적 합계
        private int mPrintingLineSTART_SOURCE2 = 9;   //내국신용장 구매확인서에 의한 공급실적 명세서
        ...
        private void SetArray1(InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid, out int[] pGDColumn, out int[] pXLColumn)
        {
            ...
            pGDColumn[0] = pGrid.GetColumnToIndex("GUBUN"); //구분
            pGDColumn[1] = pGrid.GetColumnToIndex("CNT");   //건수
            pGDColumn[2] = pGrid.GetColumnToIndex("TOTAL"); //금액(원)
        ...
        private void XLHeader(InfoSummit.Win.ControlAdv.ISDataAdapter p_adapter_PRINT_DOMESTIC_LC)
        {
            int vCountRow = 0;
            int vXLine = 0;
            int vXLColumn = 0;
            ...
            try
            {
                vCountRow = p_adapter_PRINT_DOMESTIC_LC.OraSelectData.Rows.Count;
                ...
                if (vCountRow > 0)
                {
                    ...
                    //부가가치세신고기수
                    vObject = p_adapter_PRINT_DOMESTIC_LC.OraSelectData.Rows[0]["FISCAL_YEAR"];
                    vXLine = 4;
                    vXLColumn = 2;
                    ...
                    //상호(법인명)
                    vObject = p_adapter_PRINT_DOMESTIC_LC.OraSelectData.Rows[0]["CORP_NAME"];
                    vXLine = 5;
                    vXLColumn = 11;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
                    ...
                    //사업자등록번호
                    vObject = p_adapter_PRINT_DOMESTIC_LC.OraSelectData.Rows[0]["VAT_NUMBER"];
                    vXLine = 5;
                    vXLColumn = 31;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
                    ...
                    //성명(대표자)
                    vObject = p_adapter_PRINT_DOMESTIC_LC.OraSelectData.Rows[0]["PRESIDENT_NAME"];
                    vXLine = 6;
                    vXLColumn = 12;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
                    ...
                    //사업장소재지
                    vObject = p_adapter_PRINT_DOMESTIC_LC.OraSelectData.Rows[0]["LOCATION"];
                    vXLine = 6;
                    vXLColumn = 32;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
                    ...
                    //업태
                    vObject = p_adapter_PRINT_DOMESTIC_LC.OraSelectData.Rows[0]["BUSINESS_ITEM"];
                    vXLine = 7;
                    vXLColumn = 12;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
                    ...
                    //종목
                    vObject = p_adapter_PRINT_DOMESTIC_LC.OraSelectData.Rows[0]["BUSINESS_TYPE"];
                    vXLine = 7;
                    vXLColumn = 32;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
                    ...
                    //거래기간
                    vObject = p_adapter_PRINT_DOMESTIC_LC.OraSelectData.Rows[0]["DEAL_TERM"];
                    vXLine = 8;
                    vXLColumn = 12;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
                    ...
                    //작성일자
                    vObject = p_adapter_PRINT_DOMESTIC_LC.OraSelectData.Rows[0]["CREATE_DATE"];
                    vXLine = 8;
                    vXLColumn = 32;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
                    ...
                    //관할세무서
                    vObject = p_adapter_PRINT_DOMESTIC_LC.OraSelectData.Rows[0]["TAX_OFFICE_NAME"];
                    vXLine = 26;
                    vXLColumn = 3;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
        ...
        public int LineWrite(InfoSummit.Win.ControlAdv.ISGridAdvEx p_grid_LIST_UP_DOMESTIC_LC, InfoSummit.Win.ControlAdv.ISGridAdvEx p_grid_LIST_DOWN_DOMESTIC_LC, InfoSummit.Win.ControlAdv.ISDataAdapter p_adapter_PRINT_DOMESTIC_LC)
        {
            mPageNumber = 0;
            ...
            try
            {
                int vTotal1ROW = p_grid_LIST_UP_DOMESTIC_LC.RowCount;
                int vTotal2ROW = p_grid_LIST_DOWN_DOMESTIC_LC.RowCount;

                #region ----- Header/SUM Write ----

                XLHeader(p_adapter_PRINT_DOMESTIC_LC);
                ...
                if (vTotal1ROW > 0)
                {
                    ...
                    SetArray1(p_grid_LIST_UP_DOMESTIC_LC, out vGDColumn, out vXLColumn);
                    ...
                    for (int vRow1 = 0; vRow1 < vTotal1ROW; vRow1++)
                    {
                        ...
                        vPrintingLine = XLLine_1(p_grid_LIST_UP_DOMESTIC_LC, vRow1, vPrintingLine, vGDColumn, vXLColumn);
                    }
                }
                ...
                if (vTotal2ROW > 0)
                {
                    ...
                    vPrintingLine = vPrintingLine + 3; // 17 = 14 + 3

                    SetArray2(p_grid_LIST_DOWN_DOMESTIC_LC, out vGDColumn, out vXLColumn);

                    for (int vRow2 = 0; vRow2 < vTotal2ROW; vRow2++)
                    {
                        vCountROW1++;

                        vMessage = string.Format("Grid2_Rows : {0}/{1}", vCountROW1, vTotal2ROW);
                        mAppInterface.OnAppMessageEvent(vMessage);
                        System.Windows.Forms.Application.DoEvents();

                        vPrintingLine = XLLine_2(p_grid_LIST_DOWN_DOMESTIC_LC, vRow2, vPrintingLine, vGDColumn, vXLColumn);
