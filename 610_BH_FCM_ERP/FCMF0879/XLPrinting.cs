...
namespace FCMF0879
{
    public class XLPrinting
    {
        ...
        private int mPrintingLineSTART_SOURCE1 = 39;  //Line
        ...
        private void XLSUM(InfoSummit.Win.ControlAdv.ISGridAdvEx p_grid_UP_AR_SUM_TAX)
        {
            int vRow = 0;
            int vCountRow = 0;
            ...
            bool IsConvert = false;
            int vIDX_VAT_TYPE = p_grid_UP_AR_SUM_TAX.GetColumnToIndex("VAT_TYPE");

            int vCol_COMPANY_CNT = 14;   //매입처수
            int vCol_TOTAL_RECORD = 19;  //매수
            int vCol_GL_JO = 23;         //매입금액_조단위
            int vCol_GL_UK = 25;         //매입금액_십억단위
            int vCol_GL_MAN = 28;        //매입금액_백만단위
            int vCol_GL_CHUN = 31;       //매입금액_천단위
            int vCol_GL_WON = 34;        //매입금액_원단위

            try
            {
                vCountRow = p_grid_UP_AR_SUM_TAX.RowCount;
                ...
                if (vCountRow > 0)
                {
                    ...
                    {
                        if (iConv.ISNull(p_grid_UP_AR_SUM_TAX.GetCellValue(vRow, vIDX_VAT_TYPE)) == "SUM")
                        {
                            //구분 : 합계
                            vXLine = 20;
                        }
                        else if (iConv.ISNull(p_grid_UP_AR_SUM_TAX.GetCellValue(vRow, vIDX_VAT_TYPE)) == "YC")
                        {
                            //구분 : 전자세금계산서
                            vXLine = 22;
                        }
                        else if (iConv.ISNull(p_grid_UP_AR_SUM_TAX.GetCellValue(vRow, vIDX_VAT_TYPE)) == "YP")
                        {
                            //구분 : 전자세금계산서
                            vXLine = 24;
                        }
                        else if (iConv.ISNull(p_grid_UP_AR_SUM_TAX.GetCellValue(vRow, vIDX_VAT_TYPE)) == "YS")
                        {
                            //구분 : 전자세금계산서
                            vXLine = 26;
                        }
                        else if (iConv.ISNull(p_grid_UP_AR_SUM_TAX.GetCellValue(vRow, vIDX_VAT_TYPE)) == "NC")
                        {
                            //구분 : 전자세금계산서
                            vXLine = 28;
                        }
                        else if (iConv.ISNull(p_grid_UP_AR_SUM_TAX.GetCellValue(vRow, vIDX_VAT_TYPE)) == "NP")
                        {
                            //구분 : 전자세금계산서
                            vXLine = 30;
                        }
                        else if (iConv.ISNull(p_grid_UP_AR_SUM_TAX.GetCellValue(vRow, vIDX_VAT_TYPE)) == "NS")
                        {
                            //구분 : 전자세금계산서
                            vXLine = 32;
                        }
                        ...
                        //매입처수 : 합계
                        vGDColumnIndex = p_grid_UP_AR_SUM_TAX.GetColumnToIndex("COMPANY_CNT");
                        vObject = p_grid_UP_AR_SUM_TAX.GetCellValue(vRow, vGDColumnIndex);
                        ...
                        //매수 : 합계
                        vGDColumnIndex = p_grid_UP_AR_SUM_TAX.GetColumnToIndex("TOTAL_RECORD");
                        vObject = p_grid_UP_AR_SUM_TAX.GetCellValue(vRow, vGDColumnIndex);
                        ...
                        //매입금액_조단위 : 합계
                        vGDColumnIndex = p_grid_UP_AR_SUM_TAX.GetColumnToIndex("GL_AMOUNT_1");
                        vObject = p_grid_UP_AR_SUM_TAX.GetCellValue(vRow, vGDColumnIndex);
                        ...
                        //매입금액_십억단위 : 합계
                        vGDColumnIndex = p_grid_UP_AR_SUM_TAX.GetColumnToIndex("GL_AMOUNT_2");
                        vObject = p_grid_UP_AR_SUM_TAX.GetCellValue(vRow, vGDColumnIndex);
                        ...
                        //매입금액_백만단위 : 합계
                        vGDColumnIndex = p_grid_UP_AR_SUM_TAX.GetColumnToIndex("GL_AMOUNT_3");
                        vObject = p_grid_UP_AR_SUM_TAX.GetCellValue(vRow, vGDColumnIndex);
                        ...
                        //매입금액_천단위 : 합계
                        vGDColumnIndex = p_grid_UP_AR_SUM_TAX.GetColumnToIndex("GL_AMOUNT_4");
                        vObject = p_grid_UP_AR_SUM_TAX.GetCellValue(vRow, vGDColumnIndex);
                        ...
                        //매입금액_원단위 : 합계
                        vGDColumnIndex = p_grid_UP_AR_SUM_TAX.GetColumnToIndex("GL_AMOUNT_5");
                        vObject = p_grid_UP_AR_SUM_TAX.GetCellValue(vRow, vGDColumnIndex);
        ...
        public int LineWrite(InfoSummit.Win.ControlAdv.ISGridAdvEx p_grid_UP_AR_SUM_TAX, InfoSummit.Win.ControlAdv.ISGridAdvEx p_grid_LIST_AR_SUM_TAX, InfoSummit.Win.ControlAdv.ISDataAdapter p_adapter_PRINT_SUM_TAX_TITLE)
        {
            mPageNumber = 0;
            ...
            try
            {
                int vTotal1ROW = p_grid_LIST_AR_SUM_TAX.RowCount;

                #region ----- Header/SUM Write ----

                XLHeader(p_adapter_PRINT_SUM_TAX_TITLE);
                XLSUM(p_grid_UP_AR_SUM_TAX);
                ...
                if (vTotal1ROW > 0)
                {
                    ...
                    SetArray1(p_grid_LIST_AR_SUM_TAX, out vGDColumn, out vXLColumn);
                    ...
                    for (int vRow1 = 0; vRow1 < vTotal1ROW; vRow1++)
                    {
                        ...
                        vPrintingLine = XLLine(p_grid_LIST_AR_SUM_TAX, vRow1, vPrintingLine, vGDColumn, vXLColumn);
