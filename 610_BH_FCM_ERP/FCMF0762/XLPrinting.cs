...
namespace FCMF0762
{
    public class XLPrinting
    {
        ...
        private void SetArray1(InfoSummit.Win.ControlAdv.ISGridAdvEx pGrid, out int[] pGDColumn, out int[] pXLColumn)
        {
            ...
            pGDColumn[0] = pGrid.GetColumnToIndex("ITEM_NAME");         //계정과목    : String
            pGDColumn[1] = pGrid.GetColumnToIndex("LAST_LEVEL_AMT_01"); //당기 : 금액 : Decimal
            pGDColumn[2] = pGrid.GetColumnToIndex("NON_LAST_AMT_01");   //당기 : 금액 : Decimal
            pGDColumn[3] = pGrid.GetColumnToIndex("LAST_LEVEL_AMT_02"); //전기 : 금액 : Decimal
            pGDColumn[4] = pGrid.GetColumnToIndex("NON_LAST_AMT_02");   //전기 : 금액 : Decimal

            pXLColumn[0] = 2;  //계정과목    : String
            pXLColumn[1] = 14; //당기 : 금액 : Decimal
            pXLColumn[2] = 21; //당기 : 금액 : Decimal
            pXLColumn[3] = 29; //전기 : 금액 : Decimal
            pXLColumn[4] = 36; //전기 : 금액 : Decimal
        }
        ...
        private void XLHeader(InfoSummit.Win.ControlAdv.ISDataAdapter p_adapter_PRINT_TITLE)
        {
            ...
            try
            {
                ...
                if (vCountRow > 0)
                {
                    ...
                    //당기(예>제 13기 2011년 01월 01일 부터 2011년 12월 31일 까지); 제조원가, 손익계산서에서 사용
                    vObject = p_adapter_PRINT_TITLE.OraSelectData.Rows[0]["CUR_TERM"];
                    vXLine = 4;
                    vXLColumn = 2;
                    ...
                    //전기(예>제 12기 2010년 01월 01일 부터 2010년 12월 31일 까지); 제조원가, 손익계산서에서 사용
                    vObject = p_adapter_PRINT_TITLE.OraSelectData.Rows[0]["PRE_TERM"];
                    vXLine = 5;
                    vXLColumn = 2;
                    IsConvert = IsConvertString(vObject, out vConvertString);
                    if (IsConvert == true)
        ...
        public int LineWrite(InfoSummit.Win.ControlAdv.ISGridAdvEx p_grid_FS_IS_PARADE_YEAR, InfoSummit.Win.ControlAdv.ISDataAdapter p_adapter_PRINT_TITLE)
        {
            mPageNumber = 0;
            ...
            try
            {
                int vTotal1ROW = p_grid_FS_IS_PARADE_YEAR.RowCount;
                ...
                if (vTotal1ROW > 0)
                {
                    ...
                    SetArray1(p_grid_FS_IS_PARADE_YEAR, out vGDColumn, out vXLColumn);
                    ...
                    for (int vRow1 = 0; vRow1 < vTotal1ROW; vRow1++)
                    {
                        ...
                        vPrintingLine = XLLine(p_grid_FS_IS_PARADE_YEAR, vRow1, vPrintingLine, vGDColumn, vXLColumn);
