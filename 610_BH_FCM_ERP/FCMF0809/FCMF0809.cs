...
namespace FCMF0809
{
    public partial class FCMF0809 : Office2007Form
    {
        ...
        private void SEARCH_DB()
        {
            ...
            idaZERO_TAX_RATE.Fill();
            igrZERO_TAX_RATE.Focus();
        }
        ...
        {
            igrZERO_TAX_RATE.SetCellValue("TAX_CODE", TAX_CODE_0.EditValue);
            igrZERO_TAX_RATE.SetCellValue("TAX_CODE_DESC", TAX_CODE_NAME_0.EditValue);
            igrZERO_TAX_RATE.SetCellValue("DOCUMENT_TYPE", mVAT_DOC_TYPE);
            igrZERO_TAX_RATE.SetCellValue("DOCUMENT_TYPE_DESC", mVAT_DOC_TYPE_DESC);
            igrZERO_TAX_RATE.SetCellValue("ISSUE_DATE", WRITE_DATE.EditValue);

            igrZERO_TAX_RATE.CurrentCellMoveTo(igrZERO_TAX_RATE.GetColumnToIndex("DOCUMENT_TYPE_DESC"));
            igrZERO_TAX_RATE.CurrentCellActivate(igrZERO_TAX_RATE.GetColumnToIndex("DOCUMENT_TYPE_DESC"));
            igrZERO_TAX_RATE.Focus();
        }
        ...
        {            
            ...
            igrZERO_TAX_RATE.SetCellValue("TOTAL_BASE_AMOUNT", mBASE_AMOUNT);            
        }
        ...
        private void SET_THIS_BASE_AMOUNT(object pEXCHANGE_RATE, object pCURR_AMOUNT)
        {
            ...
            mBASE_AMOUNT = iString.ISDecimaltoZero(pEXCHANGE_RATE) * iString.ISDecimaltoZero(pCURR_AMOUNT);
            mBASE_AMOUNT = Math.Round(mBASE_AMOUNT, 0);
            igrZERO_TAX_RATE.SetCellValue("THIS_BASE_AMOUNT", mBASE_AMOUNT);
        }

        private void Show_Slip_Detail()
        {
            int mSLIP_HEADER_ID = iString.ISNumtoZero(igrZERO_TAX_RATE.GetCellValue("INTERFACE_HEADER_ID"));
        ...
        {
            int vCountRow = idaZERO_TAX_RATE.OraSelectData.Rows.Count;
            if (vCountRow < 1)
            {
                return;
            }
            ...
            if (saveFileDialog1.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                bool vXLSaveOK = xlExport.XLExport(idaZERO_TAX_RATE.OraSelectData, vsSaveExcelFileName, vsSheetName);
                if (vXLSaveOK == true)
                {
        ...
        {// pOutChoice : 출력구분.
            ...
            {// 폼에 있는 항목들중 기본적으로 출력해야 하는 값.
                ...
                // open해야 할 파일명 지정.
                //-------------------------------------------------------------------------------------
                xlPrinting.OpenFileNameExcel = "FCMF0809_001.xls";
                //-------------------------------------------------------------------------------------
                // 파일 오픈.
                //-------------------------------------------------------------------------------------
                ...
                if (isOpen == true)
                {
                    ...
                    {
                        xlPrinting.SAVE("VAT_ZERO_TAX_RATE_");
                    }
        ...
        private void isAppInterfaceAdv1_AppMainButtonClick(ISAppButtonEvents e)
        {
            if (this.IsActive)
            {
                ...
                {
                    //if (idaZERO_TAX_RATE.IsFocused)
                    //{
                    //    idaZERO_TAX_RATE.AddOver();
                    //    SET_INSERT_EXPORT();
                    //}
                }
                ...
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Delete)
                {
                    if (idaZERO_TAX_RATE.IsFocused)
                    {
                        idaZERO_TAX_RATE.Delete();
                    }
                }
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Print)
                {
                    XLPrinting_1("PRINT", igrZERO_TAX_RATE);
                }
                else if (e.AppMainButtonType == ISUtil.Enum.AppMainButtonType.Export)
                {
                    XLPrinting_1("FILE", igrZERO_TAX_RATE);
                }
            }
        }
        ...
        private void igrZERO_TAX_RATE_CellDoubleClick(object pSender)
        {
            Show_Slip_Detail();
        }

        private void igrVAT_ZERO_TAX_RATE_CurrentCellChanged(object pSender, ISGridAdvExChangedEventArgs e)
        {
            int mIDX_EXCHANGE_RATE = igrZERO_TAX_RATE.GetColumnToIndex("EXCHANGE_RATE");
            int mIDX_TOTAL_CURR_AMOUNT = igrZERO_TAX_RATE.GetColumnToIndex("TOTAL_CURR_AMOUNT");
            int mIDX_THIS_CURR_AMOUNT = igrZERO_TAX_RATE.GetColumnToIndex("THIS_CURR_AMOUNT");

            if (e.ColIndex == mIDX_EXCHANGE_RATE)
            {//환율.
                SET_TOTAL_BASE_AMOUNT(e.NewValue, igrZERO_TAX_RATE.GetCellValue("TOTAL_CURR_AMOUNT"));
                // 전호수 수정 : 당기체출금액만 수정.
                //SET_THIS_BASE_AMOUNT(e.NewValue, igrZERO_TAX_RATE.GetCellValue("TOTAL_CURR_AMOUNT"));
            }
            else if (e.ColIndex == mIDX_TOTAL_CURR_AMOUNT)
            {//당기제출금액.
                SET_TOTAL_BASE_AMOUNT(igrZERO_TAX_RATE.GetCellValue("EXCHANGE_RATE"), e.NewValue);
            }
            // 전호수 수정 : 당기체출금액만 수정.
        ...
        private void ibtnSET_ZERO_TAX_RATE_ButtonClick(object pSender, EventArgs pEventArgs)
        {
            if (iString.ISNull(TAX_CODE_0.EditValue) == string.Empty)
            {
                MessageBoxAdv.Show(isMessageAdapter1.ReturnText("FCM_10007"), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            ...
            string mMESSAGE;
            idcSET_ZERO_TAX_RATE.ExecuteNonQuery();
            mMESSAGE = iString.ISNull(idcSET_ZERO_TAX_RATE.GetCommandParamValue("O_MESSAGE"));
        ...
        private void idaZERO_TAX_RATE_PreRowUpdate(ISPreRowUpdateEventArgs e)
        {
        ...
        private void idaZERO_TAX_RATE_PreDelete(ISPreDeleteEventArgs e)
        {
