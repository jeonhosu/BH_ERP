  ...
  BEGIN
    ...
    FOR C1 IN ( SELECT  --> HEADER <--
                     ...
                     || CASE
                        ...
                        END                             -- 6.신고구분상세코드;
                     || 'F01'                           -- 7.신고서종류코드;
                     || RPAD(REPLACE(WD.STD_YYYYMM, '-', ''), 6, ' ')     -- 8.귀속연월;
                     || RPAD(REPLACE(WD.PAY_YYYYMM, '-', ''), 6, ' ')     -- 9.지급연월;
                     || RPAD(CASE
                               WHEN WD.WITHHOLDING_TYPE IN('2', '3') THEN REPLACE(WD.PAY_YYYYMM, '-', '')
                               ELSE TO_CHAR(WD.SUBMIT_DATE, 'YYYYMM')
                             END, 6, ' ')                                 -- 10.제출연월;
                     || RPAD(HI.HOMETAX_ID, 20, ' ')                      -- 11.사용자ID;
                     || CASE
                          WHEN WD.WITHHOLDING_TYPE IN('1', '3') THEN 'FF001'
                          WHEN WD.WITHHOLDING_TYPE = '2' THEN 'FF101' 
                        END                                               -- 12.민원종류코드(FF001-원천징수이행상황신고서, 기한후 신고서, FF101-수정신고);
                     || RPAD(' ', 10 ,' ')                                -- 13.세무대리인사업자번호;
                     || RPAD(' ', 30, ' ')                                -- 14.세무대리인성명;
                     || RPAD(' ', 6, ' ')                                 -- 15.세무대리인관리번호;
                     || RPAD(' ', 14, ' ')                                -- 16.세무대리인전화번호;
                     || RPAD(CM.CORP_NAME, 30, ' ')                       -- 17.법인명(상호);
                     || RPAD(CM.ADDR1 || ' ' || CM.ADDR2, 70, ' ')        -- 18.사업장소재지;
                     || RPAD(CM.TEL_NUMBER, 14, ' ')                      -- 19.사업장전화번호;
                     || RPAD(NVL(CM.EMAIL, ' '), 50, ' ')                 -- 20.이메일;
                     || RPAD(CM.PRESIDENT_NAME, 30, ' ')                  -- 21.성명(대표자명);
                     || CASE
                          WHEN WD.MONTHLY_YN = 'Y' THEN '01'
                          WHEN WD.HALF_YEARLY_YN = 'Y' THEN '02'
                        END                                               -- 22.원천신고구분(01-매월, 02-반기).
                     || CASE
                          ...
                          ELSE 'N'
                        END                                               -- 23.연말정산여부;
                     || RPAD(WD.INCOME_DISPOSED_YN, 1, 'N')               -- 24.소득처분여부;
                     || RPAD(WD.REFUND_REQUEST_YN, 1, 'N')                -- 25.환급신청여부;
                     || RPAD(WD.ALL_PAYMENT_YN, 1, 'N')                   -- 26.일괄납부여부;
                     || RPAD(WD.BUSINESS_UNIT_TAX_YN, 1, 'N')             -- 27.사업자단위과세여부;
                     || 'N'                                               -- 28.신고서부표여부(이자소득, 배당소득, 양도소득, 법인원천등 지급);
                     || 'N'                                               -- 29.차월이월환급세액승계명세여부;
                     || 'N'                                               -- 30.기납부세액명세서제출여부;
                     || RPAD(' ', 3, ' ')                                 -- 예입처;
                     || RPAD(' ', 20, ' ')                                -- 계좌번호;
                     ...
                     ,  '22'                                                  -- 1.자료구분.
                     || 'C103900'                                             -- 2.서식코드.
                     || LPAD(REPLACE(LPAD(NVL(WD.RECEIVE_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 3.전월미환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.ALREADY_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 4.기환급세신청세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.REFUND_BALANCE_AMT, 0), 15, '0'), '-', ''), 15, '-')          -- 5.차감후잔액(원천).
                     || LPAD(REPLACE(LPAD(NVL(WD.GENERAL_REFUND_AMT, 0), 15, '0'), '-', ''), 15, '-')          -- 6.일반환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.FINANCIAL_AMT, 0), 15, '0'), '-', ''), 15, '-')               -- 7.신탁재산세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.ETC_REFUND_FINANCIAL_AMT, 0), 15, '0'), '-', ''), 15, '-')    -- 8.그밖의환급세액-금융회사등.
                     || LPAD(REPLACE(LPAD(NVL(WD.ETC_REFUND_MERGER_AMT, 0), 15, '0'), '-', ''), 15, '-')       -- 9.그밖의 환급세액-합병등.
                     || LPAD(REPLACE(LPAD(NVL(WD.ADJUST_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')       -- 10.조정대상환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.THIS_ADJUST_REFUND_TAX_AMT ,0), 15, '0'), '-', ''), 15, '-')  -- 11.당월조정환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.NEXT_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 12.차월이월환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.REQUEST_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 13.환급신청액.
                     || LPAD(0, 15, '0')                                      -- 14.승계대상합계 차월이월환급세액 
                     || RPAD(' ', 11, ' ')                                    -- 15.공란.
                      AS B_RECORD_FILE
                        
                     --> 원천징수이행상황신고서-원천징수 명세 및 납부세액<--
                     -- 근로소득_간이세액 
                     ,  '23'                                                  -- 1.자료구분.
                     || 'C103900'                                             -- 2.서식코드.
                     || 'A01'                                                 -- 3.원천세목코드.
                     || LPAD(NVL(WD.A01_PERSON_CNT, 0), 15, '0')              -- 4.인원.
                     || LPAD(NVL(WD.A01_PAYMENT_AMT, 0), 15, '0')             -- 5.총지급액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A01_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')          -- 6.징수세액(소득세등).
                     || LPAD(REPLACE(LPAD(NVL(WD.A01_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')              -- 7.징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A01_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')             -- 8.가산세.
                     || LPAD(0, 15, '0')                                      -- 9.당월조정환급세액.
                     || LPAD(0, 15, '0')                                      -- 10.납부세액(소득세등).
                     || LPAD(0, 15, '0')                                      -- 11.납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                    -- 공란.
                       AS A01_RECORD_FILE
                     -- 근로소득_중도퇴사   
                     ,  '23'                                                  -- 자료구분.
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A02_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')   -- 징수세액(소득세등).                          
                     || LPAD(REPLACE(LPAD(NVL(WD.A02_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')       -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A02_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 가산세.
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A03_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')   -- 징수세액(소득세등).                          
                     || LPAD(0, 15, '0')                                       -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A03_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 가산세.
                     || LPAD(0, 15, '0')                                       -- 당월조정환급세액.
                     || LPAD(0, 15, '0')                                       -- 납부세액(소득세등).
                     || LPAD(0, 15, '0')                                       -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                     -- 공란.
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A04_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')   -- 징수세액(소득세등).                        
                     || LPAD(REPLACE(LPAD(NVL(WD.A04_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')       -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A04_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 가산세.
                     || LPAD(0, 15, '0')                                       -- 당월조정환급세액.
                     || LPAD(0, 15, '0')                                       -- 납부세액(소득세등).
                     || LPAD(0, 15, '0')                                       -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                     -- 공란.
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A10_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')   -- 징수세액(소득세등).                        
                     || LPAD(REPLACE(LPAD(NVL(WD.A10_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')       -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A10_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 가산세.
                     || LPAD(REPLACE(LPAD(NVL(WD.A10_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 당월조정환급세액                     
                     || LPAD(REPLACE(LPAD(NVL(WD.A10_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')   -- 납부세액(소득세등).   
                     || LPAD(REPLACE(LPAD(NVL(WD.A10_PAY_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')       -- 납부세액(농특세).                        
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A10_RECORD_FILE
                     -- 퇴직소득_연금계좌   
                     ,  '23'                                              -- 자료구분.
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A21_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')   -- 징수세액(소득세등).                        
                     || LPAD(0, 15, '0')                                                                -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A21_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 가산세.
                     || LPAD(REPLACE(LPAD(NVL(WD.A21_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 당월조정환급세액                     
                     || LPAD(REPLACE(LPAD(NVL(WD.A21_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')   -- 납부세액(소득세등).   
                     || LPAD(0, 15, '0')                                                                    -- 납부세액(농특세).                                
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A21_RECORD_FILE
                     -- 퇴직소득_연금계좌외   
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A22_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등). 
                     || LPAD(0, 15, '0')                                  -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A22_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(REPLACE(LPAD(NVL(WD.A22_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-') -- 당월조정환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A22_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 납부세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A22_RECORD_FILE  
                     -- 퇴직소득_가감계 
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A20_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A20_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(REPLACE(LPAD(NVL(WD.A20_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-') -- 당월조정환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A20_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 납부세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A20_RECORD_FILE
                     -- 사업소득_매월징수 
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A25_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A25_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(0, 15, '0')                                  -- 당월조정환급세액.
                     || LPAD(0, 15, '0')                                  -- 납부세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A26_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(REPLACE(LPAD(NVL(WD.A26_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')          -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A26_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(0, 15, '0')                                  -- 당월조정환급세액.
                     || LPAD(0, 15, '0')                                  -- 납부세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A30_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(REPLACE(LPAD(NVL(WD.A30_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')          -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A30_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(REPLACE(LPAD(NVL(WD.A30_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-') -- 당월조정환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A30_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 납부세액(소득세등).
                     || LPAD(REPLACE(LPAD(NVL(WD.A30_PAY_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A30_RECORD_FILE
                     -- 기타소득_연금계좌   
                     ,  '23'                                              -- 자료구분.
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A41_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등)(기타소득 연금계좌).
                     || LPAD(0, 15, '0')          -- 징수세액(농특세)(기타소득 연금계좌).
                     || LPAD(REPLACE(LPAD(NVL(WD.A41_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세(기타소득 연금계좌).
                     || LPAD(REPLACE(LPAD(NVL(WD.A41_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-') -- 당월조정환급세액(기타소득 연금계좌).
                     || LPAD(REPLACE(LPAD(NVL(WD.A41_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 납부세액(소득세등)(기타소득 연금계좌).
                     || LPAD(0, 15, '0')      -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A41_RECORD_FILE  
                     -- 기타소득_기타   
                     ,  '23'                                              -- 자료구분(기타소득 그외).
                     || 'C103900'                                            -- 서식코드(기타소득 그외).
                     || 'A42'                                             -- 원천세목코드(기타소득 그외).
                     || LPAD(NVL(WD.A42_PERSON_CNT, 0), 15, '0')          -- 인원(기타소득 그외).
                     || LPAD(NVL(WD.A42_PAYMENT_AMT, 0), 15, '0')         -- 총지급액(기타소득 그외).
                     || LPAD(REPLACE(LPAD(NVL(WD.A42_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등)(기타소득 그외).
                     || LPAD(0, 15, '0')          -- 징수세액(농특세)(기타소득 그외).
                     || LPAD(REPLACE(LPAD(NVL(WD.A42_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세(기타소득 그외).
                     || LPAD(REPLACE(LPAD(NVL(WD.A42_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-') -- 당월조정환급세액(기타소득 그외).
                     || LPAD(REPLACE(LPAD(NVL(WD.A42_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 납부세액(소득세등)(기타소득 그외).
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A40_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(0, 15, '0')          -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A40_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(REPLACE(LPAD(NVL(WD.A40_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-') -- 당월조정환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A40_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 납부세액(소득세등).
                     || LPAD(0, 15, '0')      -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A40_RECORD_FILE  
                     -- 연금소득_연금계좌   
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A48_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등)(연금소득 연금계좌).
                     || LPAD(0, 15, '0')                                  -- 징수세액(농특세)(연금소득 연금계좌).
                     || LPAD(REPLACE(LPAD(NVL(WD.A48_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세(연금소득 연금계좌).
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A45_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A45_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(0, 15, '0')                                  -- 당월조정환급세액.
                     || LPAD(0, 15, '0')                                  -- 납부세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A46_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A46_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(0, 15, '0')                                  -- 당월조정환급세액.
                     || LPAD(0, 15, '0')                                  -- 납부세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A47_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(0, 15, '0')          -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A47_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(REPLACE(LPAD(NVL(WD.A47_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-') -- 당월조정환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A47_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 납부세액(소득세등).
                     || LPAD(0, 15, '0')      -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A47_RECORD_FILE
                     --이자소득   
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A50_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(REPLACE(LPAD(NVL(WD.A50_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')          -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A50_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(REPLACE(LPAD(NVL(WD.A50_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-') -- 당월조정환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A50_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 납부세액(소득세등).
                     || LPAD(REPLACE(LPAD(NVL(WD.A50_PAY_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A50_RECORD_FILE
                     -- 배당소득   
                     ,  '23'                                              -- 자료구분.
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A60_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(REPLACE(LPAD(NVL(WD.A60_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')          -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A60_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(REPLACE(LPAD(NVL(WD.A60_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-') -- 당월조정환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A60_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 납부세액(소득세등).
                     || LPAD(REPLACE(LPAD(NVL(WD.A60_PAY_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A60_RECORD_FILE
                     -- 저축해지 추징세액 등   
                     ,  '23'                                              -- 자료구분.
                     ...
                     || LPAD(0, 15, '0')                                  -- 총지급액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A69_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A69_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(REPLACE(LPAD(NVL(WD.A69_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-') -- 당월조정환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A69_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 납부세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A69_RECORD_FILE
                     -- 비거주자 양도소득   
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A70_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A70_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(REPLACE(LPAD(NVL(WD.A70_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-') -- 당월조정환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A70_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 납부세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A70_RECORD_FILE
                     -- 법인 내/외국법인원천   
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A80_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A80_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(REPLACE(LPAD(NVL(WD.A80_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-') -- 당월조정환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A80_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 납부세액(소득세등).
                     || LPAD(0, 15, '0')                                  -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A80_RECORD_FILE
                     --수정신고(세액)   
                     ...
                     || LPAD(0, 15, '0')                                  -- 인원.
                     || LPAD(0, 15, '0')                                  -- 총지급액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A90_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(REPLACE(LPAD(NVL(WD.A90_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')          -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A90_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(REPLACE(LPAD(NVL(WD.A90_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-') -- 당월조정환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A90_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 납부세액(소득세등).
                     || LPAD(REPLACE(LPAD(NVL(WD.A90_PAY_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A90_RECORD_FILE
                     -- 총합계   
                     ,  '23'                                              -- 자료구분.
                     ...
                     || LPAD(REPLACE(LPAD(NVL(WD.A99_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 징수세액(소득세등).
                     || LPAD(REPLACE(LPAD(NVL(WD.A99_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')          -- 징수세액(농특세).
                     || LPAD(REPLACE(LPAD(NVL(WD.A99_ADD_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')         -- 가산세.
                     || LPAD(REPLACE(LPAD(NVL(WD.A99_THIS_REFUND_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-') -- 당월조정환급세액.
                     || LPAD(REPLACE(LPAD(NVL(WD.A99_PAY_INCOME_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')  -- 납부세액(소득세등).
                     || LPAD(REPLACE(LPAD(NVL(WD.A99_PAY_SP_TAX_AMT, 0), 15, '0'), '-', ''), 15, '-')      -- 납부세액(농특세).
                     || RPAD(' ', 18, ' ')                                -- 공란.
                       AS A99_RECORD_FILE
    ...
    LOOP
      ...
      -- 원천징수이행상황신고서-원천징수 명세 및 납부세액(A01).
      V_SEQ_NUM := 101;
