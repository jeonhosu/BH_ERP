...
-- 입퇴사자 현황 .
  PROCEDURE DATA_SELECT_JOIN_RETIRE
            ...
            , W_DEPT_ID                           IN HRM_PERSON_MASTER.DEPT_ID%TYPE
            , W_POST_ID                           IN HRM_PERSON_MASTER.POST_ID%TYPE
            , W_FLOOR_ID                          IN HRM_PERSON_MASTER.FLOOR_ID%TYPE DEFAULT NULL
...
-- 사원사진 등록 관리 --
  PROCEDURE PERSON_PHOTO_P
            ( O_STATUS                            OUT VARCHAR2
            , O_MESSAGE                           OUT VARCHAR2 
            , P_PROC_TYPE                         IN  VARCHAR2   -- INSERT : 삽입, UPDATE : 수정, DELETE : 삭제 
            , P_SOB_ID                            IN  NUMBER
            , P_ORG_ID                            IN  NUMBER 
...
-- SELECT PERSON INFOMATION.
  PROCEDURE SELECT_PERSON_S1
            ...
            , W_PERSON_ID                         IN NUMBER
            , W_DEPT_ID                           IN NUMBER
            , W_STD_DATE                          IN DATE DEFAULT NULL 
            , W_SOB_ID                            IN NUMBER
            , W_ORG_ID                            IN NUMBER
            );
...
-- 사원명부(약식) 조회 기별 재직자 사원 조회[2015-01-15]
   PROCEDURE SELECT_PERSON_SPREAD_PERIOD
           ( P_CURSOR       OUT TYPES.TCURSOR
           , W_SOB_ID       IN  HRM_PERSON_MASTER.SOB_ID%TYPE
           , W_ORG_ID       IN  HRM_PERSON_MASTER.ORG_ID%TYPE
           , W_CORP_ID      IN  HRM_PERSON_MASTER.CORP_ID%TYPE
...
-- 사원명부(약식) 조회 재직자 사원 조회[2015-01-15]
  PROCEDURE SELECT_PERSON_SPREAD
            ( P_CURSOR2                           OUT TYPES.TCURSOR2
            , W_CORP_ID                           IN NUMBER
            , W_DEPT_ID                           IN NUMBER
            , W_FLOOR_ID                          IN NUMBER
...
-- LOOKUP PERSON INFOMATION[2011-12-19]추가
   PROCEDURE LU_PERSON_SELECT16
           ...
           , W_SOB_ID         IN  NUMBER
           , W_ORG_ID         IN  NUMBER
           , W_CORP_ID        IN  NUMBER DEFAULT NULL 
           , W_WORK_DATE_FR   IN  DATE DEFAULT NULL
...
-- LOOKUP PERSON INFOMATION[2012-03-02]추가
   PROCEDURE LU_PERSON_SELECT17
           ...
           , W_SOB_ID             IN  NUMBER
           , W_ORG_ID             IN  NUMBER
           , W_WORK_CORP_ID       IN  NUMBER DEFAULT NULL 
           , W_CORP_ID            IN  NUMBER DEFAULT NULL
           , W_WORK_DATE_FR       IN  DATE DEFAULT NULL
           , W_WORK_DATE_TO       IN  DATE           
           , W_WORK_TYPE_ID       IN  NUMBER
    ...
    OPEN P_CURSOR1 FOR
      ...
      WHERE PM.CORP_ID               = CM.CORP_ID 
        ...
        AND ((W_JOB_CLASS_ID         IS NULL AND 1 = 1)
         OR  (W_JOB_CLASS_ID         IS NOT NULL AND PM.JOB_CLASS_ID = W_JOB_CLASS_ID)) 
        AND PM.SOB_ID                = W_SOB_ID
        AND PM.ORG_ID                = W_ORG_ID
  ...
  BEGIN
   ...
   -- 권한 설정.
    IF HRM_MANAGER_G.USER_ID_CAP_F ( W_CORP_ID => P_CORP_ID
                                   , W_START_DATE => TRUNC(N_SYSDATE)
                                   , W_END_DATE => TRUNC(N_SYSDATE)
                                   , W_MODULE_CODE => '10'
                                   , W_USER_ID => P_USER_ID 
                                   , W_SOB_ID => P_SOB_ID
...
/*--------------------------------------------------------------------------------------*/
    ...
    IF V_YEAR < '2000' THEN
      ...
      BEGIN
        SELECT NVL(MAX(TO_NUMBER(REPLACE(PM.PERSON_NUM, V_PERSON_NUM, ''))), 0) + 1 AS MAX_PERSON_NUM 
          INTO N_PERSON_SEQ
          FROM HRM_PERSON_MASTER PM
      ...
      IF N_PERSON_SEQ = 1 THEN
        V_PERSON_NUM := V_PERSON_NUM || LPAD(TO_CHAR(N_PERSON_SEQ), 4, 0);
      ELSE
        V_PERSON_NUM := V_PERSON_NUM || LPAD(TO_CHAR(N_PERSON_SEQ), 4, 0);
      END IF;
    ELSE
...
-- 기존 입사일자가 다를경우 사번 및 인사발령 사항 정정.
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      RETURN;
    END; 
    N_SYSDATE := GET_LOCAL_DATE(V_SOB_ID);
...
-- DATA_DELETE.
  ...
  AS
    ...
    N_SYSDATE             DATE := TRUNC(GET_LOCAL_DATE(P_SOB_ID));
  BEGIN
    O_STATUS := 'F';
    -- 권한 설정.
    IF HRM_MANAGER_G.USER_ID_CAP_F ( W_CORP_ID => W_CORP_ID
                                   , W_START_DATE => N_SYSDATE
                                   , W_END_DATE => N_SYSDATE
                                   , W_MODULE_CODE => '10'
                                   , W_USER_ID => P_USER_ID 
                                   , W_SOB_ID => P_SOB_ID
                                   , W_ORG_ID => P_ORG_ID) = 'C' THEN
      NULL;
    ELSE
      O_MESSAGE := EAPP_MESSAGE_G.RETURN_MSG_F('EAPP_10009', '&&CAP:=Delete');
      RETURN; 
    END IF;
    
    -- 근태 자료 및 급여자료 존재시 삭제 불가 -- 
    BEGIN
      SELECT COUNT(DL.WORK_DATE) AS CNT
        INTO V_REC_COUNT
    ...
    IF V_REC_COUNT > 0 THEN
      O_STATUS := 'F';
      O_MESSAGE := '(Day Leave ) ' || EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10274');
      RETURN;
    END IF;
    
    BEGIN
      SELECT COUNT(HL.PERSON_ID) AS CNT
        INTO V_REC_COUNT
        FROM HRM_HISTORY_HEADER HH
           , HRM_HISTORY_LINE   HL
           , HRM_CHARGE_CODE_V  CC 
       WHERE HH.HISTORY_HEADER_ID   = HL.HISTORY_HEADER_ID
         AND HH.CHARGE_ID           = CC.COMMON_ID 
         
         AND HL.PERSON_ID           = W_PERSON_ID
         AND CC.NEWCOMER_YN         != 'Y' 
       ;
    EXCEPTION
      WHEN OTHERS THEN 
        V_REC_COUNT := 0;
    ...
    IF V_REC_COUNT > 0 THEN
      O_STATUS := 'F';
      O_MESSAGE := '(Person History) ' || EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10274');
      RETURN;
    END IF;
    
    BEGIN
      SELECT COUNT(PMH.START_YYYYMM) AS CNT
        INTO V_REC_COUNT
    ...
    IF V_REC_COUNT > 0 THEN
      O_STATUS := 'F';
      O_MESSAGE := '(Pay Master) ' || EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10274');
      RETURN;
    END IF;
    ...
    -- 인사빌령 삭제 --
    DELETE FROM HRM_HISTORY_LINE PH
     WHERE PH.PERSON_ID             = W_PERSON_ID
    ;

    -- 근무계획 삭제 --
     DELETE FROM HRD_WORK_CALENDAR PH
      WHERE PH.PERSON_ID            = W_PERSON_ID
    ;

    DELETE FROM HRM_PERSON_MASTER PM 
     WHERE PM.PERSON_ID             = W_PERSON_ID
    ;
...
-- 사원사진 등록 관리 --
  ...
  BEGIN
    O_STATUS := 'F';
    IF P_PROC_TYPE = 'DELETE' THEN
      BEGIN
        DELETE FROM HRM_PERSON_PHOTO PP
         WHERE PP.SOB_ID            = P_SOB_ID
           AND PP.ORG_ID            = P_ORG_ID
           AND PP.PERSON_NUM        = P_PERSON_NUM
         ;
      ...
      EXCEPTION
        WHEN OTHERS THEN
          O_STATUS := 'F';
          O_MESSAGE := 'Photo Delete Error ' || CHR(10) || SQLERRM;
          RETURN;
      END;
    ...
    ELSE
      BEGIN
        MERGE INTO HRM_PERSON_PHOTO PP
        USING ( SELECT P_SOB_ID AS SOB_ID 
                     , P_ORG_ID AS ORG_ID 
                     --, NVL(P_PERSON_ID, 0) AS PERSON_ID 
                     , NVL(P_PERSON_NUM, '-') AS PERSON_NUM 
                     , P_FILE_NAME AS FILE_NAME 
                     --, P_FILE_CONTENT_TYPE AS FILE_CONTENT_TYPE 
                     --, P_PHOTO AS PHOTO
                     , P_USER_ID AS USER_ID 
                  FROM DUAL 
              ) SX1
          ON  (PP.SOB_ID            = SX1.SOB_ID
           AND PP.ORG_ID            = SX1.ORG_ID
           AND PP.PERSON_NUM        = SX1.PERSON_NUM
              )
        WHEN MATCHED THEN
          UPDATE
             SET PP.FILE_NAME         = SX1.FILE_NAME
               /*, PP.FILE_CONTENT_TYPE = SX1.FILE_CONTENT_TYPE
               , PP.PHOTO             = SX1.PHOTO*/
               , PP.LAST_UPDATE_DATE  = V_SYSDATE
        ...
        WHEN NOT MATCHED THEN
          ...
          ( HRM_PERSON_PHOTO_S1.NEXTVAL -- PERSON_PHOTO_ID 
          , SX1.SOB_ID 
          , SX1.ORG_ID 
          --, SX1.PERSON_ID 
          , SX1.PERSON_NUM  
          , SX1.FILE_NAME 
          --, SX1.FILE_CONTENT_TYPE 
          --, SX1.PHOTO 
          , V_SYSDATE                   -- CREATION_DATE 
          , SX1.USER_ID                 -- CREATED_BY 
          , V_SYSDATE                   -- LAST_UPDATE_DATE 
          , SX1.USER_ID                 -- LAST_UPDATED_BY 
          )
        ;      
...
-- SELECT PERSON INFOMATION.
  ...
  AS
    V_LOCAL_DATE        DATE;
  BEGIN
    IF W_STD_DATE IS NULL THEN
      V_LOCAL_DATE := TRUNC(GET_LOCAL_DATE(W_SOB_ID));
    ELSE
      V_LOCAL_DATE := W_STD_DATE;
    END IF;
    ...
    OPEN P_CURSOR2 FOR
       SELECT PM.NAME AS NAME
           ...
           FROM HRM_PERSON_MASTER PM
             , (-- 시점 인사내역.
              ...
              WHERE HL.HISTORY_LINE_ID  IN ( SELECT MAX(S_HL.HISTORY_LINE_ID) AS HISTORY_LINE_ID
                              FROM HRM_HISTORY_LINE S_HL
                              WHERE S_HL.CHARGE_DATE            <= V_LOCAL_DATE
                                AND S_HL.PERSON_ID              = HL.PERSON_ID
                              GROUP BY S_HL.PERSON_ID
                             )
             ) T1
           WHERE PM.PERSON_ID                               = T1.PERSON_ID
            ...
            AND PM.ORI_JOIN_DATE                           <= V_LOCAL_DATE
            AND (PM.RETIRE_DATE IS NULL OR PM.RETIRE_DATE  >= V_LOCAL_DATE)
            AND PM.SOB_ID                                  = W_SOB_ID
            AND PM.ORG_ID                                  = W_ORG_ID
           ;
  ...
  AS
   ...
   BEGIN
             ...
             SELECT PM.PERSON_ID
                  ...
                  , PM.SEX_TYPE
                  , HRM_COMMON_G.CODE_NAME_F('SEX_TYPE', PM.SEX_TYPE, PM.SOB_ID, PM.ORG_ID) AS  SEX_NAME
                  , EAPP_REGISTER_AGE_F(PM.REPRE_NUM, V_CURR_DATE, 0) AS AGE
                  , PM.JOIN_ID
                  , HRM_COMMON_G.ID_NAME_F(PM.JOIN_ID) AS  JOIN_NAME
                  , PM.JOIN_ROUTE_ID
                  , HRM_COMMON_G.ID_NAME_F(PM.JOIN_ROUTE_ID) AS  JOIN_ROUTE_NAME
           ...
           ORDER BY PM.PERSON_NUM
                  , PM.NAME
                  ;
  END SELECT_PERSON_SPREAD_PERIOD;
...
-- 연말정산 기초관리.
  ...
  AS
    ...
    V_CONNECT_PERSON_ID           NUMBER :=1;
  BEGIN
    V_CONNECT_PERSON_ID := P_CONNECT_PERSON_ID;
    IF  HRM_MANAGER_G.USER_CAP_F( W_CORP_ID
                                , TRUNC(TO_DATE(W_STD_YYYYMM, 'YYYY-MM'), 'MONTH')
                                , LAST_DAY(TO_DATE(W_STD_YYYYMM, 'YYYY-MM'))
                                , '30'         -- 급여 담당자 -- 
                                , W_PERSON_ID
                                , P_SOB_ID
                                , P_ORG_ID) = 'C' THEN
    ...
    OPEN P_CURSOR2 FOR
      ...
      WHERE PM.PERSON_ID            = T1.PERSON_ID
        ...
        /*AND ((V_CONNECT_PERSON_ID   IS NULL AND 1 = 1)
         OR  (V_CONNECT_PERSON_ID   IS NOT NULL AND PM.PERSON_ID = V_CONNECT_PERSON_ID))*/
        AND ((P_YEAR_EMPLOYE_TYPE   IS NULL AND 1 = 1)
          OR (P_YEAR_EMPLOYE_TYPE   != '20' AND (PM.RETIRE_DATE > LAST_DAY(V_STD_DATE) OR PM.RETIRE_DATE IS NULL))
          OR (P_YEAR_EMPLOYE_TYPE   = '20' AND (PM.RETIRE_DATE BETWEEN TRUNC(V_STD_DATE, 'MONTH') AND V_STD_DATE)))
               ...
               AND HH.CHARGE_SEQ          IN 
                    (SELECT MAX(S_HH.CHARGE_SEQ) AS CHARGE_SEQ
                        FROM HRM_HISTORY_HEADER S_HH
                           , HRM_HISTORY_LINE   S_HL
                       WHERE S_HH.HISTORY_HEADER_ID = S_HL.HISTORY_HEADER_ID
                         AND S_HH.CHARGE_DATE       <= LAST_DAY(TO_DATE(W_YYYYMM, 'YYYY-MM'))
                         AND S_HL.PERSON_ID         = HL.PERSON_ID
                       GROUP BY S_HL.PERSON_ID
                     ) 
           ) T1 
               ...
               AND HH.CHARGE_SEQ          IN 
                    (SELECT MAX(S_HH.CHARGE_SEQ) AS CHARGE_SEQ
                        FROM HRM_HISTORY_HEADER S_HH
                           , HRM_HISTORY_LINE   S_HL
                       WHERE S_HH.HISTORY_HEADER_ID = S_HL.HISTORY_HEADER_ID
                         AND S_HH.CHARGE_DATE       <= TRUNC(GET_LOCAL_DATE(W_SOB_ID))
                         AND S_HL.PERSON_ID         = HL.PERSON_ID
                       GROUP BY S_HL.PERSON_ID
                     ) 
           ) T1 
               ...
               AND HH.CHARGE_SEQ          IN 
                    (SELECT MAX(S_HH.CHARGE_SEQ) AS CHARGE_SEQ
                        FROM HRM_HISTORY_HEADER S_HH
                           , HRM_HISTORY_LINE   S_HL
                       WHERE S_HH.HISTORY_HEADER_ID = S_HL.HISTORY_HEADER_ID
                         AND S_HH.CHARGE_DATE       <= LAST_DAY(TO_DATE(W_YYYYMM_TO, 'YYYY-MM'))
                         AND S_HL.PERSON_ID         = HL.PERSON_ID
                       GROUP BY S_HL.PERSON_ID
                     ) 
           ) T1 
   ...
   BEGIN
             ...
             SELECT HRM_COMMON_G.CODE_NAME_F('EMPLOYE_TYPE', PM.EMPLOYE_TYPE, PM.SOB_ID, PM.ORG_ID) AS EMPLOYE_TYPE_NAME
              ...
              WHERE PM.CORP_ID       =  CM.CORP_ID
                AND PM.PERSON_ID     =  T1.PERSON_ID
                AND PM.PERSON_ID     =  T2.PERSON_ID 
                AND ((W_CORP_ID      IS NULL AND PM.WORK_CORP_ID =  65) 
                OR   (W_CORP_ID      IS NOT NULL AND PM.WORK_CORP_ID = W_CORP_ID ))
                AND PM.SOB_ID        =  W_SOB_ID
                AND PM.ORG_ID        =  W_ORG_ID
                AND PM.JOIN_DATE     <= W_WORK_DATE_TO 
                --AND (PM.RETIRE_DATE  >= W_WORK_DATE_FR OR PM.RETIRE_DATE IS NULL) 
                AND ((W_WORK_DATE_FR IS NULL AND 1 = 1)
                OR   (W_WORK_DATE_FR IS NOT NULL AND (PM.RETIRE_DATE  >= W_WORK_DATE_FR OR PM.RETIRE_DATE IS NULL))) 
                AND ((W_DEPT_ID      IS NULL AND 1 = 1)
                OR   (W_DEPT_ID      IS NOT NULL AND T1.DEPT_ID = W_DEPT_ID)) 
                AND ((W_FLOOR_ID     IS NULL AND 1 = 1)
                OR   (W_FLOOR_ID     IS NOT NULL AND T2.FLOOR_ID = W_FLOOR_ID)) 
                AND ((W_EMPLOYE      IS NULL AND 1 = 1)
                OR   (W_EMPLOYE      IS NOT NULL AND PM.EMPLOYE_TYPE = W_EMPLOYE)) 
           --ORDER BY PM.NAME
                  ;
...
-- LOOKUP PERSON INFOMATION[2012-03-02]추가
  ...
  BEGIN
    ...
    END IF;
             OPEN P_CURSOR FOR
             SELECT ((SELECT HC.CODE_NAME
                        FROM HRM_COMMON HC
                       WHERE HC.GROUP_CODE   = 'EMPLOYE_TYPE'
                         AND HC.SOB_ID       = PM.SOB_ID
                         AND HC.ORG_ID       = PM.ORG_ID
                         AND HC.CODE         = PM.EMPLOYE_TYPE))  AS EMPLOYE_TYPE_NAME
                  , PM.PERSON_NUM                              AS PERSON_NUMBER
                  , PM.NAME                                    AS PERSON_NAME
                  , ((SELECT HC.CODE_NAME
                        FROM HRM_COMMON HC
                       WHERE HC.COMMON_ID  = T1.FLOOR_ID))     AS FLOOR_NAME
                  , ((SELECT WT.WORK_TYPE_NAME
                       FROM HRD_PERSON_HISTORY  PH
                          , HRM_WORK_TYPE_V     WT
                      WHERE PH.WORK_TYPE_ID       =   WT.WORK_TYPE_ID
                        AND PH.CORP_ID            =   PM.WORK_CORP_ID
                        AND PH.PERSON_ID          =   PM.PERSON_ID 
                        AND PH.SOB_ID             =   PM.SOB_ID
                        AND PH.ORG_ID             =   PM.ORG_ID
                        AND PH.EFFECTIVE_DATE_FR  <=  TRUNC(W_WORK_DATE_TO)
                        AND PH.EFFECTIVE_DATE_TO  >=  TRUNC(W_WORK_DATE_TO)
                        AND ROWNUM                <= 1
                    ))                                         AS WORK_TYPE
                  , CM.CORP_NAME                               AS CORP_NAME
                  , ((SELECT HC.CODE_NAME
                        FROM HRM_COMMON HC
                       WHERE HC.COMMON_ID  = T1.POST_ID))      AS POST_NAME
                  , ((SELECT HC.CODE_NAME
                        FROM HRM_COMMON HC
                       WHERE HC.COMMON_ID  = T1.PAY_GRADE_ID)) AS PAY_GRADE_NAME
                  , ((SELECT DM.DEPT_NAME
                        FROM HRM_DEPT_MASTER DM
                       WHERE DM.DEPT_ID    = T1.DEPT_ID))      AS DEPT_NAME
                  , ((SELECT HC.CODE_NAME
                        FROM HRM_COMMON HC
                       WHERE HC.COMMON_ID  = PM.OCPT_ID))      AS OCPT_NAME 
                  , ((SELECT HC.CODE_NAME
                        FROM HRM_COMMON HC
                       WHERE HC.COMMON_ID  = T1.JOB_CLASS_ID)) AS JOB_CLASS_NAME  
                  , ((SELECT HC.CODE_NAME
                        FROM HRM_COMMON HC
                       WHERE HC.COMMON_ID  = T1.JOB_CATEGORY_ID)) AS JOB_CATEGORY_NAME  
                  , PM.JOIN_DATE                               AS JOIN_DATE
                  , PM.RETIRE_DATE                             AS RETIRE_DATE
                  , PM.WORK_TYPE_ID
                  , T1.JOB_CATEGORY_ID
                         ...
                         AND HH.CHARGE_SEQ          IN 
                              (SELECT MAX(S_HH.CHARGE_SEQ) AS CHARGE_SEQ
                                  FROM HRM_HISTORY_HEADER S_HH
                                     , HRM_HISTORY_LINE   S_HL
                                 WHERE S_HH.HISTORY_HEADER_ID = S_HL.HISTORY_HEADER_ID
                                   AND S_HH.CHARGE_DATE       <= TRUNC(W_WORK_DATE_TO) 
                                   AND S_HL.PERSON_ID         = HL.PERSON_ID
                                 GROUP BY S_HL.PERSON_ID
                               )                          
                     ) T1                     
              ...
              WHERE PM.CORP_ID       =  CM.CORP_ID
                ...
                AND ((W_WORK_CORP_ID IS NULL AND PM.WORK_CORP_ID = 65)  
                OR   (W_WORK_CORP_ID IS NOT NULL AND PM.WORK_CORP_ID = W_WORK_CORP_ID))  
                AND ((W_CORP_ID      IS NULL AND 1 = 1)
                OR   (W_CORP_ID      IS NOT NULL AND PM.CORP_ID = W_CORP_ID)) 
                AND PM.SOB_ID        =  W_SOB_ID
                AND PM.ORG_ID        =  W_ORG_ID
