CREATE OR REPLACE PACKAGE HRA_YEAR_ADJUST_SET_G_2014 AS
  /*================================================================================/
       ==> 연말정산 처리(2008. 12. 25)
         0. 연말정산 처리 기초자료 생성;
...
-- 14-9. 특별공제 종합한도 초과액;
  ...
  FUNCTION TAX_REDU_SMALL_BUSINESS_CAL
            ( O_STATUS            OUT VARCHAR2
            , O_MESSAGE           OUT VARCHAR2
            , P_YEAR_YYYY         IN VARCHAR2
            , P_PERSON_ID         IN NUMBER
...
END HRA_YEAR_ADJUST_SET_G_2014;
/
CREATE OR REPLACE PACKAGE BODY HRA_YEAR_ADJUST_SET_G_2014 AS
  /*================================================================================/
...
---------------------------------------------------------------------------------------------------  
    ...
    V_TAX_REDU_SMALL_BUSINESS     NUMBER;  -- 세액감면 - 중소기업 취업 청년  
...
-------------------------------------------------------------------------------------------------------------------------------                                                                                            
      ...
      -- 16.4 세액감면 - 중소기업취업청년 감면소득;
      V_TAX_REDU_SMALL_BUSINESS := TAX_REDU_SMALL_BUSINESS_CAL
                                      ( O_STATUS        => O_STATUS
                                      , O_MESSAGE       => O_MESSAGE
                                      , P_YEAR_YYYY     => P_YEAR_YYYY
                                      , P_PERSON_ID     => C1.PERSON_ID
        ...
        BEGIN
          ...
          SELECT SUM(NVL(PW.RD_SMALL_BUSINESS_AMT, 0)) AS RD_SMALL_BUSINESS_AMT 
           ...
           WHERE PW.YEAR_YYYY         = P_YEAR_YYYY 
             ...
             AND PW.SOB_ID            = C1.SOB_ID 
             AND PW.ORG_ID            = C1.ORG_ID 
          ;      
        EXCEPTION
          WHEN OTHERS THEN
            V_TEMP_AMT := 0;
        END;
        V_TEMP_RATE := 1 - (NVL(V_TEMP_AMT, 0) / NVL(V_TOTAL_PAY, 0));
      END IF;
...
-- 14-9. 특별공제 종합한도 초과액;
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        V_TEMP_AMT := 0;
    END;
    V_SMALL_BUSINESS_AMT := NVL(V_SMALL_BUSINESS_AMT, 0) + NVL(V_TEMP_AMT, 0);
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      RETURN 0;    
  END TAX_REDU_SMALL_BUSINESS_CAL;
  ...
  BEGIN
    ...
    FOR C1 IN (SELECT T1.YEAR_YYYY
                    ...
                    , T1.IN_TAX_RATE_B
                    , T1.IN_TAX_BASE_B
                    , T1.IN_TAX_STD_B_OVER
                    , CASE
                        WHEN P_TOTAL_PAY BETWEEN 55000001 AND 70000000 THEN CASE WHEN NVL(T1.IN_TAX_LMT, 0) < 630000 THEN 630000 ELSE NVL(T1.IN_TAX_LMT, 0) END 
                        WHEN P_TOTAL_PAY > 70000000 THEN CASE WHEN NVL(T1.IN_TAX_LMT, 0) < 500000 THEN 500000 ELSE NVL(T1.IN_TAX_LMT, 0) END  
                        ELSE 660000 
                      END AS IN_TAX_LMT
                FROM ( SELECT HIT.YEAR_YYYY
                            ...
                            , 500000 AS IN_TAX_STD_B_OVER
                            , NVL(HIT.IN_TAX_STD_C, 0) IN_TAX_STD_C
                            , CASE
                                WHEN P_TOTAL_PAY BETWEEN 55000001 AND 70000000 THEN 660000
                                WHEN P_TOTAL_PAY > 70000000 THEN 630000 
                                ELSE 660000 
                              END -
                              ( CASE
                                  WHEN P_TOTAL_PAY BETWEEN 55000001 AND 70000000 THEN NVL(P_TOTAL_PAY, 0) - 55000000 
                                  WHEN P_TOTAL_PAY > 70000000 THEN NVL(P_TOTAL_PAY, 0) - 70000000 
                                  ELSE 0 
                                END * ( 1 / 2)) AS IN_TAX_LMT
    ...
    LOOP
      ...
      ELSIF NVL(C1.IN_TAX_STD_B, 0) <= NVL(P_COMP_TAX_AMT, 0) THEN
        V_TAX_DED_AMT := TRUNC((NVL(P_COMP_TAX_AMT, 0) - NVL(C1.IN_TAX_STD_B_OVER, 0)) * (NVL(C1.IN_TAX_RATE_B, 0) / 100));
