...
-- DATA UPDATE[2011-11-10]
   ...
   BEGIN
             ...
             BEGIN
                  -- 전일/금일 근무계획 조회.
                  SELECT MAX(DECODE(WC.WORK_DATE, W_WORK_DATE - 1, WC.HOLY_TYPE, '2'))    AS PRE_HOLY_TYPE
                       , MAX(DECODE(WC.WORK_DATE, W_WORK_DATE - 1, WC.DANGJIK_YN, 'N'))   AS PRE_DANGJIK_YN
                       , MAX(DECODE(WC.WORK_DATE, W_WORK_DATE - 1, WC.ALL_NIGHT_YN, 'N')) AS PRE_ALL_NIGHT_YN
                       , MAX(DECODE(WC.WORK_DATE, W_WORK_DATE, WC.HOLY_TYPE, '2'))        AS HOLY_TYPE
                       , MAX(DECODE(WC.WORK_DATE, W_WORK_DATE, WC.DANGJIK_YN, 'N'))       AS DANGJIK_YN
                       , MAX(DECODE(WC.WORK_DATE, W_WORK_DATE, WC.ALL_NIGHT_YN, 'N'))     AS ALL_NIGHT_YN
              ...
              SELECT
                     CASE
                         ...
                         CASE
                              ...
                              WHEN V_OPEN_TIME_CURRENT IS NOT NULL THEN V_PH_DUTY_ID -- 휴일근무
                              ELSE V_H_DUTY_ID                                                                                       -- 휴일
                         END
                     ELSE                                                                                                  -- 주간/야간
                        CASE
                            ...
                            WHEN V_OPEN_TIME_CURRENT IS NOT NULL THEN V_A_DUTY_ID -- 출근
                            ELSE V_NA_DUTY_ID                                                                                       -- 출근기록 없음
                        END
                     END AS DUTY_ID
                INTO V_DUTY_ID
                    ...
                    CASE
                        WHEN V_HOLY_TYPE IN('0', '1') THEN
                            CASE
                                ...
                                WHEN V_CLOSE_TIME_CURRENT IS NOT NULL THEN V_PH_DUTY_ID
                                ELSE V_H_DUTY_ID                                                                                       -- 휴일
                            END
                        ELSE
                            CASE
       ...
       BEGIN
                 ...
                 SELECT DI.WORK_DATE
                      ...
                      , CASE
                             ...
                             WHEN DI.HOLY_TYPE IN ('0', '1') AND DI.DUTY_ID = 1187 -- 휴일근무
                                                             AND DECODE(DI.MODIFY_IN_YN, 'Y', I_DM.MODIFY_TIME, DI.OPEN_TIME) > TO_DATE(TO_CHAR(DI.OPEN_TIME, 'YYYY-MM-DD') || ' ' || '17:30:00', 'YYYY-MM-DD HH24:MI:SS')
                                                            THEN DECODE(DI.MODIFY_OUT_YN
                                                                       , 'Y', O_DM.MODIFY_TIME
                                                                       , (SELECT S_DI.CLOSE_TIME
                                                                            FROM HRD_DAY_INTERFACE     S_DI
  ...
  BEGIN
    ...
    BEGIN
      ...
      WHERE DI.PERSON_ID            = PM.PERSON_ID
        ...
        AND DI.PERSON_ID            = W_PERSON_ID
        AND DI.WORK_DATE            = W_WORK_DATE
        AND DI.WORK_CORP_ID         = W_WORK_CORP_ID -- [2011-06-28] 수정 AND DI.CORP_ID  = W_CORP_ID
        AND DI.SOB_ID               = W_SOB_ID
        AND DI.ORG_ID               = W_ORG_ID
      ;
    EXCEPTION WHEN OTHERS THEN
