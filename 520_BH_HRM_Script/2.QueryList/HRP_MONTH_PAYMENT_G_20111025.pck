    ...
    OPEN P_CURSOR1 FOR
      SELECT  CASE
          ...
          , CASE
           ...
           , SUM(MA.A04) AS A04               -- 영업활동비.
           , SUM(MA.A05) AS A05               -- 사용X
           , SUM(MA.A06) AS A06               -- 자격수당X.
           , SUM(NVL(MA.A07, 0) + NVL(MA.A20, 0) + NVL(MA.A24, 0)) AS A07               -- 기타수당.
           ...
           , 0 /*SUM(MD.D11)*/ AS D11                -- 피복비.
           , 0 /*SUM(MD.D12)*/ AS D12                -- 사원증발급비.
           , SUM(MD.D13) AS D13                -- 가압류공제.
           , SUM(NVL(MD.D14, 0) + NVL(MD.D10, 0) + NVL(MD.D11, 0) + NVL(MD.D12, 0)) AS D14  -- 기타수당(전월정산액 + 기타수당).
        ...
        FROM HRP_MONTH_PAYMENT MP
          ...
          , (SELECT MT.PERSON_ID
                 ...
                 , CASE
                     ...
                     ELSE NVL(MTO.NIGHT_BONUS_TIME, 0) + NVL(MTO.HOLY_0_NIGHT, 0)
  ...
  BEGIN
    ...
    OPEN P_CURSOR1 FOR
      SELECT CASE
           ...
           , SUM(MA.A12) AS A12               -- 연장근로수당.
           , SUM(MA.A13) AS A13               -- 야간근로수당.
           , SUM(NVL(MA.A14, 0) + NVL(MA.A28, 0)) AS A14               -- 특근수당.
           ...
           , SUM(MD.D16) AS D16                -- 정산주민세.
           , SUM(MD.D17) AS D17                -- 정산농특세.
           , SUM(MD.D18) AS D18                -- 사우회비.
           , SUM(MD.D19) AS D19                -- 경조사비.
           , SUM(MD.D20) AS D20                -- 카드대.
           , SUM(MD.D21) AS D21                -- 동호회비.
           , SUM(MD.D22) AS D22
           , SUM(MD.D23) AS D23
           , SUM(MD.D24) AS D24
           , SUM(MD.D25) AS D25
