      ...
      FOR C1 IN ( SELECT PM.PERSON_ID 
                       ...
                       , NVL(S_MT.PAY_DAY, 0) + 
                         CASE 
                           ...
                           ELSE 0
                         END AS PAY_DAY
                       , HF1.DED_FAMILY_COUNT +  -- 가족사항 세액공제.
                       ...
                       , CASE
                           WHEN NVL(PMH.DED_FAMILY_COUNT, 0) + NVL(PMH.DED_CHILD_COUNT, 0) > 0 
                             THEN PMH.DED_FAMILY_COUNT + (CASE WHEN NVL(PMH.DED_CHILD_COUNT, 0) > 0 THEN NVL(PMH.DED_CHILD_COUNT, 0) - 1 ELSE 0 END)
                           ELSE 1 + NVL(HF1.DED_PERSON_COUNT, 0)
...
-- 2.2 급여 기본급 생성
  ...
  BEGIN 
    ...
    LOOP      
      ...
      ELSIF P_PAY_LEVEL = 'DAILY' THEN
      -- 일급--
        V_ALLOWANCE_AMOUNT := (NVL(R1.PAY_DAY, 0) - NVL(R1.HOLY_1_COUNT, 0)) * NVL(R1.ALLOWANCE_AMOUNT, 0);
        V_HOLIDAY_ALLOWANCE_AMOUNT := NVL(R1.HOLY_1_COUNT, 0) * NVL(R1.ALLOWANCE_AMOUNT, 0);
      ELSIF P_PAY_LEVEL = 'TIME' THEN
      -- 시급--
        V_ALLOWANCE_AMOUNT := (NVL(R1.PAY_DAY, 0) - NVL(R1.HOLY_1_COUNT, 0)) * NVL(R1.ALLOWANCE_AMOUNT, 0) * 8;
        V_HOLIDAY_ALLOWANCE_AMOUNT := NVL(R1.HOLY_1_COUNT, 0) * NVL(R1.ALLOWANCE_AMOUNT, 0) * 8;
      END IF;
    ...
    LOOP 
      ...
      -- 73 자기개발수당.
        SELF_DEV_ALLOWANCE
          ...
          , P_WAGE_TYPE         => P_WAGE_TYPE
          , P_ALLOWANCE_ID      => C1.ALLOWANCE_ID
          , P_AGE               => C1.FORMULA3    -- 나이.
          , P_SEX_TYPE          => C1.FORMULA4    -- 성별.
          , P_DEPT_ID           => P_DEPT_ID
          , P_PERSON_ID         => P_PERSON_ID
          , P_STD_DATE          => P_STD_DATE
          , P_EXCEPT_YN         => P_EXCEPT_YN
      ...
      -- 74 복지수당.
        WELFARE_ALLOWANCE
          ...
          , P_WAGE_TYPE         => P_WAGE_TYPE
          , P_ALLOWANCE_ID      => C1.ALLOWANCE_ID
          , P_WORKING_RATE      => C1.FORMULA3    -- 근무율.
          , P_LONG_MONTH        => C1.FORMULA4    -- 근속월수.
          , P_DEPT_ID           => P_DEPT_ID
          , P_PERSON_ID         => P_PERSON_ID
          , P_STD_DATE          => P_STD_DATE
          , P_EXCEPT_YN         => P_EXCEPT_YN
  ...
  BEGIN
    ...
    FOR C1 IN ( SELECT MP.MONTH_PAYMENT_ID
                 ...
                 WHERE MP.PAY_TYPE                = PT.PAY_TYPE
                   ...
                   AND (MP.EMPLOYE_TYPE            = '1'
                     OR MP.PAY_TYPE               IN('1', '3'))
  ...
  BEGIN
    ...
    LOOP
      IF C1.EXCEPT_TYPE IN ('I', 'R') OR NVL(P_TOTAL_DAY, 0) <> NVL(C1.TOTAL_DAY, 0) OR NVL(C1.TOTAL_DED_DAY, 0) > 0 THEN
        IF C1.PAY_TYPE IN ('1', '3') AND C1.EXCEPT_TYPE = 'N' AND C1.EMPLOYE_TYPE = '1' THEN
  ...
  BEGIN
    ...
    FOR R1 IN ( SELECT MA.ALLOWANCE_ID
                ...
                WHERE MA.ALLOWANCE_ID           = HA.ALLOWANCE_ID
                  ...
                  AND (HA.DAY_YN                = 'Y'  -- 일할계산 항목.
                  OR  HA.ALLOWANCE_CODE         = 'A09') -- 상여금.
               )
    LOOP 
...
-- 46.2 지급항목 일할계산
  ...
  BEGIN
    ...
    LOOP 
      ...
      ELSIF P_PAY_LEVEL = 'YEAR' THEN
      -- 연봉--
        V_ALLOWANCE_AMOUNT := (NVL(R1.ALLOWANCE_AMOUNT, 0) / P_TOTAL_DAY) * (NVL(P_PAY_DAY, 0) + NVL(P_HOLY_0_COUNT, 0));
      ELSIF P_PAY_LEVEL = 'MONTH' THEN
      -- 월급--
        V_ALLOWANCE_AMOUNT := (NVL(R1.ALLOWANCE_AMOUNT, 0) / P_TOTAL_DAY) * (NVL(P_PAY_DAY, 0) + NVL(P_HOLY_0_COUNT, 0));
      ELSIF P_PAY_LEVEL = 'DAILY' THEN
        ...
        SELECT NVL(MT.LATE_DED_COUNT, 0) + (NVL(MT.TOTAL_DED_DAY, 0) - NVL(MT.HOLY_0_COUNT, 0)) AS DED_COUNT
             , (SELECT SUM(MTO.OT_TIME) AS OT_TIME
                ...
                WHERE MTO.OT_TYPE               = OT.OT_TYPE
                  ...
                  AND OT.MONTH_TOTAL_ITEM       IN('O14', 'O15', 'O18')  -- 연장, 특근, 특근연장.
                ) AS OT_TIME
          INTO V_DED_COUNT, V_OT_TIME
          FROM HRD_MONTH_TOTAL MT
         WHERE MT.DUTY_TYPE           = C_DUTY_MONTH
  ...
  BEGIN
    ...
    LOOP
      ...
      -- 45세 이상 여자 제외.
        V_ALLOWANCE_AMOUNT := 0;
      END IF;
      IF C1.LONG_MONTH = 0 AND '15' < TO_CHAR(C1.ORI_JOIN_DATE, 'DD') THEN
        -- 15일 이전 입사자만 지급.
        V_ALLOWANCE_AMOUNT := 0;
      END IF;
      IF NVL(V_ALLOWANCE_AMOUNT, 0) <> 0 THEN
...
-- 74 복지수당 적용 여부 : 근무율 70%이상, 수습3개월 이상.
  ...
  BEGIN
    ...
    LOOP
      ...
      IF P_WORKING_RATE > V_WORKING_RATE OR P_LONG_MONTH > C1.LONG_MONTH THEN
      -- 근무율 70% 미만, 3개월 미만.
        NULL;
      ELSIF C1.LONG_MONTH = 0 AND '15' < TO_CHAR(C1.ORI_JOIN_DATE, 'DD') THEN
        -- 15일 이전 입사자만 지급.
        NULL;
      ELSE
        BEGIN
