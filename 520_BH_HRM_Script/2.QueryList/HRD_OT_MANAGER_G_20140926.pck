            ...
            UPDATE HRD_OT_MANAGER   OM
             ...
             WHERE OM.WORK_DATE     BETWEEN W_WORK_DATE_FR AND W_WORK_DATE_TO
               ...
               AND EXISTS
                     ( SELECT 'X'
                         FROM HRM_OT_TYPE_GW_V  OTG
                        WHERE OTG.OT_TYPE_ID        = OM.OT_TYPE_ID 
                          AND OTG.ALLOWANCE_CODE    = C1.ALLOWANCE_CODE
                          AND OTG.SOB_ID            = C1.SOB_ID
                          AND OTG.ORG_ID            = C1.ORG_ID 
                     )
            ;    
          EXCEPTION WHEN OTHERS THEN
            O_STATUS := 'F';
    ...
    ELSE
      ...
      ELSE 
        V_OT_START_TIME := HRD_DAY_LEAVE_G_SET.OT_START_TIME_F( V_OPEN_TIME
                                                              ...
                                                              , W_WORK_DATE + 0.75);
      END IF;
      -- 잔업종료시간 설정 : 휴일은 08:30 ~ 17:30만 잔업 적용 : 조선미J --
      IF W_HOLY_TYPE IN('0', '1') THEN
        IF (W_WORK_DATE + 0.729166666666667) < V_CLOSE_TIME THEN
          V_OT_END_TIME := (W_WORK_DATE + 0.729166666666667);
        ELSE
