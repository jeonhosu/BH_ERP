        ...
        FROM FI_AUTO_JOURNAL_MST AJM
          ...
          , ( -- 지급.
              ...
              WHERE PM.PERSON_ID          = MP.PERSON_ID
                ...
                AND MP.PAY_YYYYMM         != '2014-02'  -- 전년도 연말정산 금액때문에 예외 처리 -- 
                -- 중도 퇴사자 적용 여부 판단.
              GROUP BY MP.PAY_YYYYMM
                   , MP.WAGE_TYPE
                   , PM.SOB_ID
                   ...
                   , CASE 
                       ...
                       WHEN HD.DEDUCTION_CODE = 'D21' THEN 'S001815'  -- 서울보증보험(주).
                     END 
              -- 2014.03.07 전호수 추가 : 전년도 연말정산 금액때문에 예외 처리 --       
              UNION ALL
              -- 기타(보증보험료) 예수금.
              SELECT PM.SOB_ID
                   , PM.ORG_ID
              ...
              WHERE PM.PERSON_ID          = MP.PERSON_ID
                ...
                AND MP.SOB_ID             = P_SOB_ID
                AND MP.ORG_ID             = P_ORG_ID
                AND HD.DEDUCTION_CODE     IN('D21')
                
                AND MP.PAY_YYYYMM         = '2014-02'
                -- 중도 퇴사자 적용 여부 판단.
              GROUP BY MP.PAY_YYYYMM
                   , MP.WAGE_TYPE
                   , PM.SOB_ID
              ...
              SELECT T1.SOB_ID
                ...
                FROM( SELECT MP.SOB_ID
                           ...
                           , CASE 
                             ...
                             END AS VENDOR_CODE
                           , SUM(CASE 
                                   WHEN HD.DEDUCTION_CODE = 'D15' THEN NVL(MD.DEDUCTION_AMOUNT, 0)  -- 소득세(북인천세무서).
                                   WHEN HD.DEDUCTION_CODE = 'D16' THEN NVL(MD.DEDUCTION_AMOUNT, 0)  -- 주민세(부평구청).
                                   ELSE 0
                                 END) AS AMOUNT
                        FROM HRP_MONTH_PAYMENT MP
                           , HRP_MONTH_DEDUCTION MD
                       ...
                       WHERE MP.MONTH_PAYMENT_ID   = MD.MONTH_PAYMENT_ID
                         ...
                         AND MP.SOB_ID             = P_SOB_ID
                         AND MP.ORG_ID             = P_ORG_ID                
                         AND HD.DEDUCTION_CODE     IN('D15', 'D16')
