  ...
  BEGIN
    ...
    LOOP
      ...
      ELSIF C1.RETIRE_DATE IS NOT NULL AND C1.EXCEPT_TYPE = 'R' AND '15' > TO_CHAR(C1.RETIRE_DATE, 'DD') THEN
        -- 15일 이전 퇴사자만 제외.
        V_ALLOWANCE_AMOUNT := 0;
      ELSIF C1.LONG_YEAR < 1 AND C1.JOIN_ROUTE_CODE != '50' THEN
...
-- 75 검사수당 적용 여부
  ...
  BEGIN
    FOR C1 IN ( SELECT MP.MONTH_PAYMENT_ID
                     ...
                     , MP.PAY_TYPE
                     , MP.LONG_MONTH
                     , HRM_COMMON_G.GET_CODE_F(NVL(T1.FLOOR_ID, PM.FLOOR_ID), MP.SOB_ID, MP.ORG_ID) AS FLOOR_CODE
                     , MP.SOB_ID
                     , MP.ORG_ID
                     , MP.EXCEPT_TYPE
                     , PM.ORI_JOIN_DATE
    ...
    LOOP
      ...
      IF NVL(V_LONG_MONTH, 0) > C1.LONG_MONTH THEN
        -- 3개월 이전 입사자만 미지급.
        V_ALLOWANCE_AMOUNT := 0;
      ELSIF NVL(V_LONG_MONTH, 0) = C1.LONG_MONTH AND '15' < TO_CHAR(NVL(C1.CHARGE_DATE, C1.ORI_JOIN_DATE), 'DD') THEN
