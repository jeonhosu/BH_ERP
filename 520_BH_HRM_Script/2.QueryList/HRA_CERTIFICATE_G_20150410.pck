  ...
  BEGIN
    OPEN P_CURSOR FOR
     SELECT HSF1.YEAR_YYYY,
              ...
              --2014--
              --2014--, YA.MEDIC_INSUR_AMT
              CASE 
                WHEN NVL(HSF1.RELATION_CODE, -1) = '0' THEN DECODE((NVL(YA.MEDIC_INSUR_AMT, 0) + NVL(YA.HIRE_INSUR_AMT, 0)), 0, NULL, 
                                                      (NVL(YA.MEDIC_INSUR_AMT, 0) + NVL(YA.HIRE_INSUR_AMT, 0)))          -- 국세청-보험료 
                                                       
                ELSE NULL
              END MEDIC_HIRE_INSUR_AMT
              
         FROM HRM_PERSON_MASTER_V PM
            , HRA_YEAR_ADJUSTMENT YA
            , HRA_SUPPORT_FAMILY_V HSF1          
...
--    raise_application_error(-20001, W_CORP_ID || '/' || W_PERSON_ID || ' / ' || W_YEAR_YYYY);
     ...
     OPEN P_CURSOR FOR
        SELECT YA.YEAR_YYYY
             ...
             , CASE
                 WHEN PM.RETIRE_DATE IS NULL THEN '계속근로'
                 WHEN PM.RETIRE_DATE IS NOT NULL AND TO_DATE(YA.YEAR_YYYY || '-12-31', 'YYYY-MM-DD') < PM.RETIRE_DATE THEN '계속근로'
                 ELSE '중도퇴사'
               END AS WORK_KEEP_TYPE                                -- 연말정산구분 : 계속근로1, 중도퇴사2.
             -----------------------------------------------------------------------------------------------------------------------------------
             -- 2. 징수 의무자
             ...
             -----------------------------------------------------------------------------------------------------------------------------------
             -- 7. 근무처별 소득명세 : 종(전)3 근무지.
             -----------------------------------------------------------------------------------------------------------------------------------
             , CASE
                 WHEN PW3.COMPANY_NAME IS NOT NULL THEN '종(전)'
                 ELSE '(16)-1납세조합'
               END AS PW_COMPANY_EXIST3                                                                                          -- 종전3
             , PW3.COMPANY_NAME AS PW_COMPANY_NAME3                                                                           -- 근무처명.
             , PW3.COMPANY_NUM AS PW_COMPANY_NUM3                                                                             -- 사업자등록번호.
             , PW3.JOIN_DATE || '~' || PW2.RETR_DATE AS ADJUST_DATE3                                                          -- 근무기간.
             , NULL AS REDUCE_DATE3                                                                                           -- 감면기간.
             , DECODE(PW3.PAY_TOTAL_AMT, 0, NULL, PW3.PAY_TOTAL_AMT) AS PAY_TOTAL_AMT3                                        -- 급    여.
             , DECODE(PW3.BONUS_TOTAL_AMT, 0, NULL, PW3.BONUS_TOTAL_AMT) AS BONUS_TOTAL_AMT3                                  -- 상    여.
             , DECODE(PW3.ADD_BONUS_AMT, 0, NULL, PW3.ADD_BONUS_AMT) AS ADD_BONUS_AMT3                                        -- 인정상여.
             , DECODE(PW3.STOCK_BENE_AMT, 0, NULL, PW3.STOCK_BENE_AMT) AS STOCK_BENE_AMT3                                     -- 주식매수선택권 행사이익.
             , DECODE(PW3.EMPLOYEE_STOCK_AMT, 0, NULL, PW3.EMPLOYEE_STOCK_AMT) AS EMPLOYEE_STOCK_AMT3                         -- 우리사주조합인출금.
             , DECODE(PW3.OFFICE_RETIRE_OVER_AMT, 0, NULL, PW3.OFFICE_RETIRE_OVER_AMT) AS OFFICE_RETIRE_OVER_AMT3             -- 임원퇴직소득금액 한도초과액
             , DECODE((NVL(PW3.PAY_TOTAL_AMT, 0) + NVL(PW3.BONUS_TOTAL_AMT, 0) + NVL(PW3.ADD_BONUS_AMT, 0) + NVL(PW3.STOCK_BENE_AMT, 0)), 0, NULL,
               (NVL(PW3.PAY_TOTAL_AMT, 0) + NVL(PW3.BONUS_TOTAL_AMT, 0) + NVL(PW3.ADD_BONUS_AMT, 0) + NVL(PW3.STOCK_BENE_AMT, 0))) AS TOTAL_AMOUNT3
                                                                                                     -- 계.
             ...
             -----------------------------------------------------------------------------------------------------------------------------------
             -- 11. 비과세 및 감면소득 명세 : 종(전)3.
             -----------------------------------------------------------------------------------------------------------------------------------
             , DECODE(PW3.NT_OUTSIDE_AMT,0,NULL,PW3.NT_OUTSIDE_AMT) AS NT_OUTSIDE_AMT3                   -- 국외근로.
             , DECODE(PW3.NT_OT_AMT,0,NULL,PW3.NT_OT_AMT) AS NT_OT_AMT3                                  -- 야간근로.
             , DECODE(PW3.NT_BIRTH_AMT,0,NULL,PW3.NT_BIRTH_AMT) AS NT_BIRTH_AMT3                         -- 출생/보육수당.
             , DECODE(PW3.NT_COMPANY_AMT,0,NULL,PW3.NT_COMPANY_AMT) AS NONTAX_COMPANY_AMT3               -- 연구보조비

             , NULL AS NONTAX_TRAIN_AMT3                                                                 -- 수련보조수당
             , DECODE((NVL(PW3.NT_OUTSIDE_AMT, 0) + NVL(PW3.NT_OT_AMT, 0) + NVL(PW3.NT_BIRTH_AMT, 0) + NVL(PW3.NT_COMPANY_AMT, 0)),0,NULL,
               (NVL(PW3.NT_OUTSIDE_AMT, 0) + NVL(PW3.NT_OT_AMT, 0) + NVL(PW3.NT_BIRTH_AMT, 0) + NVL(PW3.NT_COMPANY_AMT, 0))) AS NT_TOTAL_AMOUNT3
                                                                                                          -- 비과세소득 계.
             , NULL AS REDUCE_TOTAL_AMOUNT3                                                              -- 감면소득 계.
             ...
             -----------------------------------------------------------------------------------------------------------------------------------
             -- 15. 세액명세 : 기납부 세액 - 종(전)3 근무지.
             -----------------------------------------------------------------------------------------------------------------------------------
             , PW3.COMPANY_NUM AS PW3_COMPANY_NUM3                                                                       -- 사업자등록번호.
             , DECODE(PW3.IN_TAX_AMT,0,NULL,PW3.IN_TAX_AMT) AS PW3_IN_TAX_AMT3                                           -- 소득세.
             , DECODE(PW3.LOCAL_TAX_AMT,0,NULL,PW3.LOCAL_TAX_AMT) AS PW3_LOCAL_TAX_AMT3                                  -- 주민세.
             , DECODE(PW3.SP_TAX_AMT,0,NULL,PW3.SP_TAX_AMT) AS PW3_SP_TAX_AMT3                                           -- 농특세.
             ...
             /*DECODE(YA.STAND_DED_AMT, 0, NULL, YA.STAND_DED_AMT) AS STAND_DED_AMT,   */                                           -- 표준공제
               ...
               CASE
                 WHEN W_YEAR_YYYY = '2013' THEN YA.STAND_DED_AMT
                 ELSE YA.TD_STAND_DED_AMT
               END AS STAND_DED_AMT,  
            ...
            NVL(YA.FIX_IN_TAX_AMT, 0) AS RESULT_TAX_SUM,                                                                     -- 결정세액                                                                                                            
             ...
             -----------------------------------------------------------------------------------------------------------------------------------  
             , CASE
                 WHEN PW1.RD_SMALL_BUSINESS_AMT != '0' THEN '중소기업 청년 소득세 감면'
                 WHEN PW2.RD_SMALL_BUSINESS_AMT != '0' THEN '중소기업 청년 소득세 감면'
                 WHEN PW3.RD_SMALL_BUSINESS_AMT != '0' THEN '중소기업 청년 소득세 감면'
                 ELSE '~'
               END AS RD_SMALL_BUSINESS_AMT_EXIST                                                          -- 중소기업에 취업하는 청년에 대한 소득세 감면 존재
             , DECODE(PW1.RD_SMALL_BUSINESS_AMT,0,NULL,PW1.RD_SMALL_BUSINESS_AMT) AS RD_SMALL_BUSINESS_AMT1  -- 중소기업에 취업하는 청년에 대한 소득세 감면1
             , DECODE(PW2.RD_SMALL_BUSINESS_AMT,0,NULL,PW2.RD_SMALL_BUSINESS_AMT) AS RD_SMALL_BUSINESS_AMT2  -- 중소기업에 취업하는 청년에 대한 소득세 감면2
             , DECODE(PW3.RD_SMALL_BUSINESS_AMT,0,NULL,PW3.RD_SMALL_BUSINESS_AMT) AS RD_SMALL_BUSINESS_AMT3  -- 중소기업에 취업하는 청년에 대한 소득세 감면3
            ...
            , DECODE((NVL(YA.DONAT_DED_ALL, 0) + NVL(YA.DONAT_NEXT_50, 0) + NVL(YA.DONAT_DED_RELIGION_10, 0) + NVL(YA.DONAT_DED_10, 0)) , 0, NULL, 
                     (NVL(YA.DONAT_DED_ALL, 0) + NVL(YA.DONAT_NEXT_50, 0) + NVL(YA.DONAT_DED_RELIGION_10, 0) + NVL(YA.DONAT_DED_10, 0))) AS FORWARD_DONATION_AMT              -- 2014-특별소득공제(기부금이월분)
            , DECODE(YA.DONAT_DED_30, 0, NULL, YA.DONAT_DED_30) AS COM_STOCK_DONATION_AMT                            -- 2014-그밖의소득공제(우리사주조합기부금)
            , DECODE(YA.INVEST_AMT_14, 0, NULL, YA.INVEST_AMT_14) AS INVEST_AMT_14                                                       -- 2014-투자조합출자금액  14년
            , DECODE(YA.LONG_SET_INVEST_SAVING_AMT, 0, NULL, YA.LONG_SET_INVEST_SAVING_AMT) AS LONG_SET_INVEST_SAVING_AMT                 -- 2014- 장기집합투자증권저축
            
            , DECODE(YA.TAX_REDU_SMALL_BUSINESS, 0, NULL, YA.TAX_REDU_SMALL_BUSINESS) AS TAX_REDU_SMALL_SP_LAW_AMT                    -- 2014-세감 ( 중소기업취업청년)
            , DECODE(YA.TD_CHILD_RAISE_DED_AMT, 0, NULL, YA.TD_CHILD_RAISE_DED_AMT) AS TAX_DED_CHILD_RAISE_DED                            -- 2014-세공(자녀양육)
             
            , DECODE((NVL(YA.TD_SCIENTIST_ANNU_DED_AMT, 0) + NVL(YA.TD_WORKER_RETR_ANNU_DED_AMT, 0) + NVL(YA.TD_ANNU_BANK_DED_AMT, 0)) , 0, NULL, 
                     (NVL(YA.TD_SCIENTIST_ANNU_DED_AMT, 0) + NVL(YA.TD_WORKER_RETR_ANNU_DED_AMT, 0) + NVL(YA.TD_ANNU_BANK_DED_AMT, 0))) AS TAX_DED_RETR_ANNU_AMT                                -- 2014-세감 ( 연금계좌)
            
            , DECODE(YA.TD_SCIENTIST_ANNU_AMT, 0, NULL, YA.TD_SCIENTIST_ANNU_AMT) AS SEMA_SAVING_DED_AMOUNT                               -- 2014-과학기술인 공제금액
            , DECODE(YA.TD_SCIENTIST_ANNU_DED_AMT, 0, NULL, YA.TD_SCIENTIST_ANNU_DED_AMT) AS SEMA_SAVING_REAL_DED_AMT                     -- 2014-과학기술인 세액공제
            , DECODE(YA.TD_WORKER_RETR_ANNU_AMT, 0, NULL, YA.TD_WORKER_RETR_ANNU_AMT) AS RETR_SAVING_DED_AMOUNT                           -- 2014-퇴직연금  공제금액
            , DECODE(YA.TD_WORKER_RETR_ANNU_DED_AMT, 0, NULL, YA.TD_WORKER_RETR_ANNU_DED_AMT) AS RETR_SAVING_REAL_DED_AMT                 -- 2014-퇴직연금 세액공제
            , DECODE(YA.TD_ANNU_BANK_AMT, 0, NULL, YA.TD_ANNU_BANK_AMT) AS ANNU_SAVING_DED_AMOUNT                                        -- 2014-연금저축 공제금액
            , DECODE(YA.TD_ANNU_BANK_DED_AMT, 0, NULL, YA.TD_ANNU_BANK_DED_AMT) AS ANNU_SAVING_REAL_DED_AMT                              -- 2014-연금저축 세액공제
            
            , DECODE(YA.TD_GUAR_INSUR_AMT, 0, NULL, YA.TD_GUAR_INSUR_AMT) AS TAX_DED_INSUR_TAR_AMT                                       -- 2014-세감 ( 보장성보험 공제금액)
            , DECODE(YA.TD_GUAR_INSUR_DED_AMT, 0, NULL, YA.TD_GUAR_INSUR_DED_AMT) AS TAX_DED_INSUR_AMT                                    -- 2014-세감 ( 보장성보험 세액공제)
            
            , DECODE(YA.TD_MEDIC_AMT, 0, NULL, YA.TD_MEDIC_AMT) AS TAX_DED_MEDIC_TAR_AMT                                                  -- 2014-세감 (의료비 공제금액)
            , DECODE(YA.TD_MEDIC_DED_AMT, 0, NULL, YA.TD_MEDIC_DED_AMT) AS TAX_DED_MEDIC_AMT                                             -- 2014-세감 (의료비 세액공제)
            
            , DECODE(YA.TD_EDUCATION_AMT, 0, NULL, YA.TD_EDUCATION_AMT) AS TAX_DED_EDUCATION_TAR_AMT                                      -- 2014-세감 (교육비 공제금액)
            , DECODE(YA.TD_EDUCATION_DED_AMT, 0, NULL, YA.TD_EDUCATION_DED_AMT) AS TAX_DED_EDUCATION_AMT                                 -- 2014-세감 (교육비 세액공제) 
            
            , DECODE(YA.TD_POLI_DONAT_AMT1, 0, NULL, YA.TD_POLI_DONAT_AMT1) AS TAX_DED_POLI_DONAT1                                       -- 2014-세감 (정치자금기부금-10만원이하) 공제금액
            , DECODE(YA.TD_POLI_DONAT_DED_AMT1, 0, NULL, YA.TD_POLI_DONAT_DED_AMT1) AS TAX_DED_POLI_AMT1                                             -- 2014-세감 (정치자금기부금-10만원이하) 세액공제
            , DECODE(YA.TD_POLI_DONAT_AMT2, 0, NULL, YA.TD_POLI_DONAT_AMT2) AS TAX_DED_POLI_DONAT2                                       -- 2014-세감 (정치자금기부금-10만원초과) 공제금액
            , DECODE(YA.TD_POLI_DONAT_DED_AMT2, 0, NULL, YA.TD_POLI_DONAT_DED_AMT2) AS TAX_DED_POLI_AMT2                                             -- 2014-세감 (정치자금기부금-10만원이하) 세액공제
            , DECODE(YA.TD_LEGAL_DONAT_AMT, 0, NULL, YA.TD_LEGAL_DONAT_AMT) AS TAX_DED_LEGAL_DONAT                                       -- 2014-세감 (법정기부금) 공제금액
            , DECODE(YA.TD_LEGAL_DONAT_DED_AMT, 0, NULL, YA.TD_LEGAL_DONAT_DED_AMT) AS TAX_DED_LEGAL_AMT                                             -- 2014-세감 (법정기부금) 세액공제
            , DECODE(YA.TD_DESIGN_DONAT_AMT, 0, NULL, YA.TD_DESIGN_DONAT_AMT) AS TAX_DED_DESIGN_DONAT                                    -- 2014-세감 (지정기부금) 공제금액
            , DECODE(YA.TD_DESIGN_DONAT_DED_AMT, 0, NULL, YA.TD_DESIGN_DONAT_DED_AMT) AS TAX_DED_DESIGN_AMT                                          -- 2014-세감 (정치자금기부금-10만원이하) 세액공제
            
            , DECODE((NVL(YA.PERS_ANNU_BANK_AMT, 0) + NVL(YA.SMALL_CORPOR_DED_AMT, 0) + NVL(YA.HOUSE_APP_SAVE_AMT, 0) + 
                      NVL(YA.HOUSE_APP_DEPOSIT_AMT, 0) + NVL(YA.WORKER_HOUSE_SAVE_AMT, 0) + NVL(YA.INVES_AMT, 0) + NVL(YA.CREDIT_AMT, 0) + 
                      NVL(YA.EMPL_STOCK_AMT, 0) + NVL(YA.DONAT_DED_30, 0) + NVL(YA.HIRE_KEEP_EMPLOY_AMT, 0) + 
                      NVL(YA.FIX_LEASE_DED_AMT, 0) + NVL(YA.LONG_SET_INVEST_SAVING_AMT , 0))
                      , 0, NULL,
                      (NVL(YA.PERS_ANNU_BANK_AMT, 0) + NVL(YA.SMALL_CORPOR_DED_AMT, 0) + NVL(YA.HOUSE_APP_SAVE_AMT, 0) + 
                      NVL(YA.HOUSE_APP_DEPOSIT_AMT, 0) + NVL(YA.WORKER_HOUSE_SAVE_AMT, 0) + NVL(YA.INVES_AMT, 0) + NVL(YA.CREDIT_AMT, 0) + 
                      NVL(YA.EMPL_STOCK_AMT, 0) + NVL(YA.DONAT_DED_30, 0) + NVL(YA.HIRE_KEEP_EMPLOY_AMT, 0) + 
                      NVL(YA.FIX_LEASE_DED_AMT, 0) + NVL(YA.LONG_SET_INVEST_SAVING_AMT , 0))             
                      ) AS ETC_DED_SUM_2014                                                                                              -- 2014 그 밖의 소득공제 계  
            ...
            , ((CASE
               ...
               END) + 
               NVL(YA.HOUSE_FUND_AMT, 0) + -- 주택자금(주택입차차입금 + 월세액 + 장기주택저당차입금 + 주택저축)  
               NVL(YA.DONAT_DED_ALL, 0) + NVL(YA.DONAT_NEXT_50, 0) + NVL(YA.DONAT_NEXT_30, 0) + NVL(YA.DONAT_NEXT_RELIGION_10, 0) + NVL(YA.DONAT_NEXT_10, 0)-- 기부금 이월액
               ) AS SP_DED_SUM_2014 
                                                                                                                                          -- 2014-특별소득공제 합계 --   
            , DECODE((NVL(YA.TD_GUAR_INSUR_DED_AMT, 0) + NVL(YA.TD_MEDIC_DED_AMT, 0) + NVL(YA.TD_EDUCATION_DED_AMT, 0) 
                     + NVL(YA.TD_POLI_DONAT_DED_AMT1, 0) + NVL(YA.TD_POLI_DONAT_DED_AMT2, 0) + NVL(YA.TD_LEGAL_DONAT_DED_AMT, 0) + NVL(YA.TD_DESIGN_DONAT_DED_AMT, 0))
                     , 0, NULL, 
                     (NVL(YA.TD_GUAR_INSUR_DED_AMT, 0) + NVL(YA.TD_MEDIC_DED_AMT, 0) + NVL(YA.TD_EDUCATION_DED_AMT, 0) 
                     + NVL(YA.TD_POLI_DONAT_DED_AMT1, 0) + NVL(YA.TD_POLI_DONAT_DED_AMT2, 0) + NVL(YA.TD_LEGAL_DONAT_DED_AMT, 0) + NVL(YA.TD_DESIGN_DONAT_DED_AMT, 0))) AS TAX_DED_SPC_SUM_2014
                                                                                                                                          -- 2014-특별세액공제 계   
            , DECODE((NVL(YA.TAX_DED_INCOME_AMT, 0) + NVL(YA.TD_CHILD_RAISE_DED_AMT, 0) + NVL(YA.TD_SCIENTIST_ANNU_DED_AMT, 0) + NVL(YA.TD_WORKER_RETR_ANNU_DED_AMT, 0) + NVL(YA.TD_ANNU_BANK_DED_AMT, 0) 
                     + NVL(YA.TD_GUAR_INSUR_DED_AMT, 0) + NVL(YA.TD_MEDIC_DED_AMT, 0) + NVL(YA.TD_EDUCATION_DED_AMT, 0) 
                     + NVL(YA.TD_POLI_DONAT_DED_AMT1, 0) + NVL(YA.TD_POLI_DONAT_DED_AMT2, 0) + NVL(YA.TD_LEGAL_DONAT_DED_AMT, 0) + NVL(YA.TD_DESIGN_DONAT_DED_AMT, 0) + NVL(YA.TD_HOUSE_MONTHLY_DED_AMT, 0))
                     , 0, NULL, 
                     (NVL(YA.TAX_DED_INCOME_AMT, 0) + NVL(YA.TD_CHILD_RAISE_DED_AMT, 0) + NVL(YA.TD_SCIENTIST_ANNU_DED_AMT, 0) + NVL(YA.TD_WORKER_RETR_ANNU_DED_AMT, 0) + NVL(YA.TD_ANNU_BANK_DED_AMT, 0) 
                     + NVL(YA.TD_GUAR_INSUR_DED_AMT, 0) + NVL(YA.TD_MEDIC_DED_AMT, 0) + NVL(YA.TD_EDUCATION_DED_AMT, 0) 
                     + NVL(YA.TD_POLI_DONAT_DED_AMT1, 0) + NVL(YA.TD_POLI_DONAT_DED_AMT2, 0) + NVL(YA.TD_LEGAL_DONAT_DED_AMT, 0) + NVL(YA.TD_DESIGN_DONAT_DED_AMT, 0) + NVL(YA.TD_HOUSE_MONTHLY_DED_AMT, 0))) AS TAX_DED_SUM_2014
                                                                                                                                           -- 2014-세액공제 계    
               
            , DECODE(YA.TD_HOUSE_MONTHLY_AMT, 0, NULL, YA.TD_HOUSE_MONTHLY_AMT) AS TD_HOUSE_MONTHLY_AMT                                          -- 2014 세공 월세액 공제 대상금액
            , DECODE(YA.TD_HOUSE_MONTHLY_DED_AMT, 0, NULL, YA.TD_HOUSE_MONTHLY_DED_AMT) AS TD_HOUSE_MONTHLY_DED_AMT                              -- 2014 세공 월세액 세액공제액
               
            , PM.NAME AS RECIPIENT_PERSON_NAME 
            , HOU.TAX_OFFICE_NAME AS RECIPIENT_TAX_OFFICE_NAME
            , '' AS RECIPIENT_ETC_NAME     
         ...
         FROM HRM_PERSON_MASTER_V PM
            ...
            , ------------------------------------------------------------------ 
              ...
              ( SELECT PW.YEAR_YYYY
                     ...
                     , PW.NT_COMPANY_AMT
                     , PW.RD_SMALL_BUSINESS_AMT              -- 중소기업에 취업하는 청년에 대한 소득세 감면 
                  FROM HRA_PREVIOUS_WORK PW
                WHERE PW.YEAR_YYYY        = W_YEAR_YYYY
                  AND PW.SOB_ID           = W_SOB_ID
                  AND PW.ORG_ID           = W_ORG_ID
            ...
            , ( SELECT HS.YEAR_YYYY
                     , HS.PERSON_ID
                     , HS.SAVING_DED_AMOUNT
                     , HS.SAVING_REAL_DED_AMT
                  FROM HRA_SAVING_INFO HS
                 WHERE HS.YEAR_YYYY        = W_YEAR_YYYY
                   AND HS.SOB_ID           = W_SOB_ID
                   AND HS.ORG_ID           = W_ORG_ID
                   AND HS.PERSON_ID        = NVL(W_PERSON_ID, HS.PERSON_ID)
                   AND HS.SAVING_TYPE      = '11'
              ) RETR
            , ( SELECT HS.YEAR_YYYY
                     , HS.PERSON_ID
                     , HS.SAVING_DED_AMOUNT
            ...
            , (-- 시점 인사내역.
              ...
              WHERE HH.HISTORY_HEADER_ID    = HL.HISTORY_HEADER_ID
                ...
                AND HL.POST_ID              = PC.POST_ID                
                AND HH.HISTORY_HEADER_ID           IN 
                      ( SELECT MAX(S_HH.HISTORY_HEADER_ID) AS HISTORY_HEADER_ID
        ...
        WHERE PM.PERSON_ID                = YA.PERSON_ID
          ...
          AND YA.YEAR_YYYY                = PW2.YEAR_YYYY(+)
          AND YA.PERSON_ID                = PW2.PERSON_ID(+)
          AND YA.YEAR_YYYY                = PW3.YEAR_YYYY(+)
          AND YA.PERSON_ID                = PW3.PERSON_ID(+)
          
          AND YA.YEAR_YYYY                = RETR.YEAR_YYYY(+)
          AND YA.PERSON_ID                = RETR.PERSON_ID(+)
          AND YA.YEAR_YYYY                = SEMA.YEAR_YYYY(+)
          AND YA.PERSON_ID                = SEMA.PERSON_ID(+)
          AND YA.YEAR_YYYY                = ANNU.YEAR_YYYY(+)
          AND YA.PERSON_ID                = ANNU.PERSON_ID(+)
  ...
  BEGIN
    OPEN P_CURSOR FOR
      SELECT HLI.HOUSE_LEASE_ID 
           ...
           , HLI.HOUSE_DED_AMT
           , HRM_COMMON_G.CODE_NAME_F('HOUSE_TYPE', HLI.HOUSE_TYPE, HLI.SOB_ID, HLI.ORG_ID) AS HOUSE_TYPE_NAME -- 주택TYPE명.
           , HLI.HOUSE_AREA
