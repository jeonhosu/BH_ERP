     ...
     OPEN P_CURSOR FOR
        SELECT YA.YEAR_YYYY
             ...
             DECODE(NVL(YA.LONG_HOUSE_PROF_AMT, 0) + NVL(YA.LONG_HOUSE_PROF_AMT_1, 0) + NVL(YA.LONG_HOUSE_PROF_AMT_2, 0), 0, NULL, 
                    NVL(YA.LONG_HOUSE_PROF_AMT, 0) + NVL(YA.LONG_HOUSE_PROF_AMT_1, 0) + NVL(YA.LONG_HOUSE_PROF_AMT_2, 0)) AS LONG_HOUSE_PROF_AMT,                        -- 특별공제(장기주택차입금)             
...
-------------------------------------------------------------------------------------------
  ...
  AS
    ...
    V_PAY_YYYYMM_TO               VARCHAR2(7);
  BEGIN
    V_PAY_YYYYMM_FR := W_YEAR_YYYY || '-01';
    V_PAY_YYYYMM_TO := W_YEAR_YYYY || '-12';
    ...
    OPEN P_CURSOR FOR
      SELECT -- 귀속연도
           ...
           , DECODE(HMP.PAY_AMOUNT_01, 0, TO_NUMBER(NULL), HMP.PAY_AMOUNT_01) AS PAY_AMOUNT_01         -- 급여액.
           , DECODE(HMP.BONUS_AMOUNT_01, 0, TO_NUMBER(NULL), HMP.BONUS_AMOUNT_01) AS BONUS_AMOUNT_01   -- 상여액.
           , DECODE((HMP.PAY_AMOUNT_01 + HMP.BONUS_AMOUNT_01), 0, TO_NUMBER(NULL), (HMP.PAY_AMOUNT_01 + HMP.BONUS_AMOUNT_01)) 
             AS TOTAL_PAY_BONUS_01                                                           -- 계.
           ---------------------------------------------------------------------
           -- 2 월.
           ---------------------------------------------------------------------
           , REPLACE(HMP.PAY_YYYYMM_02, '-', '.') AS PAY_YYYYMM_02                          -- 지급연월.
           , DECODE(HMP.PAY_AMOUNT_02, 0, TO_NUMBER(NULL), HMP.PAY_AMOUNT_02) AS PAY_AMOUNT_02         -- 급여액.
           , DECODE(HMP.BONUS_AMOUNT_02, 0, TO_NUMBER(NULL), HMP.BONUS_AMOUNT_02) AS BONUS_AMOUNT_02   -- 상여액.
           , DECODE((HMP.PAY_AMOUNT_02 + HMP.BONUS_AMOUNT_02), 0, TO_NUMBER(NULL), (HMP.PAY_AMOUNT_02 + HMP.BONUS_AMOUNT_02)) 
             AS TOTAL_PAY_BONUS_02                                                           -- 계.
           ---------------------------------------------------------------------
           -- 3 월.
           ---------------------------------------------------------------------
           , REPLACE(HMP.PAY_YYYYMM_03, '-', '.') AS PAY_YYYYMM_03                          -- 지급연월.
           , DECODE(HMP.PAY_AMOUNT_03, 0, TO_NUMBER(NULL), HMP.PAY_AMOUNT_03) AS PAY_AMOUNT_03         -- 급여액.
           , DECODE(HMP.BONUS_AMOUNT_03, 0, TO_NUMBER(NULL), HMP.BONUS_AMOUNT_03) AS BONUS_AMOUNT_03   -- 상여액.
           , DECODE((HMP.PAY_AMOUNT_03 + HMP.BONUS_AMOUNT_03), 0, TO_NUMBER(NULL), (HMP.PAY_AMOUNT_03 + HMP.BONUS_AMOUNT_03)) 
             AS TOTAL_PAY_BONUS_03                                                           -- 계.
           ---------------------------------------------------------------------
           -- 4 월.
           ---------------------------------------------------------------------
           , REPLACE(HMP.PAY_YYYYMM_04, '-', '.') AS PAY_YYYYMM_04                          -- 지급연월.
           , DECODE(HMP.PAY_AMOUNT_04, 0, TO_NUMBER(NULL), HMP.PAY_AMOUNT_04) AS PAY_AMOUNT_04         -- 급여액.
           , DECODE(HMP.BONUS_AMOUNT_04, 0, TO_NUMBER(NULL), HMP.BONUS_AMOUNT_04) AS BONUS_AMOUNT_04   -- 상여액.
           , DECODE((HMP.PAY_AMOUNT_04 + HMP.BONUS_AMOUNT_04), 0, TO_NUMBER(NULL), (HMP.PAY_AMOUNT_04 + HMP.BONUS_AMOUNT_04)) 
             AS TOTAL_PAY_BONUS_04                                                           -- 계.
           ---------------------------------------------------------------------
           -- 5 월.
           ---------------------------------------------------------------------
           , REPLACE(HMP.PAY_YYYYMM_05, '-', '.') AS PAY_YYYYMM_05                          -- 지급연월.
           , DECODE(HMP.PAY_AMOUNT_05, 0, TO_NUMBER(NULL), HMP.PAY_AMOUNT_05) AS PAY_AMOUNT_05         -- 급여액.
           , DECODE(HMP.BONUS_AMOUNT_05, 0, TO_NUMBER(NULL), HMP.BONUS_AMOUNT_05) AS BONUS_AMOUNT_05   -- 상여액.
           , DECODE((HMP.PAY_AMOUNT_05 + HMP.BONUS_AMOUNT_05), 0, TO_NUMBER(NULL), (HMP.PAY_AMOUNT_05 + HMP.BONUS_AMOUNT_05)) 
             AS TOTAL_PAY_BONUS_05                                                           -- 계.   
           ---------------------------------------------------------------------
           -- 6 월.
           ---------------------------------------------------------------------
           , REPLACE(HMP.PAY_YYYYMM_06, '-', '.') AS PAY_YYYYMM_06                          -- 지급연월.
           , DECODE(HMP.PAY_AMOUNT_06, 0, TO_NUMBER(NULL), HMP.PAY_AMOUNT_06) AS PAY_AMOUNT_06         -- 급여액.
           , DECODE(HMP.BONUS_AMOUNT_06, 0, TO_NUMBER(NULL), HMP.BONUS_AMOUNT_06) AS BONUS_AMOUNT_06   -- 상여액.
           , DECODE((HMP.PAY_AMOUNT_06 + HMP.BONUS_AMOUNT_06), 0, TO_NUMBER(NULL), (HMP.PAY_AMOUNT_06 + HMP.BONUS_AMOUNT_06)) 
             AS TOTAL_PAY_BONUS_06                                                           -- 계.  
           ---------------------------------------------------------------------
           -- 7 월.
           ---------------------------------------------------------------------
           , REPLACE(HMP.PAY_YYYYMM_07, '-', '.') AS PAY_YYYYMM_07                          -- 지급연월.
           , DECODE(HMP.PAY_AMOUNT_07, 0, TO_NUMBER(NULL), HMP.PAY_AMOUNT_07) AS PAY_AMOUNT_07         -- 급여액.
           , DECODE(HMP.BONUS_AMOUNT_07, 0, TO_NUMBER(NULL), HMP.BONUS_AMOUNT_07) AS BONUS_AMOUNT_07   -- 상여액.
           , DECODE((HMP.PAY_AMOUNT_07 + HMP.BONUS_AMOUNT_07), 0, TO_NUMBER(NULL), (HMP.PAY_AMOUNT_07 + HMP.BONUS_AMOUNT_07)) 
             AS TOTAL_PAY_BONUS_07                                                           -- 계.
           ---------------------------------------------------------------------
           -- 8 월.
           ---------------------------------------------------------------------
           , REPLACE(HMP.PAY_YYYYMM_08, '-', '.') AS PAY_YYYYMM_08                          -- 지급연월.
           , DECODE(HMP.PAY_AMOUNT_08, 0, TO_NUMBER(NULL), HMP.PAY_AMOUNT_08) AS PAY_AMOUNT_08         -- 급여액.
           , DECODE(HMP.BONUS_AMOUNT_08, 0, TO_NUMBER(NULL), HMP.BONUS_AMOUNT_08) AS BONUS_AMOUNT_08   -- 상여액.
           , DECODE((HMP.PAY_AMOUNT_08 + HMP.BONUS_AMOUNT_08), 0, TO_NUMBER(NULL), (HMP.PAY_AMOUNT_08 + HMP.BONUS_AMOUNT_08)) 
             AS TOTAL_PAY_BONUS_08                                                           -- 계.
           ---------------------------------------------------------------------
           -- 9 월.
           ---------------------------------------------------------------------
           , REPLACE(HMP.PAY_YYYYMM_09, '-', '.') AS PAY_YYYYMM_09                          -- 지급연월.
           , DECODE(HMP.PAY_AMOUNT_09, 0, TO_NUMBER(NULL), HMP.PAY_AMOUNT_09) AS PAY_AMOUNT_09         -- 급여액.
           , DECODE(HMP.BONUS_AMOUNT_09, 0, TO_NUMBER(NULL), HMP.BONUS_AMOUNT_09) AS BONUS_AMOUNT_09   -- 상여액.
           , DECODE((HMP.PAY_AMOUNT_09 + HMP.BONUS_AMOUNT_09), 0, TO_NUMBER(NULL), (HMP.PAY_AMOUNT_09 + HMP.BONUS_AMOUNT_09)) 
             AS TOTAL_PAY_BONUS_09                                                           -- 계.
           ---------------------------------------------------------------------
           -- 10 월.
           ---------------------------------------------------------------------
           , REPLACE(HMP.PAY_YYYYMM_10, '-', '.') AS PAY_YYYYMM_10                          -- 지급연월.
           , DECODE(HMP.PAY_AMOUNT_10, 0, TO_NUMBER(NULL), HMP.PAY_AMOUNT_10) AS PAY_AMOUNT_10         -- 급여액.
           , DECODE(HMP.BONUS_AMOUNT_10, 0, TO_NUMBER(NULL), HMP.BONUS_AMOUNT_10) AS BONUS_AMOUNT_10   -- 상여액.
           , DECODE((HMP.PAY_AMOUNT_10 + HMP.BONUS_AMOUNT_10), 0, TO_NUMBER(NULL), (HMP.PAY_AMOUNT_10 + HMP.BONUS_AMOUNT_10)) 
             AS TOTAL_PAY_BONUS_10                                                           -- 계.
           ---------------------------------------------------------------------
           -- 11 월.
           ---------------------------------------------------------------------
           , REPLACE(HMP.PAY_YYYYMM_11, '-', '.') AS PAY_YYYYMM_11                          -- 지급연월.
           , DECODE(HMP.PAY_AMOUNT_11, 0, TO_NUMBER(NULL), HMP.PAY_AMOUNT_11) AS PAY_AMOUNT_11         -- 급여액.
           , DECODE(HMP.BONUS_AMOUNT_11, 0, TO_NUMBER(NULL), HMP.BONUS_AMOUNT_11) AS BONUS_AMOUNT_11   -- 상여액.
           , DECODE((HMP.PAY_AMOUNT_11 + HMP.BONUS_AMOUNT_11), 0, TO_NUMBER(NULL), (HMP.PAY_AMOUNT_11 + HMP.BONUS_AMOUNT_11)) 
             AS TOTAL_PAY_BONUS_11                                                           -- 계.
           ---------------------------------------------------------------------
           -- 12 월.
           ---------------------------------------------------------------------
           , REPLACE(HMP.PAY_YYYYMM_12, '-', '.') AS PAY_YYYYMM_12                          -- 지급연월.
           , DECODE(HMP.PAY_AMOUNT_12, 0, TO_NUMBER(NULL), HMP.PAY_AMOUNT_12) AS PAY_AMOUNT_12         -- 급여액.
           , DECODE(HMP.BONUS_AMOUNT_12, 0, TO_NUMBER(NULL), HMP.BONUS_AMOUNT_12) AS BONUS_AMOUNT_12   -- 상여액.
           , DECODE((HMP.PAY_AMOUNT_12 + HMP.BONUS_AMOUNT_12), 0, TO_NUMBER(NULL), (HMP.PAY_AMOUNT_12 + HMP.BONUS_AMOUNT_12)) 
             AS TOTAL_PAY_BONUS_12                                                           -- 계.
           ---------------------------------------------------------------------
           -- 계.
           ---------------------------------------------------------------------
           , DECODE(HMP.TOTAL_PAY_AMOUNT, 0, TO_NUMBER(NULL), HMP.TOTAL_PAY_AMOUNT) AS TOTAL_PAY_AMOUNT       -- 총 급여액.
           , DECODE(HMP.TOTAL_BONUS_AMOUNT, 0, TO_NUMBER(NULL), HMP.TOTAL_BONUS_AMOUNT) AS TOTAL_BONUS_AMOUNT -- 총 상여액.
           , DECODE(HMP.TOTAL_PAY_BONUS, 0, TO_NUMBER(NULL), HMP.TOTAL_PAY_BONUS) AS TOTAL_PAY_BONUS          -- 계.
           --------------------------------------------------------------------------------------------------------------------
           -- 비 과 세 소 득.
           --------------------------------------------------------------------------------------------------------------------
           ---------------------------------------------------------------------
           -- 1 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.TAX_FREE_OT_01, 0, TO_NUMBER(NULL), HMP.TAX_FREE_OT_01) AS TAX_FREE_OT_01       -- 야간근로수당 등
           , DECODE(HMP.TAX_FREE_ETC_01, 0, TO_NUMBER(NULL), HMP.TAX_FREE_ETC_01) AS TAX_FREE_ETC_01    -- 차량유류비.
           ---------------------------------------------------------------------
           -- 2 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.TAX_FREE_OT_02, 0, TO_NUMBER(NULL), HMP.TAX_FREE_OT_02) AS TAX_FREE_OT_02      -- 야간근로수당 등
           , DECODE(HMP.TAX_FREE_ETC_02, 0, TO_NUMBER(NULL), HMP.TAX_FREE_ETC_02) AS TAX_FREE_ETC_02   -- 차량유류비.
           ---------------------------------------------------------------------
           -- 3 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.TAX_FREE_OT_03, 0, TO_NUMBER(NULL), HMP.TAX_FREE_OT_03) AS TAX_FREE_OT_03      -- 야간근로수당 등
           , DECODE(HMP.TAX_FREE_ETC_03, 0, TO_NUMBER(NULL), HMP.TAX_FREE_ETC_03) AS TAX_FREE_ETC_03   -- 차량유류비.
           ---------------------------------------------------------------------
           -- 4 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.TAX_FREE_OT_04, 0, TO_NUMBER(NULL), HMP.TAX_FREE_OT_04) AS TAX_FREE_OT_04      -- 야간근로수당 등
           , DECODE(HMP.TAX_FREE_ETC_04, 0, TO_NUMBER(NULL), HMP.TAX_FREE_ETC_04) AS TAX_FREE_ETC_04   -- 차량유류비.
           ---------------------------------------------------------------------
           -- 5 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.TAX_FREE_OT_05, 0, TO_NUMBER(NULL), HMP.TAX_FREE_OT_05) AS TAX_FREE_OT_05      -- 야간근로수당 등
           , DECODE(HMP.TAX_FREE_ETC_05, 0, TO_NUMBER(NULL), HMP.TAX_FREE_ETC_05) AS TAX_FREE_ETC_05   -- 차량유류비.
           ---------------------------------------------------------------------
           -- 6 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.TAX_FREE_OT_06, 0, TO_NUMBER(NULL), HMP.TAX_FREE_OT_06) AS TAX_FREE_OT_06      -- 야간근로수당 등
           , DECODE(HMP.TAX_FREE_ETC_06, 0, TO_NUMBER(NULL), HMP.TAX_FREE_ETC_06) AS TAX_FREE_ETC_06   -- 차량유류비.
           ---------------------------------------------------------------------
           -- 7 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.TAX_FREE_OT_07, 0, TO_NUMBER(NULL), HMP.TAX_FREE_OT_07) AS TAX_FREE_OT_07      -- 야간근로수당 등
           , DECODE(HMP.TAX_FREE_ETC_07, 0, TO_NUMBER(NULL), HMP.TAX_FREE_ETC_07) AS TAX_FREE_ETC_07   -- 차량유류비.
           ---------------------------------------------------------------------
           -- 8 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.TAX_FREE_OT_08, 0, TO_NUMBER(NULL), HMP.TAX_FREE_OT_08) AS TAX_FREE_OT_08      -- 야간근로수당 등
           , DECODE(HMP.TAX_FREE_ETC_08, 0, TO_NUMBER(NULL), HMP.TAX_FREE_ETC_08) AS TAX_FREE_ETC_08   -- 차량유류비.
           ---------------------------------------------------------------------
           -- 9 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.TAX_FREE_OT_09, 0, TO_NUMBER(NULL), HMP.TAX_FREE_OT_09) AS TAX_FREE_OT_09      -- 야간근로수당 등
           , DECODE(HMP.TAX_FREE_ETC_09, 0, TO_NUMBER(NULL), HMP.TAX_FREE_ETC_09) AS TAX_FREE_ETC_09   -- 차량유류비.
           ---------------------------------------------------------------------
           -- 10 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.TAX_FREE_OT_10, 0, TO_NUMBER(NULL), HMP.TAX_FREE_OT_10) AS TAX_FREE_OT_10      -- 야간근로수당 등
           , DECODE(HMP.TAX_FREE_ETC_10, 0, TO_NUMBER(NULL), HMP.TAX_FREE_ETC_10) AS TAX_FREE_ETC_10   -- 차량유류비.
           ---------------------------------------------------------------------
           -- 11 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.TAX_FREE_OT_11, 0, TO_NUMBER(NULL), HMP.TAX_FREE_OT_11) AS TAX_FREE_OT_11      -- 야간근로수당 등
           , DECODE(HMP.TAX_FREE_ETC_11, 0, TO_NUMBER(NULL), HMP.TAX_FREE_ETC_11) AS TAX_FREE_ETC_11   -- 차량유류비.
           ---------------------------------------------------------------------
           -- 12 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.TAX_FREE_OT_12, 0, TO_NUMBER(NULL), HMP.TAX_FREE_OT_12) AS TAX_FREE_OT_12      -- 야간근로수당 등
           , DECODE(HMP.TAX_FREE_ETC_12, 0, TO_NUMBER(NULL), HMP.TAX_FREE_ETC_12) AS TAX_FREE_ETC_12   -- 차량유류비.
           --------------------------------------------------------------------------------------------------------------------
           -- 근 로 소 득 원 천 징 수 액.
           --------------------------------------------------------------------------------------------------------------------
           ---------------------------------------------------------------------
           -- 1 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.SUBT_IN_TAX_AMT_01, 0, TO_NUMBER(NULL), HMP.SUBT_IN_TAX_AMT_01 ) AS SUBT_IN_TAX_AMT_01                    -- 소득세.
           , DECODE(HMP.SUBT_LOCAL_TAX_AMT_01, 0, TO_NUMBER(NULL), HMP.SUBT_LOCAL_TAX_AMT_01) AS SUBT_LOCAL_TAX_AMT_01            -- 주민세.           
           , DECODE(HMP.ANNUITY_IN_AMT_01, 0, TO_NUMBER(NULL), HMP.ANNUITY_IN_AMT_01) AS ANNUITY_IN_AMT_01                        -- 연금보험.
           , DECODE(HMP.HEALTH_IN_AMT_01, 0, TO_NUMBER(NULL), HMP.HEALTH_IN_AMT_01) AS HEALTH_IN_AMT_01                           -- 건강보험.
           , DECODE(HMP.UMP_IN_AMT_01, 0, TO_NUMBER(NULL), HMP.UMP_IN_AMT_01) AS UMP_IN_AMT_01                                    -- 고용보험.
           ---------------------------------------------------------------------
           -- 2 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.SUBT_IN_TAX_AMT_02, 0, TO_NUMBER(NULL), HMP.SUBT_IN_TAX_AMT_02) AS SUBT_IN_TAX_AMT_02                     -- 소득세.
           , DECODE(HMP.SUBT_LOCAL_TAX_AMT_02, 0, TO_NUMBER(NULL), HMP.SUBT_LOCAL_TAX_AMT_02) AS SUBT_LOCAL_TAX_AMT_02            -- 주민세.           
           , DECODE(HMP.ANNUITY_IN_AMT_02, 0, TO_NUMBER(NULL), HMP.ANNUITY_IN_AMT_02) AS ANNUITY_IN_AMT_02                        -- 연금보험.
           , DECODE(HMP.HEALTH_IN_AMT_02, 0, TO_NUMBER(NULL), HMP.HEALTH_IN_AMT_02) AS HEALTH_IN_AMT_02                           -- 건강보험.
           , DECODE(HMP.UMP_IN_AMT_02, 0, TO_NUMBER(NULL), HMP.UMP_IN_AMT_02) AS UMP_IN_AMT_02                                    -- 고용보험.
           ---------------------------------------------------------------------
           -- 3 월.
           ---------------------------------------------------------------------           
           , DECODE(HMP.SUBT_IN_TAX_AMT_03, 0, TO_NUMBER(NULL), HMP.SUBT_IN_TAX_AMT_03) AS SUBT_IN_TAX_AMT_03 
           , DECODE(HMP.SUBT_LOCAL_TAX_AMT_03, 0, TO_NUMBER(NULL), HMP.SUBT_LOCAL_TAX_AMT_03) AS SUBT_LOCAL_TAX_AMT_03
           , DECODE(HMP.ANNUITY_IN_AMT_03, 0, TO_NUMBER(NULL), HMP.ANNUITY_IN_AMT_03) AS ANNUITY_IN_AMT_03
           , DECODE(HMP.HEALTH_IN_AMT_03, 0, TO_NUMBER(NULL), HMP.HEALTH_IN_AMT_03) AS HEALTH_IN_AMT_03
           , DECODE(HMP.UMP_IN_AMT_03, 0, TO_NUMBER(NULL), HMP.UMP_IN_AMT_03) AS UMP_IN_AMT_03
           ---------------------------------------------------------------------
           -- 4 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.SUBT_IN_TAX_AMT_04, 0, TO_NUMBER(NULL), HMP.SUBT_IN_TAX_AMT_04) AS SUBT_IN_TAX_AMT_04
           , DECODE(HMP.SUBT_LOCAL_TAX_AMT_04, 0, TO_NUMBER(NULL), HMP.SUBT_LOCAL_TAX_AMT_04) AS SUBT_LOCAL_TAX_AMT_04
           , DECODE(HMP.ANNUITY_IN_AMT_04, 0, TO_NUMBER(NULL), HMP.ANNUITY_IN_AMT_04) AS ANNUITY_IN_AMT_04
           , DECODE(HMP.HEALTH_IN_AMT_04, 0, TO_NUMBER(NULL), HMP.HEALTH_IN_AMT_04) AS HEALTH_IN_AMT_04
           , DECODE(HMP.UMP_IN_AMT_04, 0, TO_NUMBER(NULL), HMP.UMP_IN_AMT_04) AS UMP_IN_AMT_04
           ---------------------------------------------------------------------
           -- 5 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.SUBT_IN_TAX_AMT_05, 0, TO_NUMBER(NULL), HMP.SUBT_IN_TAX_AMT_05) AS SUBT_IN_TAX_AMT_05
           , DECODE(HMP.SUBT_LOCAL_TAX_AMT_05, 0, TO_NUMBER(NULL), HMP.SUBT_LOCAL_TAX_AMT_05) AS SUBT_LOCAL_TAX_AMT_05
           , DECODE(HMP.ANNUITY_IN_AMT_05, 0, TO_NUMBER(NULL), HMP.ANNUITY_IN_AMT_05) AS ANNUITY_IN_AMT_05
           , DECODE(HMP.HEALTH_IN_AMT_05, 0, TO_NUMBER(NULL), HMP.HEALTH_IN_AMT_05) AS HEALTH_IN_AMT_05
           , DECODE(HMP.UMP_IN_AMT_05, 0, TO_NUMBER(NULL), HMP.UMP_IN_AMT_05) AS UMP_IN_AMT_05
           ---------------------------------------------------------------------
           -- 6 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.SUBT_IN_TAX_AMT_06, 0, TO_NUMBER(NULL), HMP.SUBT_IN_TAX_AMT_06) AS SUBT_IN_TAX_AMT_06
           , DECODE(HMP.SUBT_LOCAL_TAX_AMT_06, 0, TO_NUMBER(NULL), HMP.SUBT_LOCAL_TAX_AMT_06) AS SUBT_LOCAL_TAX_AMT_06
           , DECODE(HMP.ANNUITY_IN_AMT_06, 0, TO_NUMBER(NULL), HMP.ANNUITY_IN_AMT_06) AS ANNUITY_IN_AMT_06
           , DECODE(HMP.HEALTH_IN_AMT_06, 0, TO_NUMBER(NULL), HMP.HEALTH_IN_AMT_06) AS HEALTH_IN_AMT_06
           , DECODE(HMP.UMP_IN_AMT_06, 0, TO_NUMBER(NULL), HMP.UMP_IN_AMT_06) AS UMP_IN_AMT_06
           ---------------------------------------------------------------------
           -- 7 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.SUBT_IN_TAX_AMT_07, 0, TO_NUMBER(NULL), HMP.SUBT_IN_TAX_AMT_07) AS SUBT_IN_TAX_AMT_07
           , DECODE(HMP.SUBT_LOCAL_TAX_AMT_07, 0, TO_NUMBER(NULL), HMP.SUBT_LOCAL_TAX_AMT_07) AS SUBT_LOCAL_TAX_AMT_07
           , DECODE(HMP.ANNUITY_IN_AMT_07, 0, TO_NUMBER(NULL), HMP.ANNUITY_IN_AMT_07) AS ANNUITY_IN_AMT_07
           , DECODE(HMP.HEALTH_IN_AMT_07, 0, TO_NUMBER(NULL), HMP.HEALTH_IN_AMT_07) AS HEALTH_IN_AMT_07
           , DECODE(HMP.UMP_IN_AMT_07, 0, TO_NUMBER(NULL), HMP.UMP_IN_AMT_07) AS UMP_IN_AMT_07
           ---------------------------------------------------------------------
           -- 8 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.SUBT_IN_TAX_AMT_08, 0, TO_NUMBER(NULL), HMP.SUBT_IN_TAX_AMT_08) AS SUBT_IN_TAX_AMT_08
           , DECODE(HMP.SUBT_LOCAL_TAX_AMT_08, 0, TO_NUMBER(NULL), HMP.SUBT_LOCAL_TAX_AMT_08) AS SUBT_LOCAL_TAX_AMT_08
           , DECODE(HMP.ANNUITY_IN_AMT_08, 0, TO_NUMBER(NULL), HMP.ANNUITY_IN_AMT_08) AS ANNUITY_IN_AMT_08
           , DECODE(HMP.HEALTH_IN_AMT_08, 0, TO_NUMBER(NULL), HMP.HEALTH_IN_AMT_08) AS HEALTH_IN_AMT_08
           , DECODE(HMP.UMP_IN_AMT_08, 0, TO_NUMBER(NULL), HMP.UMP_IN_AMT_08) AS UMP_IN_AMT_08
           ---------------------------------------------------------------------
           -- 9 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.SUBT_IN_TAX_AMT_09, 0, TO_NUMBER(NULL), HMP.SUBT_IN_TAX_AMT_09) AS SUBT_IN_TAX_AMT_09
           , DECODE(HMP.SUBT_LOCAL_TAX_AMT_09, 0, TO_NUMBER(NULL), HMP.SUBT_LOCAL_TAX_AMT_09) AS SUBT_LOCAL_TAX_AMT_09
           , DECODE(HMP.ANNUITY_IN_AMT_09, 0, TO_NUMBER(NULL), HMP.ANNUITY_IN_AMT_09) AS ANNUITY_IN_AMT_09
           , DECODE(HMP.HEALTH_IN_AMT_09, 0, TO_NUMBER(NULL), HMP.HEALTH_IN_AMT_09) AS HEALTH_IN_AMT_09
           , DECODE(HMP.UMP_IN_AMT_09, 0, TO_NUMBER(NULL), HMP.UMP_IN_AMT_09) AS UMP_IN_AMT_09
           ---------------------------------------------------------------------
           -- 10 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.SUBT_IN_TAX_AMT_10, 0, TO_NUMBER(NULL), HMP.SUBT_IN_TAX_AMT_10) AS SUBT_IN_TAX_AMT_10
           , DECODE(HMP.SUBT_LOCAL_TAX_AMT_10, 0, TO_NUMBER(NULL), HMP.SUBT_LOCAL_TAX_AMT_10) AS SUBT_LOCAL_TAX_AMT_10
           , DECODE(HMP.ANNUITY_IN_AMT_10, 0, TO_NUMBER(NULL), HMP.ANNUITY_IN_AMT_10) AS ANNUITY_IN_AMT_10
           , DECODE(HMP.HEALTH_IN_AMT_10, 0, TO_NUMBER(NULL), HMP.HEALTH_IN_AMT_10) AS HEALTH_IN_AMT_10
           , DECODE(HMP.UMP_IN_AMT_10, 0, TO_NUMBER(NULL), HMP.UMP_IN_AMT_10) AS UMP_IN_AMT_10
           ---------------------------------------------------------------------
           -- 11 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.SUBT_IN_TAX_AMT_11, 0, TO_NUMBER(NULL), HMP.SUBT_IN_TAX_AMT_11) AS SUBT_IN_TAX_AMT_11
           , DECODE(HMP.SUBT_LOCAL_TAX_AMT_11, 0, TO_NUMBER(NULL), HMP.SUBT_LOCAL_TAX_AMT_11) AS SUBT_LOCAL_TAX_AMT_11
           , DECODE(HMP.ANNUITY_IN_AMT_11, 0, TO_NUMBER(NULL), HMP.ANNUITY_IN_AMT_11) AS ANNUITY_IN_AMT_11
           , DECODE(HMP.HEALTH_IN_AMT_11, 0, TO_NUMBER(NULL), HMP.HEALTH_IN_AMT_11) AS HEALTH_IN_AMT_11
           , DECODE(HMP.UMP_IN_AMT_11, 0, TO_NUMBER(NULL), HMP.UMP_IN_AMT_11) AS UMP_IN_AMT_11
           ---------------------------------------------------------------------
           -- 12 월.
           ---------------------------------------------------------------------
           , DECODE(HMP.SUBT_IN_TAX_AMT_12, 0, TO_NUMBER(NULL), HMP.SUBT_IN_TAX_AMT_12) AS SUBT_IN_TAX_AMT_12
           , DECODE(HMP.SUBT_LOCAL_TAX_AMT_12, 0, TO_NUMBER(NULL), HMP.SUBT_LOCAL_TAX_AMT_12) AS SUBT_LOCAL_TAX_AMT_12
           , DECODE(HMP.ANNUITY_IN_AMT_12, 0, TO_NUMBER(NULL), HMP.ANNUITY_IN_AMT_12) AS ANNUITY_IN_AMT_12
           , DECODE(HMP.HEALTH_IN_AMT_12, 0, TO_NUMBER(NULL), HMP.HEALTH_IN_AMT_12) AS HEALTH_IN_AMT_12
           , DECODE(HMP.UMP_IN_AMT_12, 0, TO_NUMBER(NULL), HMP.UMP_IN_AMT_12) AS UMP_IN_AMT_12
           ---------------------------------------------------------------------
           -- 계.
           ---------------------------------------------------------------------
           , DECODE(HMP.TOTAL_SUBT_IN_TAX_AMT, 0, TO_NUMBER(NULL), HMP.TOTAL_SUBT_IN_TAX_AMT) AS TOTAL_SUBT_IN_TAX_AMT           -- 소득세 계.
           , DECODE(HMP.TOTAL_SUBT_LOCAL_TAX_AMT, 0, TO_NUMBER(NULL), HMP.TOTAL_SUBT_LOCAL_TAX_AMT) AS TOTAL_SUBT_LOCAL_TAX_AMT  -- 주민세 계.
           , DECODE(HMP.TOTAL_ANNUITY_IN_AMT, 0, TO_NUMBER(NULL), HMP.TOTAL_ANNUITY_IN_AMT) AS TOTAL_ANNUITY_IN_AMT              -- 연금보험 계.
           , DECODE(HMP.TOTAL_HEALTH_IN_AMT, 0, TO_NUMBER(NULL), HMP.TOTAL_HEALTH_IN_AMT) AS TOTAL_HEALTH_IN_AMT                 -- 건강보험 계.
           , DECODE(HMP.TOTAL_UMP_IN_AMT, 0, TO_NUMBER(NULL), HMP.TOTAL_UMP_IN_AMT) AS TOTAL_UMP_IN_AMT                          -- 고용보험 계.
           , TO_CHAR(SYSDATE, 'YYYY') || '년 ' || TO_CHAR(SYSDATE, 'MM') || '월 ' || TO_CHAR(SYSDATE, 'DD') || '일' AS PRINT_DATE
     FROM HRM_PERSON_MASTER PM
           , HRM_CORP_MASTER CM
           , ( SELECT HOU.CORP_ID
                    , HOU.VAT_NUMBER 
           ...
           , ( SELECT MP.PERSON_ID
                     ...
                     , SUM(NVL(MP.PAY_AMOUNT, 0) + NVL(MP.BONUS_AMOUNT, 0)) AS TOTAL_PAY_BONUS
                     
                     -- 비과세 소득.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '01', CASE
                                                                 WHEN MP.TOT_SUPPLY_AMOUNT <= 1000000 THEN MA.TAX_FREE_OT
                                                                 ELSE 0
                                                              END, 0)) AS TAX_FREE_OT_01                   -- 야간근로수당 등.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '01', MA.TAX_FREE_ETC, 0)) AS TAX_FREE_ETC_01        -- 차량유류비.
                      --------------------------------------------------------------------------------------------------------------------------- 2 월.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '02', CASE
                                                                 WHEN MP.TOT_SUPPLY_AMOUNT <= 1000000 THEN MA.TAX_FREE_OT
                                                                 ELSE 0
                                                              END, 0)) AS TAX_FREE_OT_02                   -- 야간근로수당 등.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '02', MA.TAX_FREE_ETC, 0)) AS TAX_FREE_ETC_02        -- 차량유류비.
                      --------------------------------------------------------------------------------------------------------------------------- 3 월.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '03', CASE
                                                                 WHEN MP.TOT_SUPPLY_AMOUNT <= 1000000 THEN MA.TAX_FREE_OT
                                                                 ELSE 0
                                                              END, 0)) AS TAX_FREE_OT_03                    -- 야간근로수당 등.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '03', MA.TAX_FREE_ETC, 0)) AS TAX_FREE_ETC_03        -- 차량유류비.
                      --------------------------------------------------------------------------------------------------------------------------- 4 월.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '04', CASE
                                                                 WHEN MP.TOT_SUPPLY_AMOUNT <= 1000000 THEN MA.TAX_FREE_OT
                                                                 ELSE 0
                                                              END, 0)) AS TAX_FREE_OT_04                    -- 야간근로수당 등.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '04', MA.TAX_FREE_ETC, 0)) AS TAX_FREE_ETC_04        -- 차량유류비.
                      --------------------------------------------------------------------------------------------------------------------------- 5 월.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '05', CASE
                                                                 WHEN MP.TOT_SUPPLY_AMOUNT <= 1000000 THEN MA.TAX_FREE_OT
                                                                 ELSE 0
                                                              END, 0)) AS TAX_FREE_OT_05                    -- 야간근로수당 등.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '05', MA.TAX_FREE_ETC, 0)) AS TAX_FREE_ETC_05        -- 차량유류비.
                      --------------------------------------------------------------------------------------------------------------------------- 6 월.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '06', CASE
                                                                 WHEN MP.TOT_SUPPLY_AMOUNT <= 1000000 THEN MA.TAX_FREE_OT
                                                                 ELSE 0
                                                              END, 0)) AS TAX_FREE_OT_06                    -- 야간근로수당 등.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '06', MA.TAX_FREE_ETC, 0)) AS TAX_FREE_ETC_06         -- 차량유류비.
                      --------------------------------------------------------------------------------------------------------------------------- 7 월.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '07', CASE
                                                                 WHEN MP.TOT_SUPPLY_AMOUNT <= 1000000 THEN MA.TAX_FREE_OT
                                                                 ELSE 0
                                                              END, 0)) AS TAX_FREE_OT_07                     -- 야간근로수당 등.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '07', MA.TAX_FREE_ETC, 0)) AS TAX_FREE_ETC_07          -- 차량유류비.
                      --------------------------------------------------------------------------------------------------------------------------- 8 월.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '08', CASE
                                                                 WHEN MP.TOT_SUPPLY_AMOUNT <= 1000000 THEN MA.TAX_FREE_OT
                                                                 ELSE 0
                                                              END, 0)) AS TAX_FREE_OT_08                   -- 야간근로수당 등.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '08', MA.TAX_FREE_ETC, 0)) AS TAX_FREE_ETC_08        -- 차량유류비.
                      --------------------------------------------------------------------------------------------------------------------------- 9 월.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '09', CASE
                                                                 WHEN MP.TOT_SUPPLY_AMOUNT <= 1000000 THEN MA.TAX_FREE_OT
                                                                 ELSE 0
                                                              END, 0)) AS TAX_FREE_OT_09                   -- 야간근로수당 등.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '09', MA.TAX_FREE_ETC, 0)) AS TAX_FREE_ETC_09        -- 차량유류비.
                      --------------------------------------------------------------------------------------------------------------------------- 10 월.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '10', CASE
                                                                 WHEN MP.TOT_SUPPLY_AMOUNT <= 1000000 THEN MA.TAX_FREE_OT
                                                                 ELSE 0
                                                              END, 0)) AS TAX_FREE_OT_10                  -- 야간근로수당 등.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '10', MA.TAX_FREE_ETC, 0)) AS TAX_FREE_ETC_10       -- 차량유류비.
                      --------------------------------------------------------------------------------------------------------------------------- 11 월.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '11', CASE
                                                                 WHEN MP.TOT_SUPPLY_AMOUNT <= 1000000 THEN MA.TAX_FREE_OT
                                                                 ELSE 0
                                                              END, 0)) AS TAX_FREE_OT_11                   -- 야간근로수당 등.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '11', MA.TAX_FREE_ETC, 0)) AS TAX_FREE_ETC_11        -- 차량유류비.
                      --------------------------------------------------------------------------------------------------------------------------- 12 월.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '12', CASE
                                                                 WHEN MP.TOT_SUPPLY_AMOUNT <= 1000000 THEN MA.TAX_FREE_OT
                                                                 ELSE 0
                                                              END, 0)) AS TAX_FREE_OT_12                  -- 야간근로수당 등.
                      , SUM(DECODE(SUBSTR(MP.PAY_YYYYMM, 6, 2), '12', MA.TAX_FREE_ETC, 0)) AS TAX_FREE_ETC_12       -- 차량유류비.
