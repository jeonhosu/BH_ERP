  ...
  BEGIN
    OPEN P_CURSOR FOR
      SELECT  SX1.PERSON_ID
            , SX1.LICENSE_ID
            , SX1.LICENSE_NAME
            , SX1.LICENSE_GRADE_ID
            , SX1.LICENSE_GRADE_NAME
            , SX1.LICENSE_NO
            , SX1.LICENSE_DATE
            , SX1.LICENSE_ORG
            , SX1.RENEW_DATE
            , SX1.DESCRIPTION
        FROM ( SELECT HL.PERSON_ID
                    , HL.LICENSE_ID
                    , HRM_COMMON_G.ID_NAME_F(HL.LICENSE_ID) LICENSE_NAME
                    , HL.LICENSE_GRADE_ID
                    , HRM_COMMON_G.ID_NAME_F(HL.LICENSE_GRADE_ID) LICENSE_GRADE_NAME
                    , HL.LICENSE_NO
                    , TO_CHAR(HL.LICENSE_DATE, 'YYYY-MM-DD') AS LICENSE_DATE
                    , HL.LICENSE_ORG
                    , TO_CHAR(HL.RENEW_DATE, 'YYYY-MM-DD') AS RENEW_DATE
                    , HL.DESCRIPTION
                FROM HRM_LICENSE HL
               WHERE HL.PERSON_ID         = W_PERSON_ID
               ORDER BY HL.LICENSE_DATE DESC
             ) SX1
      WHERE ROWNUM                <= 4 
        ;
  END PRINT_LICENSE;
