...
-------------------------------------------------------------------------------
-- 2014년도 퇴직소득 파일생성 및 조회.
-------------------------------------------------------------------------------
  PROCEDURE SET_RETIRE_FILE_2014
            ( P_YEAR_YYYY         IN  VARCHAR2 
            , P_START_DATE        IN  DATE
            , P_END_DATE          IN  DATE
            , P_CORP_ID           IN  NUMBER 
  ...
  BEGIN
    OPEN P_CURSOR FOR
      SELECT 'N' AS SELECT_YN
           ...
           , SUM(NVL(S_RA.S_INCOME_TAX_AMOUNT, 0)) AS S_INCOME_TAX_AMOUNT
           , SUM(NVL(S_RA.S_RESIDENT_TAX_AMOUNT, 0)) AS S_RESIDENT_TAX_AMOUNT
           , SUM(NVL(S_RA.S_SP_TAX_AMOUNT, 0)) AS S_SP_TAX_AMOUNT
           , SUM(NVL(S_RA.S_SUM_TAX_AMOUNT, 0)) AS S_SUM_TAX_AMOUNT
           , SUM(NVL(S_RA.P_INCOME_TAX_AMOUNT, 0)) AS P_INCOME_TAX_AMOUNT
           , SUM(NVL(S_RA.P_RESIDENT_TAX_AMOUNT, 0)) AS P_RESIDENT_TAX_AMOUNT
           , SUM(NVL(S_RA.P_SP_TAX_AMOUNT, 0)) AS P_SP_TAX_AMOUNT
           , SUM(NVL(S_RA.P_SUM_TAX_AMOUNT, 0)) AS P_SUM_TAX_AMOUNT
        ...
        FROM HRM_CORP_MASTER    CM
          ...
          , ( SELECT RA.ADJUSTMENT_YYYY
                  ...
                  , SUM(NVL(RA.INCOME_TAX_AMOUNT, 0)) AS INCOME_TAX_AMOUNT
                  , SUM(NVL(RA.RESIDENT_TAX_AMOUNT, 0)) AS RESIDENT_TAX_AMOUNT 
                  , SUM(NVL(RA.SP_TAX_AMOUNT, 0)) AS SP_TAX_AMOUNT
                  , SUM(NVL(RA.INCOME_TAX_AMOUNT, 0) + 
                        NVL(RA.RESIDENT_TAX_AMOUNT, 0) + 
                        NVL(RA.SP_TAX_AMOUNT, 0)) AS SUM_TAX_AMOUNT
                  , SUM(DECODE(T2.ACCOUNT_NUM, NULL, NVL(RA.INCOME_TAX_AMOUNT, 0), 0)) AS S_INCOME_TAX_AMOUNT
                  , SUM(DECODE(T2.ACCOUNT_NUM, NULL, NVL(RA.RESIDENT_TAX_AMOUNT, 0), 0)) AS S_RESIDENT_TAX_AMOUNT 
                  , SUM(DECODE(T2.ACCOUNT_NUM, NULL, NVL(RA.SP_TAX_AMOUNT, 0), 0)) AS S_SP_TAX_AMOUNT
                  , SUM(DECODE(T2.ACCOUNT_NUM, NULL, NVL(RA.INCOME_TAX_AMOUNT, 0) + 
                                                     NVL(RA.RESIDENT_TAX_AMOUNT, 0) + 
                                                     NVL(RA.SP_TAX_AMOUNT, 0), 0)) AS S_SUM_TAX_AMOUNT
                  , SUM(DECODE(T2.ACCOUNT_NUM, NULL, 0, NVL(RA.INCOME_TAX_AMOUNT, 0))) AS P_INCOME_TAX_AMOUNT
                  , SUM(DECODE(T2.ACCOUNT_NUM, NULL, 0, NVL(RA.RESIDENT_TAX_AMOUNT, 0))) AS P_RESIDENT_TAX_AMOUNT 
                  , SUM(DECODE(T2.ACCOUNT_NUM, NULL, 0, NVL(RA.SP_TAX_AMOUNT, 0))) AS P_SP_TAX_AMOUNT
                  , SUM(DECODE(T2.ACCOUNT_NUM, NULL, 0, NVL(RA.INCOME_TAX_AMOUNT, 0) + 
                                                        NVL(RA.RESIDENT_TAX_AMOUNT, 0) + 
                                                        NVL(RA.SP_TAX_AMOUNT, 0))) AS P_SUM_TAX_AMOUNT
                FROM HRR_RETIRE_ADJUSTMENT RA
                   ...
                   , ( -- 시점 인사내역.
                    ...
                    , (SELECT RP.PERSON_ID
                            , RP.SOB_ID
                            , RP.ORG_ID
                            , MAX(RP.ACCOUNT_NUM) AS ACCOUNT_NUM 
                         FROM HRR_RETIREMENT_PENSION RP
...
--C1 ------------------------------------------------------------------------------------
        FOR C1 IN ( SELECT
                       ...
                       --> 세액환산명세-법정이외 퇴직급여;
                        ...
                        || LPAD(/*CASE
                                  WHEN NVL(RA.CHANGE_DAY, 0) < 0 
                                    THEN  CASE 
                                            WHEN TRUNC(RA.CHANGE_DAY / 30) <= 0 THEN 0
                                            WHEN TRUNC(RA.CHANGE_DAY / 30) <= 1 THEN 1
                                            ELSE TRUNC(RA.CHANGE_DAY / 30)
                                          END
                                  ELSE 0
                                END*/0, 4, 0) -- 45. 주(현)근무지 제외월수.
...
--C1 ------------------------------------------------------------------------------------
        FOR C1 IN ( SELECT
                        ...
                        || LPAD(/*CASE
                                  WHEN NVL(RA.CHANGE_DAY, 0) < 0 THEN CEIL(RA.CHANGE_DAY / 30)
                                  ELSE 0
                        ...
                        || LPAD(/*CASE
                                  WHEN NVL(RA.CHANGE_DAY, 0) < 0 THEN CEIL(RA.DED_DAY / 30)
                                  ELSE 0
...
--B1 ------------------------------------------------------------------------------------
      FOR B1 IN ( SELECT --> 자료관리번호;
                      ...
                      || RPAD(NVL(CM.PRESIDENT_NAME, ' '), 30, ' ')         -- 7.대표자 성명;
                      || RPAD(NVL(REPLACE(CM.LEGAL_NUMBER, '-', ''), ' '), 13, ' ') -- 8.법인등록번호;
                      || LPAD('1', 1, 0)  -- 9.제출대상기간 코드(연간합산제출: “1”, 휴.폐업에 따른 수시제출 : "2", 수시분할제출:"3")
                      --> 제출내역;
                      || LPAD(NVL(S_RA.PERSON_COUNT, 0), 7, 0)   -- 10.수록한 C레코드의 수(근로소득자의 수);
                      || LPAD(' ', 7, ' ')   -- 11.2013년도 수정 : 공란 처리  
                      || LPAD(NVL(S_RA.RETIRE_TOTAL_AMOUNT, 0), 14, 0)      --12.퇴직급여액 총계(C레코드 퇴직급여 합);
                    ...
                    FROM HRM_CORP_MASTER CM
                       ...
                       , ( -- 제출내역.
                          SELECT RA.ADJUSTMENT_YYYY
                              ...
                              , SUM(CASE
                                      WHEN SQ1.ACCOUNT_NUM IS NULL THEN 0
                                      ELSE NVL(RA.INCOME_TAX_AMOUNT, 0)
                                           /*(NVL(RA.COMP_TAX_AMOUNT_1, 0) +
                                            NVL(RA.COMP_TAX_AMOUNT_2, 0)) */
                                    END) AS TRANS_INCOME_TAX_AMOUNT
                              , SUM(CASE
                                      WHEN SQ1.ACCOUNT_NUM IS NOT NULL THEN 0
                              ...
                              , SUM(CASE
                                      WHEN SQ1.ACCOUNT_NUM IS NOT NULL THEN 0
                                      ELSE NVL(RA.RESIDENT_TAX_AMOUNT, 0)
                                           /*TRUNC((NVL(RA.COMP_TAX_AMOUNT_1, 0) +
                                                  NVL(RA.COMP_TAX_AMOUNT_2, 0)) / 10)*/
                                    END) AS SUB_RESIDENT_TAX_AMOUNT  -- 차감 지방소득세 
                              , SUM(CASE
                                      WHEN SQ1.ACCOUNT_NUM IS NOT NULL THEN 0
                                      ELSE  NVL(RA.SP_TAX_AMOUNT, 0)
                                            /* NVL(RA.SP_TAX_AMOUNT_1, 0) + NVL(RA.SP_TAX_AMOUNT_2, 0) */
                                    END) AS SUB_SP_TAX_AMOUNT 
                              , SUM(CASE
                                      WHEN SQ1.ACCOUNT_NUM IS NOT NULL THEN 0
                    ...
                    WHERE CM.CORP_ID            = OU.CORP_ID
                      ...
                      --AND OU.OPERATING_UNIT_ID  = S_RA.OPERATING_UNIT_ID
                      AND OU.CORP_ID            = A1.CORP_ID
                      AND OU.SOB_ID             = A1.SOB_ID
                      AND OU.ORG_ID             = A1.ORG_ID 
                      --AND OU.OPERATING_UNIT_ID  = A1.OPERATING_UNIT_ID 
                      AND CM.ENABLED_FLAG   = 'Y'
                      AND (OU.DEFAULT_FLAG  = 'Y'
                        OR (OU.DEFAULT_FLAG = 'N'
...
--C1 ------------------------------------------------------------------------------------
        FOR C1 IN ( SELECT
                        ...
                        -- [원천징수의무자] -- 
                        || RPAD(NVL(B1.VAT_NUMBER, ' '), 10, ' ')       -- 5. 사업자번호.
                        || RPAD('1', 1, 1)                              -- 6. 징수의무자 구분(1:사업장, 3:공적연금사업자)  
                        ...
                        || LPAD(CASE
                                 WHEN SQ1.ISSUE_DATE IS NULL THEN '0'
                                 ELSE NVL(TO_CHAR(SQ1.SIGN_UP_DATE, 'YYYYMMDD'), '0')
                               END, 8, '0')                                             -- 13. 확정급여형 퇴직연금제도 가입일 
                        || LPAD(CASE
                                  WHEN TO_CHAR(NVL(PM.ORI_JOIN_DATE, PM.JOIN_DATE), 'YYYYMMDD') > '20111231' THEN 0  -- 2012년 이후 입사자는 제외  
                                  WHEN NVL(RA.OFFICER_YN, 'N') = 'Y' THEN 0  -- 임원인 경우만 적용  
                        ...
                        || LPAD(/*CASE
                                  ...
                                  ELSE 0
                                END*/
                                NVL(RA.DED_DAY, 0), 4, 0)                                                --45.23. 제외월수 
                        || LPAD(/*CASE
                                  WHEN NVL(RA.CHANGE_DAY, 0) > 0 THEN CEIL(RA.CHANGE_DAY / 30)
                                  ELSE 0
                                END*/
                        ...
                        || LPAD(/*CASE
                                  ...
                                  ELSE 0
                                END*/
                                NVL(RA.DED_DAY, 0), 4, 0)                                                --54.23. 제외월수 
                        || LPAD(/*CASE
                                  WHEN NVL(RA.CHANGE_DAY, 0) > 0 THEN CEIL(RA.CHANGE_DAY / 30)
                                  ELSE 0
                                END*/
                        ...
                        || LPAD(0, 8, 0)                                                  --58.18. 입사일                                 
                        || LPAD(CASE
                                  WHEN NVL(RA.LONG_DAY_1, 0) = 0 THEN '0' 
                                  ELSE NVL(TO_CHAR(RA.RETIRE_DATE_FR, 'YYYYMMDD'), '0') 
                                END, 8, 0)                                                --59.19. 기산일 
                        || LPAD(CASE
                                  WHEN NVL(RA.LONG_DAY_1, 0) = 0 THEN '0' 
                        ...
                        || LPAD(/*CASE
                                  ...
                                  ELSE 0
                                END*/
                                NVL(RA.DED_DAY, 0), 4, 0)                                                --72.23. 제외월수
                        || LPAD(/*CASE
                                  WHEN NVL(RA.CHANGE_DAY, 0) > 0 THEN CEIL(RA.CHANGE_DAY / 30) 
                                  ELSE 0
                                END*/
                        ...
                        || LPAD(NVL(RA.TAX_STD_AMOUNT, 0) -
                                TRUNC(NVL(RA.TAX_STD_AMOUNT, 0) * 
                                      NVL(RA.LONG_YEAR_1, 0) /
                                      NVL(RA.LONG_YEAR, 0)), 11, 0)                       --90.31. 과세표준안분 
                        || LPAD(NVL(RA.AVG_TAX_STD_AMOUNT_2, 0), 11, 0)                   --91.31. 연평균과세표준
                        || LPAD(NVL(RA.CHG_TAX_STD_AMOUNT_2, 0), 11, 0)                   --92.33. 환산과세표준 
                        || LPAD(NVL(RA.CHG_COMP_TAX_AMOUNT_2, 0), 11, 0)                  --93.34. 환산산출세액                         
                        || LPAD(NVL(RA.AVG_COMP_TAX_AMOUNT_2, 0), 11, 0)                  --94.35. 연평균산출세액   
                        || LPAD(NVL(RA.COMP_TAX_AMOUNT_2, 0), 11, 0)                      --95.36. 산출세액 
                        || LPAD(0, 11, 0)                                                 --96.37. 기납부(기과세이연)세액 
                        || LPAD(0, 11, 0)                                                 --97.38. 신고대상세액 
                        ...
                        || LPAD(NVL(RA.TAX_STD_AMOUNT, 0), 11, 0)                         --98.31. 과세표준안분 
                        || LPAD(0, 11, 0)                                                 --99.32. 연평균과세표준 
                        || LPAD(NVL(RA.CHG_TAX_STD_AMOUNT_2, 0), 11, 0)                   --100.33. 환산과세표준 
                        || LPAD(NVL(RA.CHG_COMP_TAX_AMOUNT_2, 0), 11, 0)                  --101.34. 환산산출세액 
                        || LPAD(NVL(RA.AVG_COMP_TAX_AMOUNT_1, 0) + 
                                NVL(RA.AVG_COMP_TAX_AMOUNT_2, 0), 11, 0)                  --102.35. 연평균산출세액
                        || LPAD((NVL(RA.COMP_TAX_AMOUNT_1, 0) +
                                 NVL(RA.COMP_TAX_AMOUNT_2, 0)), 11, 0)                    --103.36. 산출세액 
                        || LPAD(0, 11, 0)                                                 --104.37. 기납부(기과세이연) 세액 
                        || LPAD(CASE
                        ...
                        || LPAD(CASE
                                  WHEN SQ1.ACCOUNT_NUM IS NULL THEN 0
                                  ELSE NVL(RA.RETIRE_TOTAL_AMOUNT, 0)
                                END, 11, 0)                                               --107.40. 계좌입금금액                                 
                        || LPAD(CASE
                                  WHEN SQ1.ACCOUNT_NUM IS NULL THEN 0
                                  ELSE NVL(RA.RETIRE_TOTAL_AMOUNT, 0)
                                END, 11, 0)                                               --108.41. 최종분 과세대상퇴직급여 
                        || LPAD(CASE
                                  WHEN SQ1.ACCOUNT_NUM IS NULL THEN 0
                                  ELSE CASE
                                         WHEN (NVL(RA.COMP_TAX_AMOUNT_1, 0) +
                                         ...
                                         ELSE (NVL(RA.COMP_TAX_AMOUNT_1, 0) +
                                               NVL(RA.COMP_TAX_AMOUNT_2, 0))
                                       END 
                                END, 11, 0)                                               --109.42. 이연퇴직소득세 
                        
                        -- [납부명세-신고대상세액] -- 
                        || LPAD(CASE
                                  WHEN NVL(RA.INCOME_TAX_AMOUNT , 0)
                                       /*(NVL(RA.COMP_TAX_AMOUNT_1, 0) +
                                        NVL(RA.COMP_TAX_AMOUNT_2, 0))*/ < 0 THEN 1
                                  ELSE 0
                                END, 1, 0)                                                --110.43. 소득세 부호 
                        || LPAD(ABS(NVL(RA.INCOME_TAX_AMOUNT , 0)
                                     /*(NVL(RA.COMP_TAX_AMOUNT_1, 0) +
                                     NVL(RA.COMP_TAX_AMOUNT_2, 0))*/), 11, 0)               --110.43. 소득세액 
                        || LPAD(CASE
                                  WHEN NVL(RA.RESIDENT_TAX_AMOUNT, 0)
                                       /*TRUNC((NVL(RA.COMP_TAX_AMOUNT_1, 0) +
                                              NVL(RA.COMP_TAX_AMOUNT_2, 0)) / 10)*/ < 0 THEN 1
                                  ELSE 0
                                END, 1, 0)                                                --111.43. 지방소득세 부호 
                        || LPAD(ABS(NVL(RA.RESIDENT_TAX_AMOUNT, 0)
                                    /*TRUNC((NVL(RA.COMP_TAX_AMOUNT_1, 0) +
                                           NVL(RA.COMP_TAX_AMOUNT_2, 0)) / 10)*/), 11, 0)   --111.43. 지방소득세액
                        || LPAD(CASE
                                  WHEN NVL(RA.SP_TAX_AMOUNT, 0)
                                       /*(NVL(RA.SP_TAX_AMOUNT_1, 0)  + 
                                        NVL(RA.SP_TAX_AMOUNT_2, 0))*/ < 0 THEN 1
                                  ELSE 0
                                END, 1, 0)                                                --112.43. 농특세부호 
                        || LPAD(ABS(NVL(RA.SP_TAX_AMOUNT, 0)
                                    /*(NVL(RA.SP_TAX_AMOUNT_1, 0)  + 
                                     NVL(RA.SP_TAX_AMOUNT_2, 0))*/), 11, 0)                 --112.43. 농특세액
                        || LPAD(CASE
                                  WHEN NVL(RA.INCOME_TAX_AMOUNT , 0) + 
                                       NVL(RA.RESIDENT_TAX_AMOUNT, 0) + 
                                       NVL(RA.SP_TAX_AMOUNT, 0)
                        ...
                        || LPAD(ABS(NVL(RA.INCOME_TAX_AMOUNT , 0) + 
                                       ...
                                       TRUNC((NVL(RA.COMP_TAX_AMOUNT_1, 0) +
                                              NVL(RA.COMP_TAX_AMOUNT_2, 0)) / 10) + 
                                       (NVL(RA.SP_TAX_AMOUNT_1, 0)  + 
                                        NVL(RA.SP_TAX_AMOUNT_2, 0))*/), 11, 0)                 --113.43. 합계 
                        ...
                        || LPAD(CASE
                                  ...
                                  WHEN NVL(RA.INCOME_TAX_AMOUNT, 0)  
                                       /*(NVL(RA.COMP_TAX_AMOUNT_1, 0) +
                                        NVL(RA.COMP_TAX_AMOUNT_2, 0))*/ < 0 THEN 1
                                  ELSE 0
                                END, 1, 0)                                                --118.44. 소득세 부호 
                        || LPAD(CASE
                                  WHEN SQ1.ACCOUNT_NUM IS NOT NULL THEN 0
                                  ELSE ABS(NVL(RA.INCOME_TAX_AMOUNT, 0) 
                                           /*(NVL(RA.COMP_TAX_AMOUNT_1, 0) +
                                            NVL(RA.COMP_TAX_AMOUNT_2, 0))*/)
                                END, 11, 0)                                               --118.44. 소득세액 
                        || LPAD(CASE
                                  ...
                                  WHEN NVL(RA.RESIDENT_TAX_AMOUNT, 0)  
                                       /*TRUNC((NVL(RA.COMP_TAX_AMOUNT_1, 0) +
                                              NVL(RA.COMP_TAX_AMOUNT_2, 0)) / 10)*/ < 0 THEN 1
                                  ELSE 0
                                END, 1, 0)                                                --119.44. 지방소득세 부호 
                        || LPAD(CASE
                                  WHEN SQ1.ACCOUNT_NUM IS NOT NULL THEN 0
                                  ELSE ABS(NVL(RA.RESIDENT_TAX_AMOUNT, 0)  
                                           /*TRUNC((NVL(RA.COMP_TAX_AMOUNT_1, 0) +
                                            NVL(RA.COMP_TAX_AMOUNT_2, 0)) / 10)*/)
                                END, 11, 0)                                               --119.44. 지방소득세액
                        || LPAD(CASE
                                  ...
                                  WHEN NVL(RA.SP_TAX_AMOUNT, 0)
                                       /*(NVL(RA.SP_TAX_AMOUNT_1, 0)  + 
                                        NVL(RA.SP_TAX_AMOUNT_2, 0))*/ < 0 THEN 1
                                  ELSE 0
                                END, 1, 0)                                                --120.44. 농특세부호 
                        || LPAD(CASE
                                  WHEN SQ1.ACCOUNT_NUM IS NOT NULL THEN 0
                                  ELSE ABS(NVL(RA.SP_TAX_AMOUNT, 0)
                                           /*(NVL(RA.SP_TAX_AMOUNT_1, 0)  + 
                                            NVL(RA.SP_TAX_AMOUNT_2, 0))*/)
                                END, 11, 0)                                               --120.44. 농특세액
                    ...
                    WHERE PM.PERSON_ID            = T1.PERSON_ID 
                      ...
                      AND RA.ADJUSTMENT_YYYY      = P_YEAR_YYYY
                      AND RA.CORP_ID              = A1.CORP_ID
                      AND RA.SOB_ID               = A1.SOB_ID
                      AND RA.ORG_ID               = A1.ORG_ID
                      AND RA.RETIRE_DATE_TO       BETWEEN P_START_DATE AND P_END_DATE
                      AND RA.CLOSED_YN            = 'Y'
                    ORDER BY PM.PERSON_NUM
              )
          LOOP
            ...
            --DBMS_OUTPUT.PUT_LINE(C1.RECORD_FILE);
            FOR D1 IN ( SELECT 'D'                                          -- 1.자료관리번호.
                            || '25'                                         -- 2.퇴직소득 - 숫자 “25”
                            || RPAD(NVL(B1.TAX_OFFICE_CODE, ' '), 3, ' ')   -- 3.세무서코드.
                            || LPAD(C1.C_SEQ_NO, 6, 0)                      -- 4. 일련번호.
                            
                            -- [원천징수의무자] -- 
                            || RPAD(NVL(B1.VAT_NUMBER, ' '), 10, ' ')       -- 5. 사업자번호.
                            || RPAD(' ', 50, ' ')                           -- 6. 징수의무자 구분(1:사업장, 3:공적연금사업자)  
                            
                            -- [소득자] -- 
                            || RPAD(C1.REPRE_NUM, 13, ' ')                  -- 7. 소득자 주민등록번호 
                            || LPAD(1, 2, 0)                                -- 8. 연금계좌 일련번호
                            || RPAD(NVL(RP.CORP_NAME, ' '), 30, ' ')       -- 9. 연금계좌 취급자 
                            || LPAD(NVL(REPLACE(RP.TAX_REG_NUM, '-', ''), ' '), 10, ' ')    --10. 사업자등록번호 
                            || LPAD(NVL(REPLACE(RP.ACCOUNT_NUM, '-', ''), ' '), 20, ' ')    --11. 퇴직연금계좌번호;
                            || LPAD(NVL(TO_CHAR(C1.CLOSED_DATE, 'YYYYMMDD'), '0'), 8, '0')  --12. 입금일         
                            || LPAD(NVL(C1.RETIRE_TOTAL_AMOUNT, 0), 11, 0)  -- 13. 연금계좌 입금한 금액.
                            || RPAD(' ', 924, ' ') AS RECORD_FILE                   
                             , ROW_NUMBER() OVER(ORDER BY RP.PERSON_ID) AS D_SEQ_NO
                          FROM HRR_RETIREMENT_PENSION_V RP
                         WHERE RP.PERSON_ID         = C1.PERSON_ID 
                           AND RP.SOB_ID            = P_SOB_ID
                           AND RP.ORG_ID            = P_ORG_ID
