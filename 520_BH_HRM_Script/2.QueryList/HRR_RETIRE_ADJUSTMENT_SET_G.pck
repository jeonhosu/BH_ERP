...
-----------------------------------------------------------
    ...
    --> 1.1 신규 생성
      ...
      EXCEPTION WHEN OTHERS THEN
        O_MESSAGE := 'Person Infomation Error =>' || SQLERRM;
...
--. 매월 말일 퇴사일 경우 3개월 이전 1일 기준 그외에는 ADD_MONTHS(기준일, -3)
    ...
    BEGIN
      ...
      ;
    EXCEPTION WHEN OTHERS THEN
      O_MESSAGE := 'Long Period Calculation Error : ' || SQLERRM;
      DBMS_OUTPUT.PUT_LINE(O_MESSAGE);
      ROLLBACK;
      RETURN;
    END;
          ...
          SET (RA.TOTAL_PAY_AMOUNT, RA.TOTAL_BONUS_AMOUNT)
            = (SELECT NVL(SUM(DECODE(RAP.WAGE_TYPE, 'P1', RAP.TOTAL_AMOUNT, 0)), 0) AS PAY_TOTAL_AMOUNT
               ...
               WHERE RAP.ADJUSTMENT_ID          = RA.ADJUSTMENT_ID
                 AND RAP.WAGE_TYPE              IN('P1', 'P2', 'P3', 'P5')    -- P1-급여, P2-상여, P5-특별상여.
               ) 
        WHERE RA.ADJUSTMENT_ID      = V_ADJUSTMENT_ID
        ;
      EXCEPTION WHEN OTHERS THEN
...
-- 5. 명예 퇴직금 계산.
      IF C1.HONORARY_AMOUNT > 0 THEN
        ...
        --> 5.1 근속년수
        BEGIN
          V_H_LONG_YEAR := HRM_COMMON_DATE_G.PERIOD_YEAR_F(V_H_RETIRE_DATE_FR, W_RETIRE_DATE_TO, 'CEIL');          
...
------------------------------------------------------------------------------
      ...
      UPDATE HRR_RETIRE_ADJUSTMENT RA
        SET RA.RETIRE_AMOUNT = V_RETR_AMOUNT
          ...
          , RA.H_RETIRE_DATE_TO = RA.RETIRE_DATE_TO
  ...
  EXCEPTION WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('M.Retire Main Error : ' || SQLERRM);
    ROLLBACK;
  END RETIRE_MAIN;
