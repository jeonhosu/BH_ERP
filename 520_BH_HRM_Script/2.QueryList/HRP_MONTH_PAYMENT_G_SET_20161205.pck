  ...
  BEGIN
    ...
    -- 급여 대상자 생성.
      FOR C1 IN ( SELECT PM.PERSON_ID 
                       ...
                       , PM.DIR_INDIR_TYPE
                       , CASE
                           WHEN PM.RETIRE_DATE IS NOT NULL AND P_END_DATE < PM.RETIRE_DATE THEN '1'
                           WHEN PM.RETIRE_DATE IS NOT NULL AND PM.RETIRE_DATE BETWEEN P_START_DATE AND P_END_DATE THEN '3' 
                           ELSE PM.EMPLOYE_TYPE
                         END AS EMPLOYE_TYPE
                       , CASE
                           WHEN PM.RETIRE_DATE IS NOT NULL AND 
                             NVL(PM.JOIN_DATE, PM.ORI_JOIN_DATE) <= P_START_DATE AND 
                             P_END_DATE <= PM.RETIRE_DATE THEN 'N'  -- 해당월 말일퇴사자는 만근으로 처리 함. 
                  ...
                  WHERE PM.PERSON_ID        = T1.PERSON_ID 
                    ...
                    AND (PM.RETIRE_DATE     IS NULL 
                      OR PM.RETIRE_DATE     >= DECODE(P_EXCEPT_YN, 'Y', V_PAY_END_DATE + 1, V_PAY_START_DATE))
...
-- 11 근속수당 생성
  ...
  --4) 3년~5년 : 5만
  --5) 5년이상 : 6만
        IF C1.LONG_YEAR < 2 THEN 
          IF C1.JOIN_ROUTE_CODE = '50' THEN
          -- 파견추천이 아니면 근속수당 지급 안함.
            BEGIN
              SELECT ALT.LONG_ALLOWANCE_AMOUNT
                INTO V_ALLOWANCE_AMOUNT 
                FROM HRM_ALLOWANCE_LONG_TEMP_V ALT
               WHERE C1.LONG_MONTH            BETWEEN ALT.START_LONG_ALLOWANCE
                                                  AND ALT.END_LONG_ALLOWANCE
                 AND ALT.SOB_ID               = C1.SOB_ID
                 AND ALT.ORG_ID               = C1.ORG_ID
                 AND ALT.ENABLED_FLAG         = 'Y'
                 AND ALT.EFFECTIVE_DATE_FR    <= P_STD_DATE
                 AND (ALT.EFFECTIVE_DATE_TO   IS NULL OR ALT.EFFECTIVE_DATE_TO >= P_STD_DATE)
                 AND ROWNUM                   <= 1
              ;
            EXCEPTION
              WHEN OTHERS THEN
		  ...
		  -- 연장 수당 계산.
			FOR R1 IN ( SELECT MTO.PERSON_ID
											 ...
											 , CASE
												 ...
												 END AS OT_TIME
										FROM HRD_MONTH_TOTAL MT 
											 , HRD_MONTH_TOTAL_OT MTO
											 , HRM_OT_TYPE_ALLOWANCE_V OT
									 WHERE MT.MONTH_TOTAL_ID          = MTO.MONTH_TOTAL_ID
										 AND MT.SOB_ID                  = MTO.SOB_ID
										 AND MT.ORG_ID                  = MTO.ORG_ID
										 AND MTO.OT_TYPE                = OT.OT_TYPE
...
-- 72 생산장려수당 : 기준시간 이하 연장근무, 공제일수 있을 경우, 당월 입사..
  ...
  BEGIN
    ...
    LOOP
        ...
        SELECT NVL(SX1.DUTY_11, 0) AS DED_11_COUNT
          ...
          FROM HRD_MONTH_TOTAL MT
            ...
            , ( SELECT MTD.MONTH_TOTAL_ID
                    , NVL(MTD.DUTY_31, 0) + NVL(MTD.DUTY_11, 0) + NVL(MTD.DUTY_54, 0) AS DUTY_11  -- 20140328 전호수 추가 : 조선미s요청-유계결근 포함  
      ...
      -- 중되 입사.
        NULL;
      ELSIF C1.LONG_MONTH < P_LONG_MONTH THEN
        -- 3개월 미만자 지급 안함.
        NULL;
      ELSE
        BEGIN
...
-- 74 복지수당 적용 여부 : 근무율 70%이상, 수습3개월 이상.
  ...
  BEGIN
    ...
    LOOP
      ...
      ELSE
        BEGIN
          -- 2014-03-31 전호수 수정 : 조선미J 요청 
          -- 오만원 지급 : 만기제대(예비역), 필보(방위,의가사제대), 병역특례, 기타 
          -- 삼만원 지급 : 미입력, 면제, 미필 
          SELECT CASE
                   WHEN NVL(C1.ARMY_END_TYPE, '99') IN('1', '2', '3', '6') THEN TO_NUMBER(NVL(HC.VALUE3, 0))
...
-- 75 검사수당 적용 여부
  ...
  AS
    ...
    V_INSPECTION_LONG_MONTH       NUMBER := 0;  -- 검사공정 근속월  
  BEGIN
    O_STATUS := 'F';
    FOR C1 IN ( SELECT MP.MONTH_PAYMENT_ID
                     , MP.PERSON_ID
                     ...
                     , HRM_COMMON_G.GET_CODE_F(NVL(T2.FLOOR_ID, T1.FLOOR_ID), MP.SOB_ID, MP.ORG_ID) AS FLOOR_CODE 
                     , MP.SOB_ID
                     , MP.ORG_ID
                     , MP.EXCEPT_TYPE
                     , PM.ORI_JOIN_DATE
                     , PM.RETIRE_DATE
                     , NVL((SELECT MAX(HH.CHARGE_DATE) AS CHARGE_DATE 
                            FROM HRM_HISTORY_HEADER HH
                               , HRM_HISTORY_LINE   HL 
                            WHERE HH.HISTORY_HEADER_ID  = HL.HISTORY_HEADER_ID
                              AND HL.PERSON_ID          = MP.PERSON_ID
                              AND HH.CHARGE_SEQ          IN 
                                    (SELECT MAX(S_HH.CHARGE_SEQ) AS CHARGE_SEQ
                                        FROM HRM_HISTORY_HEADER S_HH
                                           , HRM_HISTORY_LINE   S_HL
                                           ...
                                           , HRM_FLOOR_V        P_HF
                                       WHERE S_HH.HISTORY_HEADER_ID = S_HL.HISTORY_HEADER_ID
                                         AND S_HL.FLOOR_ID          = T_HF.FLOOR_ID
                                         AND S_HL.PRE_FLOOR_ID      = P_HF.FLOOR_ID
                                         AND S_HL.PERSON_ID         = HL.PERSON_ID
                                         ...
                                         AND EXISTS
                                               (SELECT 'X'
                                                ...
                                                WHERE HC.GROUP_CODE       = 'ALLOWANCE_INSPECTION'
                                                  ...
                                                  AND HC.VALUE5           = T_HF.FLOOR_CODE    -- 작업장           
                                                  AND HC.ENABLED_FLAG     = 'Y'
                                                  AND HC.EFFECTIVE_DATE_FR  <= P_STD_DATE
                                                  AND (HC.EFFECTIVE_DATE_TO IS NULL OR HC.EFFECTIVE_DATE_TO >= P_STD_DATE) 
                                               )             
                                         AND NOT EXISTS
                                               (SELECT 'X'
                                                ...
                                                WHERE HC.GROUP_CODE       = 'ALLOWANCE_INSPECTION'
                                                  ...
                                                  AND HC.ORG_ID           = MP.ORG_ID
                                                  --AND HC.VALUE2           = C1.DEPT_CODE
                                                  AND HC.VALUE5           = P_HF.FLOOR_CODE            
                                                  AND HC.ENABLED_FLAG     = 'Y'
                                                  AND HC.EFFECTIVE_DATE_FR  <= P_STD_DATE
                                                  AND (HC.EFFECTIVE_DATE_TO IS NULL OR HC.EFFECTIVE_DATE_TO >= P_STD_DATE) 
                                               )             
                                       GROUP BY S_HL.PERSON_ID
                                     )), PM.JOIN_DATE) AS CHARGE_DATE  -- 발령일자.
                     -- 검사 수당 추가 지급자 -- 
                  ...
                  FROM HRP_MONTH_PAYMENT MP
                    ...
                    , (-- 시점 인사내역.
                        ...
                        WHERE PH.CORP_ID            = P_CORP_ID
                          ...
                          /* -- 전호수 주석(2014-06-03) : 현재 시점 작업장 위해 -- 
    ...
    LOOP
      ...
      END;
			-- 검사공정 근속월 계산 -- 
      V_INSPECTION_LONG_MONTH := HRM_COMMON_DATE_G.PERIOD_MONTH_F(C1.CHARGE_DATE, P_STD_DATE, 1, 0);
      IF NVL(V_LONG_MONTH, 0) > V_INSPECTION_LONG_MONTH THEN
        -- 3개월 이전 입사자만 미지급.
        V_ALLOWANCE_AMOUNT := 0;
      END IF;
            
      IF NVL(C1.INSPECTION_ADDITION_AMT, 0) > 0 THEN
        -- 검사수당 예외 지급자 적용 -- 
        V_ALLOWANCE_AMOUNT := NVL(C1.INSPECTION_ADDITION_AMT, 0);
      END IF;
      ...
      IF NVL(V_LONG_MONTH, 0) = V_INSPECTION_LONG_MONTH AND '15' < TO_CHAR(NVL(C1.CHARGE_DATE, C1.ORI_JOIN_DATE), 'DD') THEN
