...
-- DAY INTERFACE TRANS MANAGER SELECT
  ...
  BEGIN
    ...
    LOOP
      ...
      (SELECT DISTINCT
         ...
         WHERE DM.CORP_ID               = W_CORP_ID
           ...
           AND DM.USABLE                = 'Y'
           AND DM.START_DATE            <= (W_START_DATE + C1)
           AND (DM.END_DATE IS NULL OR DM.END_DATE >= (W_START_DATE + C1))
       );
    END LOOP C1;  
    
    FOR C1 IN ( SELECT DT.SOB_ID
                     , DT.ORG_ID
                     , DT.WORK_DATE
                     , DT.DUTY_MANAGER_ID
                     , DT.DUTY_CONTROL_ID
                     , DT.WORK_TYPE_ID
                     , DT.WORK_CORP_ID  
                  FROM HRD_DAY_TRANS_GT DT
                 ORDER BY DT.WORK_DATE
              )
    LOOP
      UPDATE HRD_DAY_TRANS_GT DT 
         SET DT.DAY_INTERFACE_COUNT = NVL(( SELECT COUNT(DISTINCT DI.PERSON_ID) AS PERSON_COUNT 
                                              FROM HRD_DAY_INTERFACE DI
                                                 ...
                                                 , (-- ½ÃÁ¡ ÀÎ»ç³»¿ª.
                                                     SELECT PH.PERSON_ID
                                                          ...
                                                          , PH.WORK_TYPE_ID
                                                       FROM HRD_PERSON_HISTORY        PH
                                                      WHERE PH.EFFECTIVE_DATE_FR  <=  C1.WORK_DATE 
                                                        AND PH.EFFECTIVE_DATE_TO  >=  C1.WORK_DATE 
                                                    ) T2
                                             WHERE DI.PERSON_ID             = PM.PERSON_ID
                                               AND PM.PERSON_ID             = T2.PERSON_ID
                                               
                                               AND T2.FLOOR_ID              = DT.DUTY_CONTROL_ID
                                               
                                               AND DI.WORK_DATE             = DT.WORK_DATE
                                               AND PM.WORK_CORP_ID          = DT.WORK_CORP_ID   
                                               AND PM.SOB_ID                = DT.SOB_ID
                                               AND PM.ORG_ID                = DT.ORG_ID 
                                               AND PM.JOIN_DATE             <= DT.WORK_DATE 
                                               AND (PM.RETIRE_DATE IS NULL OR PM.RETIRE_DATE >= DT.WORK_DATE ) 
                                          ),0)
       WHERE DT.SOB_ID          = C1.SOB_ID
         AND DT.ORG_ID          = C1.ORG_ID 
         AND DT.WORK_DATE       = C1.WORK_DATE
         AND DT.DUTY_MANAGER_ID = C1.DUTY_MANAGER_ID
         ;
         
      -- ÀÏ±ÙÅÂ ³»¿ª -- 
      UPDATE HRD_DAY_TRANS_GT DT 
         SET ( DT.DAY_LEAVE_N_COUNT
             , DT.DAY_LEAVE_Y_COUNT) = ( SELECT SUM(DECODE(DL.CLOSED_YN, 'Y', 0, 1)) AS PERSON_N_COUNT 
                                              , SUM(DECODE(DL.CLOSED_YN, 'Y', 1, 0)) AS PERSON_Y_COUNT 
    ...
    OPEN P_CURSOR FOR
      SELECT DT.WORK_DATE
          , T1.DEPT_NAME
          , T1.POST_NAME
          , HF.FLOOR_NAME AS DUTY_MANAGER_NAME
          , (SELECT NAME 
               FROM HRM_PERSON_MASTER PM
              WHERE PM.PERSON_ID = DT.MANAGER_ID1) AS MANAGER_NAME
          , DT.DAY_INTERFACE_COUNT AS DAY_INTERFACE_COUNT
          , DT.DAY_LEAVE_N_COUNT AS DAY_LEAVE_COUNT
          , DT.DAY_LEAVE_Y_COUNT AS DAY_LEAVE_CLOSE_COUNT
          , 'N' AS CHECK_YN
          , DT.DUTY_MANAGER_ID AS DUTY_MANAGER_ID
        FROM HRD_DAY_TRANS_GT DT   
           , HRM_FLOOR_V      HF
           , (-- ½ÃÁ¡ ÀÎ»ç³»¿ª.
              SELECT HL.PERSON_ID
       ...
       WHERE DT.MANAGER_ID1         = T1.PERSON_ID 
         AND DT.DUTY_CONTROL_ID     = HF.FLOOR_ID 
       
       ORDER BY DT.WORK_DATE, HF.SORT_NUM, HF.FLOOR_CODE 
        ;
  ...
  FOR C1 IN ( SELECT  SESSION_ID
                    ...
                    , SOB_ID
                    , ORG_ID
                    , N_VALUE1  -- DUTY MANAGER ID 
                    , N_VALUE2  -- DUTY CONTROL ID 
                    , N_VALUE3  -- WORK TYPE ID 
  ...
  LOOP
    UPDATE HRD_WORK_DATE WD 
       SET
     WHERE WD.SESSION_ID      = C1.SESSION_ID 
       AND WD.RUN_DATETIME    = C1.RUN_DATETIME
       AND WD.WORK_DATE       = C1.WORK_DATE 
       AND WD.PERSON_ID       = C1.PERSON_ID 
       AND WD.V_VALUE1        = C1.N_VALUE1
     ;
  END LOOP C1; 
  ...
  BEGIN
            ...
            OPEN P_CURSOR FOR
             ...
             WHERE DI.PERSON_ID                                =  PM.PERSON_ID
               ...
               AND DI.WORK_TYPE_ID                             =  NVL(W_WORK_TYPE_ID, DI.WORK_TYPE_ID)
               AND T2.FLOOR_ID                                 =  NVL(W_FLOOR_ID, T2.FLOOR_ID)
               AND (DI.open_time1                              IS NOT NULL
               OR   DI.CLOSE_TIME1                             IS NOT NULL)
               AND PM.JOIN_DATE                               <=  W_WORK_DATE_TO
               AND (PM.RETIRE_DATE IS NULL OR PM.RETIRE_DATE  >=  W_WORK_DATE_FR)
               AND EXISTS (SELECT 'X'
                                   FROM HRD_DUTY_MANAGER DM
