...
--. 매월 말일 퇴사일 경우 3개월 이전 1일 기준 그외에는 ADD_MONTHS(기준일, -3)
    ...
    BEGIN
      UPDATE HRR_RETIRE_ADJUSTMENT RA
      SET RA.LONG_YEAR_2 = CASE
                             WHEN (NVL(RA.LONG_YEAR, 0) - NVL(RA.LONG_YEAR_1, 0)) < 0 THEN 0
                             ELSE (NVL(RA.LONG_YEAR, 0) - NVL(RA.LONG_YEAR_1, 0)) 
                           END 
        , RA.LONG_MONTH_2 = CASE
                              WHEN (NVL(RA.LONG_MONTH, 0) - NVL(RA.LONG_MONTH_1, 0)) < 0 THEN 0
                              ELSE (NVL(RA.LONG_MONTH, 0) - NVL(RA.LONG_MONTH_1, 0)) 
                            END
        , RA.LONG_DAY_2 = CASE
                             WHEN (NVL(RA.LONG_DAY, 0) - NVL(RA.LONG_DAY_1, 0)) < 0 THEN 0
                             ELSE (NVL(RA.LONG_DAY, 0) - NVL(RA.LONG_DAY_1, 0)) 
                           END        
      WHERE RA.ADJUSTMENT_ID      = W_ADJUSTMENT_ID
      ;
    EXCEPTION WHEN OTHERS THEN
...
---------------------------------------------------------------------------------------------------
      ...
      IF NVL(C1.LONG_YEAR_1, 0) > 0 THEN
        ...
        BEGIN
          ...
          -- 12년도까지 과세표준 금액 산정 --
          V_AVG_TAX_STD_AMOUNT_1 := V_TAX_STD_AMOUNT / (NVL(C1.LONG_YEAR_1, 0) + NVL(C1.LONG_YEAR_2, 0)) * NVL(C1.LONG_YEAR_1, 0);
          -- 12년도까지 년평균과세표준 금액 계산  
          V_AVG_TAX_STD_AMOUNT_1 := TRUNC(V_AVG_TAX_STD_AMOUNT_1 / NVL(C1.LONG_YEAR_1, 0));
        EXCEPTION 
          WHEN OTHERS THEN
            O_STATUS := 'F';
        ...
        --> 소득세
        V_IN_TAX_AMOUNT_1 := TRUNC(NVL(V_COMP_TAX_AMOUNT_1, 0) - NVL(V_TAX_DED_AMOUNT_1, 0), -1);   -- 무조건 버림
        IF V_IN_TAX_AMOUNT_1 < 0 THEN
          V_IN_TAX_AMOUNT_1 := 0;
        END IF;

        --> 주민세
        V_LOCAL_TAX_AMOUNT_1 := TRUNC(V_IN_TAX_AMOUNT_1 * (TAX_RATE_F('RESIDENT', W_SOB_ID, W_ORG_ID) / 100), -1);   -- 무조건 버림
        
        --> 실지급액 (총퇴직금 - 주민세 - 소득세 - 기타공제 - 퇴직전환금)
        V_REAL_AMOUNT := NVL(V_REAL_AMOUNT, 0) - NVL(V_IN_TAX_AMOUNT_1, 0) - NVL(V_LOCAL_TAX_AMOUNT_1, 0);
        IF V_REAL_AMOUNT < 0 THEN
          V_REAL_AMOUNT := 0;
        END IF;
      END IF;
      ...
      IF NVL(C1.LONG_YEAR_2, 0) > 0 THEN
        ...
        BEGIN
          ...
          -- 13년도 과세표준 금액 산정 --
          V_AVG_TAX_STD_AMOUNT_2 := (V_TAX_STD_AMOUNT / (NVL(C1.LONG_YEAR_1, 0) + NVL(C1.LONG_YEAR_2, 0))) * 5;
          V_AVG_TAX_STD_AMOUNT_2 := TRUNC(V_AVG_TAX_STD_AMOUNT_2);
        EXCEPTION 
          WHEN OTHERS THEN
            O_STATUS := 'F';
            O_MESSAGE := 'Avg Tax STD Amount Error : ' || SUBSTR(SQLERRM, 1, 150);       
        ...
        --> 산출세액
        V_AVG_COMP_TAX_AMOUNT_2 := AVG_TAX_AMOUNT_F(W_RETR_YYYY, W_SOB_ID, W_ORG_ID, V_AVG_TAX_STD_AMOUNT_2);

        --> 소득세액공제
        V_TAX_DED_AMOUNT_2 := TAX_SUBTRACT_F(W_RETR_YYYY, W_SOB_ID, W_ORG_ID, C1.RETIRE_DATE_TO, V_AVG_COMP_TAX_AMOUNT_2, C1.LONG_YEAR_2);

        --> 산출세액
        V_COMP_TAX_AMOUNT_2 := TRUNC((V_AVG_COMP_TAX_AMOUNT_2  / 5) * C1.LONG_YEAR_2);
        
        --> 소득세
        V_IN_TAX_AMOUNT_2 := TRUNC(NVL(V_COMP_TAX_AMOUNT_2, 0) - NVL(V_TAX_DED_AMOUNT_2, 0), -1);   -- 무조건 버림
        -- 2013-07-13 START OF MODIFY --
        -- 전호수 추가 : 원단위 보정(2012년/2013년 분할계산으로 인해 소득세 합계와 원단위 차액 발생) --
        V_TEMP_AMOUNT :=0;
        V_TEMP_AMOUNT := TRUNC((NVL(V_COMP_TAX_AMOUNT_1, 0) + NVL(V_COMP_TAX_AMOUNT_2, 0)), -1) - 
                         (TRUNC(NVL(V_COMP_TAX_AMOUNT_1, 0), -1) + TRUNC(NVL(V_COMP_TAX_AMOUNT_2, 0), -1));
        ...
        --> 주민세
        V_LOCAL_TAX_AMOUNT_2 := TRUNC(V_IN_TAX_AMOUNT_2 * (TAX_RATE_F('RESIDENT', W_SOB_ID, W_ORG_ID) / 100), -1);   -- 무조건 버림
        
        --> 실지급액 (총퇴직금 - 주민세 - 소득세 - 기타공제 - 퇴직전환금)
        V_REAL_AMOUNT := NVL(V_REAL_AMOUNT, 0) - NVL(V_IN_TAX_AMOUNT_2, 0) - NVL(V_LOCAL_TAX_AMOUNT_2, 0);
        IF V_REAL_AMOUNT < 0 THEN
          V_REAL_AMOUNT := 0;
        END IF;
      END IF;
...
------------------------------------------------------------------------------      
      V_REAL_TOTAL_AMOUNT := NVL(V_REAL_AMOUNT, 0) + NVL(V_H_REAL_AMOUNT, 0) + 
                             (NVL(C1.ETC_SUPP_AMOUNT, 0) - NVL(C1.ETC_DED_AMOUNT, 0) - NVL(C1.HEALTH_INSUR_AMOUNT, 0)) - 
