...
-- 원천징수이행상황신고서 작성 메인.
  ...
  BEGIN
    ...
    -- 당월 조정 환급세액 대상 금액 -- 
    V_THIS_REFUND_TAX_AMT := NVL(V_RECEIVE_REFUND_TAX_AMT, 0);
      ...
      SET WD.A10_PERSON_CNT         = NVL(WD.A01_PERSON_CNT, 0) + NVL(WD.A02_PERSON_CNT, 0) + 
        ...
        , WD.A10_PAY_INCOME_TAX_AMT = CASE
                                        WHEN NVL(WD.A01_INCOME_TAX_AMT, 0) + NVL(WD.A02_INCOME_TAX_AMT, 0) + 
                                             NVL(WD.A03_INCOME_TAX_AMT, 0) + NVL(WD.A04_INCOME_TAX_AMT, 0) + 
                                             NVL(WD.A01_ADD_TAX_AMT, 0) + NVL(WD.A02_Add_Tax_Amt, 0) <= 0 THEN 0
...
-- 사업소득 계산.
  ...
  BEGIN
    O_STATUS := 'F';
    BEGIN
      SELECT COUNT(DISTINCT IRB.EARNER_ID) AS PERSON_COUNT
