...
-- 5. 지급합계;  
  ...
  BEGIN
    ...
    OPEN P_CURSOR1 FOR
      ...
      SELECT  MP.CORP_ID
            , MP.PERSON_ID
            , SUM(MP.REAL_AMOUNT) AS REAL_AMOUNT  -- 총 실지급액.
            , SUM(MP.TOT_SUPPLY_AMOUNT) AS TOT_SUPPLY_AMOUNT  -- 총지급액.
            , SUM(MP.TOT_DED_AMOUNT) AS TOT_DED_AMOUNT  -- 총공제액.
            , SUM(DECODE(MP.WAGE_TYPE, 'P1', MP.REAL_AMOUNT, 0)) AS REAL_PAY_AMOUNT  -- 실지급액(월급여).
            , SUM(DECODE(MP.WAGE_TYPE, 'P1', MP.TOT_SUPPLY_AMOUNT, 0)) AS TOT_PAY_SUP_AMOUNT  -- 지급합계(월급여).
            , SUM(DECODE(MP.WAGE_TYPE, 'P1', MP.TOT_DED_AMOUNT, 0)) AS TOT_PAY_DED_AMOUNT  -- 공제하계(월급여).
            , SUM(DECODE(MP.WAGE_TYPE, 'P2', MP.REAL_AMOUNT, 0)) AS REAL_BONUS_AMOUNT  -- 실지급액(상여금).
            , SUM(DECODE(MP.WAGE_TYPE, 'P2', MP.TOT_SUPPLY_AMOUNT, 0)) AS TOT_BONUS_SUP_AMOUNT  -- 지급합계(상여금).
            , SUM(DECODE(MP.WAGE_TYPE, 'P2', MP.TOT_DED_AMOUNT, 0)) AS TOT_BONUS_DED_AMOUNT     -- 공제합계(상여금).
        FROM HRP_MONTH_PAYMENT MP
          , HRM_PERSON_MASTER PM
      WHERE MP.PERSON_ID            = PM.PERSON_ID
        AND ((MP.PAY_YYYYMM         = W_PAY_YYYYMM
