  ...
  BEGIN
    OPEN P_CURSOR FOR
      SELECT    
           ...
           , PM.RETIRE_DATE                                         -- 퇴직일자.
           , RA.CLOSED_DATE                                         -- 지급일자.
           , RA.LONG_DAY || '일(' || HRM_COMMON_DATE_G.PERIOD_YYYY_MM_DD_F(RA.RETIRE_DATE_FR, RA.RETIRE_DATE_TO, 0, 1) || ')' AS CONTINUE_DAY    -- 근무일수.
           , RA.DED_DAY                                             -- 제외일수(휴직일수).
           , RA.LONG_DAY                                            -- 산정일수.
           ...
           , TRUNC(NVL(RA.TOTAL_BONUS_AMOUNT, 0) / 4) + TRUNC(NVL(RA.YEAR_ALLOWANCE_AMOUNT, 0) / 4) AS ETC_PAY_AMOUNT  -- 상여/연차.
           ...
           , T1.START_DATE1
           , T1.END_DATE1
           , T1.END_DATE1 - T1.START_DATE1 + 1 AS DAY1
           , T1.TOTAL_AMOUNT1
           , T1.PAY_YYYYMM2
           , T1.START_DATE2
           , T1.END_DATE2
           , T1.END_DATE2 - T1.START_DATE2 + 1 AS DAY2
           , T1.TOTAL_AMOUNT2
           , T1.PAY_YYYYMM3
           , T1.START_DATE3
           , T1.END_DATE3
           , T1.END_DATE3 - T1.START_DATE3 + 1 AS DAY3
           , T1.TOTAL_AMOUNT3
           , T1.PAY_YYYYMM4
           , T1.START_DATE4                           
           , T1.END_DATE4
           , T1.END_DATE4 - T1.START_DATE4 + 1 AS DAY4
           , T1.TOTAL_AMOUNT4 
           ----------------------------------------------------------------------------------------------
           , RA.RETIRE_AMOUNT                                                                                      -- 퇴직급여.
           , NULL AS RETIRE_INSUR_AMOUNT                                                                           -- 단체퇴직보험금.
           , NVL(RA.HONORARY_AMOUNT, 0) + NVL(RA.GLORY_AMOUNT, 0) AS HONORARY_AMOUNT                               -- 명예퇴직금 + 위로금.
           ...
           , RA.INCOME_TAX_AMOUNT                                                                                  -- 소득세.
           , RA.RESIDENT_TAX_AMOUNT                                                                                -- 주민세.
           , NULL AS HEALTH_INSUR_AMOUNT                                                                           -- 건강보험정산액.
           ...
           , PM.NAME AS NAME1
           , RA.ADJUSTMENT_ID
           , TO_CHAR(PM.EXPIRE_DATE, 'YYYY-MM') AS PAY_YYYYMM
        FROM HRM_PERSON_MASTER PM
