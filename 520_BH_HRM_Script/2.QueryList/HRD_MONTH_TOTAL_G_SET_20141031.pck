  ...
  BEGIN
    ...
    BEGIN
      V_CAP_C := HRM_MANAGER_G.USER_CAP_F
                   ( HRM_CORP_MASTER_G.DEFAULT_CORP_ID_F(W_SOB_ID, W_ORG_ID)
...
---------------------------------------------------------------------------------------------------
        ...
        , V_TOTAL_DED_DAY
        , CASE 
            WHEN C1.PAY_TYPE IN ('1', '3') AND C1.JOB_CATEGORY_CODE = '10' THEN CASE
                                                                                  WHEN NVL(V_TOTAL_DAY, 0) - NVL(C1.HOLY_0_COUNT, 0) < 0 THEN 0
                                                                                  ELSE NVL(V_TOTAL_DAY, 0) - NVL(C1.HOLY_0_COUNT, 0)
                                                                                END 
...
--   지각/조퇴 4H이상에 대한 주휴공제일수
    ...
    FOR C1 IN ( SELECT DL.DUTY_ID
                ...
                WHERE DL.DUTY_ID                = DC.DUTY_ID
                  ...
                  AND DL.ORG_ID                 = W_ORG_ID
                  AND DL.CLOSED_YN              = 'Y'
                  AND DC.DUTY_CODE              IN('11', '31', '54', '78', '95', '97', '105')
               )
    LOOP
      V_ADD_DAY := 7;
      BEGIN
      ...
      BEGIN
        SELECT DISTINCT 
          ...
          FROM (SELECT TO_CHAR((SX1.START_DATE + LEVEL * 7 - 7), 'W') AS WEEK_CODE
                  ...
                  FROM (SELECT TRUNC(TO_DATE(W_DUTY_YYYYMM, 'YYYY-MM'), 'MONTH') AS START_DATE
                             , LAST_DAY(TO_DATE(W_DUTY_YYYYMM, 'YYYY-MM')) AS END_DATE
                          FROM DUAL
                       ) SX1
