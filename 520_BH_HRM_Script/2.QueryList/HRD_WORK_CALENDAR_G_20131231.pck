...
-- Table을 이용한 Work Calendar Create :입력 기간에 데이터만 생성.
  PROCEDURE WORKCAL_SET_TABLE_PERIOD
            ( P_CORP_ID                          IN HRD_WORK_CALENDAR_SET.CORP_ID%TYPE
            , P_WORK_PERIOD                      IN HRD_WORK_CALENDAR_SET.WORK_PERIOD%TYPE
            , P_PERSON_ID                        IN HRD_WORK_CALENDAR.PERSON_ID%TYPE
            , P_FLOOR_ID                         IN HRM_PERSON_MASTER.FLOOR_ID%TYPE
            ...
            , P_WORK_DATE_FR                     IN HRD_WORK_CALENDAR.WORK_DATE%TYPE
            , P_WORK_DATE_TO                     IN HRD_WORK_CALENDAR.WORK_DATE%TYPE
            , P_CHARGE_DATE_FR                   IN DATE  -- 변경 시작일자 --
            , P_CHARGE_DATE_TO                   IN DATE  -- 변경 종료일자 --  
            , P_USER_ID                          IN HRD_WORK_CALENDAR.CREATED_BY%TYPE
            , P_SOB_ID                           IN HRD_WORK_CALENDAR.SOB_ID%TYPE
            , P_ORG_ID                           IN HRD_WORK_CALENDAR.ORG_ID%TYPE
            , O_STATUS                           OUT VARCHAR2
    ...
    SELECT PM.PERSON_ID
        ...
        , (-- 시점 인사내역.
                    ...
                    IN ( SELECT MAX(S_HL.HISTORY_LINE_ID) AS HISTORY_LINE_ID
                         ...
                         WHERE S_HH.HISTORY_HEADER_ID   = S_HL.HISTORY_HEADER_ID
                           AND S_HH.SOB_ID              = P_SOB_ID
                           AND S_HH.ORG_ID              = P_ORG_ID
                           AND S_HH.CHARGE_DATE         <= P_WORK_DATE_TO
                           AND S_HL.PERSON_ID           = HL.PERSON_ID
                         GROUP BY S_HL.PERSON_ID
                       )
					) T1
      WHERE PM.WORK_TYPE_ID         = HC.COMMON_ID
        ...
        AND ((P_FLOOR_ID            IS NULL AND 1 = 1)
          OR (P_FLOOR_ID            IS NOT NULL AND PM.FLOOR_ID = P_FLOOR_ID))
      AND PM.SOB_ID                 = P_SOB_ID
      AND PM.ORG_ID                 = P_ORG_ID
      AND PM.WORK_TYPE_ID           = P_WORK_TYPE_ID
...
-------------------------------------------------------------------------------
    ...
    FOR R1 IN (SELECT DP.DUTY_PERIOD_ID
                  ...
                  FROM HRD_DUTY_PERIOD   DP
                   ...
                   AND DP.PERSON_ID                 = PM.PERSON_ID
									 AND DP.SOB_ID                    = PM.SOB_ID
									 AND DP.ORG_ID                    = PM.ORG_ID
									 AND (TRUNC(DP.WORK_START_DATE)   <= P_WORK_DATE_TO
                   AND  TRUNC(DP.WORK_END_DATE)     >= P_WORK_DATE_FR)
                   AND DP.CORP_ID                   = P_CORP_ID
                   AND DP.SOB_ID                    = P_SOB_ID
                   AND DP.ORG_ID                    = P_ORG_ID
    ...
    LOOP
      BEGIN
        ...
        -- 외근.
          ...
          ;
        ELSE
          IF V_ATTEND_FLAG = 'ALL_NIGHT' THEN
          -- 철야.
            UPDATE HRD_WORK_CALENDAR WC
               SET WC.ALL_NIGHT_YN    = 'Y' 
            WHERE EXISTS ( SELECT 'X'
                           FROM HRD_DUTY_PERIOD DP
                           WHERE DP.DUTY_PERIOD_ID    = R1.DUTY_PERIOD_ID
                             AND WC.WORK_DATE         BETWEEN DP.WORK_START_DATE AND DP.WORK_END_DATE
          ...
          UPDATE HRD_WORK_CALENDAR WC
             SET WC.C_DUTY_ID               = CASE
                                                WHEN R1.NON_PAY_DAY_FLAG != 'Y' AND WC.HOLY_TYPE IN('0', '1') THEN NULL
                                                WHEN WC.C_DUTY_ID IS NULL THEN R1.DUTY_ID
                                                ELSE WC.C_DUTY_ID
                                              END
               , WC.C_DUTY_ID1              = CASE
                                                WHEN R1.NON_PAY_DAY_FLAG != 'Y' AND WC.HOLY_TYPE IN('0', '1') THEN NULL
                                                WHEN WC.C_DUTY_ID IS NOT NULL THEN R1.DUTY_ID
                                                ELSE WC.C_DUTY_ID1
                                              END
               , WC.OPEN_TIME               = CASE
                                                WHEN R1.NON_PAY_DAY_FLAG != 'Y' AND WC.HOLY_TYPE IN('0', '1') THEN NULL
          ...
          WHERE WC.WORK_DATE                BETWEEN R1.WORK_START_DATE AND R1.WORK_END_DATE
            ...
            AND WC.ORG_ID                   = R1.ORG_ID
          ;
          /* -- 전호수주석(201306025) : 법정공휴일에 대해서 무급일 경우 무급적용 위해 --
          UPDATE HRD_WORK_CALENDAR WC
             SET WC.C_DUTY_ID               = CASE
                                                WHEN WC.HOLY_TYPE IN('0', '1') THEN NULL
                                                ELSE DECODE(WC.C_DUTY_ID, NULL, R1.DUTY_ID, WC.C_DUTY_ID)
          ...
          -- 국경일을 제외한 무휴/유휴에 대해서는 원래대로 복원함 --
          UPDATE HRD_WORK_CALENDAR WC
             SET WC.C_DUTY_ID               = CASE
                                                WHEN WC.C_DUTY_ID1 IS NOT NULL THEN R1.DUTY_ID
                                                ELSE NULL
                                              END
               , WC.C_DUTY_ID1              = NULL
               , WC.OPEN_TIME               = WC.OLD_OPEN_TIME
               , WC.CLOSE_TIME              = WC.OLD_CLOSE_TIME
               , WC.OLD_OPEN_TIME           = NULL
               , WC.OLD_CLOSE_TIME          = NULL
          WHERE WC.WORK_DATE                BETWEEN R1.WORK_START_DATE AND R1.WORK_END_DATE
            AND WC.PERSON_ID                = R1.PERSON_ID
            AND WC.SOB_ID                   = R1.SOB_ID
            AND WC.ORG_ID                   = R1.ORG_ID
            AND WC.HOLY_TYPE                IN('0', '1')
            AND NOT EXISTS
                  ( SELECT 'X'
                      FROM HRD_HOLIDAY_CALENDAR HC
                     WHERE HC.WORK_DATE   = WC.WORK_DATE                       
    ...
    FOR R1 IN ( SELECT HT.HOLY_TYPE_ID
               ...
               WHERE HT.PERSON_ID                 = PM.PERSON_ID
                 ...
                 AND (HT.START_DATE               <= P_WORK_DATE_TO
                 AND  HT.END_DATE                 >= P_WORK_DATE_FR)
  ...
  AS
    ...
    CURSOR WORKCAL_REC IS
          ...
          , (-- 시점 인사내역.
                    ...
                    IN ( SELECT MAX(S_HL.HISTORY_LINE_ID) AS HISTORY_LINE_ID
                         ...
                         WHERE S_HH.HISTORY_HEADER_ID   = S_HL.HISTORY_HEADER_ID
                           AND S_HH.SOB_ID              = P_SOB_ID
                           AND S_HH.ORG_ID              = P_ORG_ID
                           AND S_HH.CHARGE_DATE         <= P_CHARGE_DATE_TO
                           AND S_HL.PERSON_ID           = HL.PERSON_ID
                         GROUP BY S_HL.PERSON_ID
                       )
					) T1
      WHERE PM.WORK_TYPE_ID         = HC.COMMON_ID
        ...
        AND PM.ORG_ID               = P_ORG_ID
        AND PM.WORK_TYPE_ID         = P_WORK_TYPE_ID
        AND PM.JOIN_DATE            <= P_CHARGE_DATE_TO
        AND (PM.RETIRE_DATE IS NULL OR PM.RETIRE_DATE >= P_CHARGE_DATE_FR)
     ;
...
--////////////////////////////////////////////////////////////--
  ...
  LOOP
      BEGIN
        -- 기존 자료 삭제.
        DELETE FROM HRD_WORK_CALENDAR WC
        WHERE WC.WORK_DATE                            BETWEEN P_CHARGE_DATE_FR AND P_CHARGE_DATE_TO
          AND WC.SOB_ID                               = P_SOB_ID
          AND WC.ORG_ID                               = P_ORG_ID
          AND WC.PERSON_ID                            = C1.PERSON_ID
        ;
      EXCEPTION WHEN OTHERS THEN
        O_STATUS := 'F';
        O_MESSAGE := 'Work Calendar Delete Error : ' || SQLERRM;
        RETURN;
      END;
...
-------------------------------------------------------------------------------
     ...
     /=========================================================================================*/
      ...
      (SELECT WD.WORK_DATE, WD.PERSON_ID, WD.WORK_CORP_ID, WD.CORP_ID
        ...
        , WD.TMP_HOLY_TYPE, WD.CREATE_TYPE
        FROM HRD_WORK_DATE_GT WD
      WHERE WD.WORK_DATE        BETWEEN P_CHARGE_DATE_FR AND P_CHARGE_DATE_TO  -- 변경 적용일자만 해당 -- 
        AND WD.SOB_ID           = P_SOB_ID
        AND WD.ORG_ID           = P_ORG_ID
      );
      /*--> 임시테이블 DATA 삭제 <--*/
    ...
    FOR R1 IN ( SELECT PM.CORP_ID
                ...
                WHERE DI.PERSON_ID          = DL.PERSON_ID(+)
                  ...
                  AND DI.PERSON_ID          = PM.PERSON_ID
                  AND PM.PERSON_ID          = T1.PERSON_ID
                  AND DI.WORK_DATE          BETWEEN P_CHARGE_DATE_FR AND P_CHARGE_DATE_TO  -- 변경 적용일자만 해당 -- 
                  AND ((P_PERSON_ID         IS NULL AND 1 = 1)
                  ...
                  AND ((P_WORK_TYPE_ID      IS NULL AND 1 = 1)
                    OR (P_WORK_TYPE_ID      IS NOT NULL AND PM.WORK_TYPE_ID = P_WORK_TYPE_ID)) 
                  AND ( NVL(DL.DANGJIK_YN, DI.DANGJIK_YN)     = 'Y'
                    OR NVL(DL.ALL_NIGHT_YN, DI.ALL_NIGHT_YN)  = 'Y'
                      )
                ORDER BY DL.WORK_DATE, DL.PERSON_ID
    ...
    LOOP
      ...
      EXCEPTION WHEN OTHERS THEN
        O_STATUS := 'F';
        O_MESSAGE := 'DANGJIK/ALL_NIGHT_UPDATE_ERROR=>' || SUBSTR(SQLERRM, 1, 200);
        RETURN;
      END;
    END LOOP R1;
    ...
    FOR R1 IN ( SELECT OH.CORP_ID
                ...
                WHERE OH.REQ_NUM            = OL.REQ_NUM
                  ...
                  AND OL.PERSON_ID          = PM.PERSON_ID
                  AND PM.PERSON_ID          = T1.PERSON_ID
                  AND OL.WORK_DATE          BETWEEN P_CHARGE_DATE_FR AND P_CHARGE_DATE_TO  -- 변경 적용일자만 해당 -- 
                  AND ((P_PERSON_ID         IS NULL AND 1 = 1)
                    OR (P_PERSON_ID         IS NOT NULL AND OL.PERSON_ID = P_PERSON_ID)) 
                  AND OH.CORP_ID            = P_CORP_ID
                  AND OH.SOB_ID             = P_SOB_ID
                  AND OH.ORG_ID             = P_ORG_ID
                  AND ((P_FLOOR_ID          IS NULL AND 1 = 1)
    ...
    LOOP
      ...
      EXCEPTION WHEN OTHERS THEN
        O_STATUS := 'F';
        O_MESSAGE := 'OT_UPDATE_ERROR=>' || SUBSTR(SQLERRM, 1, 200);
        RETURN;
      END;
    END LOOP R1;
    << OT_UPDATE_COMPLETE >>
    ...
    FOR R1 IN (SELECT DP.DUTY_PERIOD_ID
                  ...
                  FROM HRD_DUTY_PERIOD   DP
                   ...
                   AND DP.PERSON_ID                 = PM.PERSON_ID
									 ...
									 AND DP.ORG_ID                    = PM.ORG_ID
                   AND PM.PERSON_ID                 = T1.PERSON_ID 
									 AND (TRUNC(DP.WORK_START_DATE)   <= P_CHARGE_DATE_TO    -- 변경 적용일자만 해당 -- 
                   AND  TRUNC(DP.WORK_END_DATE)     >= P_CHARGE_DATE_FR)   -- 변경 적용일자만 해당 -- 
                   AND DP.CORP_ID                   = P_CORP_ID
                   AND DP.SOB_ID                    = P_SOB_ID
                   AND DP.ORG_ID                    = P_ORG_ID
                   AND ((P_PERSON_ID                IS NULL AND 1 = 1)
                     OR (P_PERSON_ID                IS NOT NULL AND DP.PERSON_ID = P_PERSON_ID)) 
                   AND ((P_FLOOR_ID                 IS NULL AND 1 = 1)
                     OR (P_FLOOR_ID                 IS NOT NULL AND T1.FLOOR_ID = P_FLOOR_ID)) 
                   AND PM.WORK_TYPE_ID              = P_WORK_TYPE_ID
                   AND DP.APPROVE_STATUS            = 'C'
    ...
    LOOP
      ...
      EXCEPTION WHEN OTHERS THEN
        O_STATUS := 'F';
        O_MESSAGE := 'PERIOD_UPDATE_ERROR=>' || TO_CHAR(SQLCODE) || SUBSTR(SQLERRM, 1, 200);
        RETURN;
      END;
    END LOOP R1;
    << DUTY_PERIOD_UPDATE_COMPLETE >>
    ...
    FOR R1 IN ( SELECT HT.HOLY_TYPE_ID
               ...
               WHERE HT.PERSON_ID                 = PM.PERSON_ID
                 ...
                 AND HT.ORG_ID                    = PM.ORG_ID
                 AND PM.PERSON_ID                 = T1.PERSON_ID
                 AND (HT.START_DATE               <= P_CHARGE_DATE_TO
                 AND  HT.END_DATE                 >= P_CHARGE_DATE_FR)
                 AND HT.WORK_CORP_ID              = P_CORP_ID
                 AND HT.SOB_ID                    = P_SOB_ID
                 AND HT.ORG_ID                    = P_ORG_ID
                 AND ((P_PERSON_ID                IS NULL AND 1 = 1)
                   OR (P_PERSON_ID                IS NOT NULL AND HT.PERSON_ID = P_PERSON_ID)) 
                 AND ((P_FLOOR_ID                 IS NULL AND 1 = 1)
                   OR (P_FLOOR_ID                 IS NOT NULL AND T1.FLOOR_ID = P_FLOOR_ID)) 
                 AND PM.WORK_TYPE_ID              = P_WORK_TYPE_ID
                 AND HT.APPROVE_STATUS            = 'C'
    ...
    LOOP
      ...
      EXCEPTION WHEN OTHERS THEN
        O_STATUS := 'F';
        O_MESSAGE := 'HOLY_TYPE_UPDATE_ERROR=>' || SUBSTR(SQLERRM, 1, 200);
        RETURN; 
      END;
    END LOOP R1;
    << HOLY_TYPE_UPDATE_COMPLETE >>
