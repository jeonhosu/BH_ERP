...
----------------------------------------
  PROCEDURE HRM_WEB_WORKING_LIST ( P_CURSOR                 OUT     TYPES.TCURSOR
                                 ...
                                 , W_TO_DATE                IN      VARCHAR2 )
  IS
    V_FROM_DATE         DATE := TO_DATE(W_FROM_DATE, 'YYYY-MM-DD');
    V_TO_DATE           DATE := TO_DATE(W_TO_DATE, 'YYYY-MM-DD');
    V_SOB_ID            NUMBER := 10;
    V_ORG_ID            NUMBER := 101;
    V_PERSON_ID         NUMBER;
  BEGIN
          ...
          FROM (SELECT DI.WORK_DATE AS WORK_DATE
                  ...
                  FROM HRD_DAY_INTERFACE_V DI
                     ...
                     , ( SELECT DIT.WORK_DATE - 1 AS WORK_DATE
                          ...
                          WHERE DIT.PERSON_ID     = V_PERSON_ID
                            AND DIT.SOB_ID        = V_SOB_ID
                            AND DIT.ORG_ID        = V_ORG_ID
                            AND DIT.WORK_DATE     BETWEEN (V_FROM_DATE + 1) AND (V_TO_DATE + 1) ) N_DI
                     , ( SELECT DL.WORK_DATE
                              , DL.PERSON_ID
                              , DL.WORK_CORP_ID
                              , DL.SOB_ID
                          ...
                          WHERE DL.PERSON_ID      = V_PERSON_ID
                            AND DL.SOB_ID         = V_SOB_ID
                            AND DL.ORG_ID         = V_ORG_ID
                            AND DL.WORK_DATE      BETWEEN V_FROM_DATE AND V_TO_DATE
                     ...
                     WHERE DI.PERSON_ID                          = PM.PERSON_ID
                       ...
                       AND DI.WORK_DATE                          BETWEEN V_FROM_DATE AND V_TO_DATE
                       AND DI.SOB_ID                             = V_SOB_ID
                       AND DI.ORG_ID                             = V_ORG_ID
                       AND PM.PERSON_ID                          = V_PERSON_ID
    ...
    OPEN P_CURSOR FOR
      ...
      SELECT  MP.CORP_ID
            ...
            , CASE
              ...
              , V_MESSAGE AS MESSAGE   -- 전호수 추가(2013-02-18) : 급여 알림메시지.
    ...
    OPEN P_CURSOR FOR
      SELECT '근무시간' AS TITLE_NAME
           ...
           , CASE 
               ...
               --인사요청 으로 휴일연장근무 제외 처리함(2012-09-13:권춘광)
               ELSE NVL(MTO.REST_TIME, 0) + NVL(MTO.OVER_TIME, 0) + NVL(MTO.NIGHT_TIME, 0) + NVL(MTO.HOLY_0_OT, 0)  
  ...
  BEGIN
    ...
    OPEN P_CURSOR FOR
      SELECT CASE
           ...
           , ( NVL(MT.HOLY_1_COUNT, 0) - 
               NVL(HRD_HOLIDAY_CALENDAR_G.HOLIDAY_COUNT(V_START_DATE, V_END_DATE, MT.PERSON_ID, MT.SOB_ID, MT.ORG_ID), 0) -
               NVL(MT.WEEKLY_DED_COUNT, 0)) AS HOLY_1_COUNT                           -- 주휴일수(법정공휴일 및 주휴공제 제외);
           ...
           , MTD.DUTY_11                                                              -- 결근.           
           , NVL(S_HD.LATE_COUNT, 0) AS LATE_DED_COUNT                                -- 지각수.
