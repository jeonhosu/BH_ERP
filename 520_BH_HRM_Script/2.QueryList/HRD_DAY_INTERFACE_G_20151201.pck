...
-- 일일 근태집계 현황 : 입사자 리스트  --
-----------------------------------------
  PROCEDURE SELECT_DAY_JOIN_PERSON
            ( P_CURSOR1               OUT TYPES.TCURSOR1
            , W_WORK_CORP_ID          IN  NUMBER
            , W_CORP_ID               IN  NUMBER
            , W_WORK_DATE             IN  DATE
 ...
 -- 후일 퇴근 적용.
    ...
    WHERE DI.PERSON_ID                       = W_PERSON_ID
     ...
     BEGIN
       SELECT 
              CASE
                WHEN 'Y' IN(P_SX.ALL_NIGHT_YN, P_SX.DANGJIK_YN) THEN DI.WORK_DATE  
                ELSE DECODE(DI.MODIFY_IN_YN, 'Y', I_DM.MODIFY_TIME, DI.OPEN_TIME) 
              END AS OPEN_TIME
            , CASE
                   -- 후일 퇴근기록 읽어오기(단, 수정사항이 있으면 수정사항 반영).
                   WHEN 'Y' IN(DI.DANGJIK_YN, DI.ALL_NIGHT_YN) 
                     AND 'Y' IN (T_SX.ALL_NIGHT_YN, T_SX.DANGJIK_YN) THEN DI.WORK_DATE   
                   WHEN (DI.NEXT_DAY_YN   = 'Y'
                      OR DI.HOLY_TYPE    IN('N', '3')
                      OR DI.DANGJIK_YN    = 'Y'
                      OR DI.ALL_NIGHT_YN  = 'Y') THEN DECODE(DI.MODIFY_OUT_YN, 'Y', O_DM.MODIFY_TIME, T_SX.CLOSE_TIME)
                   WHEN DI.HOLY_TYPE IN ('0', '1') 
                       ...
                       AND DECODE(DI.MODIFY_IN_YN, 'Y', I_DM.MODIFY_TIME, DI.OPEN_TIME) >
                          ...
                          THEN DECODE(DI.MODIFY_OUT_YN, 'Y', O_DM.MODIFY_TIME, T_SX.CLOSE_TIME)
                   WHEN DI.MODIFY_OUT_YN = 'Y' THEN O_DM.MODIFY_TIME
                   ELSE DI.CLOSE_TIME
              END AS CLOSE_TIME  
         INTO V_IN_TIME
         ...
         FROM HRD_DAY_INTERFACE_V DI
            ...
            , HRD_DAY_MODIFY      O_DM
            , ( SELECT  DI.PERSON_ID
                      , (DI.WORK_DATE + 1) AS WORK_DATE 
                      , CASE
                          WHEN DI.WORK_HOLY_TYPE IN('0', '1', '2') 
                           AND DI.ALL_NIGHT_YN  = 'Y' THEN 'Y' 
                          ELSE 'N' 
                        END AS ALL_NIGHT_YN 
                      , DI.DANGJIK_YN 
                   FROM HRD_DAY_INTERFACE_V DI
                  WHERE DI.WORK_DATE                = (TRUNC(W_WORK_DATE) - 1)  
                    AND DI.PERSON_ID                = W_PERSON_ID
                    AND DI.SOB_ID                   = W_SOB_ID
                    AND DI.ORG_ID                   = W_ORG_ID
              ) P_SX
            , ( SELECT DI.PERSON_ID
                    ...
                    , CASE
                         -- 후일 퇴근기록 읽어오기(단, 수정사항이 있으면 수정사항 반영).
                         WHEN DI.NEXT_DAY_YN   = 'Y' AND DI.MODIFY_OUT_YN = 'N' THEN ( SELECT S_DI.CLOSE_TIME
                         ...
                         WHEN DI.WORK_HOLY_TYPE IN('0', '1', '2') 
                           AND (DI.DANGJIK_YN    = 'Y'
                            OR  DI.ALL_NIGHT_YN  = 'Y') THEN DI.WORK_DATE  
                         WHEN DI.MODIFY_OUT_YN = 'Y' THEN O_DM.MODIFY_TIME
                         ELSE DI.CLOSE_TIME
                      END AS CLOSE_TIME  
                 FROM HRD_DAY_INTERFACE_V DI
                ...
                WHERE DI.DUTY_ID                  = DC.DUTY_ID
                  ...
                  AND DI.WORK_DATE                = O_DM.WORK_DATE(+)
                  AND '2'                         = O_DM.IO_FLAG(+)
                  AND DI.WORK_DATE                = (TRUNC(W_WORK_DATE) + 1)  
                  AND DI.PERSON_ID                = W_PERSON_ID
                  AND DI.SOB_ID                   = W_SOB_ID
                  AND DI.ORG_ID                   = W_ORG_ID
              ) T_SX    
        WHERE DI.DUTY_ID                  = DC.DUTY_ID
          ...
          AND DI.PERSON_ID                = P_SX.PERSON_ID(+)
          AND DI.WORK_DATE                = P_SX.WORK_DATE(+) 
                  
          AND DI.PERSON_ID                = T_SX.PERSON_ID(+)
          AND DI.WORK_DATE                = T_SX.WORK_DATE(+) 
                  
          AND DI.WORK_DATE                = TRUNC(W_WORK_DATE)  
          AND DI.PERSON_ID                = W_PERSON_ID
          AND DI.SOB_ID                   = W_SOB_ID
          AND DI.ORG_ID                   = W_ORG_ID
        ;
     ...
     BEGIN
       UPDATE HRD_DAY_INTERFACE DI
          SET DI.DUTY_ID            = CASE
                                        WHEN DI.DUTY_ID IN(V_A_DUTY_ID, V_NA_DUTY_ID) THEN 
                                          CASE
                                            WHEN V_IN_TIME IS NOT NULL AND V_OUT_TIME IS NOT NULL THEN V_A_DUTY_ID
                                            ELSE V_NA_DUTY_ID 
                                          END 
                                        WHEN DI.DUTY_ID IN (V_PH_DUTY_ID, V_NH_DUTY_ID, V_H_DUTY_ID) THEN 
                                          CASE
                                            WHEN V_IN_TIME IS NOT NULL AND V_OUT_TIME IS NOT NULL THEN 
                                              CASE
                                                WHEN DI.HOLY_TYPE = '0' THEN V_NH_DUTY_ID 
                                                ELSE V_PH_DUTY_ID 
                                              END  
                                            ELSE V_H_DUTY_ID   
                                          END
                                        ELSE DI.DUTY_ID 
                                      END 
        WHERE DI.WORK_DATE          = TRUNC(W_WORK_DATE)  
          AND DI.PERSON_ID          = W_PERSON_ID
          AND DI.SOB_ID             = W_SOB_ID
          AND DI.ORG_ID             = W_ORG_ID 
          AND DI.DUTY_ID            IN(V_A_DUTY_ID, V_NA_DUTY_ID, V_H_DUTY_ID, V_PH_DUTY_ID, V_NH_DUTY_ID)  -- 출근/휴일근무/무휴근무일 경우만 처리 -- 
          AND NOT EXISTS
                ( SELECT 'X'
                    FROM HRD_WORK_CALENDAR WC
                   WHERE WC.WORK_DATE       = DI.WORK_DATE
                     AND WC.PERSON_ID       = DI.PERSON_ID
                     AND WC.SOB_ID          = DI.SOB_ID
                     AND WC.ORG_ID          = DI.ORG_ID
                     AND ( WC.ALL_NIGHT_YN  = 'Y'
                      OR   WC.DANGJIK_YN    = 'Y')
                )
        ; 
     EXCEPTION
       WHEN OTHERS THEN
         RAISE_APPLICATION_ERROR(-20001, 'Open Time/Close Time Clearning Error : ' || CHR(10) || SQLERRM);
         RETURN;
     END;
   END DATA_UPDATE;
...
-- [출퇴근 등록] - 2011-07-14, [2011-08-11], [2011-11-10] 수정
   ...
   END SELECT_DAY;
       ...
       BEGIN
            ...
            -- 이첩 체크.
            V_APPROVAL_STATUS := 'N';
            V_APPROVAL_STATUS := HRD_DAY_INTERFACE_G.TRANSFER_YN_F( W_CORP_ID   => W_CORP_ID
                                                                   , W_WORK_DATE => W_WORK_DATE
                                                                   , W_PERSON_ID => W_PERSON_ID
                                                                   , W_SOB_ID    => W_SOB_ID
                                                                   , W_ORG_ID    => W_ORG_ID
...
--RAISE_APPLICATION_ERROR(-20001, P_ALL_NIGHT_YN || ' | ' || V_ALL_NIGHT_YN || ' | ' || W_CORP_ID || ' | ' || W_WORK_DATE || ' | ' || W_PERSON_ID || ' | ' || W_SOB_ID || ' | ' || W_ORG_ID);
        ...
        END IF;
           ...
           END;
             ...
             EXCEPTION 
               WHEN OTHERS THEN
                 V_OPEN_TIME_CURRENT := NULL;
                 V_CLOSE_TIME_CURRENT := NULL;
             END;
             ...
             BEGIN
               UPDATE HRD_DAY_INTERFACE DI
                  SET DI.DUTY_ID            = CASE
                                                WHEN DI.DUTY_ID IN(V_A_DUTY_ID, V_NA_DUTY_ID) THEN 
                                                  CASE
                                                    WHEN V_OPEN_TIME_CURRENT IS NOT NULL AND V_CLOSE_TIME_CURRENT IS NOT NULL THEN V_A_DUTY_ID
                                                    ELSE V_NA_DUTY_ID 
                                                  END 
                                                WHEN DI.DUTY_ID IN (V_PH_DUTY_ID, V_NH_DUTY_ID, V_H_DUTY_ID) THEN 
                                                  CASE
                                                    WHEN V_OPEN_TIME_CURRENT IS NOT NULL AND V_CLOSE_TIME_CURRENT IS NOT NULL THEN 
                                                      CASE
                                                        WHEN DI.HOLY_TYPE = '0' THEN V_NH_DUTY_ID 
                                                        ELSE V_PH_DUTY_ID 
                                                      END  
...
--------------------------
  ...
  BEGIN
    ...
    -- 잔업 계산 -- 
    HRD_DAY_LEAVE_G_SET.WORKDATA_GO_IF
      ( P_CORP_ID                 => W_WORK_CORP_ID 
      , P_WORK_DATE               => W_WORK_DATE
      , P_WORK_TYPE_ID            => NULL 
      , P_HOLY_TYPE               => NULL 
      , P_DEPT_ID                 => NULL 
      , P_FLOOR_ID                => W_FLOOR_ID
      , P_JOB_CATEGORY_ID         => NULL 
      , P_PERSON_ID               => NULL 
      , P_SOB_ID                  => W_SOB_ID
      , P_ORG_ID                  => W_ORG_ID
      , P_USER_ID                 => NVL(GET_USER_ID_F, -1) 
      , P_CONNECT_PERSON_ID       => W_CONNECT_PERSON_ID
      , O_STATUS                  => V_STATUS 
      , O_MESSAGE                 => V_MESSAGE 
      );
      
    IF V_PRE_WORK_DATE != W_WORK_DATE THEN
      HRD_DAY_LEAVE_G_SET.WORKDATA_GO_IF
        ( P_CORP_ID                 => W_CORP_ID 
        , P_WORK_DATE               => V_PRE_WORK_DATE
        , P_WORK_TYPE_ID            => NULL 
        , P_HOLY_TYPE               => NULL 
        , P_DEPT_ID                 => NULL 
        , P_FLOOR_ID                => W_FLOOR_ID
    ...
    OPEN P_CURSOR FOR
      SELECT NVL(S_DI.WORK_DATE, NULL) AS WORK_DATE
           ...
           , SUM(S_DI.BASE_TIME)            AS BASE_TIME           -- 정상 근무시간 .
           , SUM(S_DI.OT_TIME)              AS OT_TIME             -- 연장근무시간. 
        FROM HRM_FLOOR_V        HF
           , (-- 주간자 처리.
               SELECT DI.WORK_DATE
                     , DI.SOB_ID
                     ...
                     , NVL(CASE
                             WHEN DC.DUTY_CODE = '00' THEN 8
                             WHEN DC.DUTY_CODE = '53' AND DI.ALL_NIGHT_YN != 'Y' THEN NVL(OT.HOLI_TIME, 0) 
                             ELSE 0
                           END, 0) - NVL(OT.LATE_TIME, 0) AS BASE_TIME 
                     , NVL(OT.OT_TIME, 0) AS OT_TIME 
                  FROM HRD_DAY_INTERFACE_V       DI
                     , HRM_DUTY_CODE_V           DC
                     , HRM_PERSON_MASTER         PM
                     , (-- 시점 인사내역.
                     ...
                     , HRD_DAY_MODIFY            I_DM
                     , HRD_DAY_MODIFY            O_DM 
                     , ( -- 잔업시간 : 12 - 지각/조회, 14 - 연장근로, 15-휴일근로, 18-휴일연장 -- 
                        SELECT DIO.PERSON_ID
                             , DIO.WORK_DATE
                             , DIO.SOB_ID
                             , DIO.ORG_ID
                             , SUM(CASE
                                     WHEN DIO.OT_TYPE IN('12') THEN DIO.OT_TIME
                                     ELSE 0
                                   END) AS LATE_TIME
                             , SUM(CASE
                                     WHEN DIO.OT_TYPE IN('14', '18') THEN DIO.OT_TIME
                                     ELSE 0
                                   END) AS OT_TIME
                             , SUM(CASE
                                     WHEN DIO.OT_TYPE IN('15') THEN DIO.OT_TIME
                                     ELSE 0
                                   END) AS HOLI_TIME
                          FROM HRD_DAY_INTERFACE_OT DIO
                         WHERE DIO.WORK_DATE          = W_WORK_DATE 
                           AND DIO.SOB_ID             = W_SOB_ID
                           AND DIO.ORG_ID             = W_ORG_ID
                           AND DIO.OT_TYPE            IN('12', '14', '15', '18')
                     ...
                     , ( -- 잔업시간 : 12 - 지각/조회, 14 - 연장근로, 15-휴일근로, 18-휴일연장 -- 
                        SELECT DIO.PERSON_ID
                             ...
                             , SUM(CASE
                                   ...
                                   END) AS HOLI_TIME
                          FROM HRD_DAY_INTERFACE_OT DIO
                         WHERE DIO.WORK_DATE          = V_PRE_WORK_DATE  
                           AND DIO.SOB_ID             = W_SOB_ID
                           AND DIO.ORG_ID             = W_ORG_ID
                           AND DIO.OT_TYPE            IN('12', '14', '15', '18')
                         GROUP BY DIO.PERSON_ID
                                   ...
                                   AND HH.CHARGE_SEQ          IN 
                                        (SELECT MAX(S_HH.CHARGE_SEQ) AS CHARGE_SEQ
                                            FROM HRM_HISTORY_HEADER S_HH
                                               , HRM_HISTORY_LINE   S_HL
                                           WHERE S_HH.HISTORY_HEADER_ID = S_HL.HISTORY_HEADER_ID
                                             AND S_HH.CHARGE_DATE       <= V_PRE_WORK_DATE
                                             AND S_HL.PERSON_ID         = HL.PERSON_ID
                                           GROUP BY S_HL.PERSON_ID
                                         ) 
                               ) T1
  ...
  BEGIN
    ...
    OPEN P_CURSOR1 FOR
      SELECT NVL(S_DI.WORK_DATE, NULL) AS WORK_DATE
           ...
           , NVL(S_DI.PERSON_NUM, NULL) AS PERSON_NUM
           , NVL(S_DI.NAME, NULL) AS NAME
           , (S_DI.RETIRE_NAME) AS RETIRE_NAME 
        FROM HRM_FLOOR_V        HF
           , (-- 주간자 처리.
               SELECT SX1.FLOOR_ID
                    , COUNT(SX1.FLOOR_PERSON_COUNT) AS FLOOR_PERSON_COUNT
