  ...
  BEGIN
    ...
    BEGIN
      SELECT MAX(HOH.REQ_NUM) AS REQ_NUM  
           , HF.FLOOR_NAME 
           , COUNT(HOH.OT_HEADER_ID) AS N_REQ_COUNT 
        INTO V_REQ_NUM
           , V_FLOOR_NAME
           , V_REQ_COUNT 
       ...
       WHERE HOH.DUTY_MANAGER_ID      = DM.DUTY_MANAGER_ID
         ...
         AND HOH.APPROVE_STATUS       = 'N' 
         AND HOH.REQ_DATE             >= TO_DATE('2014-06-29', 'YYYY-MM-DD') -- 이후 데이터만 적용 -- 
         AND EXISTS
               (SELECT OH.DUTY_MANAGER_ID 
                  FROM HRD_OT_HEADER OH 
                 WHERE OH.DUTY_MANAGER_ID       = HOH.DUTY_MANAGER_ID  
                   AND OH.REQ_TYPE              = HOH.REQ_TYPE 
                   AND OH.REQ_DATE              > HOH.REQ_DATE 
    ...
    IF DATA_STATUS_CHECK(P_OT_HEADER_ID) = 'Y' THEN
      O_STATUS := 'F';
      O_MESSAGE := EAPP_MESSAGE_G.RETURN_TEXT_F(USERENV_G.GET_TERRITORY_S_F, 'FCM_10042', '&&VALUE:=Approval Request(승인요청)');
      RETURN;
    END IF;
