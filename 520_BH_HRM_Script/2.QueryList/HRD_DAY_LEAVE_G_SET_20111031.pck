        ...
        FROM HRD_DAY_LEAVE_V1 DL
           ...
           , (-- 시점 인사내역.
              ...
              WHERE HL.PERSON_ID        = PM.PERSON_ID
							  AND PM.JOB_CATEGORY_ID  = JCC.JOB_CATEGORY_ID
   ...
   -- 주간 근무 계산 --
    ...
    -- 지각 --
      ...
      -- 계획 출근보다 실제 출근이 늦을 경우.
        ...
        END;
      END IF;
      IF NVL(W_PRE_DANGJIK_YN, 'N') = 'Y' OR NVL(W_PRE_ALL_NIGHT_YN, 'N') = 'Y' THEN
        V_LATE_TIME := 0;
        V_EARLY_TIME := 0;
      END IF;
...
/*DBMS_OUTPUT.PUT_LINE('근무일자 : ' || TO_CHAR(W_WORK_DATE, 'YYYY-MM-DD') ||
	  ...
	  FOR C1 IN C_REST_TIME
		...
		IF V_REST_TIME_4 < 0 THEN
		  V_REST_TIME_4 := 0;
		END IF;
		V_REST_TIME := V_REST_TIME_2 + V_REST_TIME_3 + V_REST_TIME_4;
        ...
        -- 3-2. 철야후 후일 아침 근무후 연장 계산.
          ...
          -- 연장 신청 적용한 종료 시간.
					V_OT_END_TIME := OT_END_TIME_F( P_CLOSE_TIME => V_OT_END_TIME
																				, P_PL_END_TIME => C1.END_TIME
																				, P_DEFAULT_RETURN_TIME => C1.END_TIME);
/*DBMS_OUTPUT.PUT_LINE('2-2. 야간근무후 연장 계산 : OT_START =>' || TO_CHAR(V_OT_START_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
                     ', OT_END=>' || TO_CHAR(V_OT_END_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
										 ', C1_START =>' || TO_CHAR(C1.START_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
                     ', C1_END=>' || TO_CHAR(C1.END_TIME, 'YYYY-MM-DD HH24:MI:SS') ||
										 ', XX =>' || TO_CHAR(V_STD_TIME, 'YYYY-MM-DD HH24:MI:SS'));*/
          ...
          IF W_NEXT_HOLY_TYPE IN ('0', '1') AND TO_DATE(TO_CHAR(W_WORK_DATE + 1, 'YYYY-MM-DD') || '08:30', 'YYYY-MM-DD HH24:MI') <  V_OT_END_TIME THEN
...
/*DBMS_OUTPUT.PUT_LINE('3-2. 근무후 연장 계산 : OT_START =>' || TO_CHAR(V_OT_START_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
          ...
          -- 근무후 야간 범위 적용.
					V_OT_END_TIME := OT_END_TIME_F( P_CLOSE_TIME => V_OT_END_TIME
																				, P_PL_END_TIME => C1.END_TIME
																				, P_DEFAULT_RETURN_TIME => C1.END_TIME);
\*DBMS_OUTPUT.PUT_LINE('3-3. 야간근로 계산 : OT_START =>' || TO_CHAR(V_OT_START_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
                     ', OT_END=>' || TO_CHAR(V_OT_END_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
										 ', C1_START =>' || TO_CHAR(C1.START_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
                     ', C1_END=>' || TO_CHAR(C1.END_TIME, 'YYYY-MM-DD HH24:MI:SS') ||
										 ', XX =>' || TO_CHAR(V_STD_TIME, 'YYYY-MM-DD HH24:MI:SS'));*\
...
/*DBMS_OUTPUT.PUT_LINE('3-4. 야간 할증 계산 : OPEN =>' || TO_CHAR(V_OPEN_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
        ...
        -- 1-9. 야간 근무후 휴일근로 계산.
				IF W_NEXT_HOLY_TYPE IN ('0', '1') AND C1.OT_YN = 'Y' AND C1.CAL_TYPE = 'OA' AND C1.HOLY_TYPE = 'DAY' 
					AND V_CLOSE_TIME IS NOT NULL AND W_AFTER_OT_START IS NOT NULL AND W_AFTER_OT_END IS NOT NULL THEN
				  -- 연장 신청 적용한 시작 시간.
					V_OT_START_TIME := OT_START_TIME_F( P_OPEN_TIME => V_OPEN_TIME
					                                  , P_PL_START_TIME => W_AFTER_OT_START
...
/*DBMS_OUTPUT.PUT_LINE('1-1. 휴일 주간 : OT_START =>' || TO_CHAR(V_OT_START_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
        ...
        -- 1-2. 주간 근무후 연장 계산.
				IF (W_ALL_NIGHT_YN <> 'Y' AND W_DANGJIK_YN <> 'Y') AND C1.OT_YN = 'Y' AND C1.CAL_TYPE = 'OA' AND C1.HOLY_TYPE = 'DAY' 
					AND V_CLOSE_TIME IS NOT NULL AND W_AFTER_OT_START IS NOT NULL AND W_AFTER_OT_END IS NOT NULL THEN
				  -- 연장 신청 적용한 시작 시간.
					V_OT_START_TIME := OT_START_TIME_F( P_OPEN_TIME => V_OPEN_TIME
					                                  , P_PL_START_TIME => W_AFTER_OT_START
...
-----------------------------------------------------------------------------------------
        -- 1-3. 야간근로 계산.
				IF W_NEXT_HOLY_TYPE IN('0', '1') AND C1.NIGHT_YN = 'Y' AND C1.CAL_TYPE = 'NT' AND C1.HOLY_TYPE = 'NIGHT' 
					AND V_CLOSE_TIME IS NOT NULL AND W_AFTER_OT_START IS NOT NULL AND W_AFTER_OT_END IS NOT NULL THEN
				  -- 야간 신청 적용한 시작 시간.
					V_OT_START_TIME := OT_START_TIME_F( P_OPEN_TIME => V_OPEN_TIME
					                                  , P_PL_START_TIME => W_AFTER_OT_START
...
/*DBMS_OUTPUT.PUT_LINE('1-3. 휴일 야간 : OT_START =>' || TO_CHAR(V_OT_START_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
          ...
          IF V_STD_TIME <= V_OT_END_TIME THEN
            ...
            END IF;
					END IF;
				END IF;
        -- 1-8. 휴일근무후 주간 근무후 연장 계산.
				IF W_NEXT_HOLY_TYPE = '2' AND C1.OT_YN = 'Y' AND C1.CAL_TYPE = 'OA' AND C1.HOLY_TYPE = 'DAY' 
					AND V_CLOSE_TIME IS NOT NULL AND W_AFTER_OT_START IS NOT NULL AND W_AFTER_OT_END IS NOT NULL THEN
				  -- 연장 신청 적용한 시작 시간.
					V_OT_START_TIME := OT_START_TIME_F( P_OPEN_TIME => V_OPEN_TIME
					                                  , P_PL_START_TIME => W_AFTER_OT_START
...
/*DBMS_OUTPUT.PUT_LINE('2-2. 근무후 연장 계산 : OT_START =>' || TO_CHAR(V_OT_START_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
          ...
          IF V_STD_TIME <= V_OT_END_TIME THEN
            ...
            END;
				...
				END IF;
        -- 1-9. 휴일근무후 야간연장 계산.
				IF W_NEXT_HOLY_TYPE = '3' AND C1.OT_YN = 'Y' AND C1.CAL_TYPE = 'OA' AND C1.HOLY_TYPE = 'NIGHT' 
					AND V_CLOSE_TIME IS NOT NULL AND W_AFTER_OT_START IS NOT NULL AND W_AFTER_OT_END IS NOT NULL THEN
				  -- 연장 신청 적용한 시작 시간.
					V_OT_START_TIME := OT_START_TIME_F( P_OPEN_TIME => V_OPEN_TIME
					                                  , P_PL_START_TIME => W_AFTER_OT_START
...
/*DBMS_OUTPUT.PUT_LINE('3-2. 근무후 연장 계산 : OT_START =>' || TO_CHAR(V_OT_START_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
          ...
          IF V_STD_TIME <= V_OT_END_TIME THEN
					  -- 식사시간 제외.
						...
						IF C1.FOOD_TIME_DED = 'Y' THEN
						  ...
						  V_FOOD_DED_TIME := V_FOOD_DED_TIME +
                                 FOOD_DED_TIME_F ( W_WORK_DATE => W_WORK_DATE + 2
							                                   , W_FOOD_TYPE => '2'
																								 , W_START_TIME => V_OT_START_TIME
																								 , W_END_TIME => V_OT_END_TIME
																								 , W_JOB_CATEGORY_CODE => W_JOB_CATEGORY_CODE
      ...
      -- 9-1. 후일 주간 휴일근로 계산.
      IF (W_ALL_NIGHT_YN = 'Y' AND W_NEXT_HOLY_TYPE IN ('0', '1')) AND C1.HOLIDAY_YN = 'Y' AND C1.CAL_TYPE = 'HD' AND C1.HOLY_TYPE = 'DAY'
        AND V_CLOSE_TIME IS NOT NULL AND W_AFTER_OT_START IS NOT NULL AND W_AFTER_OT_END IS NOT NULL THEN
        -- 신청 적용한 시작 시간.
        V_OT_START_TIME := OT_START_TIME_F( P_OPEN_TIME => V_OPEN_TIME
                                          , P_PL_START_TIME => W_AFTER_OT_START
...
/*DBMS_OUTPUT.PUT_LINE('1-6. 1.후일 휴일 주간 : OPEN_TIME => ' || TO_CHAR(V_OPEN_TIME, 'YYYY-MM-DD HH24:MI:SS') ||
                   ', CLOSE TIME =>' || TO_CHAR(V_CLOSE_TIME, 'YYYY-MM-DD HH24:MI:SS') ||
                   ', OT_START =>' || TO_CHAR(V_OT_START_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
                   ', OT_END=>' || TO_CHAR(V_OT_END_TIME, 'YYYY-MM-DD HH24:MI:SS'));*/
        ...
        V_OT_START_TIME := OT_START_TIME_F( P_OPEN_TIME => V_OT_START_TIME
                                          , P_PL_START_TIME => C1.START_TIME + 1
                                          , P_DEFAULT_RETURN_TIME => C1.END_TIME + 1);
        -- 야간 할증 범위 적용.
        V_OT_END_TIME := OT_END_TIME_F( P_CLOSE_TIME => V_OT_END_TIME
                                      , P_PL_END_TIME => C1.END_TIME + 1
                                      , P_DEFAULT_RETURN_TIME => C1.END_TIME + 1);
        
/*DBMS_OUTPUT.PUT_LINE('1-6. 2.후일 휴일 주간 : OPEN_TIME => ' || TO_CHAR(V_OPEN_TIME, 'YYYY-MM-DD HH24:MI:SS') ||
                   ', CLOSE TIME =>' || TO_CHAR(V_CLOSE_TIME, 'YYYY-MM-DD HH24:MI:SS') ||
                   ', OT_START =>' || TO_CHAR(V_OT_START_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
                   ', OT_END=>' || TO_CHAR(V_OT_END_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
                   ', C1_START =>' || TO_CHAR(C1.START_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
      ...
      END IF;      
      -- 9-2. 주간 근무후 연장 계산.
      IF (W_ALL_NIGHT_YN = 'Y' AND W_NEXT_HOLY_TYPE IN ('0', '1')) AND C1.OT_YN = 'Y' AND C1.CAL_TYPE = 'OA' AND C1.HOLY_TYPE = 'DAY' 
        AND V_CLOSE_TIME IS NOT NULL AND W_AFTER_OT_START IS NOT NULL AND W_AFTER_OT_END IS NOT NULL THEN
        -- 연장 신청 적용한 시작 시간.
        V_OT_START_TIME := OT_START_TIME_F( P_OPEN_TIME => V_OPEN_TIME
                                          , P_PL_START_TIME => W_AFTER_OT_START
...
/*DBMS_OUTPUT.PUT_LINE('1-2. 휴일 주간 근무후 : OT_START =>' || TO_CHAR(V_OT_START_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
    ...
    IF V_NIGHT_BONUS_TIME = 7 THEN
      -- 7H(만근)시 8H로 적용해줌(김일곤K 요청).
      V_NIGHT_BONUS_TIME := 8;
    END IF;
