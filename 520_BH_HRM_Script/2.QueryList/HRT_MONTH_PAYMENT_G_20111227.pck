...
-- 지급/공제 합계 처리.
  ...
  BEGIN
      ...
      -- 기업이윤 UPDATE --
      UPDATE HRT_MONTH_PAYMENT MP
        SET MP.BENEFIT_AMOUNT     = HRT_PAYMENT_G_SET.BENEFIT_AMOUNT_F(MP.STANDARD_DATE, MP.TOT_SUPPLY_AMOUNT, MP.SOB_ID, MP.ORG_ID)
          , MP.REAL_AMOUNT        = NVL(MP.REAL_AMOUNT, 0) + HRT_PAYMENT_G_SET.BENEFIT_AMOUNT_F(MP.STANDARD_DATE, MP.TOT_SUPPLY_AMOUNT, MP.SOB_ID, MP.ORG_ID)
      WHERE MP.MONTH_PAYMENT_ID   = P_MONTH_PAYMENT_ID
      ;
      ...
      UPDATE HRT_MONTH_PAYMENT MP
        SET (MP.DED_PAY_AMOUNT, MP.DED_BONUS_AMOUNT, MP.TOT_DED_AMOUNT, MP.REAL_AMOUNT)
          = ( SELECT NVL(SUM(DECODE(MD.WAGE_TYPE, 'P1', MD.DEDUCTION_AMOUNT, 0)), 0) AS DED_PAY_AMOUNT
                   ...
                   , NVL(MP.TOT_SUPPLY_AMOUNT, 0) + NVL(MP.BENEFIT_AMOUNT, 0) + NVL(SUM(MD.DEDUCTION_AMOUNT), 0) AS REAL_AMOUNT
                FROM HRT_MONTH_DEDUCTION MD
