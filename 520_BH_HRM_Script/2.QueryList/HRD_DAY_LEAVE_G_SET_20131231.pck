    ...
    V_RECORD_COUNT := 0;
        ...
        OR   (P_PERSON_ID                  IS NOT NULL
          AND DL.PERSON_ID                 = P_PERSON_ID)) 
				...
				AND DL.CLOSED_YN                   = 'Y'
				AND ((P_FLOOR_ID                   IS NULL
          AND 1                            = 1) 
        OR   (P_FLOOR_ID                   IS NOT NULL
          AND T1.FLOOR_ID                  = P_FLOOR_ID)) 
				AND ((P_WORK_TYPE_ID               IS NULL
          AND 1                            = 1) 
        OR   (P_WORK_TYPE_ID               IS NOT NULL
          AND WC.WORK_TYPE_ID              = P_WORK_TYPE_ID)) 
				AND ((P_HOLY_TYPE                  IS NULL
          AND 1                            = 1)
        OR   (P_HOLY_TYPE                  IS NOT NULL
          AND WC.HOLY_TYPE                 = P_HOLY_TYPE)) 
        ...
        OR   (P_JOB_CATEGORY_ID            IS NOT NULL
          ...
          AND 1                            = 1)
        OR   (P_DEPT_ID                    IS NOT NULL
          AND T1.DEPT_ID                   = P_DEPT_ID)) 
			;
