...
--> 기준일자 생성;      
    BEGIN
      SELECT /* --전호수 주석(2013-01-24) : 김수신D 요청-1년미만자도 퇴충 계산 --
            ADD_MONTHS(V_STD_DATE, -V_STD_CALCULATE_MONTH) AS PRE_YEAR_DATE*/
            V_STD_DATE AS PRE_YEAR_DATE
                 ...
                 FROM HRM_PERSON_MASTER PM
                   , ( SELECT PMH.PERSON_ID
                       ...
                       WHERE PMH.CORP_ID            = W_CORP_ID
                         ...
                         AND PMH.START_YYYYMM       <= W_RESERVE_YYYYMM
                         AND (PMH.END_YYYYMM         >= W_RESERVE_YYYYMM OR PMH.END_YYYYMM IS NULL)
                      ) MH
...
--> 상여 계산.
      ...
      BEGIN
        SELECT SUM(MA.ALLOWANCE_AMOUNT) AS TOTAL_PAY_AMOUNT
         ...
         WHERE MP.PERSON_ID               = PM.PERSON_ID
           AND MP.MONTH_PAYMENT_ID        = MA.MONTH_PAYMENT_ID
           AND MA.ALLOWANCE_ID            = HAV.ALLOWANCE_ID
           AND MP.PAY_YYYYMM              BETWEEN V_STR_PAY_YYYYMM AND W_RESERVE_YYYYMM  -- 전호수 주석 : BETWEEN V_STR_BONUS_YYYYMM AND W_RESERVE_YYYYMM
           AND MP.WAGE_TYPE               = 'P1'
           AND MP.PERSON_ID               = C1.PERSON_ID
           AND MP.SOB_ID                  = C1.SOB_ID
           AND MP.ORG_ID                  = C1.ORG_ID
      ...
      END;
      --> 예외처리 : 월급/연봉직 상여금은 급여에 포함.
      IF C1.PAY_TYPE IN('1', '3') THEN
        V_TOTAL_PAY_AMOUNT := NVL(V_TOTAL_PAY_AMOUNT, 0) + NVL(V_TOTAL_BONUS_AMOUNT, 0);
        V_TOTAL_AMOUNT := NVL(V_TOTAL_AMOUNT, 0) + NVL(V_TOTAL_BONUS_AMOUNT, 0);
        V_TOTAL_BONUS_AMOUNT := 0;
      END IF;
      -- 2. 상여 항목.
      BEGIN
      ...
      BEGIN
        SELECT NVL(V_TOTAL_BONUS_AMOUNT, 0) + NVL(SUM(MA.ALLOWANCE_AMOUNT), 0) AS TOTAL_PAY_AMOUNT
         ...
         WHERE MP.PERSON_ID               = PM.PERSON_ID
           ...
           AND MA.ALLOWANCE_ID            = HAV.ALLOWANCE_ID
           AND MP.PAY_YYYYMM              BETWEEN V_STR_BONUS_YYYYMM AND W_RESERVE_YYYYMM
           AND MP.WAGE_TYPE               IN('P3', 'P5')  -- 명절상여, 특별상여.
           AND MP.PERSON_ID               = C1.PERSON_ID
           AND MP.SOB_ID                  = C1.SOB_ID
           AND MP.ORG_ID                  = C1.ORG_ID
           AND HAV.RETIRE_YN              = 'Y'
