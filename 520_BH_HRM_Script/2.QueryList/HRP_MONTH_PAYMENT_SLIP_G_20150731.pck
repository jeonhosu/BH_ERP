  ...
  BEGIN
    OPEN P_CURSOR FOR
        ...
        FROM FI_AUTO_JOURNAL_MST AJM
          ...
          , ( -- 지급.
              ...
              WHERE PM.PERSON_ID          = MP.PERSON_ID
                ...
                AND MP.SOB_ID             = P_SOB_ID
                AND MP.ORG_ID             = P_ORG_ID
                AND HD.DEDUCTION_CODE     IN ('D11', 'D24')
                -- 중도 퇴사자 적용 여부 판단.
              GROUP BY MP.PAY_YYYYMM
                   , MP.WAGE_TYPE
                   , PM.SOB_ID
              ...
              SELECT PM.SOB_ID
                   , PM.ORG_ID
                   , '2' AS ACCOUNT_DR_CR
                   , '2100600' AS ACCOUNT_CODE  -- 기타유동부채(2월 중도 퇴사자)    
                   , 'S000194' AS VENDOR_CODE
                   , (SUM(NVL(SE.IN_TAX_AMT, 0))) AS AMOUNT
                FROM HRM_PERSON_MASTER PM
                  , HRP_PAYMENT_SLIP_EXCEPT SE
              WHERE PM.PERSON_NUM         = SE.PERSON_NUM                
                AND PM.CORP_ID            = P_CORP_ID
                AND PM.SOB_ID             = P_SOB_ID
                AND PM.ORG_ID             = P_ORG_ID   
                AND SE.PAY_YYYYMM         = P_PAY_YYYYMM 
                AND SE.PAY_YYYYMM         = '2014-02'
                AND SE.EXCEPT_TYPE        = 'RETIRE'  -- 중도퇴사자    
                AND SE.WAGE_TYPE          = P_WAGE_TYPE  -- 급여.
                -- 중도 퇴사자 적용 여부 판단.
              GROUP BY SE.PAY_YYYYMM
                   , SE.WAGE_TYPE
                   , PM.SOB_ID
                   , PM.ORG_ID
              UNION ALL
              ...
              SELECT PM.SOB_ID
                   ...
                   , '2' AS ACCOUNT_DR_CR
                   , '2100600' AS ACCOUNT_CODE  -- 기타유동부채(2월 중도 퇴사자)    
                   , 'S000046' AS VENDOR_CODE
                   , (SUM(NVL(SE.LOCAL_TAX_AMT, 0))) AS AMOUNT
                FROM HRM_PERSON_MASTER PM
                  , HRP_PAYMENT_SLIP_EXCEPT SE
              WHERE PM.PERSON_NUM         = SE.PERSON_NUM                
                AND PM.CORP_ID            = P_CORP_ID
              ...
              SELECT T1.SOB_ID
                   , T1.ORG_ID
                   , '2' AS ACCOUNT_DR_CR
                   , '2100600' AS ACCOUNT_CODE  -- 기타유동자산  
                   , T1.VENDOR_CODE
                   , ABS(SUM(NVL(T1.AMOUNT, 0))) * -1 AS AMOUNT
                FROM( SELECT MP.SOB_ID
                           , MP.ORG_ID 
                           , CASE 
                               WHEN HD.DEDUCTION_CODE IN('D25', 'D27') THEN 'S000194'  -- 소득세(북인천세무서).
                               WHEN HD.DEDUCTION_CODE = 'D26' THEN 'S000046'  -- 주민세(부평구청).
                             END AS VENDOR_CODE
                           , SUM(CASE 
                                   WHEN HD.DEDUCTION_CODE IN('D25', 'D27') THEN NVL(MD.DEDUCTION_AMOUNT, 0)  -- 소득세(북인천세무서).
                                   WHEN HD.DEDUCTION_CODE = 'D26' THEN NVL(MD.DEDUCTION_AMOUNT, 0)  -- 주민세(부평구청).                                   
                                   ELSE 0
                                 END) AS AMOUNT
                        FROM HRP_MONTH_PAYMENT MP
                           , HRP_MONTH_DEDUCTION MD
                       ...
                       WHERE MP.MONTH_PAYMENT_ID   = MD.MONTH_PAYMENT_ID
                         ...
                         AND MP.SOB_ID             = P_SOB_ID
                         AND MP.ORG_ID             = P_ORG_ID                
                         AND HD.DEDUCTION_CODE     IN('D25', 'D26', 'D27')
                       GROUP BY MP.SOB_ID
                               , MP.ORG_ID 
                               , CASE 
                                   WHEN HD.DEDUCTION_CODE IN('D25', 'D27') THEN 'S000194'  -- 소득세(북인천세무서).
              ...
              SELECT PM.SOB_ID
                   , PM.ORG_ID
                   , '2' AS ACCOUNT_DR_CR
                   , '2101800' AS ACCOUNT_CODE  -- 기타유동부채(2월 중도 퇴사자)    
                   , 'S000632' AS VENDOR_CODE
                   , ABS(SUM(NVL(SE.IN_TAX_AMT, 0))) AS AMOUNT
                FROM HRM_PERSON_MASTER PM
                  , HRP_PAYMENT_SLIP_EXCEPT SE
              WHERE PM.PERSON_NUM         = SE.PERSON_NUM                
                AND PM.CORP_ID            = P_CORP_ID
              ...
              SELECT PM.SOB_ID
                   ...
                   , '2101800' AS ACCOUNT_CODE  -- 기타유동부채(2월 중도 퇴사자)    
                   , 'S000632' AS VENDOR_CODE
                   , ABS(SUM(NVL(SE.LOCAL_TAX_AMT, 0))) AS AMOUNT
                FROM HRM_PERSON_MASTER PM
                  , HRP_PAYMENT_SLIP_EXCEPT SE
              WHERE PM.PERSON_NUM         = SE.PERSON_NUM                
                AND PM.CORP_ID            = P_CORP_ID
              ...
              SELECT PM.SOB_ID
                   , PM.ORG_ID
                   , '1' AS ACCOUNT_DR_CR
                   , '1112000' AS ACCOUNT_CODE  -- 기타유동자산  
                   , 'S000632' AS VENDOR_CODE
                   , ABS(SUM(NVL(SE.IN_TAX_AMT, 0))) AS AMOUNT
                FROM HRM_PERSON_MASTER PM
                  , HRP_PAYMENT_SLIP_EXCEPT SE
              ...
              WHERE PM.PERSON_ID          = MP.PERSON_ID
                ...
                AND MP.SOB_ID             = P_SOB_ID
                AND MP.ORG_ID             = P_ORG_ID
                AND HD.DEDUCTION_CODE     IN('D12', 'D19')
                -- 중도 퇴사자 적용 여부 판단.
              GROUP BY MP.PAY_YYYYMM
                   , MP.WAGE_TYPE
                   , PM.SOB_ID
              ...
              WHERE PM.PERSON_ID          = MP.PERSON_ID
                ...
                AND MP.SOB_ID             = P_SOB_ID
                AND MP.ORG_ID             = P_ORG_ID
                AND HD.DEDUCTION_CODE     IN('D18', 'D23') 
                -- 중도 퇴사자 적용 여부 판단.
              GROUP BY MP.PAY_YYYYMM
                   , MP.WAGE_TYPE
                   , PM.SOB_ID
              ...
              SELECT PM.SOB_ID
                   ...
                   , '1' AS ACCOUNT_DR_CR
                   , CASE                           
                       WHEN PM.DIR_INDIR_TYPE IN('20', '30', '40') THEN '5131803'  -- 제조.
                       ELSE '5201804'  -- 판관.
                     END AS ACCOUNT_CODE
                   , 'S000632' AS VENDOR_CODE  -- 임직원.
                   , SUM(MD.DEDUCTION_AMOUNT) * -1 AS AMOUNT
              ...
              SELECT PM.SOB_ID
                   , PM.ORG_ID
                   , '2' AS ACCOUNT_DR_CR
                   , '1111500' AS ACCOUNT_CODE
                   , 'S000632' AS VENDOR_CODE  -- 임직원 
                   , SUM(T1.ETC_AMOUNT) AS AMOUNT
                FROM HRM_PERSON_MASTER PM
                  , HRP_MONTH_PAYMENT MP
                  , HRP_MONTH_DEDUCTION MD
                  , HRM_DEDUCTION_V HD 
                  , ( SELECT SE.PAY_YYYYMM
                           , SE.WAGE_TYPE
                           , SE.PERSON_NUM
                           , SE.IN_TAX_AMT AS ETC_AMOUNT 
                           , SE.LOCAL_TAX_AMT AS INSUR_AMOUNT 
                        FROM HRP_PAYMENT_SLIP_EXCEPT SE
                       WHERE SE.PAY_YYYYMM            = '2015-06'
                         AND SE.WAGE_TYPE             = 'P1'
                         AND SE.EXCEPT_TYPE           = 'SALARY'
                      ) T1 
              WHERE PM.PERSON_ID          = MP.PERSON_ID
                AND MP.MONTH_PAYMENT_ID   = MD.MONTH_PAYMENT_ID
                AND MP.PERSON_ID          = MD.PERSON_ID
                ...
                AND MP.PAY_YYYYMM         = T1.PAY_YYYYMM
                AND MP.WAGE_TYPE          = T1.WAGE_TYPE
                AND PM.PERSON_NUM         = T1.PERSON_NUM
                ...
                AND MP.ORG_ID             = P_ORG_ID
                AND HD.DEDUCTION_CODE     = 'D14'
                AND MP.PAY_YYYYMM         = '2015-06'  
                AND PM.PERSON_NUM         IN('B03020'
                                             , 'A99003'
                                             , 'B06044'
                                             , 'B09113'
                                             , 'B03014'
              ...
              SELECT PM.SOB_ID
                   , PM.ORG_ID
                   , '1' AS ACCOUNT_DR_CR
                   , '5131301' AS ACCOUNT_CODE
                   , 'S000632' AS VENDOR_CODE  -- 임직원 
                   , SUM(T1.INSUR_AMOUNT) * -1 AS AMOUNT
                FROM HRM_PERSON_MASTER PM
                  , HRP_MONTH_PAYMENT MP
                  , HRP_MONTH_DEDUCTION MD
                  , HRM_DEDUCTION_V HD 
              ...
              GROUP BY MP.PAY_YYYYMM
                   ...
                   , PM.ORG_ID
              /*UNION ALL
              -- 미지급비용 : 기타공제 -> 복리후생비-복지환경(제)
              SELECT PM.SOB_ID
                   , PM.ORG_ID
                   , '2' AS ACCOUNT_DR_CR
                   , '1112000' AS ACCOUNT_CODE
                   , 'S000632' AS VENDOR_CODE
                   , SUM(MD.DEDUCTION_AMOUNT) AS AMOUNT
                FROM HRM_PERSON_MASTER PM
                  , HRP_MONTH_PAYMENT MP
              ...
              WHERE PM.PERSON_ID          = MP.PERSON_ID
                ...
                AND HD.DEDUCTION_CODE     = 'D14'
                AND MP.PAY_YYYYMM         = '2015-06'  
                AND PM.PERSON_NUM         NOT IN('B03020'
                                             , 'A99003'
                                             , 'B06044'
                                             , 'B09113'
                                             , 'B03014'
      ...
      WHERE AJM.SOB_ID              = AJD.SOB_ID
        ...
        AND AJD.ACCOUNT_CODE        = PAY_AL.ACCOUNT_CODE(+)        
        AND AJD.ACCOUNT_DR_CR       = PAY_AL.ACCOUNT_DR_CR(+)
        AND AJD.SOB_ID              = PAY_AL.SOB_ID(+)
...
---------------------------------------------------------------------------------------------------    
                  ...
                  FROM FI_AUTO_JOURNAL_MST AJM
                    ...
                    , ( -- 지급.
                        ...
                        WHERE PM.PERSON_ID          = MP.PERSON_ID
                          ...
                          AND MP.ORG_ID             = P_ORG_ID
                          AND HD.DEDUCTION_CODE     = 'D14'
                          AND MP.PAY_YYYYMM         NOT IN('2013-06', '2014-12') 
                          AND (MP.PAY_YYYYMM        = '2015-03' AND PM.PERSON_NUM NOT IN('B10061'))
                          AND NVL(MP.INTERFACE_YN, 'N')       = 'N'
                          -- 중도 퇴사자 적용 여부 판단.
                        GROUP BY MP.PAY_YYYYMM
                             , MP.WAGE_TYPE
    ...
    LOOP
      ...
      END IF;
      IF O_STATUS = 'F' THEN
        O_MESSAGE := '1.[' || C1.ACCOUNT_CODE || ', ' || C1.ACCOUNT_DR_CR || '] ' || O_MESSAGE;
        ROLLBACK;
        RETURN;
      END IF;
    END LOOP C1;
    ...
    FOR C1 IN ( SELECT AJM.SLIP_TYPE_CD
                     ...
                     , FI_COMMON_G.SUPP_CUST_CODE_NAME_F(PAY_AL.VENDOR_CODE, P_SOB_ID) AS VENDOR_NAME
                     , HRM_PERSON_MASTER_G.PERSON_NUMBER_F(P_CONNECT_PERSON_ID) AS PERSON_NUM
                     , NVL(PAY_AL.LINE_REMARK, AJD.SLIP_REMARKS) AS LINE_REMARK
                  FROM FI_AUTO_JOURNAL_MST AJM
                    , FI_AUTO_JOURNAL_DET AJD
                    , FI_ACCOUNT_CONTROL AC
                    , ( -- 예수금.
                        ...
                        SELECT T1.SOB_ID
                             , T1.ORG_ID
                             , '2' AS ACCOUNT_DR_CR 
                             , '2100600' AS ACCOUNT_CODE  -- 예수금  
                             , T1.VENDOR_CODE
                             , ABS(SUM(NVL(T1.AMOUNT, 0))) * -1 AS AMOUNT
                             , CASE 
                                 WHEN T1.VENDOR_CODE = 'S000194' THEN '연말정산 소득세 환급분'
                                 WHEN T1.VENDOR_CODE = 'S000046' THEN '연말정산 지방소득세 환급분'
                                 ELSE NULL
                               END AS LINE_REMARK                             
                          FROM( SELECT MP.SOB_ID
                                     , MP.ORG_ID 
                                 ...
                                 SELECT PM.SOB_ID
                                       , PM.ORG_ID 
                                       , 'S000194' AS VENDOR_CODE -- 소득세(북인천세무서).
                                       , SUM(NVL(SE.IN_TAX_AMT, 0) * CASE
                                                                       WHEN SE.EXCEPT_TYPE = 'RETIRE' THEN -1
                                                                       ELSE 1 
                                                                     END) AS AMOUNT 
                                    FROM HRM_PERSON_MASTER       PM
                                       , HRP_PAYMENT_SLIP_EXCEPT SE
                                  ...
                                  SELECT PM.SOB_ID
                                       , PM.ORG_ID 
                                       , 'S000046' AS VENDOR_CODE -- 주민세(부평구청).
                                       , SUM(NVL(SE.LOCAL_TAX_AMT, 0) * CASE
                                                                          WHEN SE.EXCEPT_TYPE = 'RETIRE' THEN -1
                                                                          ELSE 1 
                                                                        END) AS AMOUNT 
                                    FROM HRM_PERSON_MASTER       PM
                        ...
                        */
                        ...
                        WHERE PM.PERSON_ID          = MP.PERSON_ID
                          ...
                          AND MP.ORG_ID             = P_ORG_ID
                          AND HD.DEDUCTION_CODE     = 'D14'
                          AND MP.PAY_YYYYMM         = '2015-03' 
                          AND PM.PERSON_NUM         IN('B10061')
                          AND NVL(MP.INTERFACE_YN, 'N')       = 'N'
                          -- 중도 퇴사자 적용 여부 판단.
                        GROUP BY MP.PAY_YYYYMM
                             , MP.WAGE_TYPE
                  ...
                  FROM FI_AUTO_JOURNAL_MST AJM
                    ...
                    , ( -- 기타유동부채(급여압류/학자금대출).
                        ...
                        WHERE PM.PERSON_ID          = MP.PERSON_ID
                          ...
                          AND MP.SOB_ID             = P_SOB_ID
                          AND MP.ORG_ID             = P_ORG_ID
                          AND HD.DEDUCTION_CODE     IN('D23', 'D18') 
                          -- 중도 퇴사자 적용 여부 판단.
                        GROUP BY MP.PAY_YYYYMM
                             , MP.WAGE_TYPE
                             , PM.SOB_ID
