...
-- 해당 직원의 입사/퇴사일자 조회 -- 
    BEGIN
      SELECT HRM_COMMON_DATE_G.PERIOD_YEAR_F(PM.JOIN_DATE, NVL(PM.RETIRE_DATE, W_RETIRE_DATE_TO), 'TRUNC') AS REAL_LONG_YEAR 
...
--> 3개월 급여 계산-
    IF W_RETIRE_CAL_TYPE = 'NEW' AND W_REAL_LONG_YEAR > 0 THEN
      HRR_RETIRE_PAYMENT_SET_G.PAYMENT_MAIN
                              ( P_ADJUSTMENT_ID => W_ADJUSTMENT_ID
                              , P_ADJUSTMENT_TYPE => W_ADJUSTMENT_TYPE
                              , P_CORP_ID => W_CORP_ID
