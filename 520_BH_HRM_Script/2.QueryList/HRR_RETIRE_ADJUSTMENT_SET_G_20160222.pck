...
---------------------------------------------------------------------------------------------------
      ...
      IF NVL(C1.LONG_YEAR_1, 0) > 0 THEN
        ...
        --> 산출세액
        V_AVG_COMP_TAX_AMOUNT_1 := TRUNC(AVG_TAX_AMOUNT_F(W_RETR_YYYY, W_SOB_ID, W_ORG_ID, V_AVG_TAX_STD_AMOUNT_1));

        --> 산출세액
        V_COMP_TAX_AMOUNT_1 := TRUNC(V_AVG_COMP_TAX_AMOUNT_1 * C1.LONG_YEAR_1);

        --> 소득세액공제
        V_TAX_DED_AMOUNT_1 := TRUNC(TAX_SUBTRACT_F(W_RETR_YYYY, W_SOB_ID, W_ORG_ID, C1.RETIRE_DATE_TO, V_COMP_TAX_AMOUNT_1, C1.LONG_YEAR_1));
      ...
      IF NVL(C1.LONG_YEAR_2, 0) > 0 THEN
        ...
        --> 환산산출세액(환산과세표준 * 세율)  
        V_CHG_COMP_TAX_AMOUNT_2 := TRUNC(AVG_TAX_AMOUNT_F(W_RETR_YYYY, W_SOB_ID, W_ORG_ID, V_CHG_TAX_STD_AMOUNT_2));
        
        --> 연평균산출세액(환산산출세액 / 5)
        V_AVG_COMP_TAX_AMOUNT_2 := TRUNC(V_CHG_COMP_TAX_AMOUNT_2 / 5);

        --> 소득세액공제
        V_TAX_DED_AMOUNT_2 := TRUNC(TAX_SUBTRACT_F(W_RETR_YYYY, W_SOB_ID, W_ORG_ID, C1.RETIRE_DATE_TO, V_AVG_COMP_TAX_AMOUNT_2, C1.LONG_YEAR_2));
      ...
      -- 2016변경 : 환산세액        
      V_CHG_COMP_TAX_AMOUNT_3	  := TRUNC(AVG_TAX_AMOUNT_F(W_RETR_YYYY, W_SOB_ID, W_ORG_ID, V_CHG_TAX_STD_AMOUNT_3));      
      -- 2016변경 : 산출세액    
