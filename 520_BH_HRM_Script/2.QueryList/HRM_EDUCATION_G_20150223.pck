  ...
  BEGIN
    UPDATE HRM_EDUCATION HE
     SET HE.START_DATE                                    = TRUNC(P_START_DATE)
        ...
        , HE.COMPLETED_FLAG                               = NVL(P_COMPLETED_FLAG, 'N')
     ...
     OPEN P_CURSOR FOR
       SELECT  HPM.PERSON_NUM
        ...
        WHERE HPM.PERSON_ID       = T1.PERSON_ID 
          ...
          AND ((W_JOB_CATEGORY    IS NULL AND 1 = 1)
          OR   (W_JOB_CATEGORY    IS NOT NULL AND T1.JOB_CATEGORY_ID = W_JOB_CATEGORY))
  ...
  BEGIN
    OPEN P_CURSOR1 FOR
      SELECT TO_CHAR(NULL) AS PERSON_NUM
           ...
           , TO_NUMBER(NULL) AS EDU_PAY_AMOUNT  
           , TO_NUMBER(NULL) AS EDU_PAY_RETURN_AMOUNT   
...
----------------------------------------------  
  ...
  BEGIN
    ...
    IF P_PERSON_ID = -1 THEN
      RAISE_APPLICATION_ERROR(-20001, 'Person No : ' || P_PERSON_NUM || ' - ' || EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10028'));
      RETURN;
    END IF; 
