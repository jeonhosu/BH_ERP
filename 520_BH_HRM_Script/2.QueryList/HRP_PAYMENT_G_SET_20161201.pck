...
--  0. PAYMENT CALCULATE MAIN.
  ...
  BEGIN
    ...
    O_STATUS := HRM_CLOSING_G.CLOSING_CHECK_W
                  ( W_CORP_ID => P_CORP_ID
    ...
    IF HRM_MANAGER_G.USER_ID_CAP_F ( W_CORP_ID => P_CORP_ID
                                   , W_START_DATE => P_STD_DATE
                                   , W_END_DATE => P_STD_DATE
                                   , W_MODULE_CODE => '30'
                                   , W_USER_ID => P_USER_ID 
                                   , W_SOB_ID => P_SOB_ID
    ...
    ELSE
      O_STATUS := 'F';
      O_MESSAGE := EAPP_MESSAGE_G.RETURN_MSG_F('EAPP_10009', '&&CAP:=Salary Calculate');
      RETURN; 
    END IF;
...
-- 세금 계산.
  ...
  BEGIN
    IF W_STD_DATE < TO_DATE('2014-02-01', 'YYYY-MM-DD') THEN
      -- 2014-01월까지 적용 -- 
      ...
      IF NVL(W_TOTAL_AMOUNT, 0) > 28000000 THEN
        ...
        V_AMOUNT := TRUNC(NVL(V_AMOUNT, 0) * (V_TAX_RATE_3 / 100), -1);  
        V_AMOUNT := NVL(V_AMOUNT, 0) + 5985000;
      ELSIF NVL(W_TOTAL_AMOUNT, 0) >= 10000000 THEN
        V_AMOUNT := NVL(W_TOTAL_AMOUNT, 0) - 10000000;      
        V_AMOUNT := TRUNC(NVL(V_AMOUNT, 0) * (V_TAX_RATE_1 / 100));   
        V_AMOUNT := TRUNC(NVL(V_AMOUNT, 0) * (V_TAX_RATE_2 / 100), -1);   
      END IF;      
    ELSE
      -- 2014-02월부터 적용 -- 
      -- 14,000,000을 초과할 경우 : (10,000,000인 경우의 세액) + 1,372,000 + (14,000,000을 초과하는 금액중 95%를 곱한 금액의 38% 상당액)
      -- 10,000,000을 초과할 경우 10,000,000을 초과하는 금액중 95%를 곱한 금액의 35% 상당액을 더해 준다--
      V_TAX_RATE_1  := 98;
      V_TAX_RATE_2  := 35;
      V_TAX_RATE_3  := 38;
      ...
      IF NVL(W_TOTAL_AMOUNT, 0) > 14000000 THEN
        V_AMOUNT := NVL(W_TOTAL_AMOUNT, 0) - 14000000;
        V_AMOUNT := TRUNC(NVL(V_AMOUNT, 0) * (V_TAX_RATE_1 / 100));   
        V_AMOUNT := TRUNC(NVL(V_AMOUNT, 0) * (V_TAX_RATE_3 / 100), -1);  
        V_AMOUNT := NVL(V_AMOUNT, 0) + 1372000;
      ELSIF NVL(W_TOTAL_AMOUNT, 0) > 10000000 THEN
        V_AMOUNT := NVL(W_TOTAL_AMOUNT, 0) - 10000000;      
        V_AMOUNT := TRUNC(NVL(V_AMOUNT, 0) * (V_TAX_RATE_1 / 100));  
        V_AMOUNT := TRUNC(NVL(V_AMOUNT, 0) * (V_TAX_RATE_2 / 100), -1);   
