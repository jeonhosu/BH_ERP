    ...
    OPEN P_CURSOR FOR
      SELECT TO_CHAR(HMP.PERSON_ID) AS PERSON_ID
           , CASE
             ...
             END AS PAY_YYYYMM
           , NVL(SUM(CASE
                       WHEN HMP.WAGE_TYPE IN('P2', 'P3', 'P5') THEN 0
                       ELSE NVL(HMP.TOT_SUPPLY_AMOUNT, 0)
                     END), 0) AS PAY_AMOUNT
           , NVL(SUM(CASE
                       WHEN HMP.WAGE_TYPE IN('P2', 'P3', 'P5') THEN NVL(HMP.TOT_SUPPLY_AMOUNT, 0)
                       ELSE 0 
       ...
       WHERE HMP.MONTH_PAYMENT_ID     = HMD.MONTH_PAYMENT_ID(+)
         ...
         OR   (W_DEPT_ID              IS NOT NULL AND HMP.DEPT_ID = W_DEPT_ID))
         AND ((W_PAY_GRADE_ID         IS NULL AND 1 = 1)
         OR   (W_PAY_GRADE_ID         IS NOT NULL AND HMP.PAY_GRADE_ID = W_PAY_GRADE_ID))
