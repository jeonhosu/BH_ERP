      ...
      SELECT 'N' AS SELECT_YN
        ...
        FROM HRM_OPERATING_UNIT OU
          , ( SELECT YA.YEAR_YYYY AS YEAR_YYYY
                  ...
                  , COUNT(DISTINCT YA.PERSON_ID) AS PERSON_COUNT
...
-------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    ELSIF P_YEAR_YYYY IN('2012', '2013') THEN
...
-------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    ELSIF P_YEAR_YYYY IN('2013') THEN
...
--C1 ------------------------------------------------------------------------------------
        FOR C1 IN ( SELECT 
                        ...
                        || LPAD(NVL(YA.TAX_REDU_SP_LAW_AMT, 0), 10, 0) -- 129.조세특례제한법;
                        -- 2012년 연말정산 추가 START --
                        || LPAD(NVL(YA.TAX_REDU_SMALL_BUSINESS, 0), 10, 0) -- 130.조세특례제한법 : 중소기업 취업 청년 소득세 감면;
                        -- 2012년 연말정산 추가 END --
                        || LPAD(NVL(YA.TAX_REDU_TAX_TREATY, 0), 10, 0) -- 131.조세조약.
                        || LPAD(0, 10, 0) -- 132.공란;
                        || LPAD(NVL(YA.TAX_REDU_IN_LAW_AMT, 0) +
                                ...
                                NVL(YA.TAX_REDU_SMALL_BUSINESS, 0), 10, 0) -- 133.감면세액계;
                        --> 세액공제.
                        || LPAD(NVL(YA.TAX_DED_INCOME_AMT, 0), 10, 0) -- 134.근로소득세액공제;
                        || LPAD(NVL(YA.TAX_DED_TAXGROUP_AMT, 0), 10, 0) -- 135.납세조합공제;
                        || LPAD(NVL(YA.TAX_DED_HOUSE_DEBT_AMT, 0), 10, 0) -- 136.주택차입금;
                        ...
                        || LPAD(NVL(YA.TAX_DED_INCOME_AMT, 0) +
                                ...
                                NVL(YA.TAX_DED_OUTSIDE_PAY_AMT, 0), 10, 0) -- 140.세액공제계;
                        --> 결정세액.
                        || LPAD(NVL(YA.FIX_IN_TAX_AMT, 0), 10, 0) -- 141.소득세;
                        || LPAD(NVL(YA.FIX_LOCAL_TAX_AMT, 0), 10, 0) -- 142.주민세;
                        || LPAD(NVL(YA.FIX_SP_TAX_AMT, 0), 10, 0) -- 143.농특세;
                        /* -- 전호수 주석 : 합계 없음.
                        || LPAD(TRUNC(NVL(YA.FIX_IN_TAX_AMT, 0), -1) +
                       ...
                       , ( -- 제출내역.
                           SELECT YA.YEAR_YYYY
                            ...
                            WHERE YA.PERSON_ID          = PM.PERSON_ID
                              ...
                              -- 연말정산 재정산에 따른 추가 -- 
                              AND TRUNC(YA.CLOSED_DATE) >= TO_DATE('2015-05-01', 'YYYY-MM-DD')
...
--C1 ------------------------------------------------------------------------------------
        FOR C1 IN ( SELECT 
                       ...
                       || LPAD(NVL(YA.CHILD_DED_COUNT, 0), 2, 0) -- 75.자녀양육비공제인원;
                        ...
                        || LPAD(NVL(YA.TD_CHILD_RAISE_DED_AMT, 0), 10, 0) -- 118.근로소득세액공제<2014추가>;
                        || LPAD(CASE
                                  WHEN NVL(YA.TD_SCIENTIST_ANNU_DED_AMT, 0) = 0 THEN 0
                                  ELSE NVL(YA.TD_SCIENTIST_ANNU_AMT, 0)
                                END, 10, 0) -- 119.연금계좌 과학기술인공제 공제대상금액<2014추가>;
                        || LPAD(NVL(YA.TD_SCIENTIST_ANNU_DED_AMT, 0), 10, 0) -- 120.연금계좌 과학기술인공제 세액공제액<2014추가>;
                        || LPAD(CASE
                                  WHEN NVL(YA.TD_WORKER_RETR_ANNU_DED_AMT, 0) = 0 THEN 0
                                  ELSE NVL(YA.TD_WORKER_RETR_ANNU_AMT, 0)
                                END, 10, 0) -- 121.근로자 퇴직급여 공제대상금액<2014추가>;
                        || LPAD(NVL(YA.TD_WORKER_RETR_ANNU_DED_AMT, 0), 10, 0) -- 122.근로자 퇴직급여 세액공제액<2014추가>;
                        || LPAD(CASE
                                  WHEN NVL(YA.TD_ANNU_BANK_DED_AMT, 0) = 0 THEN 0
                                  ELSE NVL(YA.TD_ANNU_BANK_AMT, 0)
                                END, 10, 0) -- 123.연금계좌 연금저축 공제대상금액<2014추가>;
                        || LPAD(NVL(YA.TD_ANNU_BANK_DED_AMT, 0), 10, 0) -- 124.연금계좌 연금저축 세액공제액<2014추가>;
                        || LPAD(CASE
                                  WHEN NVL(YA.TD_GUAR_INSUR_DED_AMT, 0) = 0 THEN 0
                                  ELSE NVL(YA.TD_GUAR_INSUR_AMT, 0)
                                END, 10, 0) -- 125.보장성 보험료 공제대상금액<2014추가>;
                        || LPAD(NVL(YA.TD_GUAR_INSUR_DED_AMT, 0), 10, 0) -- 126.보장성 보험료 세액공제액<2014추가>;
                        || LPAD(CASE
                                  WHEN NVL(YA.TD_MEDIC_DED_AMT, 0) = 0 THEN 0
                                  ELSE NVL(YA.TD_MEDIC_AMT, 0)
                                END, 10, 0) -- 127.의료비 공제대상금액<2014추가>;
                        || LPAD(NVL(YA.TD_MEDIC_DED_AMT, 0), 10, 0) -- 128.의료비 세액공제액<2014추가>;
                        || LPAD(CASE
                                  WHEN NVL(YA.TD_EDUCATION_DED_AMT, 0) = 0 THEN 0
                                  ELSE NVL(YA.TD_EDUCATION_AMT, 0)
                                END, 10, 0) -- 129.교육비 공제대상금액<2014추가>;
                        || LPAD(NVL(YA.TD_EDUCATION_DED_AMT, 0), 10, 0) -- 130.교육비 세액공제액<2014추가>;
                        || LPAD(CASE
                                  WHEN NVL(YA.TD_POLI_DONAT_DED_AMT1, 0) = 0 THEN 0
                                  ELSE NVL(YA.TD_POLI_DONAT_AMT1, 0)
                                END, 10, 0) -- 131.기부금_정치자금_10만원이하_공제대상금액 공제대상금액<2014추가>;
                        || LPAD(NVL(YA.TD_POLI_DONAT_DED_AMT1, 0), 10, 0) -- 132.기부금_정치자금_10만원이하 세액공제액<2014추가>;
                        || LPAD(CASE
                                  WHEN NVL(YA.TD_POLI_DONAT_DED_AMT2, 0) = 0 THEN 0
                                  ELSE NVL(YA.TD_POLI_DONAT_AMT2, 0)
                                END, 11, 0) -- 133.기부금_정치자금_10만원초과_공제대상금액 공제대상금액<2014추가>;
                        || LPAD(NVL(YA.TD_POLI_DONAT_DED_AMT2, 0), 10, 0) -- 134.기부금_정치자금_10만원초과 세액공제액<2014추가>;
                        || LPAD(CASE
                                  WHEN NVL(YA.TD_LEGAL_DONAT_DED_AMT, 0) = 0 THEN 0
                                  ELSE NVL(YA.TD_LEGAL_DONAT_AMT, 0)
                                END, 11, 0) -- 135.기부금_법정기부금_공제대상금액 공제대상금액<2014추가>;
                        || LPAD(NVL(YA.TD_LEGAL_DONAT_DED_AMT, 0), 10, 0) -- 136.기부금_법정기부금 세액공제액<2014추가>;
                        || LPAD(CASE
                                  WHEN NVL(YA.TD_DESIGN_DONAT_DED_AMT, 0) = 0 THEN 0
                                  ELSE NVL(YA.TD_DESIGN_DONAT_AMT, 0)
                                END, 11, 0) -- 137.기부금_지정기부금_공제대상금액 공제대상금액<2014추가>;
                        || LPAD(NVL(YA.TD_DESIGN_DONAT_DED_AMT, 0), 10, 0) -- 138.기부금_지정기부금 세액공제액<2014추가>;
                        ...
                        || LPAD(NVL(YA.TAX_DED_INCOME_AMT, 0) + NVL(YA.TD_CHILD_RAISE_DED_AMT, 0) + 
                                ...
                                NVL(YA.TD_GUAR_INSUR_DED_AMT, 0) + 
...
-- 외국인 단일세율이 아닐경우만 적용.
          IF C1.FOREIGN_TAX_YN = 'N' THEN
            ...
            FOR E1 IN ( SELECT 
                            ...
                            --> 국세청 자료.
                            -- 2009년 연말정산 수정(MODIFIED BY YOUNG MIN).국세청자료 공제금액이 음수일 경우 0으로 표기;
                            || LPAD(0, 10, 0) -- 16.보험료(본인은 건강보험료 포함);
                            || LPAD(NVL(SF.INSURE_AMT, 0) + NVL(SF.DISABILITY_INSURE_AMT, 0), 10, 0) -- 17.보장성보험료(장애인 전용 보장성 보험료 포함);
                            || LPAD(NVL(SF.MEDICAL_AMT, 0), 10, 0) -- 18.의료비;
                            || LPAD(NVL(SF.EDUCATION_AMT, 0), 10, 0) -- 19.교육비;
                            || LPAD(NVL(SF.CREDIT_AMT, 0), 10, 0) -- 20.신용카드등;
                            || LPAD(NVL(SF.CHECK_CREDIT_AMT , 0), 10, 0) -- 21.직불카드;
                            || LPAD(NVL(SF.CASH_AMT, 0) + NVL(SF.ACADE_GIRO_AMT, 0), 10, 0) -- 22.현금영수증;
                            --2012년 연말정산 추가 START --
                            || LPAD(NVL(SF.TRAD_MARKET_AMT, 0), 10, 0) -- 23.전통시장사용액;
                            || LPAD(NVL(SF.PUBLIC_TRANSIT_AMT, 0), 10, 0) -- 24.대중교통이용액1;
                            --2012년 연말정산 추가 END --
                            || LPAD(CASE
                                      ...
                                      ELSE ( NVL(SF.DONAT_ALL, 0) +
                                             ...
                                             NVL(SF.ETC_DONAT_10P_RELIGION, 0)) - 
                                             NVL(T1.DONA_AMT, 0)
                                    END, 13, 0)  -- 25.기부금(정치자금 기부금중 10만원 이하 세액공제 기부금 지출금액은 제외).
                            -->국세청자료 이외.
                            || LPAD((CASE
                                       WHEN DECODE(SF.RELATION_CODE, 0, NVL(C1.MEDIC_INSUR_AMT, 0), 0) < 0 THEN 0
                                       ELSE DECODE(SF.RELATION_CODE, 0, NVL(C1.MEDIC_INSUR_AMT, 0), 0) 
                                     END), 10, 0) -- 26.국세청제공외의 건강/고용보험;
                            || LPAD(NVL(SF.ETC_INSURE_AMT, 0), 10, 0) -- 27.국세청제공외의 보험료;
                            || LPAD(NVL(SF.ETC_MEDICAL_AMT, 0), 10, 0) -- 28.국세청제공외의 의료비;
                            || LPAD(NVL(SF.ETC_EDUCATION_AMT, 0), 10, 0) -- 29.국세청제공외의 교육비;
                            || LPAD(NVL(SF.ETC_CREDIT_AMT, 0) + NVL(SF.ETC_ACADE_GIRO_AMT, 0), 10, 0) -- 30.국세청제공외의 신용카드;
                            ...
                            || LPAD(/*NVL(SF.ETC_DONAT_ALL, 0) +
                                    ...
                                    NVL(SF.ETC_DONAT_30P, 0) +
                                    NVL(SF.ETC_DONAT_10P, 0) +
                                    NVL(SF.ETC_DONAT_10P_RELIGION, 0)*/0, 13, 0) AS RECORD_LINE -- 34.국세청제공외의 기부금;
            ...
            IF V_RECORD_COUNT <> 0 THEN
              ...
              LOOP
                V_RECORD_FILE := V_RECORD_FILE 
                              ...
                              || LPAD(0, 10, 0)                 -- 16.보험료(본인은 건강보험료 포함);
                              || LPAD(0, 10, 0)                 -- 17.보장성보험료(장애인 전용 보장성 보험료 포함);
                              || LPAD(0, 10, 0)                 -- 18.의료비;
                              || LPAD(0, 10, 0)                 -- 19.교육비;
                              || LPAD(0, 10, 0)                 -- 20.신용카드등;
                              || LPAD(0, 10, 0)                 -- 21.직불카드;
                              || LPAD(0, 10, 0)                 -- 22.현금영수증;
                              || LPAD(0, 10, 0)                 -- 23.전통시장사용액;
                              || LPAD(0, 10, 0)                 -- 24.대중교통이용액1;
                              || LPAD(0, 13, 0)                 -- 25.기부금(정치자금 기부금중 10만원 이하 세액공제 기부금 지출금액은 제외).
                              -->국세청자료 이외.
                              || LPAD(0, 10, 0)                 -- 26.국세청제공외의 건강/고용보험;
                              || LPAD(0, 10, 0)                 -- 27.국세청제공외의 보험료;
                              || LPAD(0, 10, 0)                 -- 28.국세청제공외의 의료비;
                              || LPAD(0, 10, 0)                 -- 29.국세청제공외의 교육비;
                              || LPAD(0, 10, 0)                 -- 30.국세청제공외의 신용카드;
                              || LPAD(0, 10, 0)                 -- 31.국세청제공외의 직불카드;
                              || LPAD(0, 10, 0)                -- 32.전통시장사용액;
                              || LPAD(0, 10, 0)                 -- 33. 대중교통이용액;
                              || LPAD(0, 13, 0);                -- 공란.
              END LOOP E1S;
              IF V_RECORD_FILE IS NOT NULL THEN
                V_SEQ_NO := NVL(V_SEQ_NO, 0) + 1;  -- 부양가족 생성 일련번호.
...
--F1 연금/저축등 소득공제 명세 레코드 -----------------------------------------------------------------
            ...
            LOOP
              ...
              IF V_F_REC_STD <= V_RECORD_COUNT THEN
                V_SEQ_NO := NVL(V_SEQ_NO, 0) + 1;  -- 연금저축 생성 일련번호.
                V_RECORD_FILE := V_RECORD_FILE || RPAD(' ', 360, ' ');  -- 공란.
            ...
            IF V_RECORD_COUNT != 0 THEN
              FOR F1S IN V_RECORD_COUNT + 1 .. V_F_REC_STD
              LOOP
                V_RECORD_FILE := V_RECORD_FILE || RPAD(' ', 75, ' ');
              END LOOP F1S;
              IF V_RECORD_FILE IS NOT NULL THEN
                V_SEQ_NO := NVL(V_SEQ_NO, 0) + 1;  -- 연금저축 생성 일련번호.
                V_RECORD_FILE := V_RECORD_FILE || RPAD(' ', 360, ' ');  -- 연금저축  소득공제명세 순수 길이 => 공란.
...
--G1 월세/거주자간 주택임차차입금 원리금 상환액 소득공제명세서 레코드 --------------------
            ...
            LOOP
              ...
              IF V_G_REC_STD <= V_RECORD_COUNT THEN
                V_SEQ_NO := NVL(V_SEQ_NO, 0) + 1;  -- 월세/거주자간 주택임차차입금 원리금 생성 일련번호.
                V_RECORD_FILE := V_RECORD_FILE || LPAD(V_SEQ_NO, 2, 0);  -- 일련번호 
                V_RECORD_FILE := V_RECORD_FILE || RPAD(' ', 184, ' ');  -- 공란.
