...
---------------------------------------------------------------------------------------------------
      ...
      IF NVL(C1.LONG_YEAR_1, 0) > 0 THEN
        ...
        --> 실지급액 (총퇴직금 - 주민세 - 소득세 - 기타공제 - 퇴직전환금)
        V_REAL_AMOUNT := NVL(V_REAL_AMOUNT, 0) - TRUNC(NVL(V_IN_TAX_AMOUNT_1, 0), -1) - TRUNC(NVL(V_LOCAL_TAX_AMOUNT_1, 0), -1);
        IF V_REAL_AMOUNT < 0 THEN
          V_REAL_AMOUNT := 0;
        END IF;
      END IF;
      ...
      IF NVL(C1.LONG_YEAR_2, 0) > 0 THEN
        ...
        --> 실지급액 (총퇴직금 - 주민세 - 소득세 - 기타공제 - 퇴직전환금)
        V_REAL_AMOUNT := NVL(V_REAL_AMOUNT, 0) - TRUNC(NVL(V_IN_TAX_AMOUNT_2, 0), -1) - TRUNC(NVL(V_LOCAL_TAX_AMOUNT_2, 0), -1);
        IF V_REAL_AMOUNT < 0 THEN
          V_REAL_AMOUNT := 0;
        END IF;
      END IF;
