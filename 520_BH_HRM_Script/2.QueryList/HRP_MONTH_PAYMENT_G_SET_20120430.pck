...
-- 월급여 자료 SELECT - 항목별 전월대비 조회.
  ...
  BEGIN
    ...
    OPEN P_CURSOR1 FOR
      ...
      GROUP BY ROLLUP((S_HC.CODE_NAME
           , HA.ALLOWANCE_CODE
           , HA.ALLOWANCE_NAME
           , P_MA.PREV_PERSON_COUNT
           , T_MA.THIS_PERSON_COUNT
      ...
      ORDER BY GROUP_CODE, SORT_NUM, ALLOWANCE_CODE
      ;
    ...
    OPEN P_CURSOR1 FOR
      SELECT  CASE
          ...
          , CASE
           ...
           , SUM(NVL(MA.A07, 0) + NVL(MA.A15, 0) + NVL(MA.A18, 0)) AS A07  -- 기타수당(기타수당 + 야간당직 + 년차수당).
           ...
           , SUM(NVL(MD.D14, 0) + NVL(MD.D10, 0)  + NVL(MD.D12, 0)) AS D14                -- 기타공제(기타공제 + 전월정산액 + 경조사비).
...
-- 세금 다시 계산.
  ...
  BEGIN
    ...
    IF P_WAGE_TYPE IN ('P2', 'P2', 'P5') THEN
      -- 상여금.
