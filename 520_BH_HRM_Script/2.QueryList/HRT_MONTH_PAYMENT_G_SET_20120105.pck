CREATE OR REPLACE PACKAGE HRT_MONTH_PAYMENT_G_SET
AS
...
-- 2.1 급여 기본급 생성
  PROCEDURE BASIC_PAY_AMOUNT_101
            ...
            , P_PAY_YYYYMM        IN HRT_MONTH_PAYMENT.PAY_YYYYMM%TYPE
            , P_WAGE_TYPE         IN HRT_MONTH_PAYMENT.WAGE_TYPE%TYPE
            , P_EXCEPT_TYPE       IN HRT_MONTH_PAYMENT.EXCEPT_TYPE%TYPE
...
-- 2.2 급여 기본급 생성
  PROCEDURE BASIC_PAY_AMOUNT_102
            ...
            , P_PAY_YYYYMM        IN HRT_MONTH_PAYMENT.PAY_YYYYMM%TYPE
            , P_WAGE_TYPE         IN HRT_MONTH_PAYMENT.WAGE_TYPE%TYPE
            , P_EMPLOYE_TYPE      IN HRT_MONTH_PAYMENT.EMPLOYE_TYPE%TYPE
            , P_EXCEPT_TYPE       IN HRT_MONTH_PAYMENT.EXCEPT_TYPE%TYPE
            , P_PAY_LEVEL         IN HRM_COMMON.VALUE1%TYPE
            , P_OFFICER_YN        IN HRM_COMMON.CODE%TYPE
            , P_PERSON_ID         IN HRT_MONTH_PAYMENT.PERSON_ID%TYPE
...
-- 46.1 지급항목 일할계산
  PROCEDURE DAILY_PAY_CALCULATE_101
            ...
            , P_END_DATE          IN DATE
            , P_TOTAL_DAY         IN NUMBER
            , P_PAY_DAY           IN HRT_MONTH_PAYMENT.PAY_DAY%TYPE
...
-- 94. 급여 마스터 반영.
  PROCEDURE PAY_MASTER_LINE_INSERT
            ...
            , P_ALLOWANCE_TYPE    IN HRT_PAY_MASTER_LINE.ALLOWANCE_TYPE%TYPE
            , P_ALLOWANCE_ID      IN HRT_PAY_MASTER_LINE.ALLOWANCE_ID%TYPE
            , P_ALLOWANCE_AMOUNT  IN HRT_PAY_MASTER_LINE.ALLOWANCE_AMOUNT%TYPE
            , P_SOB_ID            IN HRT_MONTH_PAYMENT.SOB_ID%TYPE
            , P_ORG_ID            IN HRT_MONTH_PAYMENT.ORG_ID%TYPE
            , P_USER_ID           IN HRT_PAY_MASTER_LINE.CREATED_BY%TYPE
            );            
            
END HRT_MONTH_PAYMENT_G_SET;


 
/
CREATE OR REPLACE PACKAGE BODY HRT_MONTH_PAYMENT_G_SET
AS
/******************************************************************************/
/* PROJECT      : FPCB ERP
/* MODULE       : EAPP
...
--  0. PAYMENT CALCULATE MAIN.
  ...
  AS
    ...
    V_START_DATE                  HRT_MONTH_PAYMENT.SUPPLY_DATE%TYPE;
    V_END_DATE                    HRT_MONTH_PAYMENT.SUPPLY_DATE%TYPE;
...
--  1. 급상여 처리 대상자 생성.
  ...
  AS
    ...
    V_MONTH_PAYMENT_ID            HRT_MONTH_PAYMENT.MONTH_PAYMENT_ID%TYPE;
  ...
  BEGIN
    ...
    -- 급여 대상자 생성.
      FOR C1 IN ( SELECT PM.PERSON_ID 
                       ...
                       , PM.JOB_CATEGORY_ID
                       , S_PMH.PAY_TYPE
                       , PM.PAY_GRADE_ID AS PAY_GRADE_ID
                       , NULL AS GRADE_STEP
                       , PM.ORI_JOIN_DATE
                       , PM.JOIN_DATE
                       , PM.PAY_DATE
                       ...
                       , PM.RETIRE_DATE
                       , PM.COST_CENTER_ID
                       , NVL(PM.DIR_INDIR_TYPE, '30') AS DIR_INDIR_TYPE
                       , PM.EMPLOYE_TYPE
                       , CASE
                           ...
                           ELSE 'N'
                         END AS EXCEPT_TYPE
                       , NVL(S_PMH.INSUR_YN, 'Y') AS INSUR_YN
                       , PM.SOB_ID
                       , PM.ORG_ID
                       , P_PAY_YYYYMM
                       , P_WAGE_TYPE
                       ...
                       , NVL(S_MT.PAY_DAY, 0) + 
                         CASE 
                           WHEN S_PMH.PAY_TYPE IN ('1', '3') THEN NVL(S_MT.HOLY_0_COUNT, 0)
                           ELSE 0
                         END AS PAY_DAY
                    FROM HRM_PERSON_MASTER PM
                      , ( SELECT 1 AS CORP_ID
                          ...
                          WHERE PMH.START_YYYYMM      <= P_PAY_YYYYMM
                      ...
                      , (-- 월근태 내역.
                          SELECT MT.PERSON_ID
                           ...
                           WHERE MT.DUTY_YYYYMM             = P_PAY_YYYYMM
                             ...
                             AND MT.CORP_ID                 = NVL(P_CORP_ID, MT.CORP_ID)
      ...
      LOOP
        ...
        SELECT HRT_MONTH_PAYMENT_S1.NEXTVAL
          INTO V_MONTH_PAYMENT_ID
          FROM DUAL;
          
        INSERT INTO HRT_MONTH_PAYMENT
        ( MONTH_PAYMENT_ID
        , PERSON_ID 
        , PAY_YYYYMM 
        , WAGE_TYPE 
...
-- 2.1 급여 기본급 생성
  ...
  AS
    V_ALLOWANCE_AMOUNT            HRT_MONTH_ALLOWANCE.ALLOWANCE_AMOUNT%TYPE;
  ...
  BEGIN
    ...
    FOR R1 IN ( SELECT PML.ALLOWANCE_TYPE
                 ...
                 WHERE PMH.PERSON_ID              = DMT.PERSON_ID(+)
                   ...
                   AND PMH.CORP_ID                = NVL(P_CORP_ID, PMH.CORP_ID)
                   AND PMH.SOB_ID                 = P_SOB_ID
                   AND PMH.ORG_ID                 = P_ORG_ID
  ...
  BEGIN
    ...
    LOOP
      ...
      FOR R1 IN ( SELECT PAD.DEDUCTION_ID
                  ...
                  FROM HRT_PAYMENT_ADD_DEDUCTION PAD
    ...
    LOOP 
      ...
      ELS*/IF C1.ALLOWANCE_ETC = '150' THEN
      -- 75 검사수당.
...
-- 11 근속수당 생성
  ...
  BEGIN
    ...
    LOOP
      V_ALLOWANCE_AMOUNT := HRT_PAYMENT_G_SET.LONG_ALLOWANCE_F( P_STD_DATE, C1.LONG_MONTH, P_SOB_ID, P_ORG_ID);      
  ...
  BEGIN
    ...
    LOOP
      UPDATE HRT_MONTH_PAYMENT MP
        SET MP.GENERAL_HOURLY_AMOUNT = HRT_PAYMENT_G_SET.GENERAL_HOURLY_PAY_AMOUNT_F
                                                        ( C1.PAY_YYYYMM
                                                        , C1.PERSON_ID
                                                        , P_SOB_ID
                                                        , P_ORG_ID
...
-- 16 가족수당 생성
  ...
  BEGIN
    ...
    END;
    -- 가족수당 한도 조회.
    V_LIMIT_AMOUNT := HRT_PAYMENT_G_SET.LIMIT_AMOUNT_F ( P_STD_DATE, V_ALLOWANCE_CODE, P_SOB_ID, P_ORG_ID);
  ...
  BEGIN
    ...
    FOR R1 IN ( SELECT MA.ALLOWANCE_ID
                ...
                WHERE MA.ALLOWANCE_ID           = HA.ALLOWANCE_ID
                  ...
                  AND MA.CORP_ID                = NVL(P_CORP_ID, MA.CORP_ID)
...
-- 52. 건강보험/국민연금 생성(보수/소득월금액 기준).
  ...
  BEGIN
    FOR C1 IN ( SELECT MP.MONTH_PAYMENT_ID
                     ...
                     , HD.DEDUCTION_ID
                     , HD.DEDUCTION_CODE
                     , HRT_INSURANCE_MASTER_G.INSUR_AMOUNT_F(MP.PAY_YYYYMM, MP.PERSON_ID, IM.INSUR_TYPE, MP.SOB_ID, MP.ORG_ID) AS INSUR_AMOUNT
                     , HRM_COMMON_G.GET_ID_F('DEDUCTION', 'CODE = ''D06''', IM.SOB_ID, IM.ORG_ID) AS CARE_DEDUCTION_ID
                     , HRT_INSURANCE_MASTER_G.CARE_INSUR_AMOUNT_F(MP.PAY_YYYYMM, MP.PERSON_ID, MP.SOB_ID, MP.ORG_ID) AS CARE_INSUR_AMOUNT
                  FROM HRT_MONTH_PAYMENT MP
                    , HRT_INSURANCE_MASTER IM
                    , HRM_INSUR_TYPE_V IT
                    , HRM_DEDUCTION_V HD
...
-- 55. 보험 : (국민/건강/고용/산재) 생성
  ...
  BEGIN
    ...
    LOOP
      -- 
      FOR R1 IN ( SELECT IR.INSUR_RATE_CODE
                       , IR.INSUR_TYPE
                       , IR.INSUR_RATE
                       , IR.REDUCE_RATE     -- 장기요양보험일 경우.
                    FROM HRM_INSUR_RATE_V IR
                  WHERE IR.INSUR_RATE_CODE    BETWEEN '910' AND '999'
                    AND IR.SOB_ID             = P_SOB_ID
                    AND IR.ORG_ID             = P_ORG_ID
                    AND IR.ENABLED_FLAG       = 'Y'
                    AND IR.EFFECTIVE_DATE_FR  <= LAST_DAY(TO_DATE(P_PAY_YYYYMM, 'YYYY-MM'))
                    AND (IR.EFFECTIVE_DATE_TO IS NULL OR IR.EFFECTIVE_DATE_TO >= TRUNC(TO_DATE(P_PAY_YYYYMM, 'YYYY-MM')))
                 )
      LOOP
        V_INSUR_AMOUNT := 0;
        IF R1.INSUR_RATE_CODE = '921' THEN  -- 장기요양보험.
          V_INSUR_AMOUNT := (NVL(C1.TOTAL_AMOUNT, 0) * (NVL(R1.REDUCE_RATE, 0) / 100)) * (NVL(R1.INSUR_RATE, 0) / 100);
        ELSE
          V_INSUR_AMOUNT := NVL(C1.TOTAL_AMOUNT, 0) * (NVL(R1.INSUR_RATE, 0) / 100 );
        END IF;
        V_INSUR_AMOUNT := TRUNC(V_INSUR_AMOUNT, -1);
        IF R1.INSUR_TYPE = 'D03' THEN
          -- 국민연금.
          UPDATE HRT_MONTH_PAYMENT MP
            SET MP.NATIONAL_PENSION = NVL(MP.NATIONAL_PENSION, 0) + NVL(V_INSUR_AMOUNT, 0)
          WHERE MP.MONTH_PAYMENT_ID = C1.MONTH_PAYMENT_ID
          ;
        ELSIF R1.INSUR_TYPE = 'D05' THEN
          -- 건강.
          UPDATE HRT_MONTH_PAYMENT MP
            SET MP.HEALTH_INSUR = NVL(MP.HEALTH_INSUR, 0) + NVL(V_INSUR_AMOUNT, 0)
          WHERE MP.MONTH_PAYMENT_ID = C1.MONTH_PAYMENT_ID
          ;
        ELSIF R1.INSUR_TYPE = 'D04' THEN
          -- 고용.
          UPDATE HRT_MONTH_PAYMENT MP
            SET MP.UNEMPLOYMENT_INSUR = NVL(MP.UNEMPLOYMENT_INSUR, 0) + NVL(V_INSUR_AMOUNT, 0)
          WHERE MP.MONTH_PAYMENT_ID = C1.MONTH_PAYMENT_ID
          ;
        ELSIF R1.INSUR_TYPE = 'D22' THEN
          -- 산재.
          UPDATE HRT_MONTH_PAYMENT MP
            SET MP.SAFETY_INSUR = NVL(MP.SAFETY_INSUR, 0) + NVL(V_INSUR_AMOUNT, 0)
          WHERE MP.MONTH_PAYMENT_ID = C1.MONTH_PAYMENT_ID
          ; 
        END IF;
        COMMIT;
...
-- 56. 세금 생성
  ...
  BEGIN
    ...
    LOOP
      V_TAX_AMOUNT := 0;
      V_TAX_AMOUNT := HRT_PAYMENT_G_SET.TAX_AMOUNT_F( W_STD_DATE => C1.STANDARD_DATE
    ...
    LOOP
      ...
      IF NVL(V_ALLOWANCE_AMOUNT, 0) > 0 THEN
        ...
        EXCEPTION WHEN OTHERS THEN
          ...
          RETURN;
        END;
        -- 급여마스터 적용. : 적용안함. 공용 급여마스터 사용 함으로.
        HRT_PAY_MASTER_G.SAVE_PAY_LINE
    ...
    LOOP
      ...
      BEGIN
        SELECT NVL(SX1.DUTY_11, 0) AS DED_11_COUNT
            ...
            , (NVL(MTO.OVER_TIME, 0) + NVL(MTO.HOLIDAY_TIME, 0) + NVL(MTO.HOLYDAY_OT_TIME, 0)) AS OVER_TIME
          INTO V_DED_11_COUNT
            , V_LEAVE_TIME
            , V_LATE_TIME
            , V_OT_TIME
          FROM HRD_MONTH_TOTAL MT
            ...
            , ( SELECT MTD.MONTH_TOTAL_ID
             ...
             , ( SELECT DL.PERSON_ID
                 ...
                 WHERE DL.WORK_DATE  BETWEEN P_START_DATE AND P_END_DATE
                   AND DL.PERSON_ID  = C1.PERSON_ID
...
-- 74 복지수당 적용 여부 : 근무율 70%이상, 수습3개월 이상.
  ...
  BEGIN
    V_STD_WORK_DAY := HRT_PAYMENT_G_SET.WORK_DAY_COUNT_F
                        ( P_PAY_YYYYMM        => P_PAY_YYYYMM
                        , P_START_DATE        => P_START_DATE
                        , P_END_DATE          => P_END_DATE
                        , P_SOB_ID            => P_SOB_ID
...
-- 94. 급여 마스터 반영.
  ...
  AS
    V_PAY_HEADER_ID               HRT_PAY_MASTER_HEADER.PAY_HEADER_ID%TYPE;
    V_PAY_LINE_ID                 HRT_PAY_MASTER_LINE.PAY_LINE_ID%TYPE;
  BEGIN
    BEGIN
      SELECT PMH.PAY_HEADER_ID
        INTO V_PAY_HEADER_ID
