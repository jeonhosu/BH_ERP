...
-- 원천징수이행상황신고서 작성 메인.
  ...
  AS
    ...
    V_RECEIVE_REFUND_TAX_AMT  NUMBER := 0;    -- 12.전월 미환급세액
    V_ADJUST_REFUND_TAX_AMT   NUMBER := 0;    -- 18.조정대상 환급세액 
         
    V_THIS_REFUND_TAX_AMT     NUMBER := 0;    -- 당월조정 환급세액 
    
    V_PAY_INCOME_TAX_AMT      NUMBER := 0;    -- 소득세등(가산세 포함)
    V_PAY_SP_TAX_AMT          NUMBER := 0;    -- 농어촌 특별세 
  BEGIN
    O_STATUS := 'F';
    -- 마감 여부 체크.
    ...
    -- 전월미환급 세액 적용.
    BEGIN
      SELECT NVL(WD.NEXT_REFUND_TAX_AMT, 0) -
             NVL(WD.REQUEST_REFUND_TAX_AMT, 0) AS NEXT_REFUND_TAX_AMT
    ...
    UPDATE HRW_WITHHOLDING_DOC WD
      SET WD.A10_PERSON_CNT         = NVL(WD.A01_PERSON_CNT, 0) + NVL(WD.A02_PERSON_CNT, 0) + 
        ...
        , WD.A10_PAY_INCOME_TAX_AMT = CASE
                                        WHEN NVL(WD.A01_INCOME_TAX_AMT, 0) + NVL(WD.A02_INCOME_TAX_AMT, 0) + 
                                             NVL(WD.A03_INCOME_TAX_AMT, 0) + NVL(WD.A04_INCOME_TAX_AMT, 0) + 
                                             NVL(WD.A01_ADD_TAX_AMT, 0) + NVL(WD.A02_ADD_TAX_AMT, 0) + 
                                             NVL(WD.A03_ADD_TAX_AMT, 0) + NVL(WD.A04_ADD_TAX_AMT, 0) <= 0 THEN 0
                                        ELSE NVL(WD.A01_INCOME_TAX_AMT, 0) + NVL(WD.A02_INCOME_TAX_AMT, 0) + 
                                             NVL(WD.A03_INCOME_TAX_AMT, 0) + NVL(WD.A04_INCOME_TAX_AMT, 0) + 
                                             NVL(WD.A01_ADD_TAX_AMT, 0) + NVL(WD.A02_ADD_TAX_AMT, 0) + 
                                             NVL(WD.A03_ADD_TAX_AMT, 0) + NVL(WD.A04_ADD_TAX_AMT, 0)
                                      END 
        , WD.A10_PAY_SP_TAX_AMT     = CASE
                                        WHEN NVL(WD.A01_SP_TAX_AMT, 0) + NVL(WD.A02_SP_TAX_AMT, 0) + 
                                             NVL(WD.A04_SP_TAX_AMT, 0) <= 0 THEN 0
                                        ELSE NVL(WD.A01_SP_TAX_AMT, 0) + NVL(WD.A02_SP_TAX_AMT, 0) + 
                                             NVL(WD.A04_SP_TAX_AMT, 0)
                                      END                                 
    WHERE WD.WITHHOLDING_DOC_ID     = P_WITHHOLDING_DOC_ID
    ;
    ...
    UPDATE HRW_WITHHOLDING_DOC WD
      SET WD.GENERAL_REFUND_AMT     = CASE
                                      ...
                                      END + 
                                      CASE
                                        WHEN NVL(WD.A10_SP_TAX_AMT, 0) >= 0 THEN 0
                                        ELSE ABS(NVL(WD.A10_SP_TAX_AMT, 0))
                                      END
    WHERE WD.WITHHOLDING_DOC_ID     = P_WITHHOLDING_DOC_ID
    ;
    ...
    -- 납부세액 소득세 : 당월조정환급세액 계산.
    UPDATE HRW_WITHHOLDING_DOC WD
      SET WD.A10_THIS_REFUND_TAX_AMT  = CASE
                                          WHEN NVL(WD.NEXT_REFUND_TAX_AMT, 0) <= 0 THEN 0
                                          WHEN NVL(WD.A10_PAY_INCOME_TAX_AMT, 0) <= 0 THEN 0
                                          WHEN NVL(WD.NEXT_REFUND_TAX_AMT, 0) <= NVL(WD.A10_PAY_INCOME_TAX_AMT, 0) THEN NVL(WD.NEXT_REFUND_TAX_AMT, 0)
                                          WHEN NVL(WD.NEXT_REFUND_TAX_AMT, 0) > NVL(WD.A10_PAY_INCOME_TAX_AMT, 0) THEN NVL(WD.A10_PAY_INCOME_TAX_AMT, 0)
                                          ELSE 0
                                        END
    WHERE WD.WITHHOLDING_DOC_ID     = P_WITHHOLDING_DOC_ID
    ;
    ...
    UPDATE HRW_WITHHOLDING_DOC WD
      SET WD.A10_PAY_INCOME_TAX_AMT = CASE
                                          WHEN NVL(WD.A10_THIS_REFUND_TAX_AMT, 0) <= 0 THEN WD.A10_PAY_INCOME_TAX_AMT
                                          ELSE NVL(WD.A10_PAY_INCOME_TAX_AMT, 0) - NVL(WD.A10_THIS_REFUND_TAX_AMT, 0)
                                        END
    WHERE WD.WITHHOLDING_DOC_ID     = P_WITHHOLDING_DOC_ID
    ;
    /*
    ...
    -- 납부세액 농특세 : 당월조정환급세액 계산.
    UPDATE HRW_WITHHOLDING_DOC WD
      SET WD.A10_THIS_REFUND_TAX_AMT  = NVL(WD.A10_THIS_REFUND_TAX_AMT, 0) + 
                                        CASE
                                          WHEN (NVL(WD.NEXT_REFUND_TAX_AMT, 0) - NVL(WD.A10_THIS_REFUND_TAX_AMT, 0)) <= 0 THEN 0
                                          WHEN NVL(WD.A10_PAY_SP_TAX_AMT, 0) <= 0 THEN 0
                                          WHEN (NVL(WD.NEXT_REFUND_TAX_AMT, 0) - NVL(WD.A10_THIS_REFUND_TAX_AMT, 0)) <= NVL(WD.A10_PAY_SP_TAX_AMT, 0) 
                                            THEN (NVL(WD.NEXT_REFUND_TAX_AMT, 0) - NVL(WD.A10_THIS_REFUND_TAX_AMT, 0))
                                          WHEN (NVL(WD.NEXT_REFUND_TAX_AMT, 0) - NVL(WD.A10_THIS_REFUND_TAX_AMT, 0)) > NVL(WD.A10_PAY_SP_TAX_AMT, 0) 
                                            THEN NVL(WD.A10_PAY_SP_TAX_AMT, 0)
                                          ELSE 0
                                        END                                        
    WHERE WD.WITHHOLDING_DOC_ID     = P_WITHHOLDING_DOC_ID
    ;
    -- 납부세액 소득세 UPDATE 
    UPDATE HRW_WITHHOLDING_DOC WD
      SET WD.A10_PAY_SP_TAX_AMT     = CASE
                                          WHEN WD.A10_PAY_SP_TAX_AMT = 0 THEN 0 
                                          WHEN (NVL(WD.A10_THIS_REFUND_TAX_AMT, 0) - NVL(WD.A10_PAY_INCOME_TAX_AMT, 0)) <= 0 
                                            THEN WD.A10_PAY_SP_TAX_AMT
                                          ELSE NVL(WD.A10_PAY_SP_TAX_AMT, 0) - (NVL(WD.A10_THIS_REFUND_TAX_AMT, 0) - NVL(WD.A10_PAY_INCOME_TAX_AMT, 0))
                                        END
    WHERE WD.WITHHOLDING_DOC_ID     = P_WITHHOLDING_DOC_ID
    ;
    ...
    UPDATE HRW_WITHHOLDING_DOC WD
      SET WD.A30_PERSON_CNT         = NVL(WD.A25_PERSON_CNT, 0) + NVL(WD.A26_PERSON_CNT, 0)
        ...
        , WD.A30_SP_TAX_AMT         = NVL(WD.A26_SP_TAX_AMT, 0) 
    WHERE WD.WITHHOLDING_DOC_ID     = P_WITHHOLDING_DOC_ID
    ;
    ...
    UPDATE HRW_WITHHOLDING_DOC WD
      SET WD.A30_PAY_INCOME_TAX_AMT = CASE
                                      ...
                                      END
        , WD.A30_PAY_SP_TAX_AMT     = CASE
                                        WHEN NVL(WD.A30_SP_TAX_AMT, 0) <= 0 THEN 0
                                        ELSE NVL(WD.A30_SP_TAX_AMT, 0) 
                                      END
    WHERE WD.WITHHOLDING_DOC_ID     = P_WITHHOLDING_DOC_ID
    ;
    ...
    /*
    -- 당월조정 환급세액이 있을 경우 적용 -- 
    IF NVL(V_THIS_REFUND_TAX_AMT, 0) > 0 THEN 
\*    -- 근로소득 당월 조정 환급세액 적용 
    SELECT WD.A10_PAY_INCOME_TAX_AMT
    ...
    -- 근로소득 당월 조정 환급세액 적용 
    UPDATE HRW_WITHHOLDING_DOC WD
       SET WD.A10_THIS_REFUND_TAX_AMT =  
     WHERE WD.WITHHOLDING_DOC_ID      = P_WITHHOLDING_DOC_ID
    ;*\
  ...
  BEGIN
    ...
    BEGIN
      ...
      WHERE PM.PERSON_ID                = MP.PERSON_ID
        AND MP.MONTH_PAYMENT_ID         = HMA.MONTH_PAYMENT_ID(+)  -- 지급이 없어도  처리 
        AND MP.MONTH_PAYMENT_ID         = HMD.MONTH_PAYMENT_ID(+)  -- 공제 없어도 처리 
  ...
  BEGIN
    ...
    BEGIN
      ...
      WHERE PM.PERSON_ID                = MP.PERSON_ID
        ...
        AND MP.PAY_YYYYMM               = P_STD_YYYYMM  -- 귀속년월.
        AND MP.WAGE_TYPE                = 'P1'          -- 급여만 적용 -- 
        AND MP.CORP_ID                  = P_CORP_ID
        AND MP.SOB_ID                   = P_SOB_ID
        AND MP.ORG_ID                   = P_ORG_ID
				-- 조선미S 요청 : 연말정산 차감집계표 기준 자료 생성(2014.11.25-전호수K 수정) -- 
...
-- 연말정산 계산.
  ...
  AS
    ...
    V_PAYMENT_AMOUNT                  NUMBER := 0;
    V_INCOME_TAX_AMOUNT               NUMBER := 0;
    V_SP_TAX_AMOUNT                   NUMBER := 0;
  BEGIN
    O_STATUS := 'F';
    BEGIN
      SELECT COUNT(DISTINCT YA.PERSON_ID) AS PERSON_COUNT
           , SUM(YA.INCOME_TOT_AMT) AS ALLOWANCE_AMOUNT  -- 총지급액.
           , SUM(YA.SUBT_IN_TAX_AMT) AS INCOME_TAX_AMOUNT  -- 정산소득세.
           , SUM(YA.SUBT_SP_TAX_AMT) AS SP_TAX_AMOUNT      -- 정산 농특세 
        INTO V_PERSON_COUNT
          , V_PAYMENT_AMOUNT
          , V_INCOME_TAX_AMOUNT
          , V_SP_TAX_AMOUNT 
      ...
      WHERE PM.PERSON_ID                = YA.PERSON_ID
        ...
        AND PM.SOB_ID                   = P_SOB_ID
        AND PM.ORG_ID                   = P_ORG_ID
        -- 전호수 수정(2013-03-07) : 김수신k 요청 - 12월31일자 퇴사자는 이미 신고했으므로 연말정산신고시 빠져야 함--
    ...
    BEGIN
      UPDATE HRW_WITHHOLDING_DOC WD
        SET WD.A04_PERSON_CNT        = NVL(V_PERSON_COUNT, 0)
          ...
          , WD.A04_SP_TAX_AMT        = NVL(V_SP_TAX_AMOUNT, 0)
      WHERE WD.WITHHOLDING_DOC_ID    = P_WITHHOLDING_DOC_ID
      ;
    EXCEPTION WHEN OTHERS THEN
      O_STATUS := 'F';
...
-- 퇴직소득 계산.
  ...
  BEGIN
    ...
    -- 1. 전체.
    BEGIN
      SELECT COUNT(DISTINCT RA.PERSON_ID) AS PERSON_COUNT
           , SUM(RA.RETIRE_TOTAL_AMOUNT) AS RETIRE_TOTAL_AMOUNT
           , SUM(CASE
					         WHEN NVL((SELECT 'Y'
															FROM HRR_RETIREMENT_PENSION RP
														 WHERE RP.PERSON_ID     = RA.PERSON_ID
															 AND (RP.ISSUE_DATE   IS NULL 
																 OR RP.ISSUE_DATE   >= RA.RETIRE_DATE_TO)
													   ), 'N') = 'Y' THEN 0
								   ELSE TRUNC(NVL(RA.INCOME_TAX_AMOUNT, 0), -1) + TRUNC(NVL(RA.H_INCOME_TAX_AMOUNT, 0), -1)
    ...
    END;
		...
		-- 일반사업자는 퇴직연금 소득세 내역 표기 안함 -- 
		BEGIN
      UPDATE HRW_WITHHOLDING_DOC WD
        SET WD.A21_PERSON_CNT      = NVL(V_PERSON_COUNT, 0)
          , WD.A21_PAYMENT_AMT     = NVL(V_PAYMENT_AMOUNT, 0)
          , WD.A21_INCOME_TAX_AMT  = NVL(V_INCOME_TAX_AMOUNT, 0)
      WHERE WD.WITHHOLDING_DOC_ID  = P_WITHHOLDING_DOC_ID
      ;
    EXCEPTION WHEN OTHERS THEN
      O_STATUS := 'F';
    ...
    BEGIN
      SELECT COUNT(RA.PERSON_ID) AS PERSON_COUNT
           , SUM(RA.RETIRE_TOTAL_AMOUNT) AS RETIRE_TOTAL_AMOUNT
           , SUM(TRUNC(NVL(RA.INCOME_TAX_AMOUNT, 0), -1) + TRUNC(NVL(RA.H_INCOME_TAX_AMOUNT, 0), -1)) AS INCOME_TAX_AMOUNT
        INTO V_PERSON_COUNT
           , V_PAYMENT_AMOUNT
           , V_INCOME_TAX_AMOUNT
        FROM HRR_RETIRE_ADJUSTMENT RA
    ...
    BEGIN
      UPDATE HRW_WITHHOLDING_DOC WD
        SET WD.A22_PERSON_CNT      = NVL(V_PERSON_COUNT, 0)
          , WD.A22_PAYMENT_AMT     = NVL(V_PAYMENT_AMOUNT, 0)
          , WD.A22_INCOME_TAX_AMT  = NVL(V_INCOME_TAX_AMOUNT, 0)
      WHERE WD.WITHHOLDING_DOC_ID  = P_WITHHOLDING_DOC_ID
      ;
    EXCEPTION WHEN OTHERS THEN
      O_STATUS := 'F';
    ...
    V_INCOME_TAX_AMOUNT := 0;
    BEGIN
      SELECT NVL(WD.A20_PERSON_CNT, 0) - (NVL(WD.A21_PERSON_CNT, 0) + NVL(WD.A22_PERSON_CNT, 0)) AS GAP_PERSON_CNT
           , NVL(WD.A20_PAYMENT_AMT, 0) - (NVL(WD.A21_PAYMENT_AMT, 0) + NVL(WD.A22_PAYMENT_AMT, 0)) AS GAP_PAYMENT_AMT
           , NVL(WD.A20_INCOME_TAX_AMT, 0) - (NVL(WD.A21_INCOME_TAX_AMT, 0) + NVL(WD.A22_INCOME_TAX_AMT, 0)) AS GAP_INCOME_TAX_AMT
        INTO V_PERSON_COUNT
           , V_PAYMENT_AMOUNT
           , V_INCOME_TAX_AMOUNT              
        FROM HRW_WITHHOLDING_DOC WD        
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      O_MESSAGE := SQLERRM;
    END;
    IF V_PERSON_COUNT != 0 OR V_PAYMENT_AMOUNT != 0 OR V_INCOME_TAX_AMOUNT != 0 THEN
      O_STATUS := 'F';
      O_MESSAGE := 'Validate Error = Total Amount != Sub Item Sum';
    END IF;
...
-- 사업소득 계산.
  ...
  BEGIN
    O_STATUS := 'F';
    BEGIN
      SELECT COUNT(DISTINCT RI.EARNER_ID) AS PERSON_COUNT
           , SUM(RI.PAYMENT_AMOUNT) AS RETIRE_TOTAL_AMOUNT
           , SUM(RI.INCOME_TAX_AMT) AS INCOME_TAX_AMOUNT
        INTO V_PERSON_COUNT
           , V_PAYMENT_AMOUNT
           , V_INCOME_TAX_AMOUNT
        FROM HRW_RESIDENT_INCOME RI 
           , HRW_EARNER_MASTER   EM 
      WHERE RI.EARNER_ID            = EM.EARNER_ID 
        AND EM.EARNER_TYPE          = '10' -- 사업소득  
        AND RI.CORP_ID              = P_CORP_ID
        AND RI.SOB_ID               = P_SOB_ID
        AND RI.ORG_ID               = P_ORG_ID
        AND TO_CHAR(RI.PAY_DATE, 'YYYY-MM')     = P_STD_YYYYMM     -- 귀속연월.
        AND TO_CHAR(RI.RECEIPT_DATE, 'YYYY-MM') = P_PAY_YYYYMM  -- 지급연월.
      ;
    EXCEPTION WHEN OTHERS THEN
      V_PERSON_COUNT      := 0;
      V_PAYMENT_AMOUNT    := 0;
