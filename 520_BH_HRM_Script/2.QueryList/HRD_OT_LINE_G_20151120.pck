...
-- DATA INSERT.
  ...
  BEGIN
    IF HRD_OT_HEADER_G.DATA_STATUS_CHECK(P_OT_HEADER_ID) != 'N' THEN
...
-- DATA UPDATE.
  ...
  AS
  BEGIN
    IF HRD_OT_HEADER_G.DATA_STATUS_CHECK(W_OT_HEADER_ID) != 'N' THEN
                   ...
                   SELECT COUNT(OL.PERSON_ID)
                     ...
                     FROM HRD_OT_LINE           OL
                        ...
                        , ( SELECT PH.PERSON_ID
                              ...
                              FROM HRD_PERSON_HISTORY PH 
                             WHERE PH.EFFECTIVE_DATE_FR   <= W_WORK_DATE 
                               AND (PH.EFFECTIVE_DATE_TO  >= W_WORK_DATE OR PH.EFFECTIVE_DATE_TO IS NULL)
                          ) T2
                    WHERE OL.OT_HEADER_ID    =  OH.OT_HEADER_ID
                      AND OL.PERSON_ID       =  T2.PERSON_ID
            ...
            OPEN P_CURSOR FOR
             SELECT HRM_COMMON_G.ID_NAME_F(T2.WORK_TYPE_ID)    AS WORK_TYPE
                 ...
                 WHERE W_STD_DATE            = WC.WORK_DATE
                   ...
                   AND PM.WORK_CORP_ID       = W_CORP_ID
                   AND PM.PERSON_ID          = NVL(W_PERSON_ID, PM.PERSON_ID)
                   AND T2.WORK_TYPE_ID       = NVL(W_WORK_TYPE_ID, T2.WORK_TYPE_ID) --: 작업장별로 1개의 연장근무 신청위해 주석(2014-07-30 : 전호수K) -- 
                   AND PM.SOB_ID             = W_SOB_ID
                   AND PM.ORG_ID             = W_ORG_ID
...
-- [2011-10-28][2011-11-29]
   ...
   BEGIN
             ...
             IF V_BEFORE_OT_END < V_BEFORE_OT_START THEN
               -- 종료일자보다 시작일자가 이후 일 경우 오류 
               RAISE_APPLICATION_ERROR(-20001, 'Before : ' || V_PERSON_NAME || CHR(10) || EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10503'));
             END IF;
             ...
             IF V_AFTER_OT_END < V_AFTER_OT_START THEN
               -- 종료일자보다 시작일자가 이후 일 경우 오류 
               RAISE_APPLICATION_ERROR(-20001, 'After : ' || V_PERSON_NAME || CHR(10) || EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10509'));
             END IF;
             -- 야간일 경우 정오 이전 신청 불가 -- 
             IF (P_WORK_DATE + 1.229166666666667)  < V_AFTER_OT_START 
               AND V_BEFORE_OT_START < (P_WORK_DATE + 0.5) THEN
