...
-------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    IF P_YEAR_YYYYMM < P_PAY_YYYYMM THEN
      BEGIN
        SELECT MAX(DECODE(HD.DEDUCTION_CODE, 'D25', HD.DEDUCTION_ID, NULL)) AS IN_TAX_ID
             , MAX(DECODE(HD.DEDUCTION_CODE, 'D26', HD.DEDUCTION_ID, NULL)) AS LOCAL_TAX_ID
             , MAX(DECODE(HD.DEDUCTION_CODE, 'D27', HD.DEDUCTION_ID, NULL)) AS SP_TAX_ID
          INTO V_IN_TAX_ID, V_LOCAL_TAX_ID, V_SP_TAX_ID
          FROM HRM_DEDUCTION_V HD
         WHERE HD.DEDUCTION_CODE      IN ('D25', 'D26', 'D27')
    ...
    OPEN P_CURSOR FOR
      SELECT HA.YEAR_YYYY AS YEAR_YYYY
           ...
           , ( NVL(HA.TAX_DED_INCOME_AMT, 0) + NVL(HA.TD_CHILD_RAISE_DED_AMT, 0) +
               ...
               NVL(HA.TD_ANNU_BANK_DED_AMT, 0) + NVL(HA.TD_GUAR_INSUR_DED_AMT, 0) + 
           ...
           , NVL(HA.TD_CHILD_RAISE_DED_AMT, 0) AS TD_CHILD_RAISE_DED_AMT                  -- 2014-세공(자녀양육)   
  ...
  BEGIN
    ...
    OPEN P_CURSOR FOR
          ...
          --, NVL(HA.HOUSE_FUND_AMT, 0) AS HOUSE_FUND_AMT                -- 주택자금 합계
           ...
           -- 세액공제
           , NVL(HA.TAX_DED_INCOME_AMT, 0) AS TAX_DED_INCOME_AMT        -- 세공-근로소득
           , NVL(HA.TD_CHILD_RAISE_DED_AMT, 0) AS TD_CHILD_RAISE_DED_AMT  -- 세공-자녀양육
