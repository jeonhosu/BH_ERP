CREATE OR REPLACE PACKAGE HRT_MONTH_BONUS_G_SET
AS
...
END HRT_MONTH_BONUS_G_SET;


 
/
CREATE OR REPLACE PACKAGE BODY HRT_MONTH_BONUS_G_SET
AS
/******************************************************************************/
/* PROJECT      : FPCB ERP
/* MODULE       : EAPP
/* PROGRAM NAME : HRT_MONTH_BONUS_G_SET
...
--  1. 급상여 처리 대상자 생성.
  ...
  BEGIN
    ...
    IF P_WAGE_TYPE = C_MONTH_BONUS THEN
    -- 상여 대상자 생성.
  ...
  BEGIN
    ...
    LOOP
      ...
      LOOP
        ...
        -- 일급--
          V_ALLOWANCE_AMOUNT := 30 * NVL(R1.ALLOWANCE_AMOUNT, 0);
        ELSIF C1.PAY_LEVEL = 'TIME' THEN
        -- 시급--
          V_ALLOWANCE_AMOUNT := 209 * NVL(R1.ALLOWANCE_AMOUNT, 0);
        END IF;
    ...
    LOOP
      ...
      BEGIN
        SELECT PPR.PAYMENT_RATE
          INTO V_PAY_RATE 
          FROM HRT_PERSON_PAYMENT_RULE PPR
    ...
    FOR C1 IN ( SELECT MP.MONTH_PAYMENT_ID
                  ...
                  FROM HRT_MONTH_PAYMENT MP
                    , (SELECT MA.MONTH_PAYMENT_ID
                            , NVL(SUM(CASE WHEN HA.TAX_FREE IN('OT', 'ETC') THEN 0 ELSE MA.ALLOWANCE_AMOUNT END), 0) AS ALLOWANCE_AMOUNT
                        FROM HRT_MONTH_ALLOWANCE MA
                          , HRM_ALLOWANCE_V HA
                       WHERE MA.ALLOWANCE_ID      = HA.ALLOWANCE_ID
