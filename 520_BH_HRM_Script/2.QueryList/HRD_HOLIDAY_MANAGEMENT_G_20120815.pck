  ...
  AS
    ...
    BEGIN
			SELECT HT.START_DATE, TO_CHAR(HT.START_DATE, 'YYYY-MM') AS START_YYYYMM
			 ...
			 WHERE HT.HOLIDAY_YEAR          = SUBSTR(W_YYYYMM, 1, 4)
			   AND HT.SOB_ID                = W_SOB_ID
				 AND HT.ORG_ID                = W_ORG_ID
				 ;
		EXCEPTION WHEN OTHERS THEN
		  V_START_DATE := TRUNC(TO_DATE(W_YYYYMM || '-01', 'YYYY-MM-DD'), 'YEAR');
			V_START_YYYYMM := TO_CHAR(V_START_DATE, 'YYYY-MM');
			V_END_DATE := LAST_DAY(TO_DATE(W_YYYYMM || '-01', 'YYYY-MM-DD'));
				...
				FROM EAPP_CALENDAR_YYYYMM_V CY
					...
					, (-- 월근태 정보.
						SELECT MT.DUTY_YYYYMM             
							...
							FROM HRD_MONTH_TOTAL MT
								 , HRM_CLOSING_TYPE_V CT
						 WHERE MT.DUTY_TYPE          = CT.CLOSING_TYPE
						   AND MT.SOB_ID             = CT.SOB_ID
							 AND MT.ORG_ID             = CT.ORG_ID
					...
					, (-- 근태 자료.
						SELECT DL.PERSON_ID
									 ...
									 , '11' AS DUTY_GROUP_11  
									 , SUM(DECODE(DC.HOLIDAY_MANAGE_DUTY_GROUP, '11', DC.APPLY_DAY, 0)) AS DUTY_DAY_11
									 , '20' AS DUTY_GROUP_20  
									 , SUM(DECODE(DC.HOLIDAY_MANAGE_DUTY_GROUP, '20', DC.APPLY_DAY, 0)) AS DUTY_DAY_20
									 , '22' AS DUTY_GROUP_22  
									 , SUM(DECODE(DC.HOLIDAY_MANAGE_DUTY_GROUP, '22', DC.APPLY_DAY, 0)) AS DUTY_DAY_22
									 , '23' AS DUTY_GROUP_23 
									 , SUM(DECODE(DC.HOLIDAY_MANAGE_DUTY_GROUP, '23', DC.APPLY_DAY, 0)) AS DUTY_DAY_23
									 , '54' AS DUTY_GROUP_54  
									 , SUM(DECODE(DC.HOLIDAY_MANAGE_DUTY_GROUP, '54', DC.APPLY_DAY, 0)) AS DUTY_DAY_54
									 , '55' AS DUTY_GROUP_55  
									 , SUM(DECODE(DC.HOLIDAY_MANAGE_DUTY_GROUP, '55', DC.APPLY_DAY, 0)) AS DUTY_DAY_55
									 , '56' AS DUTY_GROUP_56  
									 , SUM(DECODE(DC.HOLIDAY_MANAGE_DUTY_GROUP, '56', DC.APPLY_DAY, 0)) AS DUTY_DAY_56
