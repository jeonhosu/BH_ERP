...
-- 퇴직충당금 산출.
  ...
  AS
    V_SYSDATE                     HRR_RETIRE_RESERVE.CREATION_DATE%TYPE;
...
--> 해당 년월에 대한 자료 삭제;
    ...
    EXCEPTION WHEN OTHERS THEN
      O_MESSAGE := 'Delete_Error => ' || SQLERRM;
      RETURN;
    END;
...
--> 기준일자 생성;      
    ...
    EXCEPTION WHEN OTHERS THEN
      O_MESSAGE := 'Reserve Date Term Error => ' || SQLERRM;
      RETURN;
    END;
...
---> 3개월 평균일수;
    ...
    EXCEPTION WHEN OTHERS THEN
      O_MESSAGE := '3rd Average Day Error => ' || SQLERRM;
      RETURN;
    END;
...
--> 중도정산 여부 체크 및 정산 시작일자 설정;
      BEGIN
        SELECT DECODE(C1.EXPIRE_DATE, NULL, C1.JOIN_DATE, C1.EXPIRE_DATE + 1) AS RETIRE_START_DATE
...
--> 근속년수, 근속월수, 근속일수 계산  
      ...
      EXCEPTION WHEN OTHERS THEN
        O_MESSAGE := 'Long Calculate Error =>' || SQLERRM;
        RETURN;
      END;
      ...
      BEGIN
        SELECT SUM(MA.ALLOWANCE_AMOUNT) AS TOTAL_PAY_AMOUNT
         ...
         WHERE MP.PERSON_ID               = PM.PERSON_ID
           ...
           AND EXISTS ( SELECT 'X'  -- 급여항목만 조회.
                          FROM HRM_CLOSING_TYPE_V CT
                         WHERE CT.CLOSING_TYPE    = MP.WAGE_TYPE
                           AND CT.SOB_ID          = MP.SOB_ID
                           AND CT.ORG_ID          = MP.ORG_ID
                           AND CT.MODULE_TYPE     = 'PAY'
                           AND CT.PERIOD_TYPE     = 'PAY'
                      )
           AND EXISTS ( SELECT 'X'
                          FROM HRM_ALLOWANCE_V HA
                         WHERE HA.ALLOWANCE_ID    = MA.ALLOWANCE_ID
                           AND HA.ALLOWANCE_TYPE  NOT IN('YEAR', 'BONUS')
                           AND HA.RETIRE_YN       = 'Y'
                           AND HA.ENABLED_FLAG    = 'Y'
                           AND HA.EFFECTIVE_DATE_FR <= V_RETR_END_DATE
                           AND (HA.EFFECTIVE_DATE_TO IS NULL OR HA.EFFECTIVE_DATE_TO >= V_RETR_START_DATE)
                      )
...
--> 누적 총금액.
      IF NVL(V_STD_MONTH, 0) = 0 THEN
        V_STD_MONTH := 3;        
      END IF;
      V_TOTAL_AMOUNT := NVL(V_TOTAL_PAY_AMOUNT, 0) * (NVL(V_STD_PAY_MONTH, 0) / V_STD_MONTH);
...
--> 상여 계산.
      ...
      END;
--> 누적 총금액.      
      IF V_STD_MONTH = 0 THEN
        V_STD_MONTH := 3;        
      END IF;
      V_TOTAL_AMOUNT := V_TOTAL_AMOUNT + (NVL(V_TOTAL_BONUS_AMOUNT, 0) * (NVL(V_STD_BONUS_MONTH ,0) / V_STD_MONTH));
...
--> 누적 총금액.      
      IF V_STD_MONTH = 0 THEN
        V_STD_MONTH := 3;        
      END IF;
      V_TOTAL_AMOUNT := NVL(V_TOTAL_AMOUNT, 0) + (NVL(V_YEAR_ALOWANCE_AMOUNT, 0) * (12 / V_STD_MONTH));
