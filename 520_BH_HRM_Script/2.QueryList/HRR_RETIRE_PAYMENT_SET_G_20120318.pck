...
----> 인사정보 조회.
    BEGIN
      ...
      WHERE PMH.PERSON_ID       = P_PERSON_ID
        AND PMH.START_YYYYMM    <=  TO_CHAR(P_RETIRE_DATE_TO, 'YYYY-MM')
        AND PMH.END_YYYYMM      >=  TO_CHAR(P_RETIRE_DATE_TO, 'YYYY-MM')
        AND ROWNUM              <= 1
      ;
    EXCEPTION WHEN OTHERS THEN
      V_PAY_TYPE := '3';
  ...
  BEGIN
    ...
    */
    ...
    LOOP 
      ...
      END IF;
      IF NVL(V_ALLOWANCE_AMOUNT, 0) = 0 THEN
        DELETE FROM HRR_RETIRE_ADJUST_PAY_DETAIL APD
        WHERE APD.PAY_YYYYMM        = P_PAY_YYYYMM
          AND APD.WAGE_TYPE         = P_WAGE_TYPE
          AND APD.PERSON_ID         = C1.PERSON_ID
          AND APD.ALLOWANCE_ID      = C1.ALLOWANCE_ID
