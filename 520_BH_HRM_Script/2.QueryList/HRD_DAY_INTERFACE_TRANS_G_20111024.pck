...
-- DAY INTERFACE TRANSFER GO1.
 ...
 END DATA_TRANSFER_GO1;
       ...
       --ÃâÅð±ÙÁ¶È¸(±â°£) [2011-07-29][2011-08-11]
       PROCEDURE SELECT_DATA
               ( P_CURSOR             OUT TYPES.TCURSOR
               , W_SOB_ID             IN  HRD_DAY_INTERFACE.SOB_ID%TYPE
               , W_ORG_ID             IN  HRD_DAY_INTERFACE.ORG_ID%TYPE
       ...
       BEGIN
                 ...
                 SELECT DI.WORK_DATE
                      ...
                      /*
                      , CASE WHEN DI.MODIFY_FLAG = 'N' AND DI.OPEN_TIME  IS NOT NULL AND DI.CLOSE_TIME IS NOT NULL AND (DI.DUTY_ID = 1168 OR DI.DUTY_ID = 1187) THEN ''
                             WHEN DI.MODIFY_FLAG = 'N' AND DI.OPEN_TIME  IS NULL AND DI.CLOSE_TIME IS NULL AND (DI.DUTY_ID = 1182 OR DI.DUTY_ID = 1188) THEN ''
                             WHEN DI.MODIFY_FLAG = 'N' AND DI.OPEN_TIME  IS NULL AND (SELECT S_DI.HOLY_TYPE
                                                                                        FROM HRD_DAY_INTERFACE S_DI
                                                                                       WHERE S_DI.SOB_ID       = DI.SOB_ID
                                                                                         AND S_DI.ORG_ID       = DI.ORG_ID
                                                                                         AND S_DI.WORK_CORP_ID = DI.WORK_CORP_ID
                             ...
                             WHEN DI.MODIFY_FLAG = 'N' AND DI.OPEN_TIME  IS NOT NULL AND DI.DUTY_ID = 1187 AND (SELECT S_DI.CLOSE_TIME
                                                                                                                  FROM HRD_DAY_INTERFACE S_DI
                                                                                                                 WHERE S_DI.SOB_ID        =  DI.SOB_ID
                                                                                                                   AND S_DI.ORG_ID        =  DI.ORG_ID
                                                                                                                   AND S_DI.WORK_CORP_ID  =  DI.WORK_CORP_ID
