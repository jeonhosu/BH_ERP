...
-- DATA INSERT.
  PROCEDURE DATA_INSERT
            ...
            , P_WORK_DATE                             IN HRD_OT_HEADER.WORK_DATE%TYPE 
  ...
  BEGIN
    ...
    BEGIN
      SELECT PM.SOB_ID
       ...
       WHERE PM.PERSON_ID      = W_CONNECT_PERSON_ID
       ;
    EXCEPTION
      WHEN OTHERS THEN
        V_SOB_ID := 10;
...
-- DATA INSERT.
  ...
  BEGIN
    ...
    -- 작업장 ID 조회 --
    BEGIN
      SELECT DM.DUTY_CONTROL_ID AS FLOOR_ID 
           , DM.START_DATE
        INTO V_FLOOR_ID
           , V_START_DATE 
        FROM HRD_DUTY_MANAGER DM
       WHERE DM.CORP_ID             = P_CORP_ID
         ...
         AND DM.DUTY_MANAGER_ID     = P_DUTY_MANAGER_ID
         ;
    EXCEPTION
      WHEN OTHERS THEN
        V_FLOOR_ID := 0;
        V_START_DATE := TRUNC(P_WORK_DATE, 'MONTH');
    END;
    
    -- 1.기존에 신청한 갯수 
    BEGIN
      SELECT COUNT(DISTINCT HOH.WORK_DATE) AS REQ_COUNT 
        INTO V_REQ_SEQ
        FROM HRD_OT_HEADER    HOH  
       WHERE HOH.SOB_ID                     = P_SOB_ID 
         AND HOH.ORG_ID                     = P_ORG_ID 
         AND HOH.WORK_DATE                  >= TRUNC(P_WORK_DATE, 'MONTH') 
         AND HOH.WORK_DATE                  < P_WORK_DATE                         -- 당일 제외 -- 
         AND HOH.WORK_DATE                  >= TO_DATE('2014-08-01', 'YYYY-MM-DD') -- 이후 데이터만 적용 -- 
         AND HOH.WORK_DATE                  >= V_START_DATE                        -- 해당 작업장 사용 시작일 이후 부터 체크 -- 
         AND EXISTS
               ( SELECT 'X'
                   FROM HRD_DUTY_MANAGER DM
                  WHERE DM.DUTY_MANAGER_ID  = HOH.DUTY_MANAGER_ID
                    AND EXISTS
                          ( SELECT 'X'
                              FROM HRD_DUTY_MANAGER S_DM
                             WHERE S_DM.CORP_ID           = DM.CORP_ID
                               AND S_DM.DUTY_CONTROL_ID   = DM.DUTY_CONTROL_ID
                               AND S_DM.DUTY_MANAGER_ID   = P_DUTY_MANAGER_ID
                          )
               )
       ;
      
      
      SELECT (P_WORK_DATE - CASE
                              WHEN TRUNC(P_WORK_DATE, 'MONTH') < V_START_DATE THEN V_START_DATE 
                              WHEN TRUNC(P_WORK_DATE, 'MONTH') < TO_DATE('2014-08-01', 'YYYY-MM-DD') THEN TO_DATE('2014-08-01', 'YYYY-MM-DD')
                              ELSE TRUNC(P_WORK_DATE, 'MONTH')
                            END) AS DAY_COUNT
        INTO V_RECORD_COUNT 
        FROM DUAL; 
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ...
        V_RECORD_COUNT  := 1;
    END;
    IF P_WORK_DATE < TO_DATE('2014-08-01', 'YYYY-MM-DD') THEN
      NULL;
    ELSIF V_REQ_SEQ != V_RECORD_COUNT THEN
      RAISE_APPLICATION_ERROR(-20001, '[' || TO_DATE(P_WORK_DATE, 'YYYY-MM-DD') || '] ' || EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10512'));
      RETURN;
    END IF;  
		
    -- 2. 미신청 인원이 있으면 저장 안되게 --
    CREATE_OT_REQ_LIST
      ( O_STATUS                  => V_STATUS 
      , O_MESSAGE                 => V_MESSAGE  
      , P_SOB_ID                  => P_SOB_ID 
      ...
      , P_CORP_ID                 => NULL 
      , P_PERIOD_NAME             => TO_CHAR(P_WORK_DATE, 'YYYY-MM') -- P_PERIOD_NAME 
      , P_FLOOR_ID                => V_FLOOR_ID 
      , P_CONNECT_PERSON_ID       => P_REQ_PERSON_ID  
      );
    IF V_STATUS = 'F' THEN
      RAISE_APPLICATION_ERROR(-20001, '[' || TO_DATE(P_WORK_DATE, 'YYYY-MM-DD') || '] ' || V_MESSAGE);
      RETURN; 
    END IF;
    BEGIN
      SELECT SX1.WORK_DATE AS WORK_DATE 
           , NVL(( SELECT (NVL(ORG.ATTRIBUTE_1, 0) - NVL(ORG.ATTRIBUTE_2, 0)) AS GAP 
                    FROM HRD_OT_REQ_GT ORG
                       , HRM_FLOOR_V   HF
                   WHERE ORG.FLOOR_ID           = HF.FLOOR_ID 
                     AND ORG.WORK_DATE          = SX1.WORK_DATE
                     AND ORG.FLOOR_ID           = V_FLOOR_ID 
                   ), 0) AS GAP 
        INTO V_WORK_DATE
           , V_GAP
        FROM (SELECT MIN(ORG.WORK_DATE) AS WORK_DATE  
                FROM HRD_OT_REQ_GT ORG
                   , HRM_FLOOR_V   HF
               WHERE ORG.FLOOR_ID           = HF.FLOOR_ID 
                 AND ORG.WORK_DATE          < P_WORK_DATE
                 AND ORG.FLOOR_ID           = V_FLOOR_ID
                 AND (NVL(ORG.ATTRIBUTE_1, 0) - NVL(ORG.ATTRIBUTE_2, 0)) != 0
              ) SX1
              ;
    EXCEPTION
      WHEN OTHERS THEN
        ...
        V_GAP := 0;
    END;
    IF V_WORK_DATE IS NOT NULL AND V_GAP != 0 THEN
      RAISE_APPLICATION_ERROR(-20001, '[' || TO_DATE(P_WORK_DATE, 'YYYY-MM-DD') || '] ' || 
                                      EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10513', '&&WORK_DATE:=' || TO_CHAR(V_WORK_DATE, 'YYYY-MM-DD') ||
                                                                               '&&PERSON_COUNT:=' || TO_CHAR(V_GAP, '999,999,999')));
      RETURN; 
    END IF;
    
    /*-- 해당 근무일자에 대해 신청내역 존재 체크 : 있으면 다시 신청 못함 -- 
    BEGIN
      SELECT COUNT(OH.OT_HEADER_ID) AS RECORD_COUNT
        INTO V_RECORD_COUNT
        FROM HRD_OT_HEADER OH
      WHERE OH.SOB_ID             = P_SOB_ID
        AND OH.ORG_ID             = P_ORG_ID
        AND OH.WORK_DATE          = P_WORK_DATE 
        AND EXISTS
             ( SELECT 'X'
                 FROM HRD_DUTY_MANAGER DM
    ...
    END;
    IF V_RECORD_COUNT > 0 THEN
      RAISE_APPLICATION_ERROR(-20001, '[' || TO_DATE(P_WORK_DATE, 'YYYY-MM-DD') || '] ' || EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10301'));
      RETURN;
    END IF;*/
    ...
    BEGIN
      SELECT SX1.REQ_NUM
        ...
        INTO V_REQ_NUM
           , V_FLOOR_NAME
           , V_REQ_COUNT 
        FROM ( SELECT HOH.REQ_NUM AS REQ_NUM 
                   , HF.FLOOR_NAME 
                   , HOH.WORK_DATE
                   , HOH.DUTY_MANAGER_ID
                   , DM.DUTY_CONTROL_ID 
               ...
               WHERE HOH.DUTY_MANAGER_ID                    = DM.DUTY_MANAGER_ID
                 ...
                 AND HOH.WORK_DATE                         >= TO_DATE('2014-07-21', 'YYYY-MM-DD') -- 이후 데이터만 적용 -- 
                 AND HOH.APPROVE_STATUS                    = 'N'                                  -- 승인 미요청 -- 
                 AND EXISTS
                       (SELECT 'X' 
                          FROM HRD_OT_HEADER OH 
                         WHERE OH.DUTY_MANAGER_ID          = HOH.DUTY_MANAGER_ID  
                           AND OH.REQ_TYPE                 = HOH.REQ_TYPE 
                           AND OH.WORK_DATE                > HOH.WORK_DATE 
                           AND OH.WORK_DATE                >= TRUNC(HOH.WORK_DATE, 'MONTH')       -- 매월1일자 부터 적용 -- 
                           AND OH.WORK_DATE                <= LAST_DAY(HOH.WORK_DATE) 
                           AND OH.OT_HEADER_ID             = P_OT_HEADER_ID 
                       )
               ORDER BY HOH.WORK_DATE 
             ) SX1
       WHERE ROWNUM                 <= 1
       ;
       /*
      SELECT MAX(HOH.REQ_NUM) AS REQ_NUM  
       ...
       WHERE HOH.DUTY_MANAGER_ID                    = DM.DUTY_MANAGER_ID
         AND DM.DUTY_CONTROL_ID                     = HF.FLOOR_ID  
         AND HOH.WORK_DATE                         >= TO_DATE('2014-07-21', 'YYYY-MM-DD') -- 이후 데이터만 적용 -- 
         AND HOH.WORK_DATE                         >= TRUNC(HOH.WORK_DATE, 'MONTH')       -- 매월1일자 부터 적용 -- 
         AND EXISTS
        ...
        BEGIN
          SELECT 0 AS REQ_COUNT 
           ...
           WHERE OH.OT_HEADER_ID                      = P_OT_HEADER_ID 
             AND OH.WORK_DATE                         < TO_DATE('2014-07-21', 'YYYY-MM-DD') -- 이후 데이터만 적용 -- 
           ;
        EXCEPTION
          WHEN OTHERS THEN
            V_REQ_COUNT := 0; 
...
--RAISE_APPLICATION_ERROR(-20001, W_CORP_ID || '/' || W_STD_DATE || ',' || W_CONNECT_PERSON_ID || '/' || V_CONNECT_PERSON_ID || '/' || W_SOB_ID || '/' || W_ORG_ID);
           ...
           V_REQUEST_DATE_FR := TRUNC(ADD_MONTHS(W_STD_DATE, -2), 'MONTH');
           V_REQUEST_DATE_TO := LAST_DAY(TRUNC(SYSDATE));
  ...
  BEGIN
    ...
    LOOP
      ...
      SELECT DISTINCT
        ...
        FROM HRD_DUTY_MANAGER DM
           , HRM_FLOOR_V      HF
       WHERE DM.DUTY_CONTROL_ID                         = HF.FLOOR_ID 
         AND DM.CORP_ID                                 = P_WORK_CORP_ID 
         AND (NVL(V_CONNECT_PERSON_ID, DM.MANAGER_ID1)  IN (DM.MANAGER_ID1, DM.MANAGER_ID2, DM.APPROVER_ID1, DM.APPROVER_ID2))
         ...
         AND DM.USABLE                                   = 'Y'  
         AND DM.START_DATE                               <= C1.WORK_DATE
         AND (DM.END_DATE                                >= C1.WORK_DATE OR DM.END_DATE IS NULL) 
         
         AND HF.ENABLED_FLAG                             = 'Y' 
         AND HF.EFFECTIVE_DATE_FR                        <= C1.WORK_DATE
         AND (HF.EFFECTIVE_DATE_TO                       >= C1.WORK_DATE OR HF.EFFECTIVE_DATE_TO IS NULL) 
                                                 ...
                                                 AND HH.CHARGE_SEQ          IN 
                                                      (SELECT MAX(S_HH.CHARGE_SEQ) AS CHARGE_SEQ
                                                         ...
                                                         WHERE S_HH.HISTORY_HEADER_ID = S_HL.HISTORY_HEADER_ID
                                                           AND S_HH.CHARGE_DATE       <= C1.WORK_DATE 
