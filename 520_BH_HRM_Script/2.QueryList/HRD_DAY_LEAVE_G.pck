    ...
    OPEN P_CURSOR FOR
      SELECT DL.PERSON_ID
        ...
        AND (T1.RETIRE_DATE IS NULL OR T1.RETIRE_DATE >= W_START_DATE)
			...
			ORDER BY DL.WORK_DATE, DL.HOLY_TYPE, DL.WORK_TYPE_ID, T1.FLOOR_ID, T1.PERSON_NUM
      ;
  END DATA_SELECT;
...
-- DAY LEAVE CLOSE PROCESS GO.
  ...
  AS
    ...
    /* -- 마감처리시 미마감 자료만 처리하므로 마감여부 체크 필요 없음. (JHS)
    BEGIN
		  SELECT NVL(SUM(DECODE(DL.CLOSED_YN, 'Y', 1, 0)), 0) AS CLOSE_COUNT
			  INTO V_CLOSE_COUNT
			  FROM HRD_DAY_LEAVE DL
      ...
      -- 중도 입/퇴사자의 경우 입퇴사일 사이만 적용.      
        AND DL.WORK_DATE              BETWEEN CASE
                                          ...
                                          AND CASE 
                                                ...
                                                ELSE W_END_DATE
                                              END
        AND T1.PERSON_ID              = NVL(W_PERSON_ID, T1.PERSON_ID)
        AND T1.CORP_ID                = W_CORP_ID
        AND T1.SOB_ID                 = W_SOB_ID
        AND T1.ORG_ID                 = W_ORG_ID
        AND T1.FLOOR_ID               = NVL(W_FLOOR_ID, T1.FLOOR_ID)
...
--				AND DL.CLOSED_YN              = DECODE(W_CLOSE_YN, 'A', DL.CLOSED_YN, NVL(W_CLOSE_YN, DL.CLOSED_YN))
  ...
  AS	  
  BEGIN
--    RAISE_APPLICATION_ERROR(-20001, W_CORP_ID || '/' || W_START_DATE || '/' || W_END_DATE || W_SOB_ID);
    OPEN P_CURSOR1 FOR
      SELECT DL.PERSON_ID
           , DL.WORK_DATE
           , HRM_COMMON_G.CODE_NAME_F('WEEK', TO_CHAR(DL.WORK_DATE, 'D'), DL.SOB_ID, DL.ORG_ID) AS WORK_WEEK
           ...
           , DL.HOLY_TYPE
           , HRM_COMMON_G.CODE_NAME_F('HOLY_TYPE', DL.HOLY_TYPE, DL.SOB_ID, DL.ORG_ID) AS HOLY_TYPE_NAME
           , TO_CHAR(DL.OPEN_TIME, 'YYYY-MM-DD HH24:MI') AS OPEN_TIME
           , TO_CHAR(DL.CLOSE_TIME, 'YYYY-MM-DD HH24:MI') AS CLOSE_TIME
           , DL.OPEN_TIME1
           , DL.CLOSE_TIME1
           , DL.NEXT_DAY_YN
           , DL.DANGJIK_YN
           ...
           , DL.WORK_CORP_ID AS CORP_ID           
           , DL.DAY_LEAVE_ID
           , T1.DEPT_NAME || ' ' || T1.NAME || ' ' || T1.PAY_GRADE_NAME AS LOGIN_PERSON_INFO
        FROM HRD_DAY_LEAVE_V1 DL
           , (-- 시점 인사내역.
              SELECT HL.PERSON_ID
                  , PM.NAME
...
/*        AND DL.HOLY_TYPE              = NVL(W_HOLY_TYPE, DL.HOLY_TYPE)
      ...
      ORDER BY T1.PERSON_NUM, DL.WORK_DATE, T1.POST_SORT_NUM
      ;
  END SELECT_FLOOR_PERSON;
