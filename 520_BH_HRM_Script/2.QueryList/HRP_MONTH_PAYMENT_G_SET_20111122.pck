...
-- 74 복지수당 적용 여부 : 근무율 70%이상, 수습3개월 이상.
  ...
  BEGIN
    ...
    LOOP
      ...
      -- 근무율 계산.
      BEGIN
        SELECT CEIL((DECODE(NVL(SX1.WORK_COUNT, 0), 0, 1, SX1.WORK_COUNT) - (NVL(MT.TOTAL_DED_DAY, 0) - NVL(MT.WEEKLY_DED_COUNT, 0))) / DECODE(NVL(SX1.WORK_COUNT, 0), 0, 1, SX1.WORK_COUNT) * 100) AS OVER_RATE
          INTO V_WORKING_RATE
          FROM HRD_MONTH_TOTAL MT
            , ( SELECT WC.PERSON_ID
                     , COUNT(WC.WORK_DATE) AS WORK_COUNT
                  FROM HRD_WORK_CALENDAR WC
                WHERE WC.WORK_DATE        BETWEEN P_START_DATE AND P_END_DATE
                  AND WC.PERSON_ID        = C1.PERSON_ID
                  AND WC.SOB_ID           = C1.SOB_ID
                  AND WC.ORG_ID           = C1.ORG_ID
                  AND WC.HOLY_TYPE        IN ('2', '3')  -- 주간/야간수 카운트.
                GROUP BY WC.PERSON_ID
               ) SX1
        WHERE MT.PERSON_ID                = SX1.PERSON_ID
          AND MT.PERSON_ID                = C1.PERSON_ID
          AND MT.DUTY_TYPE                = C_DUTY_MONTH
          AND MT.DUTY_YYYYMM              = C1.PAY_YYYYMM
          AND MT.CORP_ID                  = C1.CORP_ID
