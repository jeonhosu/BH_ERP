CREATE OR REPLACE PACKAGE HRA_YEAR_ADJUST_SET_G_2016 AS
  /*================================================================================/
       ==> 연말정산 처리(2008. 12. 25)
         0. 연말정산 처리 기초자료 생성;
...
-- 14-9. 특별공제 종합한도 초과액;
  ...
  -- 65.1 기부금 이월 처리(표준공제 선택시 기부금에 대해 이월또는 소멸 처리) 
  FUNCTION TAX_DED_DONATION_NEXT
            ( O_STATUS            OUT VARCHAR2
            , O_MESSAGE           OUT VARCHAR2
            , P_YEAR_YYYY         IN VARCHAR2
            , P_PERSON_ID         IN NUMBER
...
END HRA_YEAR_ADJUST_SET_G_2016;
/
CREATE OR REPLACE PACKAGE BODY HRA_YEAR_ADJUST_SET_G_2016 AS
  /*================================================================================/
...
----- <주현> ------------------------------------------------------------------------------
      ...
      BEGIN
        SELECT SUM(CASE  -- 소득세.
            ...
            , SUM(CASE  -- 국민.
                   WHEN HD.DEDUCTION_CODE IN('D03') THEN MD.DEDUCTION_AMOUNT
                   ELSE 0
            ...
            , SUM(CASE  -- 건강+장기요양.
                   WHEN HD.DEDUCTION_CODE IN('D05', 'D06', 'D07', 'D08') THEN MD.DEDUCTION_AMOUNT
                   ELSE 0
...
-------------------------------------------------------------------------------  
      ...
      IF NVL(C1.STD_TAX_DED_FIX_FLAG, 'N') = 'Y' THEN
      -- 2014 추가 - 특별세액공제 고정 : 'Y' 이면 해당 금액 0 적용 --
        V_TD_DONAT_DED_AMT := TAX_DED_DONATION_NEXT
                             ( O_STATUS      => O_STATUS
                             , O_MESSAGE     => O_MESSAGE
                             , P_YEAR_YYYY   => P_YEAR_YYYY
                             , P_PERSON_ID   => C1.PERSON_ID
...
-----> 근로소득 공제   UPDATE
  ...
  BEGIN
    ...
    FOR C1 IN ( SELECT HIT.YEAR_YYYY
                    ...
                    , NVL(HIT.POLI_DONA_SEPARATE_AMT, 0) AS POLI_DONA_SEPARATE_AMT
                    , NVL(HIT.LEGAL_DONA_SEPARATE_AMT, 0) AS LEGAL_DONA_SEPARATE_AMT 
                    , NVL(HIT.COM_STOCK_DONA_SEPARATE_AMT, 0) AS COM_STOCK_DONA_SEPARATE_AMT
                    , NVL(HIT.DESIGN_DONA_SEPARATE_AMT, 0) AS DESIGN_DONA_SEPARATE_AMT  
 
                    -- 2015년도 이전.
                    , NVL(T1.POLI_DONA_SEPARATE_AMT, 0) AS P_POLI_DONA_SEPARATE_AMT
                    , NVL(T1.POLI_DONA_RATE2, 0) AS P_POLI_DONA_RATE2 
                    , NVL(T1.POLI_DONA_RATE3, 0) AS P_POLI_DONA_RATE3 
                    , NVL(T1.LEGAL_DONA_SEPARATE_AMT, 0) AS P_LEGAL_DONA_SEPARATE_AMT 
                    , NVL(T1.LEGAL_DONA_RATE1, 0) AS P_LEGAL_DONA_RATE1 
                    , NVL(T1.LEGAL_DONA_RATE2, 0) AS P_LEGAL_DONA_RATE2 
                    , NVL(T1.COM_STOCK_DONA_SEPARATE_AMT, 0) AS P_COM_STOCK_DONA_SEPARATE_AMT
                    , NVL(T1.COM_STOCK_DONA_LMT_RATE1, 0) AS P_COM_STOCK_DONA_LMT_RATE1 
                    , NVL(T1.COM_STOCK_DONA_LMT_RATE2, 0) AS P_COM_STOCK_DONA_LMT_RATE2 
                    , NVL(T1.DESIGN_DONA_SEPARATE_AMT, 0) AS P_DESIGN_DONA_SEPARATE_AMT            
                    , NVL(T1.DESIGN_DONA_RATE1, 0) AS P_DESIGN_DONA_RATE1 
                    , NVL(T1.DESIGN_DONA_RATE2, 0) AS P_DESIGN_DONA_RATE2                      
                  FROM HRA_INCOME_TAX_STANDARD HIT 
                     , ( SELECT P_YEAR_YYYY AS YEAR_YYYY 
                              , S_HIT.SOB_ID
                              , NVL(S_HIT.POLI_DONA_SEPARATE_AMT, 0) AS POLI_DONA_SEPARATE_AMT
                              , NVL(S_HIT.POLI_DONA_RATE2, 0) AS POLI_DONA_RATE2 
                              , NVL(S_HIT.POLI_DONA_RATE3, 0) AS POLI_DONA_RATE3 
                              , NVL(S_HIT.LEGAL_DONA_SEPARATE_AMT, 0) AS LEGAL_DONA_SEPARATE_AMT 
                              , NVL(S_HIT.LEGAL_DONA_RATE1, 0) AS LEGAL_DONA_RATE1 
                              , NVL(S_HIT.LEGAL_DONA_RATE2, 0) AS LEGAL_DONA_RATE2 
                              , NVL(S_HIT.COM_STOCK_DONA_SEPARATE_AMT, 0) AS COM_STOCK_DONA_SEPARATE_AMT
                              , NVL(S_HIT.COM_STOCK_DONA_LMT_RATE1, 0) AS COM_STOCK_DONA_LMT_RATE1 
                              , NVL(S_HIT.COM_STOCK_DONA_LMT_RATE2, 0) AS COM_STOCK_DONA_LMT_RATE2 
                              , NVL(S_HIT.DESIGN_DONA_SEPARATE_AMT, 0) AS DESIGN_DONA_SEPARATE_AMT            
                              , NVL(S_HIT.DESIGN_DONA_RATE1, 0) AS DESIGN_DONA_RATE1 
                              , NVL(S_HIT.DESIGN_DONA_RATE2, 0) AS DESIGN_DONA_RATE2 
                           FROM HRA_INCOME_TAX_STANDARD S_HIT
                          WHERE S_HIT.YEAR_YYYY     = '2015'
                            AND S_HIT.SOB_ID        = P_SOB_ID
                            AND P_YEAR_YYYY         = '2016'
                        ) T1 
                WHERE HIT.YEAR_YYYY     = T1.YEAR_YYYY(+) 
                  AND HIT.SOB_ID        = T1.SOB_ID(+) 
                  AND HIT.YEAR_YYYY     = P_YEAR_YYYY
                  AND HIT.SOB_ID        = P_SOB_ID 
                  AND ROWNUM            <= 1
               )
    LOOP
      ...
      LOOP
        ...
        ELSE
          ...
          -- 3천만원 초과 = 25/100 
          IF R1.DONA_YYYY = P_YEAR_YYYY THEN 
            IF C1.POLI_DONA_SEPARATE_AMT < V_TD_DONAT_AMT1 THEN
            ...
            END IF;
          ELSE 
            IF C1.P_POLI_DONA_SEPARATE_AMT < V_TD_DONAT_AMT1 THEN
              V_TD_DONAT_DED_AMT1 := TRUNC((NVL(V_TD_DONAT_AMT1, 0))* (C1.P_POLI_DONA_RATE3 / 100));
            ELSE
              V_TD_DONAT_DED_AMT1 := TRUNC((NVL(V_TD_DONAT_AMT1, 0))* (C1.P_POLI_DONA_RATE2 / 100));
            END IF;
          END IF;
      ...
      IF NVL(V_TAX_DED_RELIGION, 0) > 0 THEN
        ...
        IF ROUND(NVL(V_DONAT_LMT, 0) * (C1.DESIGN_DONA_LMT_RATE2/100)) < NVL(V_TAX_DED_DESIGN, 0) THEN
          V_DONAT_LMT := TRUNC((NVL(V_DONAT_LMT, 0) * (C1.DESIGN_DONA_LMT_RATE1/100)) + (NVL(V_DONAT_LMT, 0) * (C1.DESIGN_DONA_LMT_RATE2/100)));
        ELSE
          V_DONAT_LMT := TRUNC((NVL(V_DONAT_LMT, 0) * (C1.DESIGN_DONA_LMT_RATE1/100)) + (NVL(V_TAX_DED_DESIGN, 0)));
        END IF;
      ...
      -- 5.2 종교단체기부금이 없는 경우;
        -- 소득금액 * 30% -- 
        V_DONAT_LMT := TRUNC(NVL(V_DONAT_LMT, 0) * (C1.DESIGN_DONA_LMT_RATE/100)); 
        FOR R1 IN ( SELECT DA.DONATION_ADJUSTMENT_ID
                        , DA.YEAR_YYYY
                        , DA.PERSON_ID
                        , DA.SOB_ID
      ...
      IF R1.DONA_YYYY = '2016' THEN 
        IF C1.LEGAL_DONA_SEPARATE_AMT < NVL(V_TD_LEGAL_DONAT_AMT, 0)+ NVL(V_TD_COM_STOCK_DONAT_AMT, 0) + NVL(V_TD_DESIGN_DONAT_AMT, 0) + NVL(V_TD_DESIGN_DONAT_R_AMT, 0) THEN
          V_TD_DONAT_OVER_AMT := (NVL(V_TD_LEGAL_DONAT_AMT, 0)+ NVL(V_TD_COM_STOCK_DONAT_AMT, 0) + 
                                  NVL(V_TD_DESIGN_DONAT_AMT, 0) + NVL(V_TD_DESIGN_DONAT_R_AMT, 0)) - C1.LEGAL_DONA_SEPARATE_AMT;  -- 30000000 
          -- 1.1 법정기부금 안분액-- 
        ...
        END IF;
      ELSE
        IF C1.P_LEGAL_DONA_SEPARATE_AMT < NVL(V_TD_LEGAL_DONAT_AMT, 0)+ NVL(V_TD_COM_STOCK_DONAT_AMT, 0) + NVL(V_TD_DESIGN_DONAT_AMT, 0) + NVL(V_TD_DESIGN_DONAT_R_AMT, 0) THEN
          V_TD_DONAT_OVER_AMT := (NVL(V_TD_LEGAL_DONAT_AMT, 0)+ NVL(V_TD_COM_STOCK_DONAT_AMT, 0) + 
                                  NVL(V_TD_DESIGN_DONAT_AMT, 0) + NVL(V_TD_DESIGN_DONAT_R_AMT, 0)) - NVL(C1.P_LEGAL_DONA_SEPARATE_AMT,0);  -- 30000000   
          -- 1.1 법정기부금 안분액-- 
          V_TD_LEGAL_OVER_AMT := TRUNC(NVL(V_TD_DONAT_OVER_AMT, 0) * (NVL(V_TD_LEGAL_DONAT_AMT, 0) / NVL(V_TD_DONAT_OVER_AMT, 0)));
  ...
  BEGIN
    ...
    LOOP
      ...
      LOOP
        -- 기부금 건당 세액공제 금액.
        V_DONAT_AMT := NVL(R1.TOTAL_DONA_AMT, 0); 
        
        V_DONAT_NEXT_AMT := 0;   -- 정치자금 기부금 이월 될 금액 (10만원 이하)
        V_DONAT_LAPSE_AMT := 0;  -- 정치자금 기부금 소멸 될 금액(10만원 이하)     
        
        -- 기부금액소멸.(가능한년도가 아니라면 소멸처리한다)
        IF (TO_NUMBER(R1.DONA_YYYY) + NVL(R1.CARRIED_OVER_YEAR, 0)) > TO_NUMBER(R1.YEAR_YYYY) THEN
          V_DONAT_NEXT_AMT := V_DONAT_AMT;
        ELSE
          V_DONAT_LAPSE_AMT := V_DONAT_AMT;
        END IF;  

        -- 기부금조정명세서 UPDATE.        
        UPDATE HRA_DONATION_ADJUSTMENT DA
          SET DA.LAPSE_DONA_AMT   = NVL(DA.LAPSE_DONA_AMT, 0) + NVL(V_DONAT_LAPSE_AMT, 0)
            , DA.NEXT_DONA_AMT    = NVL(DA.NEXT_DONA_AMT, 0) + NVL(V_DONAT_NEXT_AMT, 0)
        WHERE DA.DONATION_ADJUSTMENT_ID = R1.DONATION_ADJUSTMENT_ID
        ;
      END LOOP R1;      
