...
-- 년월 조회 LOOK UP : 해당월의 1일, 말일 표시.
  PROCEDURE LU_CALENDAR_YYYYMM_1
...
-- 년월 조회 LOOK UP : 해당월의 1일, 말일 표시.
  ...
  BEGIN
    V_END_YYYYMM := TO_CHAR(ADD_MONTHS(SYSDATE, 1), 'YYYY-MM');
		...
		OPEN P_CURSOR FOR
			SELECT CY.YYYYMM
           , TRUNC(TO_DATE(CY.YYYYMM, 'YYYY-MM'), 'MONTH') AS DATE_FR
           , LAST_DAY(TO_DATE(CY.YYYYMM, 'YYYY-MM')) AS DATE_TO
			FROM EAPP_CALENDAR_YYYYMM_V CY
			WHERE CY.YYYYMM                BETWEEN NVL(W_START_YYYYMM, CY.YYYYMM) AND NVL(W_END_YYYYMM, V_END_YYYYMM)
			ORDER BY CY.YYYYMM DESC
			;
    
  END LU_CALENDAR_YYYYMM_1;
...
-- 해당일자에 대해 년 주차 조회.
  ...
  BEGIN
    BEGIN
      SELECT TO_CHAR(W_DATE, 'YYYY') || TO_CHAR(W_DATE - 1, 'WW')
        INTO O_WEEKLY
        FROM DUAL;
