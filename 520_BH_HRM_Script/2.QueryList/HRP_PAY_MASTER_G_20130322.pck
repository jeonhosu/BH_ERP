  ...
  BEGIN
    OPEN P_CURSOR2 FOR
      SELECT CASE
           ...
           , PMH.END_YYYYMM
           , T1.DEPT_NAME
           , NVL(T1.JOIN_DATE, NULL) AS JOIN_DATE
        ...
        FROM HRP_PAY_MASTER_HEADER  PMH
          ...
          , (-- 시점 인사내역.
            ...
            WHERE HL.PERSON_ID        = PM.PERSON_ID
              ...
              AND HL.HISTORY_LINE_ID  IN ( SELECT MAX(S_HL.HISTORY_LINE_ID) AS HISTORY_LINE_ID
                                            FROM HRM_HISTORY_LINE S_HL
                                           WHERE S_HL.CHARGE_DATE            <= LAST_DAY(TO_DATE(W_PAY_YYYYMM, 'YYYY-MM'))
                                             AND S_HL.PERSON_ID              = HL.PERSON_ID
                                           GROUP BY S_HL.PERSON_ID
                                         )
          ) T1 
      WHERE PMH.PAY_HEADER_ID           = PML.PAY_HEADER_ID
        ...
        AND PMH.PERSON_ID               = T1.PERSON_ID
        AND PMH.CORP_ID                 = W_CORP_ID
        AND PMH.PERSON_ID               = NVL(W_PERSON_ID, PMH.PERSON_ID)
      ...
      GROUP BY ROLLUP((PMH.START_YYYYMM
           ...
           , T1.JOIN_DATE
           , T1.RETIRE_DATE))
       ORDER BY T1.POST_SORT_NUM, T1.POST_CODE, T1.FLOOR_SORT_NUM, T1.FLOOR_CODE, T1.PERSON_NUM
      ;
