...
-- 일근태 마감 자료 집계 조회.
  PROCEDURE SELECT_DAY_LEAVE_TOTAL
            ( P_CURSOR1                           OUT TYPES.TCURSOR1
            , W_CORP_ID                           IN HRD_DAY_LEAVE.CORP_ID%TYPE
            , W_START_DATE                        IN HRD_DAY_LEAVE.WORK_DATE%TYPE
						, W_END_DATE                          IN HRD_DAY_LEAVE.WORK_DATE%TYPE
    ...
    OPEN P_CURSOR FOR
      ...
      WHERE DL.PERSON_ID              = T1.PERSON_ID
        ...
        AND DL.WORK_DATE              BETWEEN CASE 
                                                WHEN W_START_DATE < T1.JOIN_DATE THEN T1.JOIN_DATE
                                                ELSE W_START_DATE
                                              END
                                      AND CASE 
...
-- 근태 오류자 조회.
  ...
  BEGIN
    ...
    OPEN P_CURSOR1 FOR
      ...
      WHERE DL.PERSON_ID              = PM.PERSON_ID
        AND DL.DUTY_ID                = DC.DUTY_ID 
        AND PM.PERSON_ID              = T1.PERSON_ID
        AND DL.WORK_DATE              BETWEEN CASE 
                                                WHEN W_START_DATE < PM.JOIN_DATE THEN PM.JOIN_DATE
                                                ELSE W_START_DATE
                                              END
                                      AND CASE 
        ...
        OR   (W_PERSON_ID             IS NOT NULL AND DL.PERSON_ID = W_PERSON_ID))
        AND DL.WORK_CORP_ID           = W_CORP_ID
        AND DL.SOB_ID                 = W_SOB_ID
        AND DL.ORG_ID                 = W_ORG_ID
        AND ((W_CLOSE_YN              IS NULL AND 1 = 1)
        OR   (W_CLOSE_YN              = 'A' AND 1 = 1)        
        OR   (W_CLOSE_YN              != 'A' AND DL.CLOSED_YN = W_CLOSE_YN))
        AND ((W_HOLY_TYPE             IS NULL AND 1 = 1)
        OR   (W_HOLY_TYPE             IS NOT NULL AND DL.HOLY_TYPE = W_HOLY_TYPE))
        AND ((W_DUTY_ID               IS NULL AND 1 = 1)
        OR   (W_DUTY_ID               IS NOT NULL AND DL.DUTY_ID = W_DUTY_ID))
        ...
        AND ((EXISTS ( SELECT 'X'
          ...
          OR (EXISTS ( SELECT 'X'
                         FROM HRM_DUTY_CODE_V DC
                       WHERE DC.DUTY_ID       = DL.DUTY_ID
                         AND DC.DUTY_CODE     IN ('11')  -- 결근.
                      ))
          OR ((DL.HOLY_TYPE             IN ('2', '3')  -- 주/야 => 계획근무시간대비 2시간 이전 대상 -- 
            AND NVL(DL.OPEN_TIME, DL.PL_OPEN_TIME) <= (DL.PL_OPEN_TIME - (1/24*2)))
           OR (DL.HOLY_TYPE             IN ('0', '1')  -- 무/유휴 => 철야또는 당직에 따라 21시 OR 08시 대비 2시간 이전 대상 -- 
            AND (((DL.ALL_NIGHT_YN      = 'Y' OR DL.DANGJIK_YN = 'Y')
            AND    NVL(DL.OPEN_TIME, (DL.WORK_DATE+ (1/24*21))) <= (DL.WORK_DATE+ (1/24*19))) 
            OR  (((DL.ALL_NIGHT_YN      != 'Y' OR DL.DANGJIK_YN != 'Y')
            AND    NVL(DL.OPEN_TIME, (DL.WORK_DATE+ (1/24*8.5))) <= (DL.WORK_DATE+ (1/24*6.5))))))))
          AND EXISTS 
                ( SELECT 'X'
                    FROM HRD_DUTY_MANAGER DM
                  WHERE DM.CORP_ID                                = DL.WORK_CORP_ID
  ...
  ORDER BY DL.WORK_DATE, DL.HOLY_TYPE, DL.WORK_TYPE_GROUP, T1.FLOOR_SORT_NUM, T1.DEPT_SORT_NUM, PM.PERSON_NUM
  ;
  ...
  BEGIN
    OPEN P_CURSOR1 FOR
      SELECT T1.DEPT_CODE
           ...
           , PM.NAME
           , SUM(CASE 
                   WHEN DL.HOLY_TYPE IN ('0', '1') THEN 0
           ...
           , SUM(CASE 
                   ...
                   ELSE 0
                 END) AS EARLY_COUNT
           , SUM(CASE WHEN DC.DUTY_CODE IN('100') AND DL.HOLY_TYPE IN ('2', '3') THEN 1 ELSE 0 END) AS LEAVE_COUNT1  -- 공적외근.
           , SUM(CASE WHEN DC.DUTY_CODE IN('101') AND DL.HOLY_TYPE IN ('2', '3') THEN 1 ELSE 0 END) AS LEAVE_COUNT2  -- 사적외출.
           ...
           , SUM(CASE WHEN DC.DUTY_CODE IN('30', '98') AND DL.HOLY_TYPE IN ('2', '3') THEN 1 ELSE 0 END) AS DUTY_DAY_30   -- 공상/공가.
           , SUM(CASE WHEN DC.DUTY_CODE IN('77', '78', '95', '96', '97', '105') AND DL.HOLY_TYPE IN ('2', '3') THEN 1 ELSE 0 END) AS DUTY_DAY_95  -- 휴직/산휴/육아휴직.
           , SUM(CASE WHEN DC.DUTY_CODE IN('11') AND DL.HOLY_TYPE IN ('2', '3') THEN 1 ELSE 0 END) AS DUTY_DAY_11        -- 결근.
           , SUM(CASE WHEN DC.DUTY_CODE IN('54') AND DL.HOLY_TYPE IN ('2', '3') THEN 1 ELSE 0 END) AS DUTY_DAY_54        -- 유계결근.
           , SUM(CASE WHEN DC.DUTY_CODE IN('31') AND DL.HOLY_TYPE IN ('2', '3') THEN 1 ELSE 0 END) AS DUTY_DAY_31        -- 병가.
      ...
      -- 중도 입/퇴사자의 경우 입퇴사일 사이만 적용.      
        AND DL.WORK_DATE              BETWEEN CASE
                                                WHEN PM.JOIN_DATE > W_START_DATE THEN PM.JOIN_DATE
                                                ELSE W_START_DATE 
                                              END
                                          AND CASE 
                                                WHEN PM.RETIRE_DATE IS NOT NULL AND PM.RETIRE_DATE < W_END_DATE THEN PM.RETIRE_DATE
                                                ELSE W_END_DATE
                                              END
        AND ((W_PERSON_ID             IS NULL AND 1 = 1)
      ...
      GROUP BY T1.DEPT_CODE
             ...
             , T1.FLOOR_SORT_NUM
             , T1.POST_SORT_NUM
      ORDER BY T1.DEPT_SORT_NUM, T1.FLOOR_SORT_NUM, T1.POST_SORT_NUM, PM.PERSON_NUM
      ;
  END SELECT_DAY_LEAVE_TOTAL;
