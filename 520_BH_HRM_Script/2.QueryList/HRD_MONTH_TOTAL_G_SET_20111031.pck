...
-- 월근태 집계 MAIN.
  ...
  BEGIN
    ...
    END IF;
      ...
      WHERE WD.WORK_DATE           BETWEEN V_MONTH_START_DATE AND V_MONTH_END_DATE
        ...
        AND EXISTS(SELECT 'X'
                   ...
                   WHERE MT.PERSON_ID           = PM.PERSON_ID
                     ...
                     AND MT.WORK_CORP_ID        = WD.WORK_CORP_ID
                     AND MT.SOB_ID              = WD.SOB_ID
                     AND MT.ORG_ID              = WD.ORG_ID
                     AND MT.PERSON_ID           = NVL(W_PERSON_ID, MT.PERSON_ID)
                     AND MT.DUTY_TYPE           = W_DUTY_TYPE
                     AND MT.DUTY_YYYYMM         = W_DUTY_YYYYMM
                     AND MT.WORK_CORP_ID        = W_CORP_ID
...
---------------------------------------------------------------------------------------------------
        ...
        , V_TOTAL_DED_DAY
        , CASE 
            WHEN NVL(V_TOTAL_DAY, 0) - NVL(V_TOTAL_DED_DAY, 0) < 0 THEN 0
            ELSE NVL(V_TOTAL_DAY, 0) - NVL(V_TOTAL_DED_DAY, 0)
          END
        , V_HOLY_0_DED_FLAG                            ---
        , V_STD_HOLY_0_COUNT
        , W_SOB_ID, W_ORG_ID
  ...
  BEGIN
    ...
    , LAST_UPDATE_DATE, LAST_UPDATED_BY
		...
		SELECT P_MONTH_TOTAL_ID
         , CASE 
             WHEN DL.HOLY_TYPE IN('0', '1') AND DLO.OT_TYPE = '14' THEN '18'     -- 휴일(특근) : 연장.
             ELSE DLO.OT_TYPE
           END OT_TYPE
...
-- 월근태 주휴공제 계산 : 입사일자에 대한 주휴공제 계산.
  ...
  BEGIN
    ...
    LOOP
      ...
      BEGIN
        SELECT CASE
                 WHEN WC.ATTRIBUTE5 IN ('32') THEN 4  -- 유휴, 무휴 순.
                 ELSE 6
      ...
      SELECT SX1.WORK_DATE  -- 주휴공제일자.
          , C1.WORK_DATE AS SOURCE_DATE  -- 발생일자.
          , SX1.PERSON_ID
          , SX1.WORK_CORP_ID
          , SX1.CORP_ID
          , SX1.SOB_ID
    ...
    -- 주휴공제 테이블 반영.
    FOR C1 IN ( SELECT WORK_DATE, OPEN_TIME AS SOURCE_DATE
                  FROM HRD_WORK_DATE_GT WD
    ...
    LOOP
      UPDATE HRD_WEEKLY_DED WD
        SET WD.SOURCE_DATE        = C1.SOURCE_DATE
...
-- 지각/조퇴 4H 이상자에 대한 주휴공제 계산.
  ...
  BEGIN
    ...
    -- 주휴공제 테이블 반영.
    FOR C1 IN ( SELECT DISTINCT WD.WORK_DATE, WD.OPEN_TIME AS SOURCE_DATE
                  FROM HRD_WORK_DATE_GT WD
                WHERE WD.WORK_DATE            BETWEEN W_START_DATE AND W_END_DATE
                  AND WD.PERSON_ID            = W_PERSON_ID
                  AND WD.WORK_CORP_ID         = W_CORP_ID
...
--   지각/조퇴 4H이상에 대한 주휴공제일수
    ...
    LOOP
      ...
      SELECT SX1.WORK_DATE
          , C1.WORK_DATE AS SOURCE_DATE
          , SX1.PERSON_ID
          , SX1.WORK_CORP_ID
          , SX1.CORP_ID
          , SX1.SOB_ID
