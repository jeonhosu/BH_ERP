  ...
  BEGIN
    ...
    BEGIN
      V_CAP_C := HRM_MANAGER_G.USER_CAP_F
                   ( W_CORP_ID
                   , TRUNC(TO_DATE(W_STD_YYYYMM, 'YYYY-MM'))
...
-- 급여마스터 헤더 저장(INSERT/UPDATE) 제어.
  ...
  BEGIN
    ...
    END;
    IF V_LAST_YN = 'N' THEN
      RAISE_APPLICATION_ERROR(-20001, EAPP_MESSAGE_G.RETURN_TEXT_F(USERENV_G.GET_TERRITORY_S_F, 'FCM_10326', NULL));
      RETURN;
    END IF;
...
-- PAY_HEADER_ID_CHECK_F RETURN.
  ...
  BEGIN
    BEGIN
      SELECT MAX(PMH.PAY_HEADER_ID) AS PAY_HEADER_ID
      ...
      ;
--DBMS_OUTPUT.PUT_LINE('V_PAY_HEADER_ID : ' || V_PAY_HEADER_ID);
    EXCEPTION WHEN OTHERS THEN
      V_PAY_HEADER_ID := 0;
    END;
