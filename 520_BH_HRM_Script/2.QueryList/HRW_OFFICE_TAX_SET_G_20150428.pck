...
-- 과세 제외급여 계산.
  ...
  BEGIN
    ...
    BEGIN
      SELECT SUM(/* -- 조선미J 요청 : 연장수당 제외 (전호수 수정-2014-08-22) -- 
                 CASE
                ...
                END + 
                CASE
                  WHEN NVL(SX1.TAX_FREE_FOOD, 0) > V_STD_FOOD THEN V_STD_FOOD
                  ELSE NVL(SX1.TAX_FREE_FOOD, 0)
                END) AS TAX_FREE_AMT  --TAX_FREE_BABY
        INTO V_TAX_FREE_AMT
        FROM (SELECT MP.CORP_ID 
                  , MP.PERSON_ID
                  ...
                  , SUM(DECODE(HA.TAX_FREE, 'FOOD', MA.ALLOWANCE_AMOUNT)) AS TAX_FREE_FOOD
      ...
      SELECT COUNT(DISTINCT RA.PERSON_ID) AS PERSON_COUNT
           ...
           , SUM(CASE
					         WHEN NVL((SELECT 'Y'
								   ...
								   ELSE TRUNC(RA.INCOME_TAX_AMOUNT, -1)
