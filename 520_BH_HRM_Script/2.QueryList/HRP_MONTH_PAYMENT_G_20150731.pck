      ...
      SELECT  CASE
          ...
          , CASE
           ...
           , SUM(NVL(MD.D14, 0) + NVL(MD.D10, 0) + NVL(MD.D12, 0) + NVL(MD.D19, 0) + 
                 NVL(MD.D20, 0) + NVL(MD.D22, 0) + NVL(MD.D23, 0) + NVL(MD.D24, 0) + 
                 NVL(MD.D18, 0) + NVL(MD.D28, 0)) AS D14  -- 기타공제(기타공제 + 전월정산액 + 경조사비 + 교육훈련비 + 식대공제 + 학자금 대출 + 교육훈련 교재비 ).
           , SUM(MD.D15) AS D15                -- 정산소득세.
           , SUM(MD.D16) AS D16                -- 정산주민세.
           , SUM(MD.D17) AS D17                -- 정산농특세.
           , 0 /*SUM(MD.D18)*/ AS D18                -- 학자금대출 
