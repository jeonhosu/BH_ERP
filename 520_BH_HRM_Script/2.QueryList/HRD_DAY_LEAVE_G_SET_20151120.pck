...
-----------------------------------------------------------------------------------------
        ...
        -- 잔업 사항 저장. --
        -- LEAVE_TIME.
        IF 8 < (NVL(V_LEAVE_TIME, 0) + NVL(V_LATE_TIME, 0)) THEN
          V_LEAVE_TIME := NVL(V_LEAVE_TIME, 0) - (NVL(V_LEAVE_TIME, 0) + NVL(V_LATE_TIME, 0) - 8);
        END IF;
  ...
  AS
    ...
    IS
      SELECT DI.PERSON_ID
           ...
           , NULL AS PL_BEFORE_OT_END
           , CASE
               WHEN TO_CHAR(DI.OPEN_TIME, 'MI') BETWEEN '01' AND '29' THEN TRUNC(DI.OPEN_TIME, 'HH24') + (30/24/60)
               WHEN TO_CHAR(DI.OPEN_TIME, 'MI') BETWEEN '31' AND '59' THEN TRUNC(DI.OPEN_TIME, 'HH24') + (60/24/60)
               ELSE DI.OPEN_TIME
             END AS PL_AFTER_OT_START
           , CASE
               WHEN TO_CHAR(CASE
                              WHEN DI.CLOSE_TIME1 IS NULL THEN DI.CLOSE_TIME
                              ELSE DI.CLOSE_TIME1
                            END, 'MI') BETWEEN '01' AND '29' THEN TRUNC(CASE
                                                                          WHEN DI.CLOSE_TIME1 IS NULL THEN DI.CLOSE_TIME
                                                                          ELSE DI.CLOSE_TIME1
