...
-- 작업장 및 교대유형 저장.
  ...
  AS
    ...
    V_DEPT_ID               NUMBER;
    V_JOB_CLASS_ID          NUMBER;
    V_JOB_ID                NUMBER;
    V_POST_ID               NUMBER;
    V_OCPT_ID               NUMBER;
    V_ABIL_ID               NUMBER;
    V_PAY_GRADE_ID          NUMBER;
    V_JOB_CATEGORY_ID       NUMBER;
    V_FLOOR_ID              NUMBER;      
  BEGIN
    BEGIN
      O_MODIFY_SUCCESS := 'N';
    ...
    IF NVL(P_INTERFACE_FLAG, 'N') != 'Y' AND (P_FLOOR_ID != V_PRE_FLOOR_ID) OR (P_DEPT_ID != V_PRE_DEPT_ID) THEN
      -- 변경전 데이터 조회 --
      BEGIN
        SELECT PM.CORP_ID
      ...
      IF V_SAVE_MODE IN('NEW', 'UPDATE/INSERT') THEN
        HRM_HISTORY_LINE_G.INTERFACE_INSERT_HISTORY
         (  P_HISTORY_LINE_ID                     => V_HISTORY_LINE_ID
          ...
          , P_RETIRE_ID                           => NULL
          , P_OPERATING_UNIT_ID                   => V_OPERATING_UNIT_ID
          , P_DEPT_ID                             => V_DEPT_ID
          , P_JOB_CLASS_ID                        => V_JOB_CLASS_ID
          , P_JOB_ID                              => V_JOB_ID
          , P_POST_ID                             => V_POST_ID
          , P_OCPT_ID                             => V_OCPT_ID
          , P_ABIL_ID                             => V_ABIL_ID
          , P_PAY_GRADE_ID                        => V_PAY_GRADE_ID
          , P_JOB_CATEGORY_ID                     => V_JOB_CATEGORY_ID
...
-- [2013-07-29 전호수 추가] 인사발령 삭제 -> 근태작업장 변경 삭제.
  ...
  BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
      O_STATUS := 'F';  
      RAISE_APPLICATION_ERROR(-20001, 'Not found Data. Person History Log not exists');
      RETURN;
    END;
  ...
  BEGIN
            BEGIN
                 SELECT PH.DEPT_ID
                   INTO V_DEPT_ID
                   FROM HRD_PERSON_HISTORY       PH
                  WHERE PH.CORP_ID             = P_WORK_CORP_ID
                    AND PH.PERSON_ID           = P_PERSON_ID
                    AND PH.EFFECTIVE_DATE_FR  <= P_STD_DATE
                    AND PH.EFFECTIVE_DATE_TO  >= P_STD_DATE
                    AND PH.SOB_ID              = P_SOB_ID
            ...
            SAVE_PERSON_HISTORY
            ( O_MODIFY_SUCCESS    => O_SUCCESS_FLAG
                   ...
                   SELECT TO_CHAR(PH.EFFECTIVE_DATE_FR, 'YYYY-MM')
                     ...
                     FROM (SELECT ROWNUM AS ROWNUMBER
                            ...
                            WHERE S_PH.SOB_ID    = W_SOB_ID
                              ...
                              AND S_PH.CORP_ID   = W_WORK_CORP_ID
                              AND S_PH.PERSON_ID = W_PERSON_ID
                         ORDER BY S_PH.EFFECTIVE_DATE_FR DESC
                          ) PH
                    WHERE PH.ROWNUMBER = 2
                        ;
...
-- Table을 이용한 Work Calendar Create.
  ...
  AS
    ...
    CURSOR WORK_CALENDAR_ROW IS
      SELECT PM.PERSON_ID
       ...
       WHERE PM.WORK_TYPE_ID                            = HC.COMMON_ID
         ...
         AND PM.ORG_ID                                  = P_ORG_ID
         AND PM.PERSON_ID                               = P_PERSON_ID
         AND PM.WORK_CORP_ID                            = P_WORK_CORP_ID
    ...
    V_CREATED_METHOD     HRD_WORK_CALENDAR_SET.CREATED_METHOD%TYPE := 'A';
    ...
    V_SEARCH_START_DATE  DATE := TO_DATE(P_WORK_PERIOD || '-01', 'YYYY-MM-DD');
