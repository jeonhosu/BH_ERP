...
-- 출근 시간 정리.
	FUNCTION OPEN_TIME_F
						...
						, P_DANGJIK_YN                        IN VARCHAR2 DEFAULT 'N' 
            , P_ALL_NIGHT_YN                      IN VARCHAR2 DEFAULT 'N' 
					  ) RETURN DATE;
...
-- DAY_INTERFACE_OT SAVE(INSERT/UPDATE).
  PROCEDURE DAY_INTERFACE_OT_SAVE
            ( P_PERSON_ID                         IN HRD_DAY_INTERFACE_OT.PERSON_ID%TYPE
            , P_WORK_DATE                         IN HRD_DAY_INTERFACE_OT.WORK_DATE%TYPE
            , P_CORP_ID                           IN HRD_DAY_INTERFACE_OT.CORP_ID%TYPE
            , P_OT_TYPE                           IN HRD_DAY_INTERFACE_OT.OT_TYPE%TYPE
            , P_OT_TIME                           IN HRD_DAY_INTERFACE_OT.OT_TIME%TYPE                        
            , P_SOB_ID                            IN HRD_DAY_INTERFACE_OT.SOB_ID%TYPE
            , P_ORG_ID                            IN HRD_DAY_INTERFACE_OT.ORG_ID%TYPE
            , P_USER_ID                           IN HRD_DAY_INTERFACE_OT.CREATED_BY%TYPE
            );       
...
-- 일근태 계산 MAIN.
  ...
  BEGIN
    ...
    V_RECORD_COUNT := 0;
        ...
        OR   (P_PERSON_ID                  IS NOT NULL AND DL.PERSON_ID = P_PERSON_ID)) 
				AND DL.WORK_DATE                   BETWEEN  CASE 
                                                      WHEN P_START_DATE < PM.JOIN_DATE THEN PM.JOIN_DATE
                                                      ELSE P_START_DATE
                                                    END
                                            AND CASE 
                                                  WHEN PM.RETIRE_DATE IS NULL THEN P_END_DATE
                                                  WHEN PM.RETIRE_DATE < P_END_DATE THEN PM.RETIRE_DATE
                                                  ELSE P_END_DATE
                                                END
        ...
        OR   (P_FLOOR_ID                   IS NOT NULL AND T1.FLOOR_ID              = P_FLOOR_ID)) 
				AND ((P_WORK_TYPE_ID               IS NULL AND 1                            = 1) 
        OR   (P_WORK_TYPE_ID               IS NOT NULL AND WC.WORK_TYPE_ID          = P_WORK_TYPE_ID)) 
				AND ((P_HOLY_TYPE                  IS NULL AND 1                            = 1)
        OR   (P_HOLY_TYPE                  IS NOT NULL AND WC.HOLY_TYPE             = P_HOLY_TYPE)) 
    ...
    O_STATUS := 'F';
	  V_DAY_COUNT := (P_END_DATE - P_START_DATE) + 1;
        ...
        BEGIN
          ...
          ;
        EXCEPTION WHEN OTHERS THEN
          O_MESSAGE := 'Delete Error - Person ID:' || C1.PERSON_ID || ', Date:' || TO_CHAR(C1.WORK_DATE, 'YYYY-MM-DD')
        ...
        V_HOLIDAY_OT_TIME           := 0;
				...
				-- 계산 방법 정의 끝 --
				IF NVL(C1.DUTY_CONTROL_YN, 'N') = 'N' THEN
				-- 일근태 계산 제외 --.
				  NULL;
        ELSIF NVL(C1.DAY_LEAVE_CONTROL_YN, 'N') = 'N' THEN
        -- 일근태 계산 제외 근태계 --.
				  NULL;
				ELSE
				-- 일근태 계산 처리 --.
-----------------------------------------------------------------------------------------
					...
					ELSE
						V_OPEN_TIME := C1.OPEN_TIME;
					END IF;
          IF V_OPEN_TIME IS NULL AND C1.DUTY_CODE = '104' THEN 
          -- 직출/직퇴시 계획 출근시간 적용.
...
-----------------------------------------------------------------------------------------
          ...
          ELSE
						V_CLOSE_TIME := C1.CLOSE_TIME;
					END IF;
          IF V_CLOSE_TIME IS NULL AND C1.DUTY_CODE = '104' THEN 
          -- 직출/직퇴시 계획 퇴근시간 적용.
                           ...
                           AND HH.CHARGE_SEQ          IN 
                                (SELECT MAX(S_HH.CHARGE_SEQ) AS CHARGE_SEQ
                                    FROM HRM_HISTORY_HEADER S_HH
                                       , HRM_HISTORY_LINE   S_HL
                                   WHERE S_HH.HISTORY_HEADER_ID = S_HL.HISTORY_HEADER_ID
                                     AND S_HH.CHARGE_DATE       <= W_END_DATE
                                     AND S_HL.PERSON_ID         = HL.PERSON_ID
                                   GROUP BY S_HL.PERSON_ID
                                 )                                                   
                        ) T1
		  ...
		  UPDATE HRD_DAY_LEAVE DL
			  SET ( DL.DUTY_ID, DL.HOLY_TYPE
			      ...
			      , DL.LAST_UPDATE_DATE, DL.LAST_UPDATED_BY			
						...
						( SELECT DI.SOURCE_DUTY_ID AS DUTY_ID                      
									 , DI.HOLY_TYPE
									 , CASE
                   ...
                   , CASE
                       ...
                       ELSE DI.CLOSE_TIME1
                     END AS CLOSE_TIME1
									 , NVL(DI.NEXT_DAY_YN, 'N') AS NEXT_DAY_YN
                   ...
                   SELECT HL.PERSON_ID
                    ...
                    WHERE HH.HISTORY_HEADER_ID  = HL.HISTORY_HEADER_ID
                      ...
                      AND HH.HISTORY_NUM        IN
                            ( SELECT MAX(S_HH.HISTORY_NUM) AS HISTORY_NUM 
                                FROM HRM_HISTORY_HEADER S_HH
                                   , HRM_HISTORY_LINE   S_HL
                               WHERE S_HH.HISTORY_HEADER_ID   = S_HL.HISTORY_HEADER_ID
                                 AND S_HH.CHARGE_DATE         <= W_END_DATE
        ...
        AND DL.CLOSED_YN               = 'N'
              ...
              ( SELECT 'X'
                ...
                WHERE DI.PERSON_ID              = WC.PERSON_ID
                  ...
                  AND DI.SOB_ID                 = DL.SOB_ID
                  AND DI.ORG_ID                 = DL.ORG_ID
                  AND ((W_WORK_TYPE_ID          IS NULL
                    AND 1                       = 1) 
                  OR   (W_WORK_TYPE_ID          IS NOT NULL
                    AND WC.WORK_TYPE_ID         = W_WORK_TYPE_ID)) 
                  AND ((W_HOLY_TYPE             IS NULL
                    AND 1                       = 1) 
                  OR   (W_HOLY_TYPE             IS NOT NULL
                    AND WC.HOLY_TYPE            = W_HOLY_TYPE)) 
                  AND ((W_DEPT_ID               IS NULL
                    AND 1                       = 1) 
                  OR   (W_DEPT_ID               IS NOT NULL
                    AND T1.DEPT_ID              = W_DEPT_ID)) 
              ...
              ( SELECT 'X'
                ...
                WHERE DLV.PERSON_ID         = T1.PERSON_ID
                  ...
                  AND ((W_WORK_TYPE_ID          IS NULL
                    AND 1                       = 1) 
                  OR   (W_WORK_TYPE_ID          IS NOT NULL
                    AND DLV.WORK_TYPE_ID        = W_WORK_TYPE_ID)) 
                  AND ((W_HOLY_TYPE             IS NULL
                    AND 1                       = 1) 
                  OR   (W_HOLY_TYPE             IS NOT NULL
                    AND DLV.HOLY_TYPE           = W_HOLY_TYPE)) 
                  AND ((W_DEPT_ID               IS NULL
                    AND 1                       = 1) 
                  OR   (W_DEPT_ID               IS NOT NULL
                    AND T1.DEPT_ID              = W_DEPT_ID)) 
...
-- DAY_LEAVE_OT SAVE(INSERT/UPDATE).
  ...
  BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
      O_MESSAGE := 'Insert Error - Person ID:' || P_PERSON_ID || ', Date:' || TO_CHAR(P_WORK_DATE, 'YYYY-MM-DD') 
                                      || ', OT Type:' || P_OT_TYPE || ', Leave ID:' || P_DAY_LEAVE_ID 
                                      || ' : ' || SUBSTR(SQLERRM, 1, 150);                                        
      RETURN;
    END;  
    /*IF P_OT_TIME > 0 THEN
...
-- 일근태 계산 처리(실적만 가지고 계산-연장근무 신청 반영 안함).
  ...
  AS
    ...
    IS
      SELECT DI.PERSON_ID
           ...
           , CASE
               WHEN (DI.NEXT_DAY_YN = 'Y' OR DI.HOLY_TYPE IN('3', 'N') OR DI.DANGJIK_YN = 'Y' OR DI.ALL_NIGHT_YN = 'Y') 
                    THEN DECODE(DI.MODIFY_OUT_YN, 'Y', O_DM.MODIFY_TIME, N_DI.CLOSE_TIME)
               WHEN DI.MODIFY_OUT_YN = 'Y' THEN O_DM.MODIFY_TIME
               ELSE DI.CLOSE_TIME
             END AS CLOSE_TIME
           , CASE
           ...
           , DI.NIGHT_TIME
           , DI.NIGHT_BONUS_TIME
           , TO_DATE(TO_CHAR(DI.WORK_DATE + NVL(ST1.I_ADD_DAYS, 0), 'YYYY-MM-DD') 
                            || ' ' || NVL(ST1.I_TIME, TO_CHAR(DI.PL_OPEN_TIME, 'HH24:MI')), 'YYYY-MM-DD HH24:MI') AS STD_OPEN_TIME
           , TO_DATE(TO_CHAR(DI.WORK_DATE + NVL(ST1.O_ADD_DAYS, 0), 'YYYY-MM-DD') 
                            || ' ' || NVL(ST1.O_TIME, TO_CHAR(DI.PL_CLOSE_TIME, 'HH24:MI')), 'YYYY-MM-DD HH24:MI') AS STD_CLOSE_TIME
           , DI.PL_OPEN_TIME
           , DI.PL_CLOSE_TIME
           -- 30" 단위 변경 계산.
           , CASE
               WHEN TO_CHAR(DI.PL_BEFORE_OT_START, 'MI') BETWEEN '01' AND '29' THEN TRUNC(DI.PL_BEFORE_OT_START, 'HH24') + (30/24/60)
               WHEN TO_CHAR(DI.PL_BEFORE_OT_START, 'MI') BETWEEN '31' AND '59' THEN TRUNC(DI.PL_BEFORE_OT_START, 'HH24') + (60/24/60)
               ELSE DI.PL_BEFORE_OT_START
             END AS PL_BEFORE_OT_START
           , CASE
               WHEN TO_CHAR(DI.PL_BEFORE_OT_END, 'MI') BETWEEN '01' AND '29' THEN TRUNC(DI.PL_BEFORE_OT_END, 'HH24')
               WHEN TO_CHAR(DI.PL_BEFORE_OT_END, 'MI') BETWEEN '31' AND '59' THEN TRUNC(DI.PL_BEFORE_OT_END, 'HH24') + (30/24/60)
               ELSE DI.PL_BEFORE_OT_END
             END AS PL_BEFORE_OT_END
           , CASE
               WHEN TO_CHAR(DI.PL_AFTER_OT_START, 'MI') BETWEEN '01' AND '29' THEN TRUNC(DI.PL_AFTER_OT_START, 'HH24') + (30/24/60)
               WHEN TO_CHAR(DI.PL_AFTER_OT_START, 'MI') BETWEEN '31' AND '59' THEN TRUNC(DI.PL_AFTER_OT_START, 'HH24') + (60/24/60)
               ELSE DI.PL_AFTER_OT_START
             END AS PL_AFTER_OT_START
           , CASE
               WHEN TO_CHAR(DI.PL_AFTER_OT_END, 'MI') BETWEEN '01' AND '29' THEN TRUNC(DI.PL_AFTER_OT_END, 'HH24')
               WHEN TO_CHAR(DI.PL_AFTER_OT_END, 'MI') BETWEEN '31' AND '59' THEN TRUNC(DI.PL_AFTER_OT_END, 'HH24') + (30/24/60)
               ELSE DI.PL_AFTER_OT_END
             END AS PL_AFTER_OT_END
           , DI.PL_BREAKFAST_YN
           , DI.PL_LUNCH_YN
           , DI.PL_DINNER_YN
      ...
      WHERE DI.DUTY_ID                = DC.DUTY_ID
        AND DI.SOB_ID                 = DC.SOB_ID
        AND DI.ORG_ID                 = DC.ORG_ID
        AND DI.PERSON_ID              = T1.PERSON_ID
        AND DI.SOB_ID                 = T1.SOB_ID
        AND DI.ORG_ID                 = T1.ORG_ID
        AND DI.WORK_DATE              = PW1.WORK_DATE(+)
        AND DI.PERSON_ID              = PW1.PERSON_ID(+)
        AND DI.SOB_ID                 = PW1.SOB_ID(+)
        AND DI.ORG_ID                 = PW1.ORG_ID(+)
        AND DI.PERSON_ID              = I_DM.PERSON_ID(+)
				AND DI.WORK_DATE              = I_DM.WORK_DATE(+)
				AND '1'                       = I_DM.IO_FLAG(+)
				AND DI.PERSON_ID              = O_DM.PERSON_ID(+)
        ...
        AND DI.SOB_ID                 = N_DI.SOB_ID(+)
        AND DI.ORG_ID                 = N_DI.ORG_ID(+)         
        AND DI.WORK_DATE              = NW1.WORK_DATE(+)
        AND DI.PERSON_ID              = NW1.PERSON_ID(+)
        AND DI.SOB_ID                 = NW1.SOB_ID(+)
        AND DI.ORG_ID                 = NW1.ORG_ID(+)
        AND DI.WORK_TYPE_GROUP        = ST1.WORK_TYPE(+)
        AND DI.HOLY_TYPE              = ST1.HOLY_TYPE(+)
        AND DI.WORK_DATE              = W_WORK_DATE
        AND DI.PERSON_ID              = NVL(W_PERSON_ID, DI.PERSON_ID)
        AND DI.WORK_CORP_ID           = W_CORP_ID
        AND DI.SOB_ID                 = W_SOB_ID
        AND DI.ORG_ID                 = W_ORG_ID
        AND DI.HOLY_TYPE              = NVL(W_HOLY_TYPE, DI.HOLY_TYPE)
        ...
        AND EXISTS 
              (SELECT 'X'
               ...
               WHERE DM.CORP_ID                                  = DI.WORK_CORP_ID
                 AND DM.DUTY_CONTROL_ID                          = NVL(T1.FLOOR_ID, DM.DUTY_CONTROL_ID)
      ...
      ORDER BY DI.WORK_DATE, DI.HOLY_TYPE
      ;
    
    V_DAY_COUNT                 NUMBER := 0;                          -- 계산총 일수.
    V_WORK_DATE                 HRD_DAY_LEAVE.WORK_DATE%TYPE;         -- 근무일자.
    
    V_LEAVE_TIME                HRD_DAY_INTERFACE_OT.OT_TIME%TYPE;    -- 외출시간 적용.
    V_LATE_TIME                 HRD_DAY_INTERFACE_OT.OT_TIME%TYPE;    -- 지각시간.
    V_REST_TIME                 HRD_DAY_INTERFACE_OT.OT_TIME%TYPE;    -- 휴식연장.
    V_ADD_NIGHT_BONUS_TIME      HRD_DAY_LEAVE_OT.OT_TIME%TYPE;        -- <추가>야간할증-휴식연장(야간)근무시 추가적용되는 야간할증.
    V_OVER_TIME                 HRD_DAY_INTERFACE_OT.OT_TIME%TYPE;    -- 연장시간.
    V_HOLIDAY_TIME              HRD_DAY_INTERFACE_OT.OT_TIME%TYPE;    -- 휴일근로.
    V_HOLIDAY_OT_TIME           HRD_DAY_LEAVE_OT.OT_TIME%TYPE;        -- 휴일연장근로.
    V_NIGHT_TIME                HRD_DAY_INTERFACE_OT.OT_TIME%TYPE;    -- 야간근로.
    V_NIGHT_BONUS_TIME          HRD_DAY_INTERFACE_OT.OT_TIME%TYPE;    -- 야간할증.
    
    V_FOOD_DED_TIME             NUMBER := 0;                          -- 식사 공제시간.
    V_REST_DED_TIME             NUMBER := 0;                          -- 휴식시간 공제 시간.
        
    V_OPEN_TIME                 HRD_DAY_INTERFACE.OPEN_TIME%TYPE;         -- 정리된 출근시간.
    V_CLOSE_TIME                HRD_DAY_INTERFACE.CLOSE_TIME%TYPE;        -- 정리된 퇴근시간.
    V_OT_OPEN_TIME              HRD_DAY_INTERFACE.OPEN_TIME%TYPE;         -- 30" 정리된 출근시간.
    V_OT_CLOSE_TIME             HRD_DAY_INTERFACE.CLOSE_TIME%TYPE;        -- 30" 정리된 퇴근시간.
    
    V_OT_START_TIME             HRD_DAY_INTERFACE.OPEN_TIME%TYPE;         -- 연장등 계산 시작 시간.
    V_OT_MID_TIME               HRD_DAY_INTERFACE.OPEN_TIME%TYPE;         -- 연장등 계산 중간 시간.
    V_OT_END_TIME               HRD_DAY_INTERFACE.CLOSE_TIME%TYPE;        -- 연장등 계산 종료 시간.
    
    V_PRE_HOLY_TYPE             HRD_DAY_INTERFACE.HOLY_TYPE%TYPE := 'N';  -- 전일 근무/당직/철야 여부.
  ...
  BEGIN
    ...
    -- 근태권한 설정.
    IF HRM_MANAGER_G.USER_CAP_F(W_CORP_ID     => P_CORP_ID
                              , W_START_DATE  => P_WORK_DATE
                              , W_END_DATE    => P_WORK_DATE
                              , W_MODULE_CODE => '20'
    ...
    -- 기존 HRD_DAY_INTERFACE_OT 자료 삭제.
    DELETE FROM HRD_DAY_INTERFACE_OT DIO
     WHERE EXISTS (SELECT 'X'
                     FROM HRD_DAY_INTERFACE DL
                        , HRD_WORK_CALENDAR WC
                        , (-- 시점 인사내역.
                            SELECT  HL.PERSON_ID
                                  , HL.DEPT_ID
                               ...
                               AND HH.CHARGE_SEQ           IN 
                                    (SELECT MAX(S_HH.CHARGE_SEQ) AS CHARGE_SEQ
                                        FROM HRM_HISTORY_HEADER S_HH
                                           , HRM_HISTORY_LINE   S_HL
                                       WHERE S_HH.HISTORY_HEADER_ID = S_HL.HISTORY_HEADER_ID
                                         AND S_HH.CHARGE_DATE       <= P_WORK_DATE
                                         AND S_HL.PERSON_ID         = HL.PERSON_ID
                                       GROUP BY S_HL.PERSON_ID
                                     )  
                            ) T1
                    WHERE DL.PERSON_ID                   = WC.PERSON_ID(+)
                      ...
                      AND DL.ORG_ID                      = WC.ORG_ID(+)
                      AND DL.PERSON_ID                   = T1.PERSON_ID
                      AND DL.PERSON_ID                   = DIO.PERSON_ID
                      AND DL.WORK_DATE                   = DIO.WORK_DATE
                      AND DL.SOB_ID                      = DIO.SOB_ID
                      AND DL.ORG_ID                      = DIO.ORG_ID
                      AND DL.PERSON_ID                   = NVL(P_PERSON_ID, DL.PERSON_ID)
                      AND DL.WORK_DATE                   = P_WORK_DATE
                      AND DL.WORK_CORP_ID                = P_CORP_ID
                      AND DL.SOB_ID                      = P_SOB_ID
                      AND DL.ORG_ID                      = P_ORG_ID
    ...
    V_DAY_COUNT := (P_WORK_DATE - P_WORK_DATE) + 1;
    
    -- V_DAY_COUNT 만큼_일근태_계산.--
    FOR D1 IN 0 .. V_DAY_COUNT - 1
    LOOP 
      V_WORK_DATE := P_WORK_DATE + D1;
...
-----------------------------------------------------------------------------------------          
          ...
          V_OT_OPEN_TIME := OPEN_TIME_F( P_JOB_CATEGORY_CODE => C1.JOB_CATEGORY_CODE
                                        , P_HOLY_TYPE => C1.HOLY_TYPE
                                        , P_OPEN_TIME => V_OPEN_TIME
                                        , P_DANGJIK_YN => C1.DANGJIK_YN
                                        , P_ALL_NIGHT_YN => C1.ALL_NIGHT_YN 
                                        );
          V_OT_CLOSE_TIME := CLOSE_TIME_F( P_JOB_CATEGORY_CODE => C1.JOB_CATEGORY_CODE
                                          , P_HOLY_TYPE => C1.HOLY_TYPE
                                          , P_CLOSE_TIME => V_CLOSE_TIME
...
-----------------------------------------------------------------------------------------
        ...
        -- 잔업 사항 저장. --
        -- LEAVE_TIME.
        DAY_INTERFACE_OT_SAVE ( P_PERSON_ID => C1.PERSON_ID
                              , P_WORK_DATE => C1.WORK_DATE
                              , P_CORP_ID => C1.CORP_ID
                              , P_OT_TYPE => C_LEAVE_TIME      -- '11'
                              , P_OT_TIME => V_LEAVE_TIME
    ...
    END LOOP D1;
    O_STATUS := 'S';
    O_MESSAGE := EAPP_MESSAGE_G.RETURN_TEXT_F(USERENV_G.GET_TERRITORY_S_F, 'FCM_10358', NULL);
  END WORKDATA_GO_IF;
...
-- DAY_INTERFACE_OT SAVE(INSERT/UPDATE).
  ...
  AS
    V_SYSDATE       HRD_DAY_INTERFACE_OT.CREATION_DATE%TYPE := GET_LOCAL_DATE(P_SOB_ID);
    V_RE            NUMBER := 0;
  ...
  BEGIN
    IF P_OT_TIME > 0 THEN
      /*SELECT COUNT(X.DAY_LEAVE_ID) AS COUNT
       INTO V_RE
      FROM HRD_DAY_INTERFACE_OT X
      WHERE X.DAY_LEAVE_ID  = P_DAY_LEAVE_ID  
        AND X.OT_TYPE       = P_OT_TYPE
       ;*/
--DBMS_OUTPUT.PUT_LINE('ID : ' || P_DAY_LEAVE_ID || ', OT TYPE : ' || P_OT_TYPE || '레코드 수 : ' || V_RE);
    
      INSERT INTO HRD_DAY_INTERFACE_OT
      ( PERSON_ID
      , WORK_DATE
      , CORP_ID
      , SOB_ID
