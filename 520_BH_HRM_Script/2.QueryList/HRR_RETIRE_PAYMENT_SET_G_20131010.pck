...
--  0. PAYMENT CALCULATE MAIN.
  ...
  AS
    ...
    V_PAY_DAY                     NUMBER := 0;      -- 급여일수.
    V_MONTH_DAY                   NUMBER := 0;      -- 월일수.
    V_LAST_ERROR_FLAG             VARCHAR2(2);      -- 최종 급상여 계산시 3개월 시작시점 일자와 상호 검증 --
  BEGIN
...
---------------------------------------------------------------------------
    ...
    -- 급여.
      ...
      LOOP
        ...
        V_MONTH_DAY := LAST_DAY(TO_DATE(V_PAY_YYYYMM, 'YYYY-MM')) - TRUNC(TO_DATE(V_PAY_YYYYMM, 'YYYY-MM'), 'MONTH') + 1;
        
        V_LAST_ERROR_FLAG := 'N';
        IF V_STD_MONTH < (V_R1 + 1) AND V_PAY_YYYYMM != TO_CHAR(V_START_DATE, 'YYYY-MM') THEN
          -- 최종개월에 대한 일할계산시 급여일자와 3개월 이전일자가 다를 경우 급여계산 안함 --
          V_LAST_ERROR_FLAG := 'Y';
        ELSIF V_PAY_DATE_FR <= V_START_DATE THEN
        ...
        END IF;
        V_PAY_DAY := V_PAY_DATE_TO - V_PAY_DATE_FR + 1;
        
        IF V_LAST_ERROR_FLAG = 'Y' THEN
          NULL;
        ELSIF P_ADJUSTMENT_TYPE = 'M' AND (V_MONTH_DAY != V_PAY_DAY) THEN
          -- 중도정산자중 급여일수와 정산월 일수가 다를경우 일할계산함.
    ...
    -- 명절상여 포함, 특별상여 포함 : 조선미s 요청 2013-03-12.
    /*ELSIF P_WAGE_TYPE IN(\*C_SPECIAL_BONUS, *\C_BONUS_P3) THEN
