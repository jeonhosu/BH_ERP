...
-- 출퇴근 정리 및 집계 MAIN.
  ...
  AS 
        ...
        , (-- 시점 인사내역.
              ...
              AND HH.CHARGE_SEQ          IN 
                    (SELECT MAX(S_HH.CHARGE_SEQ) AS CHARGE_SEQ
                       ...
                       WHERE S_HH.HISTORY_HEADER_ID = S_HL.HISTORY_HEADER_ID
                         AND S_HH.CHARGE_DATE       <= TRUNC(P_WORK_DATE)
         ...
         , (-- 시점 인사내역.
            SELECT PH.PERSON_ID
             ...
             WHERE PH.EFFECTIVE_DATE_FR  <=  TRUNC(P_WORK_DATE)
               AND PH.EFFECTIVE_DATE_TO  >=  TRUNC(P_WORK_DATE)
           ) T2
			WHERE DI.PERSON_ID              = PM.PERSON_ID
        AND DI.PERSON_ID              = T1.PERSON_ID
        AND DI.PERSON_ID              = T2.PERSON_ID
			  AND DI.WORK_DATE              = TRUNC(P_WORK_DATE)
        ...
        AND DI.TRANS_YN               = 'N'
				AND EXISTS ( SELECT 'X'
											...
											WHERE DM.CORP_ID               = DI.WORK_CORP_ID
												...
												AND DM.INOUT_YN              = 'Y'
												AND DM.START_DATE            <= TRUNC(P_WORK_DATE)
												AND (DM.END_DATE IS NULL OR DM.END_DATE >= TRUNC(P_WORK_DATE))
										)	
       ;										
    ...
    -- 후일 출퇴근 정리 및 집계 처리 프로시져 호출.
		WORKDATA_GO( P_CONNECT_PERSON_ID => V_CONNECT_PERSON_ID
		           , P_WORK_DATE => TRUNC(P_WORK_DATE) + 1
							 , P_CORP_ID => P_CORP_ID
							 , P_SOB_ID => P_SOB_ID
							 , P_ORG_ID => P_ORG_ID
							 , P_USER_ID => P_USER_ID
  ...
  AS
    ...
    IS
           ...
           , NVL(T_WC.WORK_TYPE_ID, PM.WORK_TYPE_ID) AS WORK_TYPE_ID
					 ...
					 , NVL(T_WC.WORK_HOLY_TYPE, '2') AS WORK_HOLY_TYPE  -- 주간/야간 구분 
					 , T_WC.OPEN_TIME
					 , T_WC.CLOSE_TIME
					 ...
					 , CASE
               WHEN NVL(P_WC.WORK_HOLY_TYPE, '2') = '3' THEN 'N'
               ELSE NVL(P_WC.DANGJIK_YN, 'N') 
             END AS P_DANGJIK_YN                              -- 근무가 야간일 경우 당직은 'N' 적용   
					 , CASE
               WHEN NVL(P_WC.WORK_HOLY_TYPE, '2') = '3' THEN 'N'
               ELSE NVL(P_WC.ALL_NIGHT_YN, 'N') 
             END AS P_ALL_NIGHT_YN                            -- 근무가 야간일 경우 철야는 'N' 적용 
                 ...
                 AND PH.EFFECTIVE_DATE_TO  >=  TRUNC(P_WORK_DATE)
					...
					, (-- 당일 근무 계획. 
							SELECT WC.PERSON_ID
									 , WC.WORK_DATE
									 , WC.WORK_TYPE_ID
                   , WC.ATTRIBUTE3 AS WORK_HOLY_TYPE   -- 주간/야간 구분 
                   ...
                   , WC.WORK_CORP_ID
							 ...
							 WHERE WC.WORK_DATE                   = TRUNC(P_WORK_DATE)
                   ...
                   , WC.WORK_CORP_ID
									 ...
									 , WC.ORG_ID					 
								FROM HRD_WORK_CALENDAR WC
							 WHERE WC.WORK_DATE                   = (TRUNC(P_WORK_DATE) - 1)
								 AND WC.WORK_CORP_ID                = P_CORP_ID
								 AND WC.SOB_ID                      = P_SOB_ID
								 AND WC.ORG_ID                      = P_ORG_ID
						) P_WC          
        ...
        AND PM.PERSON_ID                            = T2.PERSON_ID
			  AND PM.PERSON_ID                            = T_WC.PERSON_ID(+)
				...
				AND PM.JOIN_DATE                            <= TRUNC(P_WORK_DATE)
				AND (PM.RETIRE_DATE IS NULL OR PM.RETIRE_DATE >= TRUNC(P_WORK_DATE))
				AND EXISTS ( SELECT 'X'
                       FROM HRD_DUTY_MANAGER DM
...
---------------------------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    END IF;
      ...
      ++++ 당일 출퇴근 시간 정리
      /========================================================*/
      FOR C_IO IN  CUR_IO_DATE(C_PERSON.PERSON_ID, TRUNC(P_WORK_DATE), P_SOB_ID, P_ORG_ID)
      LOOP
      ...
      BEGIN
        ...
        WHERE DM.PERSON_ID     = C_PERSON.PERSON_ID
          AND DM.WORK_DATE     = TRUNC(P_WORK_DATE)
          AND DM.IO_FLAG       = '1'
        ;
      EXCEPTION WHEN OTHERS THEN
        M_IN_TIME := NULL;
...
/------------------------------------------------------------------------------------------*/
      SELECT /*CASE
        WHEN WORKDATA_CUR.P_DUTY_CODE IN('00', '11', '53', '40') THEN*/
            CASE
              WHEN C_PERSON.HOLY_TYPE IN('0', '1') THEN  --휴일 --
                CASE
                  ...
                  WHEN C_PERSON.HOLY_TYPE = '0' THEN V_NH_DUTY_ID                                                        -- 무급휴일.
                  ELSE V_H_DUTY_ID                                                                                       -- 휴일.
                END
              ELSE                                                                                                       -- 주간/야간
                CASE
   ...
   /*\*-- 기존 데이터 존재 체크 - 미 존재 : 삽입, 존재 : 수정 --*\
      BEGIN
        SELECT DECODE(COUNT(DI.PERSON_ID), 0, 'I', 'U') AS DATA_FLAG
           ...
           WHERE DI.WORK_DATE             = TRUNC(P_WORK_DATE)
    ...
    IF (SQL%NOTFOUND) THEN
      ...
      ( C_PERSON.PERSON_ID, TRUNC(P_WORK_DATE), C_PERSON.WORK_CORP_ID, C_PERSON.CORP_ID, C_PERSON.DEPT_ID, C_PERSON.POST_ID
     ...
     SELECT 
       ...
       FROM HRD_DAY_INTERFACE_V DI
          ...
          , ( SELECT  DI.PERSON_ID
                    ...
                    , DI.DANGJIK_YN 
                 FROM HRD_DAY_INTERFACE_V DI
                WHERE DI.WORK_DATE                = (TRUNC(P_WORK_DATE) - 1)  
                  AND DI.PERSON_ID                = C_PERSON.PERSON_ID
                  AND DI.SOB_ID                   = P_SOB_ID
                  AND DI.ORG_ID                   = P_ORG_ID
            ) P_SX
          , ( SELECT DI.PERSON_ID
              ...
              WHERE DI.DUTY_ID                  = DC.DUTY_ID
                ...
                AND '2'                         = O_DM.IO_FLAG(+)
                AND DI.WORK_DATE                = (TRUNC(P_WORK_DATE) + 1)  
                AND DI.PERSON_ID                = C_PERSON.PERSON_ID
                AND DI.SOB_ID                   = P_SOB_ID
                AND DI.ORG_ID                   = P_ORG_ID
            ) T_SX    
   ...
   BEGIN
     UPDATE HRD_DAY_INTERFACE DI
        SET DI.DUTY_ID            = CASE 
                                      WHEN DI.DUTY_ID IN (V_H_DUTY_ID, V_NH_DUTY_ID) THEN DI.DUTY_ID 
                                      WHEN DI.DUTY_ID IN (V_PH_DUTY_ID) THEN DECODE(DI.HOLY_TYPE, '0', V_NH_DUTY_ID, V_H_DUTY_ID) 
                                      ELSE V_NA_DUTY_ID 
                                    END
                                    /*-- 무급휴일시 휴일로 처리되는문제 발생으로 주석 
                                    CASE 
                                      WHEN DI.DUTY_ID IN (V_H_DUTY_ID, V_PH_DUTY_ID, V_NH_DUTY_ID) THEN V_H_DUTY_ID  
                                      ELSE V_NA_DUTY_ID 
                                    END */
      WHERE DI.WORK_DATE          = TRUNC(P_WORK_DATE)  
        AND DI.PERSON_ID          = C_PERSON.PERSON_ID
        ...
        AND ( IN_TIME             IS NULL
         OR   OUT_TIME            IS NULL)
        --AND DI.DUTY_ID            IN(V_A_DUTY_ID, V_PH_DUTY_ID, V_NH_DUTY_ID)  -- 출근/휴일근무/무휴근무일 경우만 처리 -- 
        AND NOT EXISTS
              ( SELECT 'X'
                  FROM HRD_WORK_CALENDAR WC
   ...
   EXCEPTION
     WHEN OTHERS THEN
       O_MESSAGE := 'Open Time/Close Time Clearning Error : ' || CHR(10) || SQLERRM;
       RETURN;
   END;
  END LOOP C_PERSON;
