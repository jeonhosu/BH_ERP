...
-- 신청기간에 대한 근무계획 및 출퇴근 시간 조회 -- 
  PROCEDURE DATA_SELECT_WORKCALENDAR 
...
-- 신청기간에 대한 근무계획 및 출퇴근 시간 조회 -- 
 ...
 BEGIN
   OPEN P_CUR1 FOR
     SELECT WC.WORK_DATE
          , HRM_COMMON_G.ID_NAME_F(NVL(WC.C_DUTY_ID1, NVL(WC.C_DUTY_ID, WC.DUTY_ID))) AS DUTY_NAME
          , WC.HOLY_TYPE
      ...
      FROM HRD_WORK_CALENDAR   WC  
         ...
         , HRD_DAY_MODIFY      I_DM
         , HRD_DAY_MODIFY      O_DM
     WHERE WC.PERSON_ID               = DI.PERSON_ID(+)
       AND WC.WORK_DATE               = DI.WORK_DATE(+)    
       AND DI.DUTY_ID                 = DC.DUTY_ID(+)
   ...
   -- 근무시간 정리.
    ...
    END;
		IF HRM_MANAGER_G.USER_CAP_F( W_CORP_ID => P_CORP_ID
		                           , W_START_DATE => D_WORK_START_DATE
															 , W_END_DATE => D_WORK_END_DATE
															 , W_MODULE_CODE => '20'
    ...
    -- 보건휴가 1일 초과 사용시 오류 --
    V_RECORD_COUNT := 0;
    IF V_DUTY_CODE = '22' AND V_PERSON_CAP_C = 'N' THEN
      -- 유급휴일수 제외 -- 
      V_HOLY_1_COUNT := HRD_WORK_CALENDAR_G.HOLY_1_COUNT_F
      ...
      -- 무급휴일수 제외 -- 
      V_HOLY_1_COUNT := NVL(V_HOLY_1_COUNT, 0) + 
                        HRD_WORK_CALENDAR_G.HOLY_0_COUNT_F
                          ( W_PERSON_ID   => P_PERSON_ID
                          , W_START_DATE  => D_WORK_START_DATE 
                          , W_END_DATE    => D_WORK_END_DATE
                          , W_SOB_ID      => P_SOB_ID
      ...
      BEGIN
        SELECT SUM(DP.WORK_END_DATE - DP.WORK_START_DATE + 1) AS DAY_COUNT
          INTO V_RECORD_COUNT 
          FROM HRD_DUTY_PERIOD DP
         WHERE DP.PERSON_ID       = P_PERSON_ID 
           AND DP.DUTY_ID         = P_DUTY_ID
           AND DP.APPROVE_STATUS  != 'R'
           AND DP.WORK_START_DATE >= TRUNC(D_WORK_START_DATE, 'MONTH')
           AND DP.WORK_END_DATE   <= LAST_DAY(D_WORK_END_DATE)
         ;
      EXCEPTION
        WHEN OTHERS THEN
          V_RECORD_COUNT := 0;
      END;
      V_RECORD_COUNT := NVL(V_RECORD_COUNT, 0) + (D_WORK_END_DATE - D_WORK_START_DATE + 1);  -- 이번 신청건 -- 
      V_RECORD_COUNT := NVL(V_RECORD_COUNT, 0) - NVL(V_HOLY_1_COUNT, 0);                     -- 유급/무급 휴일 제외 -- 
      IF V_RECORD_COUNT > 1 THEN
        RAISE_APPLICATION_ERROR(-20001, EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10514'));
      END IF;
    END IF;
    
    -- 유계결근 3일 이상 오류 --
    V_RECORD_COUNT := 0;
    IF V_DUTY_CODE = '54' AND V_PERSON_CAP_C = 'N' THEN
      -- 유급휴일수 제외 -- 
      V_HOLY_1_COUNT := HRD_WORK_CALENDAR_G.HOLY_1_COUNT_F
                          ( W_PERSON_ID   => P_PERSON_ID
                          , W_START_DATE  => D_WORK_START_DATE 
      ...
      V_RECORD_COUNT := (D_WORK_END_DATE - D_WORK_START_DATE + 1);  -- 신청건 -- 
      V_RECORD_COUNT := NVL(V_RECORD_COUNT, 0) - NVL(V_HOLY_1_COUNT, 0);                     -- 유급/무급 휴일 제외 -- 
      IF V_RECORD_COUNT > 2 THEN
        RAISE_APPLICATION_ERROR(-20001, EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10515') || CHR(10) || 
                                        RPAD(' ', 17, ' ') || EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10516'));
      END IF;
    END IF;
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        V_CONNECT_PERSON_ID := -1;
    END;
		IF HRM_MANAGER_G.USER_CAP_F( W_CORP_ID => V_CORP_ID
		                           , W_START_DATE => D_WORK_START_DATE
															 , W_END_DATE => D_WORK_END_DATE
															 , W_MODULE_CODE => '20'
															 , W_PERSON_ID => V_CONNECT_PERSON_ID
    ...
    IF V_DUTY_CODE = '22' AND V_PERSON_CAP_C = 'N' THEN
      ...
      BEGIN
        SELECT SUM(DP.WORK_END_DATE - DP.WORK_START_DATE + 1) AS DAY_COUNT
          INTO V_RECORD_COUNT 
          FROM HRD_DUTY_PERIOD DP
         WHERE DP.PERSON_ID       = V_PERSON_ID 
           AND DP.DUTY_ID         = P_DUTY_ID
             ...
             AND HH.CHARGE_SEQ          IN 
                  (SELECT MAX(S_HH.CHARGE_SEQ) AS CHARGE_SEQ
                      FROM HRM_HISTORY_HEADER S_HH
                         , HRM_HISTORY_LINE   S_HL
                     WHERE S_HH.HISTORY_HEADER_ID = S_HL.HISTORY_HEADER_ID
                       AND S_HH.CHARGE_DATE       <= ( SELECT TRUNC(DP1.END_DATE) END_DATE
      ...
      ELSE      
        ...
        UPDATE HRD_WORK_CALENDAR WC
           SET (WC.C_DUTY_ID
             ...
             =
             ( SELECT CASE
                        WHEN DC.NON_PAY_DAY_FLAG != 'Y' AND WC.HOLY_TYPE IN('0', '1') THEN NULL
                        WHEN WC.C_DUTY_ID IS NULL THEN DP.DUTY_ID
                        ELSE WC.C_DUTY_ID
                      END AS C_DUTY_ID
                    , CASE
                        WHEN DC.NON_PAY_DAY_FLAG != 'Y' AND WC.HOLY_TYPE IN('0', '1') THEN NULL
                        WHEN WC.C_DUTY_ID IS NOT NULL THEN DP.DUTY_ID
                        ELSE WC.C_DUTY_ID1
                      END AS C_DUTY_ID1
                    , CASE
                        WHEN DC.NON_PAY_DAY_FLAG != 'Y' AND WC.HOLY_TYPE IN('0', '1') THEN NULL
               ...
               /*--전호수주석(20130625) : 법정공휴일에 대해서 무급일경우 무급처리 위해 -- 
               SELECT CASE
        ...
        UPDATE HRD_WORK_CALENDAR WC
           SET (WC.C_DUTY_ID   
             ...
             ( SELECT CASE
                        WHEN WC.C_DUTY_ID1 IS NOT NULL THEN DP.DUTY_ID
                        ELSE NULL
                      END AS C_DUTY_ID
                    , NULL AS C_DUTY_ID1
                    , WC.OLD_OPEN_TIME AS OPEN_TIME
                    , WC.OLD_CLOSE_TIME AS CLOSE_TIME
                    , NULL AS OLD_START_TIME
                    , NULL AS OLD_CLOSE_TIME
               FROM HRD_DUTY_PERIOD DP
                  , HRM_DUTY_CODE_V DC
             WHERE DP.DUTY_ID           = DC.DUTY_ID
               AND DP.DUTY_PERIOD_ID    = W_DUTY_PERIOD_ID
...
-- WORK DATE TIME 정리.
  ...
  BEGIN
   ...
   -- 실제근무기간 생성 및 반환.
    ...
    ELSIF V_END_TIME BETWEEN '00:01' AND '08:30' THEN
      D_WORK_END_DATE := TRUNC(P_END_DATE) - 1;
    ELSE
      D_WORK_END_DATE := TRUNC(P_END_DATE);
    END IF;
    
    IF D_WORK_START_DATE > D_WORK_END_DATE THEN
      D_WORK_END_DATE := D_WORK_START_DATE;
    END IF;
