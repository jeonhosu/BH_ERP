  ...
  BEGIN
    OPEN P_CURSOR FOR
      SELECT ROWNUM AS ROW_NUM
           ...
           , RA.COMP_TAX_AMOUNT
           , RA.TAX_DED_AMOUNT
           , TRUNC(RA.INCOME_TAX_AMOUNT, -1) AS INCOME_TAX_AMOUNT 
           , TRUNC(RA.RESIDENT_TAX_AMOUNT, -1) AS RESIDENT_TAX_AMOUNT 
           , TRUNC(RA.SP_TAX_AMOUNT, -1) AS SP_TAX_AMOUNT 
           ...
           , RA.H_TAX_DED_AMOUNT
           , TRUNC(RA.H_INCOME_TAX_AMOUNT, -1) AS H_INCOME_TAX_AMOUNT 
           , TRUNC(RA.H_RESIDENT_TAX_AMOUNT, -1) AS H_RESIDENT_TAX_AMOUNT 
           , TRUNC(RA.H_SP_TAX_AMOUNT, -1) AS H_SP_TAX_AMOUNT 
           , RA.H_REAL_AMOUNT
  ...
  BEGIN
    OPEN P_CURSOR FOR
      SELECT   NVL(PPM.CORP_ID, NULL) AS CORP_ID                      -- 업체ID
              ...
              , SUM(TRUNC(RA.H_INCOME_TAX_AMOUNT, -1)) AS H_INCOME_TAX_AMOUNT    -- 소득세(법정이외 퇴직급여)
              , SUM(TRUNC(RA.H_RESIDENT_TAX_AMOUNT, -1)) AS H_RESIDENT_TAX_AMOUNT-- 주민세(법정이외 퇴직급여)
  ...
  BEGIN
    ...
    OPEN P_CURSOR FOR
      SELECT -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- 
           ...
           -------------------------------------------------------------------------------------------------------------------  
           -- 2012.12.31이전
           , RA.AVG_TAX_STD_AMOUNT_1 AS AVG_TAX_ANBON1                                         -- 과세표준안분(2012.12.31이전)
           , RA.AVG_TAX_STD_AMOUNT_1 AS AVG_TAX_STD1                                           -- 연평균과세표준(2012.12.31이전)
           , RA.COMP_TAX_AMOUNT_1    AS AVG_YEAR_COMP_TAX1                                     -- 연평균산출세액(2012.12.31이전)
           , RA.COMP_TAX_AMOUNT_1    AS COMP_TAX1                                              -- 산출세액 (2012.12.31이전)
           
           -- 2013.01.01이후 
          , RA.AVG_TAX_STD_AMOUNT_2/5  AS AVG_TAX_ANBON2                                       -- 과세표준안분(2013.01.01이후 )   
          , RA.AVG_TAX_STD_AMOUNT_2/5  AS AVG_TAX_STD2                                         -- 연평균과세표준(2013.01.01이후 )
          , RA.AVG_TAX_STD_AMOUNT_2    AS CHANGE_AVG_TAX_2                                     -- 환산과세표준 (2013.01.01이후 )
          , RA.AVG_COMP_TAX_AMOUNT_2   AS CAHNGE_AVG_COMP_TAX2                                 -- 환산산출세액 (2013.01.01이후 )
          , RA.COMP_TAX_AMOUNT_2       AS AVG_YEAR_COMP_TAX2                                   -- 연평균산출세액 (2013.01.01이후 )
          , RA.COMP_TAX_AMOUNT_2       AS COMP_TAX2                                            -- 산출세액 (2013.01.01이후 )
          
          -- 정산( 합산) 
          , NVL(RA.AVG_TAX_STD_AMOUNT_1, 0) + NVL(RA.AVG_TAX_STD_AMOUNT_2/5 , 0)   AS AVG_TAX_ANBON_SUM              -- 과세표준안분(정산(합산))
          , NVL(RA.AVG_TAX_STD_AMOUNT_1, 0) + NVL(RA.AVG_TAX_STD_AMOUNT_2/5 , 0)   AS AVG_TAX_STD_SUM                -- 연평균과세표준(정산(합산))
          , RA.AVG_TAX_STD_AMOUNT_2                                                AS CHANGE_AVG_TAX_SUM             -- 환산과세표준 
          , RA.AVG_COMP_TAX_AMOUNT_2                                               AS CAHNGE_AVG_COMP_TAX_SUM        -- 환산산출세액액 
          , NVL(RA.COMP_TAX_AMOUNT_1, 0) + NVL(RA.COMP_TAX_AMOUNT_2, 0)            AS COMP_TAX_AMOUNT_SUM            -- 연평균산출세액
          ...
          , DECODE(HRP.PERSON_ID, NULL, 
                   TO_NUMBER(NULL),
                   RA.RETIRE_AMOUNT) AS DEPOSIT_AMOUNT                                                               -- 계좌입금금액 
          , DECODE(HRP.PERSON_ID, NULL, 
                   TO_NUMBER(NULL),                                                 	
