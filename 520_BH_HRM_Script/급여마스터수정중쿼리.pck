...
-- 급여 마스터 헤더 저장(기준년월과 최종 시작년월 비교하여 처리).
...
-- 급여 마스터 라인 저장(기준년월과 최종 시작년월 비교하여 처리).
...
-- PAY_MASTER_LINE_COPY : 급여마스터 라인정보 COPY.
...
-- 급여마스터 헤더의 최종 적용되는 자료의 시작 년월 리턴.
...
-- 급여 마스터 저장(기준년월과 최종 시작년월 비교하여 처리).
  PROCEDURE PAY_HEADER_SAVE
            ( P_PAY_HEADER_ID    OUT HRP_PAY_MASTER_HEADER.PAY_HEADER_ID%TYPE
            , P_PERSON_ID        IN HRP_PAY_MASTER_HEADER.PERSON_ID%TYPE
            , P_CORP_ID          IN HRP_PAY_MASTER_HEADER.CORP_ID%TYPE
  ...
  BEGIN
    ...
    IF V_PAY_HEADER_ID = 0 AND V_LAST_START_YYYYMM IS NULL THEN  -- 신규 INSERT.
    ...
    ELSIF V_LAST_START_YYYYMM < P_STD_YYYYMM THEN     -- 기준년월보다 시작년월이 클 경우 : 데이터 백업후 생성.
      -- 기존자료 BACKUP.
    ...
    ELSIF V_LAST_START_YYYYMM = P_STD_YYYYMM THEN  -- 기준년월과 시작년월이 같은 경우.
      PAY_HEADER_UPDATE
            ( V_PAY_HEADER_ID
            , P_PERSON_ID
            , P_CORP_ID
...
-- 급여 마스터 라인 저장(기준년월과 최종 시작년월 비교하여 처리).
  ...
  BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE_APPLICATION_ERROR(-20001, EAPP_MESSAGE_G.RETURN_TEXT_F(USERENV_G.GET_TERRITORY_S_F, 'FCM_10236', '&&VALUE:=START YYYYMM(시작년월)'));
    END;
    -- 해당 급여년월의 급여 마스터 헤더 ID 값.
    V_PAY_HEADER_ID := PAY_HEADER_ID_CHECK_F
                        ( P_PERSON_ID => P_PERSON_ID
