...
-- 근무변경신청 조회.
  ...
  IF W_APPROVE_STATUS IN('A') THEN
    ...
    OPEN P_CURSOR FOR
      ...
      , HT.CORP_ID
      , HT.HOLY_TYPE
      , HRM_COMMON_G.CODE_NAME_F('HOLY_TYPE', HT.HOLY_TYPE, HT.SOB_ID, HT.ORG_ID) HOLY_TYPE_NAME
      , TRUNC(HT.START_DATE) AS START_DATE
      , TRUNC(HT.END_DATE) AS END_DATE
      , HT.DESCRIPTION
      , 'N' AS APPROVE_YN
      , HT.APPROVE_STATUS
      , HRM_COMMON_G.CODE_NAME_F('DUTY_APPROVE_STATUS', HT.APPROVE_STATUS, HT.SOB_ID, HT.ORG_ID) APPROVE_STATUS_NAME
      , HRM_PERSON_MASTER_G.NAME_F(HT.APPROVED_PERSON_ID) APPROVERD_PERSON_NAME
      , HRM_PERSON_MASTER_G.NAME_F(HT.CONFIRMED_PERSON_ID) CONFIRMED_PERSON_NAME
      , HT.HOLY_TYPE_ID
      , WT.WORK_TYPE_GROUP
  ...
  BEGIN
   BEGIN
      SELECT HRM_MANAGER_G.USER_CAP_F
                             (PM.WORK_CORP_ID --< PM.CORP_ID [2011-06-28] 수정
                             , TRUNC(HT.START_DATE)
                             , TRUNC(HT.END_DATE)
                             , '20'
                             , P_CONNECT_PERSON_ID
      ...
      FROM HRD_HOLY_TYPE HT
        ...
        , (-- 시점 인사내역.
            ...
            WHERE HL.HISTORY_LINE_ID  IN ( SELECT MAX(S_HL.HISTORY_LINE_ID) AS HISTORY_LINE_ID
                                            FROM HRM_HISTORY_LINE S_HL
                                           WHERE S_HL.CHARGE_DATE            <= ( SELECT TRUNC(HT1.END_DATE) END_DATE
