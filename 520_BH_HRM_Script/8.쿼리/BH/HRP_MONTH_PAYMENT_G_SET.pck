...
---------------------------------------------------------------------------
    ...
    G_TOTAL_DAY := V_END_DATE - V_START_DATE + 1;
...
-- 3 기본급이외 기본자료 생성
  ...
  BEGIN
    ...
    LOOP
      ...
      ELSIF C1.ALLOWANCE_CLASS = 'WORKING_RATE' THEN
      -- 74 복지수당.
        WORKING_RATE( P_CORP_ID
                    , P_PAY_YYYYMM
                    , P_WAGE_TYPE
                    , C1.ALLOWANCE_ID
                    , C1.FORMULA3    -- 근무율.
  ...
  BEGIN
    FOR C1 IN ( SELECT MP.MONTH_PAYMENT_ID
                  ...
                  FROM HRP_MONTH_PAYMENT MP
                    ...
                    , HRM_PAY_TYPE_V PT
                 WHERE MP.JOB_CATEGORY_ID         = JCC.JOB_CATEGORY_ID
                   AND MP.PAY_TYPE                = PT.PAY_TYPE
                   AND MP.SOB_ID                  = PT.SOB_ID
                   AND MP.ORG_ID                  = PT.ORG_ID
                   AND MP.PAY_YYYYMM              = P_PAY_YYYYMM
    ...
    LOOP
      ...
      LOOP
        ...
        BEGIN
          ...
          WHERE AH.JOB_CATEGORY_CODE  = C1.JOB_CATEGORY_CODE
            AND AH.SOB_ID         = C1.SOB_ID
            AND AH.ORG_ID         = C1.ORG_ID
            AND AH.START_TIME     <= R1.OT_TIME
            AND AH.END_TIME       > R1.OT_TIME
    ...
    LOOP
      ...
      BEGIN
        ...
        WHERE ASD.PAY_TYPE              = C1.PAY_TYPE
          ...
          AND ASD.EFFECTIVE_DATE_FR     <= C1.STANDARD_DATE
          AND (ASD.EFFECTIVE_DATE_TO IS NULL OR ASD.EFFECTIVE_DATE_TO >= C1.STANDARD_DATE)
        ;
      EXCEPTION WHEN OTHERS THEN
        NULL;
      END;
