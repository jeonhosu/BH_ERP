...
/*DBMS_OUTPUT.PUT_LINE('휴일 야할 : OT_START =>' || TO_CHAR(V_OT_START_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
        ...
        -- 4. 근무후 연장 계산.
				IF C1.OT_YN = 'Y' AND C1.CAL_TYPE = 'OA' AND (C1.HOLY_TYPE = 'NIGHT' OR W_ALL_NIGHT_YN = 'Y')
					AND V_CLOSE_TIME IS NOT NULL AND W_AFTER_OT_START IS NOT NULL AND W_AFTER_OT_END IS NOT NULL THEN
				  -- 연장 신청 적용한 시작 시간.
					V_OT_START_TIME := OT_START_TIME_F( P_OPEN_TIME => V_OPEN_TIME
					                                  , P_PL_START_TIME => W_AFTER_OT_START
...
-- 연장 종료 시간 정리.
  ...
  AS
	...
	BEGIN
    DBMS_OUTPUT.PUT_LINE('CLOSE TIME : ' || TO_CHAR(P_CLOSE_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
                         ', PL END : ' || TO_CHAR(P_PL_END_TIME, 'YYYY-MM-DD HH24:MI:SS') || 
                         ', DEF END : ' || TO_CHAR(P_DEFAULT_RETURN_TIME, 'YYYY-MM-DD HH24:MI:SS'));
	  BEGIN
		  SELECT CASE
