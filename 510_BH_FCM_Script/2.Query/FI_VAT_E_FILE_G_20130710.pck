...
IF t_E_FILE_NO_DEDUCTION_YN = 'Y' THEN  --전자신고파일생성대상여부_공제받지못할매입세액명세서
    ...
    SELECT
        ...
        || LPAD(REPLACE(LPAD(NVL(GL_AMOUNT, 0), 15, '0'), '-', ''), 15, '-') --4.공급가액합계_세금계산서	NUMBER	15
        || LPAD(REPLACE(LPAD(NVL(VAT_AMOUNT, 0), 15, '0'), '-', ''), 15, '-') --5.매입세액합계_세금계산서	NUMBER	15
    ...
    FROM
        (
            ...
            FROM FI_SLIP_LINE A
            WHERE A.SLIP_LINE_ID NOT IN(201426, 199895)
                AND A.SOB_ID = W_SOB_ID
                AND A.ORG_ID = W_ORG_ID 
    ...
    FOR REC_2_12 IN (
        ...
        SELECT
            ...
            || LPAD(REPLACE(LPAD(NVL(GL_AMOUNT, 0), 13, '0'), '-', ''), 13, '-')   --5.공급가액	NUMBER	13
            || LPAD(REPLACE(LPAD(NVL(VAT_AMOUNT, 0), 13, '0'), '-', ''), 13, '-')  --6.매입세액	NUMBER	13    
...
IF t_E_FILE_SUM_CALC_YN = 'Y' THEN      --전자신고파일생성대상여부_계산서합계표
    ...
    FOR REC_TAX IN (
        ...
        SELECT
            ...
            || LPAD(NVL(SX1.VAT_COUNT, 0), 6, '0')         -- 10.계산서매수합계 
