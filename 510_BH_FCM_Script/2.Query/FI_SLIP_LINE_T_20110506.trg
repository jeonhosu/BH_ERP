...
-- 관리항목 저장.
  IF DELETING THEN
    -- 기존 자료 삭제.
    IF  :OLD.CONFIRM_YN = 'Y' THEN  -- 확정전표만 적용.
      FOR C1 IN ( SELECT SMI.SLIP_LINE_ID
                    ...
                    FROM FI_SLIP_MANAGEMENT_ITEM SMI
                  WHERE SMI.SLIP_LINE_ID         = :OLD.SLIP_LINE_ID
                    AND SMI.SOB_ID               = :OLD.SOB_ID
      ...
      LOOP
        BEGIN 
          FI_MANAGEMENT_BALANCE_G.MANAGEMENT_BALANCE_SAVE_P
              ( P_GUBUN             => 'D'
              , P_GL_DATE           => :OLD.GL_DATE
              , P_SOB_ID            => :OLD.SOB_ID
              , P_ORG_ID            => :OLD.ORG_ID
              , P_ACCOUNT_CONTROL_ID  => :OLD.ACCOUNT_CONTROL_ID
              , P_MANAGEMENT_ID     => C1.MANAGEMENT_ID
              , P_MANAGEMENT_VALUE  => C1.MANAGEMENT_VALUE
              , P_ACCOUNT_DR_CR     => :OLD.ACCOUNT_DR_CR
              , P_GL_AMOUNT         => :OLD.GL_AMOUNT
              , P_CURRENCY_CODE     => :OLD.CURRENCY_CODE
              , P_EXCHANGE_RATE     => :OLD.EXCHANGE_RATE
              , P_GL_CURRENCY_AMOUNT  => :OLD.GL_CURRENCY_AMOUNT
              , P_ACCOUNT_CODE      => :OLD.ACCOUNT_CODE
              , P_USER_ID           => :OLD.CREATED_BY
              );
          DELETE FROM FI_SLIP_MANAGEMENT_ITEM MI
          WHERE MI.SLIP_LINE_ID         = C1.SLIP_LINE_ID
            AND MI.SOB_ID               = C1.SOB_ID
            AND MI.MANAGEMENT_SEQ       = C1.MANAGEMENT_SEQ
          ;
        EXCEPTION WHEN OTHERS THEN
          NULL;
        END;
  ...
  ELSE
    IF  :OLD.CONFIRM_YN = 'Y' THEN  -- 확정전표만 적용.
      IF UPDATING THEN -- UPDATE 일 경우 기존 자료 삭제 후 INSERT.
        FOR C1 IN ( SELECT SMI.SLIP_LINE_ID
                         , SMI.SOB_ID
                         , SMI.MANAGEMENT_SEQ
                         , SMI.MANAGEMENT_ID
      ...
      LOOP
        ...
        -- 전표 관리항목 내역 저장.
          ...
          FI_MANAGEMENT_BALANCE_G.MANAGEMENT_BALANCE_SAVE_P
              ...
              , P_MANAGEMENT_ID     => V_MANAGEMENT_ID
              , P_MANAGEMENT_VALUE  => V_MANAGEMENT_VALUE
              , P_ACCOUNT_DR_CR     => :NEW.ACCOUNT_DR_CR
              , P_GL_AMOUNT         => :NEW.GL_AMOUNT
              , P_CURRENCY_CODE     => :NEW.CURRENCY_CODE
              , P_EXCHANGE_RATE     => :NEW.EXCHANGE_RATE
              , P_GL_CURRENCY_AMOUNT  => :NEW.GL_CURRENCY_AMOUNT
