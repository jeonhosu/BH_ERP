...
PROCEDURE CREATE_SURTAX_CARD(
    ...
    , W_VAT_LEVIER_GB       IN  FI_SURTAX_CARD.VAT_LEVIER_GB%TYPE       -- 과세자구분 
    , W_VAT_MAKE_GB         IN  FI_SURTAX_CARD.VAT_MAKE_GB%TYPE DEFAULT '01'    --신고구분(01 : 정기신고, 01:수정신고)
    , W_MODIFY_DESC         IN  FI_SURTAX_CARD.MODIFY_DESC%TYPE                 -- 수정신고 사유 -- 
    ...
    , O_MODIFY_DEGREE       OUT NUMBER                                  -- 수정신고 차수 --
    , O_MODIFY_DESC         OUT VARCHAR2                                -- 수정신고 사유 --
);
...
PROCEDURE LIST_SURTAX_CARD(
    ...
    , W_MODIFY_DEGREE       IN  FI_SURTAX_CARD.MODIFY_DEGREE%TYPE               -- 수정신고 차수 -- 
);
...
PROCEDURE UPDATE_SURTAX_CARD(
    ...
    , P_A_TAX_RECEIVE_DELAY_VAT IN FI_SURTAX_CARD.A_TAX_RECEIVE_DELAY_VAT%TYPE  -- 세금계산서지연수취세액.
    ...
    , P_P_EXPORT_IMPORT_POSTPONE_VAT  IN FI_SURTAX_CARD.P_EXPORT_IMPORT_POSTPONE_VAT%TYPE DEFAULT 0 
);
...
t_COL42_1   FI_SURTAX_CARD.COL42_1%TYPE;    --기타공제_재활용_금액
t_COL42_2   FI_SURTAX_CARD.COL42_2%TYPE;    --기타공제_신용카드_고정자산매입_세액
...
t_COL49_1   FI_SURTAX_CARD.COL49_1%TYPE;    --공제받지못할매입세액_공제받지못할_공통매입세액 면세사업분 금액
t_COL49_2   FI_SURTAX_CARD.COL49_2%TYPE;    --공제받지못할매입세액_공제받지못할_공통매입세액 면세사업분 세액
...
t_COL18_2   FI_SURTAX_CARD.COL18_2%type;    -- 18.경감공제세액-기타경감/공제세액;
t_COL20_2   FI_SURTAX_CARD.COL20_2%TYPE;    -- 20. 경감공제_합계 
t_COL53_2   FI_SURTAX_CARD.COL53_2%TYPE;    -- 53. 기타경감공제세액_전자세금계산서발급세액공제_세액 
t_COL57_2   FI_SURTAX_CARD.COL57_2%TYPE;    -- 57. 기타경감공제세액_합계_세액 

t_COL29_3   FI_SURTAX_CARD.COL29_3%TYPE;    --과세표준_금액4

t_COL73     FI_SURTAX_CARD.COL73%TYPE;    --계산서발급금액
...
BEGIN
    ...
    IF W_VAT_MAKE_GB = '01' THEN
      ...
      WHERE   SOB_ID  = W_SOB_ID  --회사아이디
          ...
          AND VAT_MAKE_GB  = W_VAT_MAKE_GB --신고구분(01 : 정기신고)
      ;
    ELSE
      -- 수정신고 -- 
      BEGIN
        SELECT NVL(MAX(SC.MODIFY_DEGREE), 0) + 1 AS NEXT_MODIFY_DEGREE
         ...
         WHERE SOB_ID  = W_SOB_ID  --회사아이디
           ...
           AND TAX_CODE   = W_TAX_CODE   --사업장아이디
           AND VAT_MNG_SERIAL   = W_VAT_MNG_SERIAL      --부가세신고기간구분번호
           AND VAT_MAKE_GB  = W_VAT_MAKE_GB --신고구분(01 : 정기신고, 02:수정신고)
           AND CLOSED_FLAG  = 'Y'           -- 마감처리된 데이터 기준 -- 
        ;
      EXCEPTION WHEN OTHERS THEN
        O_MODIFY_DEGREE := 1;
      END;
    ...
    -- 총괄납부사업자(주)의 경우 1건의 데이터만 존재해야 함
    -- 기존 데이터 존재시 ERROR 
      t_RECORD_COUNT := 0;
      ...
      BEGIN
        SELECT COUNT(SC.TAX_CODE) AS RECORD_COUNT
         ...
         WHERE  SC.TAX_CODE             = VRM.TAX_CODE
            ...
            AND SC.VAT_MAKE_GB          = W_VAT_MAKE_GB --신고구분(01 : 정기신고)
      ...
      IF t_RECORD_COUNT > 0 THEN
        O_STATUS := 'F';
        O_MESSAGE := '이미 [총괄납부사업자의 종사업자]의 데이터가 존재합니다. 중복 생성할 수 없습니다.';
        RETURN;
      END IF;
    END IF;
        
    
    -- 해당 신고기간에 대한 기존 신고서 최종구분 UPDATE -- 
    BEGIN
    ...
    SELECT
        ...
        , W_VAT_MAKE_GB     --신고구분(01 : 정기신고, 02:수정신고)
        , O_MODIFY_DEGREE   -- 수정신고 차수 --
        , O_MODIFY_DESC     -- 수정신고 사유 -- 
        
        , W_TITLE_1_1       --신고기간_시작
        , W_TITLE_1_2       --신고기간_종료
        , A.CORP_NAME || ' ' || B.OPERATING_UNIT_NAME       --상호(법인명)
...
--라인수만 길 뿐이지 동일한 QUERY가 반복된다.
    ...
    t_COL15_1 := NVL(t_COL10_1, 0) + NVL(t_COL11_1, 0) + NVL(t_COL14_1, 0); --매입_합계_금액
    t_COL15_2 := NVL(t_COL10_2, 0) + NVL(t_COL11_2, 0) + NVL(t_COL14_2, 0); --매입_합계_세액
    ...
    BEGIN
      -- 전호수 추가 : 공제받지못할 매입세액 변경 --
      SELECT                
            SUM(NVL(A.GL_AMOUNT, 0)) AS GL_AMOUNT     --공급가액
          , SUM(NVL(A.VAT_AMOUNT, 0)) AS VAT_AMOUNT    --세액   
      ...
      WHERE A.SOB_ID = W_SOB_ID
          ...
          AND A.VAT_DATE_TO = W_TITLE_1_2   --신고기준일자
          AND A.NO_DED_CODE = '990'
      ;
      /*-- 전호수 주석 : 공제받지못할 매입세액 변경 --
      SELECT                
            SUM(TO_NUMBER(REPLACE(TRIM(NVL(A.REFER5, 0)), ',', ''))) AS GL_AMOUNT     --공급가액
          , SUM(TO_NUMBER(REPLACE(TRIM(NVL(A.REFER8, 0)), ',', ''))) AS VAT_AMOUNT    --세액   
      INTO t_COL16_1, t_COL16_2
    ...
    t_COL17_1 := NVL(t_COL15_1, 0) - NVL(t_COL51_1, 0); --매입_차감계_금액
    t_COL17_2 := NVL(t_COL15_2, 0) - NVL(t_COL51_2, 0); --매입_차감계_세액
    
    /*-- 전호수 주석 : 공제받지못할 매입세액 변경 --
    t_COL17_1 := NVL(t_COL15_1, 0) - NVL(t_COL16_1, 0); --매입_차감계_금액
    t_COL17_2 := NVL(t_COL15_2, 0) - NVL(t_COL16_2, 0); --매입_차감계_세액
    */
/*
    -- 18-53 기타 경감/공제 세액명세  
    t_COL18_2 := 0;  -- 경감공제세액 - 기타 경감/공제세액;
    t_COL20_2 := 0;  -- 경감공제세액 합계 ;
    t_COL53_2 := 0;  -- (18)기타경감/공제세액명세 - 전자세금계산서 발급세액공제;
    t_COL57_2 := 0;  -- (18)합계;
    BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
      t_COL53_2 := 0;
    END;
    t_COL18_2 := t_COL53_2;  -- 경감공제세액 - 기타 경감/공제세액;
    t_COL20_2 := t_COL53_2;  -- 경감공제세액 합계 ;
    t_COL57_2 := t_COL53_2;  -- (18)합계;
    */
    ...
    BEGIN
      ...
      WHERE A.SOB_ID = W_SOB_ID
          ...
          --AND TO_DATE(A.REFER2) BETWEEN TO_DATE('20110401') AND TO_DATE('20110630')   --신고기준일자 
          AND TO_DATE(A.REFER1) BETWEEN W_TITLE_1_1 AND W_TITLE_1_2   --신고기준일자
      ;               
    EXCEPTION WHEN OTHERS THEN
      t_COL73 := 0;
    END;
    ...
    SET   COL1_1 = t_COL1_1   --과세_세금계산서발급분_금액
        ...
        , COL9_1 = NVL(t_COL1_1, 0) + NVL(t_COL4_1, 0) + NVL(t_COL5_1, 0) + NVL(t_COL6_1, 0)    --합계_금액
        , COL30 = NVL(t_COL1_1, 0) + NVL(t_COL4_1, 0) + NVL(t_COL5_1, 0) + NVL(t_COL6_1, 0)     --과세표준_합계
        , COL9_2 = NVL(t_COL1_2, 0) + NVL(t_COL4_2, 0)  --합계_세액
        ...
        , COL39_1 = NVL(t_COL14_1, 0) - NVL(t_COL40_1, 0)   --기타공제_신용카드_일반매입_금액
        , COL39_2 = NVL(t_COL14_2, 0) - NVL(t_COL40_2, 0)  --기타공제_신용카드_일반매입_세액
        , COL40_1 = t_COL40_1   --기타공제_신용카드_고정자산매입_금액
        , COL40_2 = t_COL40_2   --기타공제_신용카드_고정자산매입_세액                              
        
        , COL42_1 = t_COL42_1   -- 기타공제 - 재활용폐자원 금액 
        , COL42_2 = t_COL42_2   -- 기타공제 - 재활용폐자원 세액 
        
        , COL47_1 = NVL(t_COL14_1, 0) + NVL(t_COL42_1, 0)    --기타공제_합계_금액
        , COL47_2 = NVL(t_COL14_2, 0) + NVL(t_COL42_2, 0)    --기타공제_헙계_세액 
        ...
        , COL48_1 = t_COL48_1   --공제받지못할매입세액_공제받지못할_금액
        , COL48_2 = t_COL48_2   --공제받지못할매입세액_공제받지못할_세액
        , COL49_1 = t_COL49_1   --공제받지못할매입세액_공제받지못할_금액
        , COL49_2 = t_COL49_2   --공제받지못할매입세액_공제받지못할_세액
        , COL51_1 = t_COL51_1   --공제받지못할매입세액_합계_금액
        , COL51_2 = t_COL51_2   --공제받지못할매입세액_합계_세액
        
        , COL17_1 = t_COL17_1   --매입_차감계_금액
        , COL17_2 = t_COL17_2   --매입_차감계_세액
        
        , COL_DA = (NVL(t_COL1_2, 0) + NVL(t_COL4_2, 0)) - (NVL(t_COL15_2, 0) - NVL(t_COL16_2, 0))   --납부세액
        , COL25 = (NVL(t_COL1_2, 0) + NVL(t_COL4_2, 0)) - (NVL(t_COL15_2, 0) - NVL(t_COL16_2, 0))   --차가감하여납부할세액
...
BEGIN
    ...
    SELECT
        ...
        -- 2013추가 --
        , A_TAX_INV_SUM_BAD_AMT_1  -- 가산세명세 - 세금계산서 합계표 지연제출(AMT-금액) 
        , A_TAX_INV_SUM_BAD_VAT_1  -- 가산세명세 - 세금계산서 합계표 지연제출(VAT-세액)
        , A_REPORT_BAD_AMT_1       -- 가산세명세 - 신고불성실(AMT-금액)-무신고(일반) 
        , A_REPORT_BAD_VAT_1       -- 가산세명세 - 신고불성실(VAT-세액)-무신고(일반) 
        , A_REPORT_BAD_AMT_2       -- 가산세명세 - 신고불성실(AMT-금액)-무신고(부당) 
        , A_REPORT_BAD_VAT_2       -- 가산세명세 - 신고불성실(VAT-세액)-무신고(부당)
        , A_REPORT_BAD_AMT_3       -- 가산세명세 - 신고불성실(AMT-금액)-과소/초과 환급신고(일반) 
        , A_REPORT_BAD_VAT_3       -- 가산세명세 - 신고불성실(VAT-세액)-과소/초과 환급신고(일반) 
        , A_REPORT_BAD_AMT_4       -- 가산세명세 - 신고불성실(AMT-금액)-과소/초과 환급신고(부당)
        , A_REPORT_BAD_VAT_4       -- 가산세명세 - 신고불성실(VAT-세액)-과소/초과 환급신고(부당) 
        , A_REALTY_LEASE_UNREPORT_AMT  -- 가산세명세 - 부동산임대공급가액명세서 불성실(AMT-금액) 
        , A_REALTY_LEASE_UNREPORT_VAT  -- 가산세명세 - 부동산임대공급가액명세서 불성실(VAT-세액) 
        ...
        , P_EXPORT_IMPORT_POSTPONE_VAT  -- 매입 수출기업 수입 납부유예.
    FROM FI_SURTAX_CARD SC
    WHERE SOB_ID = W_SOB_ID
        AND ORG_ID = W_ORG_ID
        AND TAX_CODE = W_TAX_CODE
...
BEGIN
  OPEN P_CURSOR FOR
    ...
    WHERE SOB_ID = W_SOB_ID
        ...
        AND TAX_CODE = W_TAX_CODE
        AND VAT_MNG_SERIAL = W_VAT_MNG_SERIAL
        AND VAT_MAKE_GB = '01' --신고구분(01 : 정기신고, 02:수정시곤)
    ;
...
BEGIN
    ...
    SET
        ...
        , COL64_1	= (NVL(P_A_REPORT_BAD_AMT_1, 0) + NVL(P_A_REPORT_BAD_AMT_2, 0) + 
                     NVL(P_A_REPORT_BAD_AMT_3, 0) + NVL(P_A_REPORT_BAD_AMT_4, 0)) -- P_COL64_1	--가산세명세_신고불성실_금액 합계
        , COL64_2	= (NVL(P_A_REPORT_BAD_VAT_1, 0) + NVL(P_A_REPORT_BAD_VAT_2, 0) + 
                     NVL(P_A_REPORT_BAD_VAT_3, 0) + NVL(P_A_REPORT_BAD_VAT_4, 0))  -- P_COL64_2	--가산세명세_신고불성실_세액 합계 
        ...
        , R_ORIGIN_PLACE_VAT       = P_R_ORIGIN_PLACE_VAT  -- 원산지확인서 발급공제세액.
        , A_TAX_RECEIVE_DELAY_AMT  = P_A_TAX_RECEIVE_DELAY_AMT  -- 세금계산서지연수취금액.
        , A_TAX_RECEIVE_DELAY_VAT  = P_A_TAX_RECEIVE_DELAY_VAT  -- 세금계산서지연수취세액.
        ...
        , P_EXPORT_IMPORT_POSTPONE_VAT  = NVL(P_P_EXPORT_IMPORT_POSTPONE_VAT, 0) 
...
BEGIN
  OPEN P_CURSOR1 FOR
    ...
    WHERE SC.SOB_ID             = W_SOB_ID
        ...
        AND SC.VAT_MAKE_GB      != '01' --신고구분(01 : 정기신고) 제외 -- 
