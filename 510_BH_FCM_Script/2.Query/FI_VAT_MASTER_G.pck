...
-- 사업자 정보 조회 2.
  PROCEDURE SELECT_BUSINESS_MASTER
            ( P_CURSOR1           OUT TYPES.TCURSOR1
            , W_TAX_CODE          IN VARCHAR2
            , W_SOB_ID            IN NUMBER
            );
...
-- 부가세 신고기간 조회.
  ...
  AS
    ...
    V_PERIOD_DESC       VARCHAR2(50);
  BEGIN
    V_PERIOD_YEAR := TO_CHAR(W_ISSUE_DATE_FR, 'YYYY');
    BEGIN
      SELECT  FC.CODE_NAME
        INTO V_PERIOD_DESC
        FROM FI_COMMON FC
      WHERE FC.GROUP_CODE         = 'VAT_PERIOD'
        AND FC.SOB_ID             = W_SOB_ID
        AND FC.VALUE1             <= TO_CHAR(W_ISSUE_DATE_FR, 'MM')             
        AND FC.VALUE2             >= TO_CHAR(W_ISSUE_DATE_TO, 'MM')
        AND FC.DEFAULT_FLAG       = 'Y'
    ...
    EXCEPTION WHEN OTHERS THEN
      V_PERIOD_DESC :='1 기';
    END;
    V_PERIOD_DESC := EAPP_MESSAGE_G.RETURN_TEXT_F(USERENV_G.GET_TERRITORY_S_F, 'FCM_10295', '&&YEAR:=' || V_PERIOD_YEAR) || ' ' || V_PERIOD_DESC;
    RETURN V_PERIOD_DESC;
  END VAT_PERIOD_F;
...
-- 부가세 신고기간 예정/확정 조회.
  ...
  AS
  BEGIN
    O_PERIOD := VAT_PERIOD_TYPE_F
                  ( W_ISSUE_DATE_FR
                  , W_ISSUE_DATE_TO
                  , W_SOB_ID                  
                  );
  END VAT_PERIOD_TYPE_P;
  ...
  BEGIN
    ...
    BEGIN
      SELECT  FC.CODE_NAME
        INTO V_PERIOD_DESC
        FROM FI_COMMON FC
      WHERE FC.GROUP_CODE         = 'VAT_PERIOD_TYPE'
        AND FC.SOB_ID             = W_SOB_ID
        AND FC.VALUE1             <= TO_CHAR(W_ISSUE_DATE_FR, 'MM')             
        AND FC.VALUE2             >= TO_CHAR(W_ISSUE_DATE_TO, 'MM')
        AND FC.ENABLED_FLAG       = 'Y'
  ...
  BEGIN
    OPEN P_CURSOR1 FOR
      SELECT BM.BUSINESS_NAME
           ...
           , CM.CORPORATE_NAME
           , CM.LEGAL_NUM
           , SUBSTR(REPLACE(BM.VAT_NUM, '-', ''), 1, 1) AS VAT_NUM_1
           , SUBSTR(REPLACE(BM.VAT_NUM, '-', ''), 2, 1) AS VAT_NUM_2
           , SUBSTR(REPLACE(BM.VAT_NUM, '-', ''), 3, 1) AS VAT_NUM_3
           , SUBSTR(REPLACE(BM.VAT_NUM, '-', ''), 4, 1) AS VAT_NUM_4
           , SUBSTR(REPLACE(BM.VAT_NUM, '-', ''), 5, 1) AS VAT_NUM_5
           , SUBSTR(REPLACE(BM.VAT_NUM, '-', ''), 6, 1) AS VAT_NUM_6
           , SUBSTR(REPLACE(BM.VAT_NUM, '-', ''), 7, 1) AS VAT_NUM_7
           , SUBSTR(REPLACE(BM.VAT_NUM, '-', ''), 8, 1) AS VAT_NUM_8
           , SUBSTR(REPLACE(BM.VAT_NUM, '-', ''), 9, 1) AS VAT_NUM_9
           , SUBSTR(REPLACE(BM.VAT_NUM, '-', ''), 10, 1) AS VAT_NUM_10     
           , FB.BANK_NAME
           , BA.BANK_ACCOUNT_NUM
