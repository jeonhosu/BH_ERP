...
-- 전표 생성 데이터 삽입.
  ...
  BEGIN
    ...
    IF NVL(P_ACCOUNT_DR_CR, '0') NOT IN ('1', '2') THEN
      O_MESSAGE := 'Account Code : ' || P_ACCOUNT_CODE || ' ' || CHR(10) || EAPP_MESSAGE_G.RETURN_TEXT_F(USERENV_G.GET_TERRITORY_S_F, 'FCM_10122', NULL);
      RETURN;
    END IF;    
    -- 계정ID.
    BEGIN
    ...
    IF V_ACCOUNT_CONTROL_ID = -1 THEN
      O_MESSAGE := 'Account Code : ' || P_ACCOUNT_CODE || ' ' || CHR(10) || EAPP_MESSAGE_G.RETURN_TEXT_F(USERENV_G.GET_TERRITORY_S_F, 'FCM_10413', NULL);
      RETURN;
    END IF;    
