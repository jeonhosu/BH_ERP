  ...
  BEGIN
    ...
    OPEN P_CURSOR FOR
      SELECT CASE
          ...
          , SX1.CURRENCY_CODE
          , NVL(SX1.DR_CURR_SUM, 0) AS DR_CURR_SUM
          , NVL(SX1.CR_CURR_SUM, 0) AS CR_CURR_SUM
        ...
        FROM ( SELECT MBG.GL_DATE + 1 AS GL_DATE
                ...
                SELECT SL.GL_DATE AS  GL_DATE         -- 회계일자(Account Date)
                     ...
                     , SL.CURRENCY_CODE
                     , DECODE(SL.ACCOUNT_DR_CR, '1', SL.GL_CURRENCY_AMOUNT, 0) AS DR_CURR_SUM      -- 차변 외화금액.
                     , DECODE(SL.ACCOUNT_DR_CR, '2', SL.GL_CURRENCY_AMOUNT, 0) AS CR_CURR_SUM      -- 대변 외화금액.
