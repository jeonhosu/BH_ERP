...
-- 전표 라인 조회 : 분개 정보.
  PROCEDURE SELECT_SLIP_LINE_JOURNALIZE
            ...
            , P_SOB_ID            IN NUMBER
            , P_ORG_ID            IN NUMBER
            , P_SLIP_DETAIL_YN    IN VARCHAR2 DEFAULT 'N'
            );
...
-- 전표 조회 : 관리항목별.
  PROCEDURE SELECT_SLIP_MANAGEMENT
            ...
            , P_GL_CURR_AMOUNT_FR IN NUMBER
            , P_GL_CURR_AMOUNT_TO IN NUMBER
...
-- 전표 헤더 삽입.
  ...
  BEGIN
    ...
    -- 전표일자와 전표번호 체크.
    IF TO_CHAR(P_GL_DATE, 'YYYYMMDD') <> SUBSTR(P_GL_NUM, 4, 8) THEN
      RAISE_APPLICATION_ERROR(-20001, EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10453'));
      RETURN;
    END IF;
...
-- 전표 헤더 수정.
  ...
  BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
      V_GL_NUM := '-';
    END;
    IF TO_CHAR(V_GL_DATE, 'YYYYMMDD') <> SUBSTR(V_GL_NUM, 4, 8) THEN
      RAISE_APPLICATION_ERROR(-20001, EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10453'));
      RETURN;
    END IF;    
    
    UPDATE FI_SLIP_HEADER
      SET /* -- 전호수 수정 : 전표일자와 전표번호 불일치 발생 => 전표일자 수정이 문제인거 같아 전표일자 수정 안함.
          SLIP_DATE           = P_SLIP_DATE
    ...
    ;
    /* -- 전호수 수정 : 전표일자와 전표번호 불일치 발생 => 전표일자 수정이 문제인거 같아 전표일자 수정 안함.
    -- 라인 업데이트.
...
-- 전표 헤더 삭제.
  ...
  BEGIN
    ...
    BEGIN
      ...
      ;
    EXCEPTION WHEN OTHERS THEN
      RAISE_APPLICATION_ERROR(-20001, EAPP_MESSAGE_G.RETURN_MSG_F('FCM_10128'));
      RETURN;
    END;
    IF GL_FISCAL_PERIOD_G.PERIOD_STATUS_F(NULL, V_GL_DATE, V_SOB_ID, V_ORG_ID) IN('C', 'N') THEN
      RAISE_APPLICATION_ERROR(ERRNUMS.Data_Not_opened_Code, ERRNUMS.Data_Not_Opened_Desc);
    END IF;
...
-- 전표라인 수정.
  ...
  BEGIN
    ...
    IF GL_FISCAL_PERIOD_G.PERIOD_STATUS_F(NULL, V_GL_DATE, W_SOB_ID, W_ORG_ID) IN('C', 'N') THEN
      RAISE_APPLICATION_ERROR(ERRNUMS.Data_Not_opened_Code, ERRNUMS.Data_Not_Opened_Desc);
    END IF;
...
-- 전표 라인 조회 : 분개 정보.
  ...
  BEGIN
    OPEN P_CURSOR2 FOR
      SELECT NVL(SL.GL_DATE, NULL) AS GL_DATE     
           , SL.ACCOUNT_CODE     
           , CASE
           ...
           , NVL(SL.SLIP_HEADER_ID, NULL) AS SLIP_HEADER_ID
        FROM FI_SLIP_LINE                          SL
           , FI_SLIP_HEADER                        SH
           , FI_ACCOUNT_CONTROL                    AC
      ...
      WHERE SL.SLIP_HEADER_ID                   = SH.SLIP_HEADER_ID
        ...
        AND ((P_SLIP_DETAIL_YN                  = 'Y' AND 1 = 1)
        OR   (P_SLIP_DETAIL_YN                  = 'N'
        ...
        AND EXISTS
              ( SELECT 'X'
                ...
                WHERE FSL.SLIP_HEADER_ID        = SH.SLIP_HEADER_ID
                  AND FSL.ACCOUNT_CODE          BETWEEN NVL(P_ACCOUNT_CODE_FR, FSL.ACCOUNT_CODE) AND NVL(P_ACCOUNT_CODE_TO, FSL.ACCOUNT_CODE)
              )
...
/*    RAISE_APPLICATION_ERROR(-20001, '//' || P_MANAGEMENT_DESC);*/
    OPEN P_CURSOR2 FOR
           ...
           , ( SELECT S_SM.SLIP_LINE_ID
                 ...
                 FROM (SELECT SM.SLIP_LINE_ID
                            ...
                            , CASE MC.DATA_TYPE
                                WHEN 'NUMBER' THEN 
                                  CASE
                                    WHEN IS_NUMBER_F(SM.MANAGEMENT_VALUE) = 'N' THEN '-1'
                                    ELSE TO_CHAR(TO_NUMBER(SM.MANAGEMENT_VALUE), 'FM999,999,999,999,999,999')
                                  END
                                WHEN 'RATE' THEN 
                                   CASE 
                                     WHEN IS_NUMBER_F(SM.MANAGEMENT_VALUE) = 'N' THEN '-1'
                                ...
                                ELSE
                                  /*CASE
                                    WHEN MC.LOOKUP_TYPE = 'CUSTOMER' THEN SM.MANAGEMENT_VALUE || DECODE(SM.MANAGEMENT_VALUE, NULL, NULL, '-')
                                    ELSE NULL
                                  END ||*/ NVL(FI_ACCOUNT_CONTROL_G.ITEM_DESC_F(MC.LOOKUP_TYPE, SM.MANAGEMENT_VALUE, SM.SOB_ID), SM.MANAGEMENT_VALUE)
      ...
      WHERE SH.SLIP_HEADER_ID                   = SL.SLIP_HEADER_ID
        ...
        AND ((P_REMARK                          IS NULL 
         AND 1                                  = 1)
         OR (P_REMARK                           IS NOT NULL
         AND SL.REMARK                          LIKE P_REMARK || '%'))
        AND SL.CURRENCY_CODE                    = NVL(P_CURRENCY_CODE, SL.CURRENCY_CODE)
        AND NVL(SL.GL_CURRENCY_AMOUNT, 0)       BETWEEN NVL(P_GL_CURR_AMOUNT_FR, NVL(SL.GL_CURRENCY_AMOUNT, 0)) AND NVL(P_GL_CURR_AMOUNT_TO, NVL(SL.GL_CURRENCY_AMOUNT, 0))
        AND ((P_MANAGEMENT_ID IS NULL AND 1 = 1)
          OR (P_MANAGEMENT_ID IS NOT NULL 
          AND EXISTS 
             ( SELECT 'X'
                 FROM FI_SLIP_MANAGEMENT_ITEM SM
                   , FI_MANAGEMENT_CODE_V MC
               WHERE SM.MANAGEMENT_ID       = MC.MANAGEMENT_ID
                 ...
                 AND ((NVL(P_LOOKUP_YN, 'N')= 'Y'
                   AND (( P_MANAGEMENT_VALUE IS NULL AND 1 = 1)
                     OR ( P_MANAGEMENT_VALUE IS NOT NULL AND SM.MANAGEMENT_VALUE  = P_MANAGEMENT_VALUE)))
                   OR (NVL(P_LOOKUP_YN, 'N') = 'N'
                   AND (P_MANAGEMENT_DESC IS NULL AND 1 = 1)
                     OR (P_MANAGEMENT_DESC IS NOT NULL AND SM.MANAGEMENT_VALUE  LIKE P_MANAGEMENT_DESC || '%')))
              )))
      ORDER BY SL.GL_DATE, SL.GL_NUM, SL.SLIP_LINE_SEQ
      ;
