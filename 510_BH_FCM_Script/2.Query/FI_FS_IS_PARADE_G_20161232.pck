...
FUNCTION INVENTORY_DATA_F(
    ...
    , W_FS_SET_ID           IN  VARCHAR2 DEFAULT NULL   --보고서기준세트아이디
    , W_FS_ITEM_CODE        IN  VARCHAR2 DEFAULT NULL 
) RETURN NUMBER;
    ...
    IF    W_DATA_GB = 'M' THEN
        ...
        FOR REPEAT_INVENTORY IN 01..t_TERM_END_M LOOP
            ...
            IF t_REPEAT = '01' THEN               
                ...
                --기말제품재고액(8)
                t_TERM_END_LINE_AMT_0 := INVENTORY_DATA_F('A', W_SOB_ID, W_ORG_ID, W_CLOSING_YEAR || '-' || t_REPEAT, '302', W_FS_SET_ID, '8');
            ...
            ELSIF t_REPEAT = '12' THEN
                ...
                IF t_TERM_END_LINE_AMT_0 = 99999999999999 THEN
                    ...
                    SELECT
                        ...
                        WHERE ITEM_CODE = '4300000')  --관세환급금(4300000)
                        +
                        (SELECT sum(LAST_LEVEL_AMT_12) as LAST_LEVEL_AMT_12
                        FROM FI_FS_IS_PARADE
                        WHERE ITEM_CODE in('5301702', '5301000'))  --재고자산평가손실(5301000)                        
                    INTO t_TERM_END_AMT
                    FROM DUAL;                     
                ELSE
                    t_TERM_END_AMT := t_TERM_END_LINE_AMT_0;
...
BEGIN
    ...
    ELSIF W_DATA_GB = 'A' THEN
      ...
      IF W_FORM_ITEM_TYPE_CD = '302' THEN
        t_TEMP := 0;
        -- 제품폐기손실 적용.
        FOR A1 IN (
              SELECT D.ACCOUNT_CONTROL_ID                   
                FROM FI_FORM_MST T
                   , FI_FORM_DET D
              WHERE T.SOB_ID            = D.SOB_ID
                AND T.ORG_ID            = D.ORG_ID
                AND T.FS_SET_ID         = D.FS_SET_ID
                AND T.FORM_TYPE_ID      = D.FORM_TYPE_ID
                AND T.ITEM_CODE         = D.ITEM_CODE
                AND T.SOB_ID            = W_SOB_ID                 --회사아이디
                AND T.ORG_ID            = W_ORG_ID               --사업부아이디
                AND T.FS_SET_ID         = W_FS_SET_ID
                --AND T.FORM_TYPE_ID      = t_FORM_TYPE_ID   --보고서양식ID(공통코드)
                AND T.ITEM_CODE         = W_FS_ITEM_CODE
                AND D.ITEM_SIGN_SHOW    = '-'
                AND D.ENABLED_FLAG      = 'Y' 
                AND T.ENABLED_FLAG      = 'Y'
        ) LOOP
            BEGIN
              SELECT NVL(t_TEMP,0) + 
                     NVL(SUM(NVL(SL.GL_AMOUNT,0) * DECODE(SL.ACCOUNT_DR_CR, AC.ACCOUNT_DR_CR, 1, -1)), 0) AS AMT
                INTO t_TEMP
                FROM FI_SLIP_LINE       SL 
                   , FI_ACCOUNT_CONTROL AC 
                WHERE SL.ACCOUNT_CONTROL_ID   = AC.ACCOUNT_CONTROL_ID
                  AND SL.SOB_ID               = W_SOB_ID
                  AND SL.ORG_ID               = W_ORG_ID
                        
                    --전표분개자료에서 자료 추출 시 전표유형이 '기초잔액이월-BLS'과 '결산대체분개-CRJ'는 제외하고 구한다.
                  AND SLIP_TYPE               NOT IN ('BLS', 'CRJ') 
                  AND GL_DATE                 BETWEEN TRUNC(TO_DATE(W_PERIOD, 'YYYY-MM'), 'MONTH') 
                                                  AND LAST_DAY(TO_DATE(W_PERIOD, 'YYYY-MM'))
                  AND SL.ACCOUNT_CONTROL_ID   = A1.ACCOUNT_CONTROL_ID 
              GROUP BY SL.ACCOUNT_CODE
              ;
            EXCEPTION WHEN OTHERS THEN
              NULL;
            END; 
        END LOOP A1;    
        t_DATA := NVL(t_DATA,0) - NVL(t_TEMP,0);
      END IF;
    END IF;
