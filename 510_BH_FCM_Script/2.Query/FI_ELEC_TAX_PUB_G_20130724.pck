...
t_TOTAL_DED_TAX NUMBER := 0;  -- 누적 공제금액 ;
...
BEGIN
    ...
    SELECT
        ...
        , NVL(SUM(CNT), 0) * 200 AS DED_TAX_AMT -- 공제금액 
       ...
       WHERE TP.SOB_ID        = W_SOB_ID
         ...
         AND EXISTS
               ( SELECT 'X'
                  ...
                  WHERE RM.SOB_ID         = TP.SOB_ID
                    AND RM.ORG_ID         = TP.ORG_ID
                    AND RM.TAX_CODE       = TP.TAX_CODE
                    AND RM.VAT_MNG_SERIAL = TP.VAT_MNG_SERIAL
                    AND RM.FY             IN ( SELECT VRM.FY
    ...
    EXCEPTION WHEN OTHERS THEN
      t_TOTAL_DED_TAX := 0;
    END;
    IF t_TOTAL_DED_TAX > 1000000 THEN
      t_TOTAL_DED_TAX := NVL(t_TOTAL_DED_TAX, 0) - 1000000;
      
      UPDATE FI_ELEC_TAX_PUB TP
        SET TP.DEDUCT_TAX = CASE
                              WHEN NVL(TP.DEDUCT_TAX, 0) - NVL(t_TOTAL_DED_TAX, 0) < 0 THEN 0
                              ELSE NVL(TP.DEDUCT_TAX, 0) - NVL(t_TOTAL_DED_TAX, 0)
                            END 
       WHERE TP.SOB_ID    = W_SOB_ID
         AND TP.ORG_ID    = W_ORG_ID
         AND TP.TAX_CODE  = W_TAX_CODE
...
BEGIN
    ...
    SELECT
        ...
        , /*CASE 
          ...
          END*/ NVL(DEDUCT_TAX, 0) AS MIN_AMT    --해당 공제세액
