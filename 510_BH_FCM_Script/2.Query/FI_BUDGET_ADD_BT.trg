CREATE OR REPLACE TRIGGER FI_BUDGET_ADD_BT
BEFORE INSERT OR UPDATE OR DELETE ON FI_BUDGET_ADD
FOR EACH ROW
...
BEGIN
  IF INSERTING THEN
    ...
    -- INSERT/승인 상태가 확정승인시 예산수립 반영.
      FI_BUDGET_G.SAVE_BUDGET_CONFIRM
        ( W_GB => 'I'
        , W_BUDGET_TYPE => :NEW.BUDGET_TYPE
        , W_BUDGET_PERIOD => :NEW.BUDGET_PERIOD
        , W_DEPT_ID => :NEW.DEPT_ID
  ...
  ELSIF UPDATING THEN
    ...
    -- DELETE/기존 상태가 확정승인시 예산수립에서 감소 반영.
      FI_BUDGET_G.SAVE_BUDGET_CONFIRM
        ( W_GB => 'D'
        , W_BUDGET_TYPE => :OLD.BUDGET_TYPE
        , W_BUDGET_PERIOD => :OLD.BUDGET_PERIOD
        , W_DEPT_ID => :OLD.DEPT_ID
        , W_ACCOUNT_CONTROL_ID => :OLD.ACCOUNT_CONTROL_ID
        , W_SOB_ID => :OLD.SOB_ID
