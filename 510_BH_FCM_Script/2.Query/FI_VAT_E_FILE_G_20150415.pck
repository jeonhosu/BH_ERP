...
BEGIN
    ...
    WHERE   SOB_ID  = W_SOB_ID  --회사아이디
        ...
        AND TAX_CODE            = W_TAX_CODE   --사업장아이디
        AND VAT_MNG_SERIAL      = W_VAT_MNG_SERIAL         --부가세신고기간구분번호
        AND VAT_MAKE_GB         = NVL(W_VAT_MAKE_GB, '01') --신고구분
    ;
...
IF t_E_FILE_SURTAX_YN = 'Y' THEN          --전자신고파일생성대상여부_부가세신고서
    ...
    SELECT
        ...
        || LPAD(REPLACE(LPAD(NVL(COL2_1, 0), 15, 0), '-', ''), 13, '-')      --5.매출과세매입자발행세금계산서금액-  (1) NUMBER      15
        ...
        || LPAD(REPLACE(LPAD(NVL(COL8_2, 0), 15, 0), '-', ''), 15, '-')      --23.대손세액가감세액          (8)      NUMBER  13
        ...
        || RPAD(NVL(DEAL_BRANCH, ' '), 30, ' ')                                       --78.은행지점명              CHAR  30  Null 허용
...
-- 총괄납부사업자(주) 사업자일 경우만 생성 -- 
    FOR REC_2_10 IN ( SELECT SC.SOB_ID
                       ...
                       WHERE  SC.TAX_CODE             = VRM.TAX_CODE
                          ...
                          AND SC.VAT_MAKE_GB          = NVL(W_VAT_MAKE_GB, '01') --신고구분
...
BEGIN
    ...
    SELECT
        ...
        || RPAD(W_CODE, 2, ' ')           --3.수입금액종류구분	CHAR	1   Not Null	[1,2,4,7,8,E]
