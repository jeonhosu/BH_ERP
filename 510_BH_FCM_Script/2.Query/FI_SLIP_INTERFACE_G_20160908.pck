...
-- 전표 인터페이스 헤더 삽입.
  PROCEDURE INSERT_HEADER_IF
            ...
            , O_GL_DATE             OUT FI_SLIP_HEADER_INTERFACE.GL_DATE%TYPE 
...
-- 전표 라인 조회 : 분개 정보.
  PROCEDURE SELECT_SLIP_JOURNALIZE
            ( P_CURSOR2             OUT TYPES.TCURSOR2
            , P_SLIP_DATE_FR        IN DATE
            , P_SLIP_DATE_TO        IN DATE
...
-- 전표 인터페이스 헤더 삽입.
  ...
  BEGIN
    ...
    --기표일자 = 전표일자 -- 
    O_GL_DATE := P_SLIP_DATE;
    IF P_SLIP_TYPE = 'TSE' THEN       
      O_GL_NUM  := P_SLIP_NUM; 
    ELSE
      -- 문서 타입.  
      BEGIN
      ...
      END;
      -- 기표번호 생성.
      O_GL_NUM  := FI_DOCUMENT_NUM_G.DOCUMENT_NUM_F(V_DOCUMENT_TYPE, P_SOB_ID, P_SLIP_DATE, P_USER_ID); 
    END IF;
...
-- 전표 라인 조회 : 분개 정보.
  ...
  BEGIN
    OPEN P_CURSOR2 FOR
      SELECT CASE
               WHEN GROUPING(SL.SLIP_NUM) = 1 THEN TO_CHAR(NULL)
               WHEN GROUPING(SL.ACCOUNT_CODE) = 1 THEN TO_CHAR(NULL)
               ELSE TO_CHAR(SL.SLIP_DATE, 'YYYY-MM-DD')
             END AS SLIP_DATE
           , SL.ACCOUNT_CODE     
           , CASE
               WHEN GROUPING(SL.SLIP_NUM) = 1 THEN EAPP_MESSAGE_G.RETURN_TEXT_F(USERENV_G.GET_TERRITORY_S_F, 'EAPP_10045', NULL)
             ...
             END AS ACCOUNT_DESC
           , SLIP_IF_ITEM_VALUE_F(SL.LINE_INTERFACE_ID, 'CUSTOMER', SL.SOB_ID) AS SUPP_CUST_CODE
           , FI_COMMON_G.SUPP_CUST_CODE_NAME_F(SLIP_IF_ITEM_VALUE_F(SL.LINE_INTERFACE_ID, 'CUSTOMER', SL.SOB_ID), SL.SOB_ID) AS SUPP_CUST_NAME
           ...
           , NVL(SL.HEADER_INTERFACE_ID, NULL) AS HEADER_INTERFACE_ID
        FROM FI_SLIP_LINE_INTERFACE             SL
          , FI_SLIP_HEADER_INTERFACE            SH
          , FI_ACCOUNT_CONTROL                  AC
      WHERE SL.HEADER_INTERFACE_ID              = SH.HEADER_INTERFACE_ID
        AND SL.ACCOUNT_CONTROL_ID               = AC.ACCOUNT_CONTROL_ID
        AND SL.SLIP_DATE                        BETWEEN P_SLIP_DATE_FR AND P_SLIP_DATE_TO
        ...
        AND SL.HEADER_INTERFACE_ID              = NVL(P_HEADER_INTERFACE_ID, SL.HEADER_INTERFACE_ID)
         ...
         AND EXISTS ( SELECT 'X'
                      ...
                      WHERE PM.DEPT_ID        = DM.HR_DEPT_ID
                        ...
                        AND EXISTS ( SELECT 'X'
                                       FROM HRM_PERSON_MASTER PM1
                                     WHERE PM1.PERSON_ID      = PM.PERSON_ID
                                       AND PM1.PERSON_ID      = P_CONNECT_PERSON_ID
                                       AND PM1.SOB_ID         = P_SOB_ID
                                   )
                     )
      GROUP BY ROLLUP((SL.SLIP_DATE
           , SL.SLIP_NUM)
           , (SL.SLIP_LINE_SEQ
           ...
           , SH.GL_DATE
           , SH.GL_NUM
           , SLIP_IF_ITEM_VALUE_F(SL.LINE_INTERFACE_ID, 'CUSTOMER', SL.SOB_ID)
           , FI_COMMON_G.SUPP_CUST_CODE_NAME_F(SLIP_IF_ITEM_VALUE_F(SL.LINE_INTERFACE_ID, 'CUSTOMER', SL.SOB_ID), SL.SOB_ID)
           , HRM_PERSON_MASTER_G.NAME_F(SL.PERSON_ID)
           , SL.HEADER_INTERFACE_ID))
      ORDER BY SL.SLIP_DATE, SL.SLIP_NUM, SL.SLIP_LINE_SEQ
      ;
  END SELECT_SLIP_JOURNALIZE;
...
-- 선택한 전표 인터페이스 승인 조회.
  ...
  BEGIN
    OPEN P_CURSOR2 FOR
      SELECT SH.HEADER_INTERFACE_ID
           , DECODE(SH.CONFIRM_YN, 'Y', SH.GL_DATE, SH.OLD_GL_DATE) AS GL_DATE 
           , DECODE(SH.CONFIRM_YN, 'Y', SH.GL_NUM, TO_CHAR(NULL)) AS GL_NUM 
           , SH.CONFIRM_YN
           , DECODE(SH.CONFIRM_YN, 'Y', HRM_PERSON_MASTER_G.NAME_F(SH.CONFIRM_PERSON_ID), TO_CHAR(NULL)) AS CONFIRM_PERSON_NAME
        FROM FI_SLIP_HEADER_INTERFACE SH
...
-- 선택한 전표 인터페이스 승인/승인 취소 처리.
  ...
  AS
    ...
    V_GL_DATE                        DATE;  -- 기처리된 GL일자 : 일자가 다를경우만 번호 채번 -- 
    V_DR_AMOUNT                      NUMBER := -1;
    V_CR_AMOUNT                      NUMBER := 0;
  BEGIN
    -- 중복 승인 체크.
    IF P_CONFIRM_YN = 'Y' AND 'Y' = SLIP_CONFIRM_YN_F(W_HEADER_INTERFACE_ID) THEN
      RAISE_APPLICATION_ERROR(-20001, '1785.' || EAPP_MESSAGE_G.RETURN_TEXT_F(USERENV_G.GET_TERRITORY_S_F, 'FCM_10115', NULL));
      RETURN;
    END IF;
    
    IF GL_FISCAL_PERIOD_G.PERIOD_STATUS_F(NULL, P_GL_DATE, W_SOB_ID, W_ORG_ID) IN('C', 'N') THEN
      RAISE_APPLICATION_ERROR(ERRNUMS.Data_Not_opened_Code, '1790.' || TO_CHAR(P_GL_DATE, 'YYYY-MM-DD') || '.' || ERRNUMS.Data_Not_Opened_Desc);
      RETURN;
    END IF;
    ...
    BEGIN
      SELECT NVL(SUM(DECODE(SLI.ACCOUNT_DR_CR, '1', SLI.GL_AMOUNT, 0)), 0) AS DR_AMOUNT
        ...
        FROM FI_SLIP_HEADER_INTERFACE SHI
           , FI_SLIP_LINE_INTERFACE   SLI
      WHERE SHI.HEADER_INTERFACE_ID   = SLI.HEADER_INTERFACE_ID 
        AND SHI.HEADER_INTERFACE_ID   = W_HEADER_INTERFACE_ID
        AND SHI.SOB_ID                = W_SOB_ID
      GROUP BY SHI.SLIP_NUM
      ;
    EXCEPTION WHEN OTHERS THEN
      NULL;
    END;
    ...
    IF P_CONFIRM_YN = 'Y' THEN
      -- 전표 승인 -- 
      BEGIN
        SELECT NVL(SHI.GL_DATE, SHI.OLD_GL_DATE) AS GL_DATE 
             , NVL(SHI.GL_NUM, SHI.OLD_GL_NUM) AS GL_NUM 
          INTO V_GL_DATE
             , O_GL_NUM
          FROM FI_SLIP_HEADER_INTERFACE SHI
      ...
      EXCEPTION
        WHEN OTHERS THEN
          V_GL_DATE := TO_DATE(NULL);
          O_GL_NUM := TO_CHAR(NULL);
      END;
      IF V_GL_DATE IS NULL OR V_GL_DATE != P_GL_DATE OR O_GL_NUM IS NULL THEN
        V_GL_DATE := P_GL_DATE;
    ...
    BEGIN
      UPDATE FI_SLIP_HEADER_INTERFACE SHI
        SET SHI.GL_DATE           = DECODE(P_CONFIRM_YN, 'Y', P_GL_DATE, V_GL_DATE)
          , SHI.GL_NUM            = O_GL_NUM
          , SHI.CONFIRM_YN        = P_CONFIRM_YN
          , SHI.CONFIRM_DATE      = V_CONFIRM_DATE
          , SHI.CONFIRM_PERSON_ID = P_CONFIRM_PERSON_ID 
          , SHI.OLD_GL_DATE       = DECODE(P_CONFIRM_YN, 'Y', P_GL_DATE, SHI.OLD_GL_DATE)
          , SHI.OLD_GL_NUM        = DECODE(P_CONFIRM_YN, 'Y', O_GL_NUM, SHI.OLD_GL_NUM)
      WHERE SHI.HEADER_INTERFACE_ID = W_HEADER_INTERFACE_ID
        AND SHI.SOB_ID              = W_SOB_ID
      ;
    EXCEPTION
      WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20001, 'Confirm Update Error : ' || SQLERRM);
        RETURN;
    END;
  ...
  BEGIN
    BEGIN
      SELECT CASE W_LOOKUP_TYPE
               WHEN FI_ACCOUNT_CONTROL_G.CONTROL_ITEM_LOOKUP_TYPE_F(AC.ACCOUNT_CONTROL_ID, 'REFER1_ID', AC.SOB_ID) THEN SL.MANAGEMENT1
               WHEN FI_ACCOUNT_CONTROL_G.CONTROL_ITEM_LOOKUP_TYPE_F(AC.ACCOUNT_CONTROL_ID, 'REFER2_ID', AC.SOB_ID) THEN SL.MANAGEMENT2
               WHEN FI_ACCOUNT_CONTROL_G.CONTROL_ITEM_LOOKUP_TYPE_F(AC.ACCOUNT_CONTROL_ID, 'REFER3_ID', AC.SOB_ID) THEN SL.REFER1
               WHEN FI_ACCOUNT_CONTROL_G.CONTROL_ITEM_LOOKUP_TYPE_F(AC.ACCOUNT_CONTROL_ID, 'REFER4_ID', AC.SOB_ID) THEN SL.REFER2
               WHEN FI_ACCOUNT_CONTROL_G.CONTROL_ITEM_LOOKUP_TYPE_F(AC.ACCOUNT_CONTROL_ID, 'REFER5_ID', AC.SOB_ID) THEN SL.REFER3
               WHEN FI_ACCOUNT_CONTROL_G.CONTROL_ITEM_LOOKUP_TYPE_F(AC.ACCOUNT_CONTROL_ID, 'REFER6_ID', AC.SOB_ID) THEN SL.REFER4
               WHEN FI_ACCOUNT_CONTROL_G.CONTROL_ITEM_LOOKUP_TYPE_F(AC.ACCOUNT_CONTROL_ID, 'REFER7_ID', AC.SOB_ID) THEN SL.REFER5
               WHEN FI_ACCOUNT_CONTROL_G.CONTROL_ITEM_LOOKUP_TYPE_F(AC.ACCOUNT_CONTROL_ID, 'REFER8_ID', AC.SOB_ID) THEN SL.REFER6
               WHEN FI_ACCOUNT_CONTROL_G.CONTROL_ITEM_LOOKUP_TYPE_F(AC.ACCOUNT_CONTROL_ID, 'REFER9_ID', AC.SOB_ID) THEN SL.REFER7
               WHEN FI_ACCOUNT_CONTROL_G.CONTROL_ITEM_LOOKUP_TYPE_F(AC.ACCOUNT_CONTROL_ID, 'REFER10_ID', AC.SOB_ID) THEN SL.REFER8
               WHEN FI_ACCOUNT_CONTROL_G.CONTROL_ITEM_LOOKUP_TYPE_F(AC.ACCOUNT_CONTROL_ID, 'REFER11_ID', AC.SOB_ID) THEN SL.REFER9               
               WHEN FI_ACCOUNT_CONTROL_G.CONTROL_ITEM_LOOKUP_TYPE_F(AC.ACCOUNT_CONTROL_ID, 'REFER12_ID', AC.SOB_ID) THEN SL.REFER10
               WHEN FI_ACCOUNT_CONTROL_G.CONTROL_ITEM_LOOKUP_TYPE_F(AC.ACCOUNT_CONTROL_ID, 'REFER13_ID', AC.SOB_ID) THEN SL.REFER11
               WHEN FI_ACCOUNT_CONTROL_G.CONTROL_ITEM_LOOKUP_TYPE_F(AC.ACCOUNT_CONTROL_ID, 'REFER14_ID', AC.SOB_ID) THEN SL.REFER12
               ELSE NULL
             END AS ITEM_VALUE
        INTO V_ITEM_VALUE
        FROM FI_SLIP_LINE_INTERFACE SL
