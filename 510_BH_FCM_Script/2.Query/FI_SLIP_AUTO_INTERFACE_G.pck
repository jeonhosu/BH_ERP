...
-- 전표 생성 데이터 삭제.
  PROCEDURE DELETE_SLIP_AUTO_INTERFACE_1
...
-- 전표 생성 데이터 생성.
  ...
  BEGIN
    ...
    IF NVL(V_DR_AMOUNT, 0) <> NVL(V_CR_AMOUNT, 0) THEN
      V_DR_AMOUNT := ABS(NVL(V_DR_AMOUNT, 0) - NVL(V_CR_AMOUNT, 0));      
      O_MESSAGE := EAPP_MESSAGE_G.RETURN_TEXT_F(USERENV_G.GET_TERRITORY_S_F, 'FCM_10249', '&&AMOUNT:=' || TO_CHAR(V_DR_AMOUNT, 'FM999,999,999,999,999'));
      RETURN;
    END IF;
    /*
    BEGIN
...
-- 전표 생성 데이터 삭제.
  ...
  BEGIN
    ...
    END;
    -- 전표 삭제.
    DELETE_SLIP_AUTO_INTERFACE(V_HEADER_ID, O_STATUS, O_MESSAGE);
    
  END DELETE_SLIP_AUTO_INTERFACE_1;
