...
BEGIN
  ...
  -- INSERT.
    ...
    -- 승인 전표시 전표 생성.
      ...
      BEGIN
        ...
        -- 승인자 실제 승인자로 업뎃.
        -- (전표 헤더 생성시 발의자 정보를 가지고 승인자로 적용하므로 헤더 생성후 승인자를 실제 승인자로 다시 업뎃해야 함).
        UPDATE FI_SLIP_HEADER SH
          SET SH.CONFIRM_PERSON_ID    = :NEW.CONFIRM_PERSON_ID
  ...
  ELSIF UPDATING THEN
  -- UPDATE.
    IF :OLD.CONFIRM_YN = 'N' AND :NEW.CONFIRM_YN = 'Y' THEN
    -- 승인 전표시 전표 생성.
      IF GL_FISCAL_PERIOD_G.PERIOD_STATUS_F(NULL, :NEW.GL_DATE, :NEW.SOB_ID, :NEW.ORG_ID) IN('C', 'N') THEN
        RAISE_APPLICATION_ERROR(ERRNUMS.Data_Not_opened_Code, ERRNUMS.Data_Not_Opened_Desc);
        RETURN;
    ...
    -- 승인취소시 전표 삭제 생성.
      IF GL_FISCAL_PERIOD_G.PERIOD_STATUS_F(NULL, :OLD.GL_DATE, :OLD.SOB_ID, :OLD.ORG_ID) IN('C', 'N') THEN
        RAISE_APPLICATION_ERROR(ERRNUMS.Data_Not_opened_Code, ERRNUMS.Data_Not_Opened_Desc);
        RETURN;
      END IF;
