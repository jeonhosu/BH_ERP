...
/******************************************************************************/
 ...
 PROCEDURE AP_SUPPLIER_INSERT ( P_SUPPLIER_ID              OUT NUMBER
                              ,	P_SOB_ID                   IN  NUMBER
                              ,	P_ORG_ID                   IN  NUMBER
                              ,	P_SUPPLIER_CODE            OUT AP_SUPPLIER.SUPPLIER_CODE%TYPE
                              ...
                              , P_USER_ID                  IN  NUMBER
                              , P_TAX_BILL_TYPE_LCODE      IN  VARCHAR2
                              );
...
-- 은행계좌 수정.
 ...
 -------------------------------------------------------------------
 -- TAX_REG_NO Duplication check  : 해당 공급사는 제외하고 체크   --
 -------------------------------------------------------------------
 PROCEDURE TAX_REG_NO_DUP_P   ( P_SOB_ID          IN   NUMBER
                              , P_ORG_ID          IN   NUMBER
                              , P_SUPPLIER_ID     IN   AP_SUPPLIER.SUPPLIER_ID%TYPE
                              , P_TAX_REG_NO      IN   AP_SUPPLIER.TAX_REG_NO%TYPE
                              , X_CHECK_RESULT    OUT  VARCHAR2);
...
/******************************************************************************/
 ...
 BEGIN
       ...
       SELECT APS.SUPPLIER_ID
            ...
            ,	APS.LAST_UPDATED_BY
            , APS.TAX_BILL_TYPE_LCODE
            , EAPP_COMMON_G.GET_LOOKUP_DESC(APS.SOB_ID, APS.ORG_ID, 'TAX_BILL_TYPE', APS.TAX_BILL_TYPE_LCODE) AS TAX_BILL_TYPE_DESC
 ...
 BEGIN
       AR_CUSTOMER_SITE_G.FIND_MAX_CODE(P_SOB_ID, P_ORG_ID, 'S', P_SUPPLIER_CODE);
...
PROCEDURE AP_SUPPLIER_UPDATE (  W_SUPPLIER_ID              IN  NUMBER
 ...
 BEGIN
     OPEN P_CURSOR FOR
       ...
       WHERE BA.VENDOR_TYPE            = 'SUPPLIER'
         AND BA.VENDOR_ID              = W_SUPPLIER_ID
       ;
...
-- 은행계좌 삽입.
  ...
  BEGIN
    ...
    IF V_RECORD_COUNT > 0 THEN
      RAISE_APPLICATION_ERROR(-20001, 'Supplier Bank Account Number : ' || ERRNUMS.Exist_Data_Desc);  
    END IF;
    
    -- 계좌코드 생성. - 거래처코드-XX
    BEGIN
      SELECT ASP.SUPPLIER_CODE
        INTO V_VENDOR_CODE
        FROM AP_SUPPLIER ASP
       WHERE ASP.SUPPLIER_ID      = P_VENDOR_ID
      ;
    EXCEPTION WHEN OTHERS THEN
      RAISE_APPLICATION_ERROR(-20001, 'Insert error : Supplier Code not found. Check Supplier...');
    END;
    ...
    BEGIN
      SELECT COUNT(*)
       ...
       WHERE APS.SOB_ID        = P_SOB_ID
         AND APS.ORG_ID        = P_ORG_ID
         AND APS.TAX_REG_NO    = P_TAX_REG_NO
         AND ((P_SUPPLIER_ID   IS NULL AND 1 = 1)
         OR   (APS.SUPPLIER_ID != P_SUPPLIER_ID))         
         ;
    EXCEPTION WHEN OTHERS THEN
        V_CHK_COUNT := 0;
    END;
