...
-- 선택한 전표 인터페이스 승인 조회.
  ...
  BEGIN
    OPEN P_CURSOR2 FOR
      SELECT SH.HEADER_INTERFACE_ID
           , DECODE(SH.CONFIRM_YN, 'Y', SH.GL_DATE, TO_DATE(NULL)) AS GL_DATE 
...
-- 선택한 전표 인터페이스 승인/승인 취소 처리.
  ...
  BEGIN
    ...
    IF P_CONFIRM_YN = 'Y' THEN
      -- 전표 승인 -- 
      BEGIN
        SELECT SHI.GL_DATE AS GL_DATE 
             , SHI.GL_NUM  AS GL_NUM 
          INTO V_GL_DATE
             , O_GL_NUM
          FROM FI_SLIP_HEADER_INTERFACE SHI
    ...
    BEGIN
      UPDATE FI_SLIP_HEADER_INTERFACE SHI
        SET SHI.GL_DATE           = DECODE(P_CONFIRM_YN, 'Y', P_GL_DATE, SHI.GL_DATE)
          , SHI.GL_NUM            = O_GL_NUM
