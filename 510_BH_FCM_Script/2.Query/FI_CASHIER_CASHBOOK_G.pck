CREATE OR REPLACE PACKAGE FI_CASHIER_CASHBOOK_G
AS

-- 현금출납장 조회.
  PROCEDURE SELECT_CASHBOOK
...
END FI_CASHIER_CASHBOOK_G; 
/
CREATE OR REPLACE PACKAGE BODY FI_CASHIER_CASHBOOK_G
AS
/******************************************************************************/
/* Project      : FPCB ERP
/* Module       : HR
/* Program Name : FI_CASHIER_CASHBOOK_G
/* Description  : 현금출납장 조회.
/*
/* Reference by :
/* Program History :
/*------------------------------------------------------------------------------
        ...
        FROM (SELECT ROWNUM AS ROW_NUM
                   ...
                   , SUM
                     (CASE
                      ...
                      END) OVER ( ORDER BY ROWNUM, SX1.PERIOD_NAME, GL_DATE, SLIP_HEADER_ID, SX1.REMARK) AS REMAIN_AMOUNT
                FROM (
                      -- 이월 잔액 산출.
                        ...
                        FROM (SELECT '1' AS ACCOUNT_DR_CR
                              ...
                              SELECT AC.ACCOUNT_DR_CR
                               ...
                               WHERE SL.ACCOUNT_CONTROL_ID    = AC.ACCOUNT_CONTROL_ID
                                 ...
                                 AND SL.GL_DATE               BETWEEN TRUNC(W_GL_DATE_FR, 'YEAR') AND W_GL_DATE_FR - 1
