...
---------------------------------------------------------------------------------------------------
-- 원재료 기초 금액.
  FUNCTION MAT_BEGIN_AMOUNT
            ( W_PERIOD_NAME       IN FI_CLOSING_AMOUNT.PERIOD_NAME%TYPE
            , W_SOB_ID            IN FI_CLOSING_AMOUNT.SOB_ID%TYPE
            ) RETURN NUMBER;
            
-- 원재료 기말 금액.
  FUNCTION MAT_ENDING_AMOUNT
            ( W_PERIOD_NAME       IN FI_CLOSING_AMOUNT.PERIOD_NAME%TYPE
            , W_SOB_ID            IN FI_CLOSING_AMOUNT.SOB_ID%TYPE
            ) RETURN NUMBER;

-- 부재료 기초 금액.
  FUNCTION PAT_BEGIN_AMOUNT
            ( W_PERIOD_NAME       IN FI_CLOSING_AMOUNT.PERIOD_NAME%TYPE
            , W_SOB_ID            IN FI_CLOSING_AMOUNT.SOB_ID%TYPE
            ) RETURN NUMBER;
            
-- 부재료 기말 금액.
  FUNCTION PAT_ENDING_AMOUNT
            ( W_PERIOD_NAME       IN FI_CLOSING_AMOUNT.PERIOD_NAME%TYPE
            , W_SOB_ID            IN FI_CLOSING_AMOUNT.SOB_ID%TYPE
            ) RETURN NUMBER;
            
-- 재공품 기초 금액.
  FUNCTION WIP_BEGIN_AMOUNT
            ( W_PERIOD_NAME       IN FI_CLOSING_AMOUNT.PERIOD_NAME%TYPE
            , W_SOB_ID            IN FI_CLOSING_AMOUNT.SOB_ID%TYPE
            ) RETURN NUMBER;
            
-- 재공품 기말 금액.
  FUNCTION WIP_ENDING_AMOUNT
            ( W_PERIOD_NAME       IN FI_CLOSING_AMOUNT.PERIOD_NAME%TYPE
            , W_SOB_ID            IN FI_CLOSING_AMOUNT.SOB_ID%TYPE
            ) RETURN NUMBER;
            
-- 제품 기초 금액.
  FUNCTION FG_BEGIN_AMOUNT
            ( W_PERIOD_NAME       IN FI_CLOSING_AMOUNT.PERIOD_NAME%TYPE
            , W_SOB_ID            IN FI_CLOSING_AMOUNT.SOB_ID%TYPE
            ) RETURN NUMBER;
            
-- 제품 기말 금액.
  FUNCTION FG_ENDING_AMOUNT
            ( W_PERIOD_NAME       IN FI_CLOSING_AMOUNT.PERIOD_NAME%TYPE
            , W_SOB_ID            IN FI_CLOSING_AMOUNT.SOB_ID%TYPE
            ) RETURN NUMBER;

-- 상품 기초 금액.
  FUNCTION SG_BEGIN_AMOUNT
            ( W_PERIOD_NAME       IN FI_CLOSING_AMOUNT.PERIOD_NAME%TYPE
            , W_SOB_ID            IN FI_CLOSING_AMOUNT.SOB_ID%TYPE
            ) RETURN NUMBER;
            
-- 상품 기말 금액.
  FUNCTION SG_ENDING_AMOUNT
            ( W_PERIOD_NAME       IN FI_CLOSING_AMOUNT.PERIOD_NAME%TYPE
            , W_SOB_ID            IN FI_CLOSING_AMOUNT.SOB_ID%TYPE
            ) RETURN NUMBER;
  ...
  BEGIN
    ...
    BEGIN
      ...
      WHERE CA.CLOSING_GROUP        = '101'
        ...
        AND CA.ACCOUNT_CONTROL_ID IN (SELECT MIN(CA.ACCOUNT_CONTROL_ID) AS ACCOUNT_CONTROL_ID
                                         FROM FI_CLOSING_ACCOUNT CA
                                      WHERE CA.CLOSING_GROUP        = '101'
                                        AND CA.CLOSING_ACCOUNT_TYPE = '20'
                                     )
    ...
    END;
    -- 1.1 원재료 기초금액.
    V_BEGIN_AMOUNT := MAT_BEGIN_AMOUNT(W_PERIOD_NAME, W_SOB_ID);
    ...
    BEGIN
      ...
      WHERE SL.SLIP_HEADER_ID           = SH.SLIP_HEADER_ID
        ...
        AND NOT EXISTS 
              ( SELECT 'X'
                  FROM FI_SLIP_HEADER FSH
                WHERE FSH.SLIP_HEADER_ID  = SH.SLIP_HEADER_ID
                  AND FSH.SOB_ID          = SH.SOB_ID
                  AND FSH.SOURCE_TABLE    = 'FI_CLOSING_SLIP'
                  AND FSH.CREATED_TYPE    = 'I'
              )
        AND EXISTS (SELECT 'X'
                      FROM FI_CLOSING_ACCOUNT CA
    ...
    -- 1.4 원재료 기말 금액.
    V_ENDING_AMOUNT := MAT_ENDING_AMOUNT(W_PERIOD_NAME, W_SOB_ID);
    ...
    END;
    -- 2.1 부재료 기초금액.
    V_BEGIN_AMOUNT := PAT_BEGIN_AMOUNT(W_PERIOD_NAME, W_SOB_ID);
    ...
    -- 2.4 부재료 기말 금액.
    V_ENDING_AMOUNT := PAT_ENDING_AMOUNT(W_PERIOD_NAME, W_SOB_ID);
    ...
    EXCEPTION
      WHEN OTHERS THEN        
        DBMS_OUTPUT.PUT_LINE('SG ITEM : Account Code Find Error');
        V_ACCOUNT_CODE := '1120101';
        BEGIN
          SELECT AC.ACCOUNT_CONTROL_ID
            INTO V_ACCOUNT_CONTROL_ID
            FROM FI_ACCOUNT_CONTROL AC
    ...
    END;
    -- 4.1 제품 기초금액.
    V_BEGIN_AMOUNT := SG_BEGIN_AMOUNT(W_PERIOD_NAME, W_SOB_ID);
    ...
    -- 4.4 상품 기말 금액.
    V_ENDING_AMOUNT := SG_ENDING_AMOUNT(W_PERIOD_NAME, W_SOB_ID);
    ...
    END;
    -- 5.1 재공품 기초금액.
    V_BEGIN_AMOUNT := WIP_BEGIN_AMOUNT(W_PERIOD_NAME, W_SOB_ID);
      ...
      WHERE CA.PERIOD_NAME              = W_PERIOD_NAME
        ...
        AND EXISTS (SELECT 'X'
                    ...
                    WHERE FC.GROUP_CODE   = 'CLOSING_GROUP'
                      ...
                      AND FC.CODE         IN('101', '201', '301')    -- 원재료(101), 부재료(201), 제조경비(301)
                   )
      ;
    EXCEPTION 
      WHEN OTHERS THEN
    ...
    -- 5.4 재공품 기말 금액.
    V_ENDING_AMOUNT := WIP_ENDING_AMOUNT(W_PERIOD_NAME, W_SOB_ID);
    ...
    END;
    -- 6.1 제품 기초금액.
    V_BEGIN_AMOUNT := FG_BEGIN_AMOUNT(W_PERIOD_NAME, W_SOB_ID);
    ...
    -- 6.4 제품 기말 금액.
    V_ENDING_AMOUNT := FG_ENDING_AMOUNT(W_PERIOD_NAME, W_SOB_ID);
  ...
  BEGIN
    ...
    LOOP
      ...
      FI_SLIP_G.INSERT_SLIP_HEADER
            ...
            , NULL
            , NULL
            , EAPP_MESSAGE_G.RETURN_TEXT_F(USERENV_G.GET_TERRITORY_S_F, 'FCM_10251', NULL)
            , P_USER_ID
