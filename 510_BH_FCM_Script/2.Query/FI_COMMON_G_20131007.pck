...
-----------------------------------------------------------------------------------------
-- 발의부서별 결재라인 조회.
  PROCEDURE SELECT_DOC_APPROVAL_LINE
            ( P_CURSOR1          OUT TYPES.TCURSOR1
            , W_DEPT_ID          IN FI_COMMON.CODE%TYPE
            , W_SOB_ID           IN FI_COMMON.SOB_ID%TYPE
            , W_ORG_ID           IN FI_COMMON.ORG_ID%TYPE
            );
...
--회계모듈에서 이용하는 거래처로 [LU_SUPP_CUST]에다 국적 항목을 추가한 것임.            
PROCEDURE LU_SUPP_CUST_MOD( 
      P_CURSOR3           OUT TYPES.TCURSOR3
    , W_SOB_ID            IN NUMBER
    , W_SUPP_CUST_TYPE    IN VARCHAR2
    , W_ENABLED_YN        IN VARCHAR2 DEFAULT 'Y'
...
--------------------------------------------
  PROCEDURE LU_CURRENCY
            ...
            , W_SOB_ID            IN NUMBER
            , W_ORG_ID            IN NUMBER
            , W_EXCEPT_BASE_YN    IN VARCHAR2 DEFAULT 'N'
            , W_ENABLED_YN        IN VARCHAR2 DEFAULT 'Y'
            );
...
-- 발의부서별 결재라인 조회.
  ...
  BEGIN
    OPEN P_CURSOR1 FOR
      SELECT DECODE(SX1.DEPT_CODE, NULL, FC.CODE, SX1.DEPT_CODE) AS DEPT_CODE
          , DECODE(SX1.DEPT_CODE, NULL, FC.CODE_NAME, SX1.DEPT_NAME) AS DEPT_NAME
          , DECODE(SX1.DEPT_CODE, NULL, FC.VALUE1, SX1.APPROVAL_1) AS APPROVAL_1
          , DECODE(SX1.DEPT_CODE, NULL, FC.VALUE2, SX1.APPROVAL_2) AS APPROVAL_2
          , DECODE(SX1.DEPT_CODE, NULL, FC.VALUE3, SX1.APPROVAL_3) AS APPROVAL_3
          , DECODE(SX1.DEPT_CODE, NULL, FC.VALUE4, SX1.APPROVAL_4) AS APPROVAL_4
          , DECODE(SX1.DEPT_CODE, NULL, FC.VALUE5, SX1.APPROVAL_5) AS APPROVAL_5
          , DECODE(SX1.DEPT_CODE, NULL, FC.VALUE6, SX1.APPROVAL_6) AS APPROVAL_6
          , DECODE(SX1.DEPT_CODE, NULL, FC.VALUE7, SX1.APPROVAL_7) AS APPROVAL_7
          , DECODE(SX1.DEPT_CODE, NULL, FC.VALUE8, SX1.APPROVAL_8) AS APPROVAL_8
          , DECODE(SX1.DEPT_CODE, NULL, FC.VALUE9, SX1.APPROVAL_9) AS APPROVAL_9
          , DECODE(SX1.DEPT_CODE, NULL, FC.VALUE10, SX1.APPROVAL_10) AS APPROVAL_10
        FROM FI_COMMON FC LEFT OUTER JOIN
            ( SELECT 'A' AS COMMON_ID
                  , FC.CODE AS DEPT_CODE
                  , FC.CODE_NAME AS DEPT_NAME
                  , FC.VALUE1 AS APPROVAL_1
                  , FC.VALUE2 AS APPROVAL_2
                  , FC.VALUE3 AS APPROVAL_3
                  , FC.VALUE4 AS APPROVAL_4
                  , FC.VALUE5 AS APPROVAL_5
                  , FC.VALUE6 AS APPROVAL_6
                  , FC.VALUE7 AS APPROVAL_7
                  , FC.VALUE8 AS APPROVAL_8
                  , FC.VALUE9 AS APPROVAL_9
                  , FC.VALUE10 AS APPROVAL_10
                FROM FI_COMMON FC
                  , FI_DEPT_MASTER DM
              WHERE FC.CODE                   = DM.DEPT_CODE
                AND FC.SOB_ID                 = DM.SOB_ID
                AND FC.GROUP_CODE             = 'DOC_APPROVAL_LINE'
                AND DM.DEPT_ID                = W_DEPT_ID
                AND FC.SOB_ID                 = W_SOB_ID
                AND FC.ORG_ID                 = W_ORG_ID
                AND FC.ENABLED_FLAG           = 'Y'
                AND FC.EFFECTIVE_DATE_FR      <= TRUNC(SYSDATE)
                AND (FC.EFFECTIVE_DATE_TO IS NULL OR FC.EFFECTIVE_DATE_TO >= TRUNC(SYSDATE, 'MONTH'))
            ) SX1 ON 'A' = SX1.COMMON_ID
      WHERE FC.GROUP_CODE             = 'DOC_APPROVAL_LINE'
        AND FC.CODE                   = '000000'
        AND FC.SOB_ID                 = W_SOB_ID
        AND FC.ORG_ID                 = W_ORG_ID
        AND FC.ENABLED_FLAG           = 'Y'
...
--------------------------------------------
  ...
  BEGIN
    BEGIN
      SELECT EO.ORG_ID
        INTO V_ORG_ID
        FROM EAPP_ORGANIZATIONS EO
       WHERE EO.SOB_ID        = W_SOB_ID
         AND EO.DEFAULT_FLAG  = 'Y'
         AND ROWNUM <= 1
      ;
    EXCEPTION WHEN OTHERS THEN
      V_ORG_ID := 0;
...
BEGIN
    ...
    SELECT SCV.SUPP_CUST_NAME
       ...
       , SCV.SUPP_CUST_CODE
       , SCV.SUPP_CUST_ID
       , SCV.COUNTRY_CODE --국가코드
       , B.COUNTRY_SHORT_NAME   --국적
    FROM FI_SUPP_CUST_V SCV
        , EAPP_COUNTRY B
    WHERE SCV.SOB_ID                 = W_SOB_ID
        AND (SCV.SUPP_CUST_TYPE        = NVL(W_SUPP_CUST_TYPE, SCV.SUPP_CUST_TYPE)
        ...
        AND TRUNC(SCV.EFFECTIVE_DATE_FR) <= NVL(V_STD_DATE, TRUNC(SCV.EFFECTIVE_DATE_FR))
        AND (TRUNC(SCV.EFFECTIVE_DATE_TO) IS NULL OR TRUNC(SCV.EFFECTIVE_DATE_TO) >= NVL(V_STD_DATE, TRUNC(SCV.EFFECTIVE_DATE_TO)))
        AND SCV.COUNTRY_CODE = B.COUNTRY_CODE(+)
    ;

END LU_SUPP_CUST_MOD;
  ...
  BEGIN
    ...
    OPEN P_CURSOR FOR
      ...
      WHERE EC.SOB_ID             = W_SOB_ID
        ...
        AND EC.CURRENCY_CODE      NOT IN DECODE(W_EXCEPT_BASE_YN, 'Y', V_BASE_CURRENCY, '-')
...
-- 외화 금액을 입력 통화로 환산.
  ...
  BEGIN
    BEGIN
      ...
      ;
    EXCEPTION WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('Currency Set error : ' || SUBSTR(SQLERRM, 1, 150));
      V_ROUND_TYPE := 'ROUND';
      V_DECIMAL_POINT := 0;
    END;
