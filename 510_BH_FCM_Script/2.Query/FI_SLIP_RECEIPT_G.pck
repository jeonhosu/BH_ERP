CREATE OR REPLACE PACKAGE FI_SLIP_RECEIPT_G
AS
...
END FI_SLIP_RECEIPT_G;
/
CREATE OR REPLACE PACKAGE BODY FI_SLIP_RECEIPT_G
AS
/******************************************************************************/
/* Project      : FPCB ERP
/* Module       : HR
/* Program Name : FI_SLIP_RECEIPT_G
/* Description  : (입금/수금) 전표 헤더/라인 정보.
/*
/* Reference by :
/* Program History :
/*------------------------------------------------------------------------------
    ...
    OPEN P_CURSOR1 FOR
      SELECT SL.SLIP_LINE_ID
       ...
       WHERE SL.CUSTOMER_ID             = SCV.SUPP_CUST_ID(+)
         ...
         AND AC.ACCOUNT_CONTROL_ID      = ACI.ACCOUNT_CONTROL_ID(+)
         AND AC.ACCOUNT_DR_CR           = ACI.ACCOUNT_DR_CR(+)
