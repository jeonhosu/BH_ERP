CREATE OR REPLACE PACKAGE FI_DPR_EXPENSE_CG_G
AS
...
Project      : BH FLEX erp
Module       : Financial
Package Name : FI_DPR_EXPENSE_CG_G
Description  : 감가상각비현황 Package

Reference by : calling assmbly-program id(호출 프로그램) : (감가상각비현황)
Program History :
    참조>FI_DPR_EXPENSE_CG(감가상각비현황) 테이블은 감가상각비현황 자료를 추출하기 위해 임시적으로 사용하는 테이블이다.
...
PROCEDURE CREATE_DPR_EXPENSE(
      W_SOB_ID          IN  FI_DPR_EXPENSE_CG.SOB_ID%TYPE          --회사아이디
    , W_ORG_ID          IN  FI_DPR_EXPENSE_CG.ORG_ID%TYPE          --사업부아이디
    , W_EXPENSE_TYPE    IN  FI_DPR_EXPENSE_CG.EXPENSE_TYPE%TYPE    --경비구분
...
PROCEDURE LIST_DPR_EXPENSE_DET( 
    ...
    , W_SOB_ID              IN  FI_DPR_EXPENSE_CG.SOB_ID%TYPE              --회사아이디
    , W_ORG_ID              IN  FI_DPR_EXPENSE_CG.ORG_ID%TYPE              --사업부아이디
    , W_ASSET_CATEGORY_ID   IN  FI_DPR_EXPENSE_CG.ASSET_CATEGORY_ID%TYPE   --자산유형아이디
    , W_EXPENSE_TYPE        IN  FI_DPR_EXPENSE_CG.EXPENSE_TYPE%TYPE        --경비구분
    , W_WPERIOD_FR          IN  VARCHAR2    --상각기간_시작(예>2011-01)
    , W_WPERIOD_TO          IN  VARCHAR2    --상각기간_종료(예>2011-12)  
    , W_TAX_CODE            IN  VARCHAR2    --공장구분. 
...
END FI_DPR_EXPENSE_CG_G;
/
CREATE OR REPLACE PACKAGE BODY FI_DPR_EXPENSE_CG_G
AS
...
t_ASSET_ID      FI_DPR_EXPENSE_CG.ASSET_ID%TYPE;       --자산아이디
t_AMT_SIGN      VARCHAR2(2);
t_THIS_DPR_AMT  FI_DPR_EXPENSE_CG.THIS_DPR_AMT%TYPE;   --당기감가상각비

BEGIN

    --기존 자료를 삭제한다.
    DELETE FI_DPR_EXPENSE_CG;
    
    --기초 자료를 생성한다.
    INSERT INTO FI_DPR_EXPENSE_CG(
          SOB_ID	        --회사아이디
        , ORG_ID	        --사업부아이디
...
BEGIN
    ...
    FROM
        (
            SELECT
                  DE.ASSET_CATEGORY_ID	            --자산유형아이디
                , SUM(DE.START_AMT) AS	START_AMT   --기초가액
                , SUM(DE.THIS_INC) AS THIS_INC     --당기증가액
                , SUM(DE.THIS_DEC) AS THIS_DEC	    --당기감소액
                , SUM(DE.END_AMT) AS END_AMT	    --기말잔액
                , SUM(DE.PRE_ALLOWANCES_ACCU) AS PRE_ALLOWANCES_ACCU   --전기충당금누계액
                , SUM(DE.THIS_DPR_AMT) AS THIS_DPR_AMT	                --당기감가상각비
                , SUM(DE.PRE_ALLOWANCES_DEC) AS PRE_ALLOWANCES_DEC	    --전기충당금감소액
                , SUM(DE.DPR_ACCUMULATE_AMT) AS DPR_ACCUMULATE_AMT	    --감가상각누계액
                , SUM(DE.BOOK_VALUE) AS BOOK_VALUE	                    --미상각잔액 
            FROM FI_DPR_EXPENSE_CG DE
            WHERE DE.SOB_ID = W_SOB_ID     --회사아이디
                AND DE.ORG_ID = W_ORG_ID   --사업부아이디
                AND ((W_TAX_CODE        IS NULL 
                  AND 1                 = 1)
                OR   (W_TAX_CODE        IS NOT NULL 
                  AND EXISTS
                        ( SELECT 'X'
                            FROM FI_ASSET_MASTER_CG AM
                           WHERE AM.ASSET_ID      = DE.ASSET_ID
                             AND AM.TAX_CODE      = W_TAX_CODE
                        )))
