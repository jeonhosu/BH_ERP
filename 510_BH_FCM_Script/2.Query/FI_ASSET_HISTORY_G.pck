...
-- 자산변동 내역 전체.
  PROCEDURE ASSET_HISTORY_ALL_SELECT
            ...
            , W_DATE_FR         IN DATE
            , W_DATE_TO         IN DATE
            , W_AST_CATEGORY_ID IN FI_ASSET_MASTER.AST_CATEGORY_ID%TYPE
  ...
  BEGIN
    OPEN P_CURSOR FOR
      SELECT ACV.AST_CATEGORY_NAME
           ...
           , AH.QTY
           , AH.USE_DEPT_ID
           , HRM_COMMON_G.ID_NAME_F(AH.USE_DEPT_ID) AS DEPT_NAME
           , AH.LOCATION_ID
           ...
           , AH.BF_QTY
           , AH.BF_USE_DEPT_ID
           , HRM_COMMON_G.ID_NAME_F(AH.BF_USE_DEPT_ID) AS BF_DEPT_NAME
           , AH.BF_LOCATION_ID
        ...
        FROM FI_ASSET_HISTORY AH
          , FI_ASSET_MASTER AM
          , FI_ASSET_CATEGORY_V1 ACV
      WHERE AH.ASSET_ID               = AM.ASSET_ID
        AND AH.SOB_ID                 = AM.SOB_ID
        AND AM.AST_CATEGORY_ID        = ACV.AST_ITEM_ID
        AND AM.SOB_ID                 = ACV.SOB_ID
        ...
        AND EXISTS ( SELECT 'X'
                       FROM FI_ASSET_CATEGORY_V1 ACV
                     WHERE ACV.AST_ITEM_ID      = AM.AST_CATEGORY_ID
                       AND ACV.SOB_ID           = AM.SOB_ID
                       AND ACV.AST_CATEGORY_ID  = NVL(W_AST_CATEGORY_ID, ACV.AST_CATEGORY_ID)
                   )
  ...
  BEGIN
    OPEN P_CURSOR FOR
      SELECT AH.HISTORY_NUM
           ...
           , AH.FIRST_USER AS FIRST_USER_NAME
           , AH.SECOND_USER AS SECOND_USER_NAME
           , AH.COST_CENTER_ID
           , FI_COMMON_G.COST_CENTER_DESC_F(AH.COST_CENTER_ID) AS COST_CENTER_NAME
           , AH.BF_CURR_AMOUNT
           , AH.BF_AMOUNT
           ...
           , AH.BF_FIRST_USER AS BF_FIRST_USER_NAME
           , AH.BF_SECOND_USER AS BF_SECOND_USER_NAME
           , AH.BF_COST_CENTER_ID
           , FI_COMMON_G.COST_CENTER_DESC_F(AH.BF_COST_CENTER_ID) AS BF_COST_CENTER_NAME
           , AH.DESCRIPTION
           , AH.ASSET_ID
