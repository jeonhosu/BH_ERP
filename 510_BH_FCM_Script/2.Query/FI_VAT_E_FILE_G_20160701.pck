...
IF t_E_FILE_SURTAX_YN = 'Y' THEN          --전자신고파일생성대상여부_부가세신고서
    ...
    SELECT
        ...
        || RPAD(CASE
                  ...
                  ELSE '0'
                END, 1, '0')                                                          --84.조기환급취소구분 
        || LPAD(NVL(P_EXPORT_IMPORT_POSTPONE_VAT,0), 15, '0')                         --85. 수출기업수입납부유.
        || RPAD(' ', 28, ' ')                                                         --86.공란  CHAR    47
    INTO g_REPORT_CONTENT
    FROM FI_SURTAX_CARD SC
...
IF t_E_FILE_REAL_ESTATE_YN = 'Y' THEN   --전자신고파일생성대상여부_부동산임대공급가액명세서
    ...
    FOR REC_2_7 IN (
        SELECT
            ...
            || CASE
                 WHEN LEND_AREA - TRUNC(LEND_AREA) > 0 THEN LPAD(TO_CHAR(LEND_AREA,'FM9999999.00'),10, '0')
                 ELSE LPAD(LEND_AREA, 10, '0')
               END                                      --8.면적              CHAR  10
...
IF t_E_FILE_DOMESTIC_LC_YN = 'Y' THEN   --전자신고파일생성대상여부_내국신용장구매확인서전자발급명세서
    ...
    FOR REC_2_27 IN (
        ...
        SELECT
            ...
            || LPAD(REPLACE(LPAD(NVL(SUPPLY_AMT, 0), 15, '0'), '-', ''), 15, '-') --8.금액  NUMBER  15
...
-- 203.07.15 전호수 추가 --
  ...
  FOR C1 IN ( SELECT RPAD('17', 2, ' ') || -- 자료구분 
                     ...
                     LPAD(NVL(ZRD.T_105_01_01_1_1, 0), 15, '0') || -- 군부대공급석유류
                     LPAD(NVL(ZRD.T_105_01_05_1_1, 0), 15, '0') || -- 어민에게공급하는 어업용기자재
                     RPAD(' ', 21, ' ') AS RECORD_FILE
...
IF t_E_FILE_SUM_VAT_YN = 'Y' THEN       --전자시고파일생성대상여부_세금계산서합계표
    ...
    FOR REC_4_1 IN (
        ...
        SELECT
            ...
            || LPAD(
                    CASE
                        WHEN NVL(GL_AMOUNT,0) >= 0 THEN TO_CHAR(NVL(GL_AMOUNT,0))
                        ELSE SUBSTRB(REPLACE(NVL(GL_AMOUNT,0), '-', ''), 1, LENGTH(NVL(GL_AMOUNT,0)) - 2) ||
                            CASE SUBSTR(NVL(GL_AMOUNT,0), -1, 1)
                                WHEN '1' THEN 'J'
                                WHEN '2' THEN 'K'
                                WHEN '3' THEN 'L'
                                WHEN '4' THEN 'M'
            ...
            || LPAD(
                    CASE
                        WHEN NVL(VAT_AMOUNT,0) >= 0 THEN TO_CHAR(NVL(VAT_AMOUNT,0))
                        ELSE SUBSTRB(REPLACE(NVL(VAT_AMOUNT,0), '-', ''), 1, LENGTH(NVL(VAT_AMOUNT,0)) - 2) ||
                            CASE SUBSTR(NVL(VAT_AMOUNT,0), -1, 1)
                                WHEN '1' THEN 'J'
                                WHEN '2' THEN 'K'
                                WHEN '3' THEN 'L'
                                WHEN '4' THEN 'M'
    ...
    SELECT
        ...
        --(합계분)    
        || LPAD(NVL(TOT_CNT,0), 7, 0)  --3.거래처수      NUMBER  7
        || LPAD(NVL(TOT_RECORD,0), 7, 0)   --4.세금계산서매수  NUMBER  7
        || LPAD(
                CASE
                    WHEN NVL(TOT_GL_AMOUNT,0) >= 0 THEN TO_CHAR(NVL(TOT_GL_AMOUNT,0))
                    ELSE SUBSTRB(REPLACE(NVL(TOT_GL_AMOUNT,0), '-', ''), 1, LENGTH(NVL(TOT_GL_AMOUNT,0)) - 2) ||
                        CASE SUBSTR(NVL(TOT_GL_AMOUNT,0), -1, 1)
                            WHEN '1' THEN 'J'
                            WHEN '2' THEN 'K'
                            WHEN '3' THEN 'L'
                            WHEN '4' THEN 'M'
        ...
        || LPAD(
                CASE
                    WHEN NVL(TOT_VAT_AMOUNT,0) >= 0 THEN TO_CHAR(NVL(TOT_VAT_AMOUNT,0))
                    ELSE SUBSTRB(REPLACE(NVL(TOT_VAT_AMOUNT,0), '-', ''), 1, LENGTH(NVL(TOT_VAT_AMOUNT,0)) - 2) ||
                        CASE SUBSTR(NVL(TOT_VAT_AMOUNT,0), -1, 1)
                            WHEN '1' THEN 'J'
                            WHEN '2' THEN 'K'
                            WHEN '3' THEN 'L'
                            WHEN '4' THEN 'M'
        ...
        --(사업자번호발행분)        
        || LPAD(NVL(C_CNT,0), 7, 0)  --7.거래처수      NUMBER  7
        || LPAD(NVL(C_RECORD,0), 7, 0)   --8.세금계산서매수  NUMBER  7
        || LPAD(
                CASE
                    WHEN NVL(C_GL_AMOUNT,0) >= 0 THEN TO_CHAR(NVL(C_GL_AMOUNT,0))
                    ELSE SUBSTRB(REPLACE(NVL(C_GL_AMOUNT,0), '-', ''), 1, LENGTH(NVL(C_GL_AMOUNT,0)) - 2) ||
                        CASE SUBSTR(NVL(C_GL_AMOUNT,0), -1, 1)
                            WHEN '1' THEN 'J'
                            WHEN '2' THEN 'K'
                            WHEN '3' THEN 'L'
                            WHEN '4' THEN 'M'
        ...
        || LPAD(
                CASE
                    WHEN NVL(C_VAT_AMOUNT,0) >= 0 THEN TO_CHAR(NVL(C_VAT_AMOUNT,0))
                    ELSE SUBSTRB(REPLACE(NVL(C_VAT_AMOUNT,0), '-', ''), 1, LENGTH(NVL(C_VAT_AMOUNT,0)) - 2) ||
                        CASE SUBSTR(NVL(C_VAT_AMOUNT,0), -1, 1)
                            WHEN '1' THEN 'J'
                            WHEN '2' THEN 'K'
                            WHEN '3' THEN 'L'
                            WHEN '4' THEN 'M'
        ...
        --(주민번호발행분)  
        || LPAD(NVL(P_CNT,0), 7, 0)  --11.거래처수      NUMBER  7
        || LPAD(NVL(P_RECORD,0), 7, 0)   --12.세금계산서매수  NUMBER  7
        || LPAD(
                CASE
                    WHEN NVL(P_GL_AMOUNT,0) >= 0 THEN TO_CHAR(NVL(P_GL_AMOUNT,0))
                    ELSE SUBSTRB(REPLACE(NVL(P_GL_AMOUNT,0), '-', ''), 1, LENGTH(NVL(P_GL_AMOUNT,0)) - 2) ||
                        CASE SUBSTR(NVL(P_GL_AMOUNT,0), -1, 1)
                            WHEN '1' THEN 'J'
                            WHEN '2' THEN 'K'
                            WHEN '3' THEN 'L'
                            WHEN '4' THEN 'M'
                        ...
                        END
                END
              , 15, '0') --13.공급가액  NUMBER  15
        || LPAD(
                CASE
                    WHEN NVL(P_VAT_AMOUNT,0) >= 0 THEN TO_CHAR(NVL(P_VAT_AMOUNT,0))
                    ELSE SUBSTRB(REPLACE(NVL(P_VAT_AMOUNT,0), '-', ''), 1, LENGTH(NVL(P_VAT_AMOUNT,0)) - 2) ||
                        CASE SUBSTR(NVL(P_VAT_AMOUNT,0), -1, 1)
                            WHEN '1' THEN 'J'
                            WHEN '2' THEN 'K'
                            WHEN '3' THEN 'L'
                            WHEN '4' THEN 'M'
    ...
    FROM
        (
            SELECT
                ...
                , NVL(SUM(GL_AMOUNT), 0) AS TOT_GL_AMOUNT       --공급가액
                , NVL(SUM(VAT_AMOUNT), 0) AS TOT_VAT_AMOUNT     --세액                
    ...
    SELECT
        ...
        --(합계분)    
        || LPAD(TOT_CNT, 7, 0)  --3.거래처수      NUMBER  7
        || LPAD(TOT_RECORD, 7, 0)   --4.세금계산서매수  NUMBER  7
        || LPAD(
                CASE
                    WHEN TOT_GL_AMOUNT >= 0 THEN TO_CHAR(TOT_GL_AMOUNT)
                    ELSE SUBSTRB(REPLACE(TOT_GL_AMOUNT, '-', ''), 1, LENGTH(TOT_GL_AMOUNT) - 2) ||
                        CASE SUBSTR(TOT_GL_AMOUNT, -1, 1)
                            WHEN '1' THEN 'J'
                            WHEN '2' THEN 'K'
                            WHEN '3' THEN 'L'
                            WHEN '4' THEN 'M'
        ...
        || LPAD(
                CASE
                    WHEN TOT_VAT_AMOUNT >= 0 THEN TO_CHAR(TOT_VAT_AMOUNT)
                    ELSE SUBSTRB(REPLACE(TOT_VAT_AMOUNT, '-', ''), 1, LENGTH(TOT_VAT_AMOUNT) - 2) ||
                        CASE SUBSTR(TOT_VAT_AMOUNT, -1, 1)
                            WHEN '1' THEN 'J'
                            WHEN '2' THEN 'K'
                            WHEN '3' THEN 'L'
                            WHEN '4' THEN 'M'
        ...
        --(사업자번호발행분)        
        || LPAD(C_CNT, 7, 0)  --7.거래처수      NUMBER  7
        || LPAD(C_RECORD, 7, 0)   --8.세금계산서매수  NUMBER  7
        || LPAD(
                CASE
                    WHEN C_GL_AMOUNT >= 0 THEN TO_CHAR(C_GL_AMOUNT)
                    ELSE SUBSTRB(REPLACE(C_GL_AMOUNT, '-', ''), 1, LENGTH(C_GL_AMOUNT) - 2) ||
                        CASE SUBSTR(C_GL_AMOUNT, -1, 1)
                            WHEN '1' THEN 'J'
                            WHEN '2' THEN 'K'
                            WHEN '3' THEN 'L'
                            WHEN '4' THEN 'M'
        ...
        || LPAD(
                CASE
                    WHEN C_VAT_AMOUNT >= 0 THEN TO_CHAR(C_VAT_AMOUNT)
                    ELSE SUBSTRB(REPLACE(C_VAT_AMOUNT, '-', ''), 1, LENGTH(C_VAT_AMOUNT) - 2) ||
                        CASE SUBSTR(C_VAT_AMOUNT, -1, 1)
                            WHEN '1' THEN 'J'
                            WHEN '2' THEN 'K'
                            WHEN '3' THEN 'L'
                            WHEN '4' THEN 'M'
        ...
        --(주민번호발행분)    
        || LPAD(P_CNT, 7, 0)  --11.거래처수      NUMBER  7
        || LPAD(P_RECORD, 7, 0)   --12.세금계산서매수  NUMBER  7
        || LPAD(
                CASE
                    WHEN P_GL_AMOUNT >= 0 THEN TO_CHAR(P_GL_AMOUNT)
                    ELSE SUBSTRB(REPLACE(P_GL_AMOUNT, '-', ''), 1, LENGTH(P_GL_AMOUNT) - 2) ||
                        CASE SUBSTR(P_GL_AMOUNT, -1, 1)
                            WHEN '1' THEN 'J'
                            WHEN '2' THEN 'K'
                            WHEN '3' THEN 'L'
                            WHEN '4' THEN 'M'
        ...
        || LPAD(
                CASE
                    WHEN P_VAT_AMOUNT >= 0 THEN TO_CHAR(P_VAT_AMOUNT)
                    ELSE SUBSTRB(REPLACE(P_VAT_AMOUNT, '-', ''), 1, LENGTH(P_VAT_AMOUNT) - 2) ||
                        CASE SUBSTR(P_VAT_AMOUNT, -1, 1)
                            WHEN '1' THEN 'J'
                            WHEN '2' THEN 'K'
                            WHEN '3' THEN 'L'
                            WHEN '4' THEN 'M'
    ...
    FROM
        (
            SELECT
                ...
                -- 사업자번호 
                , NVL(COUNT(DECODE(BUSINESS_TYPE_S, 'P', NULL, SUPPLIER_CODE)),0) AS C_CNT   --매입처수
                , NVL(SUM(DECODE(BUSINESS_TYPE_S, 'P', 0, CNT)),0) AS C_RECORD                --매수
                , NVL(SUM(DECODE(BUSINESS_TYPE_S, 'P', 0, GL_AMOUNT)),0) AS C_GL_AMOUNT       --공급가액
                , NVL(SUM(DECODE(BUSINESS_TYPE_S, 'P', 0, VAT_AMOUNT)),0) AS C_VAT_AMOUNT     --세액
                
                -- 주민번호 -- 
                , NVL(COUNT(DECODE(BUSINESS_TYPE_S, 'P', SUPPLIER_CODE, NULL)),0) AS P_CNT   --매입처수
                , NVL(SUM(DECODE(BUSINESS_TYPE_S, 'P', CNT, 0)),0) AS P_RECORD                --매수
                , NVL(SUM(DECODE(BUSINESS_TYPE_S, 'P', GL_AMOUNT, 0)),0) AS P_GL_AMOUNT       --공급가액
                , NVL(SUM(DECODE(BUSINESS_TYPE_S, 'P', VAT_AMOUNT, 0)),0) AS P_VAT_AMOUNT     --세액   
            FROM
                (
                  SELECT             
                          A.MANAGEMENT1 AS SUPPLIER_CODE    --거래처코드
    ...
    FROM
        (
            SELECT
                  NVL(COUNT(SUPPLIER_CODE),0) AS TOT_CNT   --매입처수
                , NVL(SUM(CNT),0) AS TOT_RECORD                --매수
                , NVL(SUM(GL_AMOUNT),0) AS TOT_GL_AMOUNT       --공급가액
                , NVL(SUM(VAT_AMOUNT),0) AS TOT_VAT_AMOUNT     --세액       
