...
/******************************************************************************/
  ...
  BEGIN
    ...
    -- 1. 신규월 시작일자.
    V_N_FIRST_DATE := TRUNC(TO_DATE(V_N_PERIOD_NAME, 'YYYY-MM'));
    ...
    BEGIN
      ...
      ;
    EXCEPTION WHEN OTHERS THEN
      V_MESSAGE := '1.Daily Customer Balance Delete : ' || SUBSTR(SQLERRM, 1, 200);
      RAISE E_DELETE_ERR;
    END;
    BEGIN
      ...
      ;
    EXCEPTION WHEN OTHERS THEN
      V_MESSAGE := '1.Daily Customer Balance Insert : ' || SUBSTR(SQLERRM, 1, 200);
      RAISE E_INSERT_ERR;
    END;
    -- 금액 0 인것 삭제.
    BEGIN
      ...
      ;
    EXCEPTION WHEN OTHERS THEN
      V_MESSAGE := '2.Daily Customer Balance Delete : ' || SUBSTR(SQLERRM, 1, 200);
      RAISE E_DELETE_ERR;
    END;
    ---------------------------------------------------------------------------
