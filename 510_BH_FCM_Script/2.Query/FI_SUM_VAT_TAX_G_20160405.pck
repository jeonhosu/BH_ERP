...
BEGIN
    ...
    SELECT
        ...
        , SUBSTR(C_GL_AMOUNT, -AMT_LEN, AMT_LEN - 12) AS COM_GL_1      --공급가액_조단위
        , CASE
            WHEN AMT_LEN > 12 THEN SUBSTR(C_GL_AMOUNT, -12, 3)
            WHEN AMT_LEN > 9 THEN SUBSTR(C_GL_AMOUNT, -AMT_LEN, AMT_LEN - 9)
            ELSE ''
          END AS COM_GL_2      --공급가액_십억단위
        , CASE
            WHEN AMT_LEN > 9 THEN SUBSTR(C_GL_AMOUNT, -9, 3)
            WHEN AMT_LEN > 6 THEN SUBSTR(C_GL_AMOUNT, -AMT_LEN, AMT_LEN - 6)
            ELSE ''
          END AS COM_GL_3      --공급가액_백만단위
        , CASE
            WHEN AMT_LEN > 6 THEN SUBSTR(C_GL_AMOUNT, -6, 3)
            WHEN AMT_LEN > 3 THEN SUBSTR(C_GL_AMOUNT, -AMT_LEN, AMT_LEN - 3)
            ELSE ''
          END AS COM_GL_4      --공급가액_천단위
        , CASE
            WHEN AMT_LEN <= 3 THEN TO_CHAR(C_GL_AMOUNT)
            ELSE SUBSTR(C_GL_AMOUNT, -3, 3)
          END AS COM_GL_5      --공급가액_원단위
            
        , SUBSTR(C_VAT_AMOUNT, -VAT_LEN, VAT_LEN - 12) AS COM_VAT_1      --세액_조단위
        , CASE
            WHEN VAT_LEN > 12 THEN SUBSTR(C_VAT_AMOUNT, -12, 3)
            WHEN VAT_LEN > 9 THEN SUBSTR(C_VAT_AMOUNT, -VAT_LEN, VAT_LEN - 9)
            ELSE ''
          END AS COM_VAT_2      --세액_십억단위
        , CASE
            WHEN VAT_LEN > 9 THEN SUBSTR(C_VAT_AMOUNT, -9, 3)
            WHEN VAT_LEN > 6 THEN SUBSTR(C_VAT_AMOUNT, -VAT_LEN, VAT_LEN - 6)
            ELSE ''
          END AS COM_VAT_3      --세액_백만단위
        , CASE
            WHEN VAT_LEN > 6 THEN SUBSTR(C_VAT_AMOUNT, -6, 3)
            WHEN VAT_LEN > 3 THEN SUBSTR(C_VAT_AMOUNT, -VAT_LEN, VAT_LEN - 3)
            ELSE ''
          END AS COM_VAT_4      --세액_천단위
        , CASE
            WHEN VAT_LEN <= 3 THEN TO_CHAR(C_VAT_AMOUNT)
            ELSE SUBSTR(C_VAT_AMOUNT, -3, 3)
          END AS COM_VAT_5      --세액_원단위   
        ...
        , SUBSTR(P_GL_AMOUNT, -AMT_LEN, AMT_LEN - 12) AS SN_GL_1      --공급가액_조단위
        , CASE
            WHEN AMT_LEN > 12 THEN SUBSTR(P_GL_AMOUNT, -12, 3)
            WHEN AMT_LEN > 9 THEN SUBSTR(P_GL_AMOUNT, -AMT_LEN, AMT_LEN - 9)
            ELSE ''
          END AS SN_GL_2      --공급가액_십억단위
        , CASE
            WHEN AMT_LEN > 9 THEN SUBSTR(P_GL_AMOUNT, -9, 3)
            WHEN AMT_LEN > 6 THEN SUBSTR(P_GL_AMOUNT, -AMT_LEN, AMT_LEN - 6)
            ELSE ''
          END AS SN_GL_3      --공급가액_백만단위
        , CASE
            WHEN AMT_LEN > 6 THEN SUBSTR(P_GL_AMOUNT, -6, 3)
            WHEN AMT_LEN > 3 THEN SUBSTR(P_GL_AMOUNT, -AMT_LEN, AMT_LEN - 3)
            ELSE ''
          END AS SN_GL_4      --공급가액_천단위
        , CASE
            WHEN AMT_LEN <= 3 THEN TO_CHAR(P_GL_AMOUNT)
            ELSE SUBSTR(P_GL_AMOUNT, -3, 3)
          END AS SN_GL_5      --공급가액_원단위
            
        , SUBSTR(P_VAT_AMOUNT, -VAT_LEN, VAT_LEN - 12) AS SN_VAT_1      --세액_조단위
        , CASE
            WHEN VAT_LEN > 12 THEN SUBSTR(P_VAT_AMOUNT, -12, 3)
            WHEN VAT_LEN > 9 THEN SUBSTR(P_VAT_AMOUNT, -VAT_LEN, VAT_LEN - 9)
            ELSE ''
          END AS SN_VAT_2      --세액_십억단위
        , CASE
            WHEN VAT_LEN > 9 THEN SUBSTR(P_VAT_AMOUNT, -9, 3)
            WHEN VAT_LEN > 6 THEN SUBSTR(P_VAT_AMOUNT, -VAT_LEN, VAT_LEN - 6)
            ELSE ''
          END AS SN_VAT_3      --세액_백만단위
        , CASE
            WHEN VAT_LEN > 6 THEN SUBSTR(P_VAT_AMOUNT, -6, 3)
            WHEN VAT_LEN > 3 THEN SUBSTR(P_VAT_AMOUNT, -VAT_LEN, VAT_LEN - 3)
            ELSE ''
          END AS SN_VAT_4      --세액_천단위
        , CASE
            WHEN VAT_LEN <= 3 THEN TO_CHAR(P_VAT_AMOUNT)
            ELSE SUBSTR(P_VAT_AMOUNT, -3, 3)
          END AS SN_VAT_5      --세액_원단위   
        ...
        , SUBSTR(TOT_GL_AMOUNT, -AMT_LEN, AMT_LEN - 12) AS TOT_GL_1      --공급가액_조단위
        , CASE
            WHEN AMT_LEN > 12 THEN SUBSTR(TOT_GL_AMOUNT, -12, 3)
            WHEN AMT_LEN > 9 THEN SUBSTR(TOT_GL_AMOUNT, -AMT_LEN, AMT_LEN - 9)
            ELSE ''
          END AS TOT_GL_2      --공급가액_십억단위
        , CASE
            WHEN AMT_LEN > 9 THEN SUBSTR(TOT_GL_AMOUNT, -9, 3)
            WHEN AMT_LEN > 6 THEN SUBSTR(TOT_GL_AMOUNT, -AMT_LEN, AMT_LEN - 6)
            ELSE ''
          END AS TOT_GL_3      --공급가액_백만단위
        , CASE
            WHEN AMT_LEN > 6 THEN SUBSTR(TOT_GL_AMOUNT, -6, 3)
            WHEN AMT_LEN > 3 THEN SUBSTR(TOT_GL_AMOUNT, -AMT_LEN, AMT_LEN - 3)
            ELSE ''
          END AS TOT_GL_4      --공급가액_천단위
        , CASE
            WHEN AMT_LEN <= 3 THEN TO_CHAR(TOT_GL_AMOUNT)
            ELSE SUBSTR(TOT_GL_AMOUNT, -3, 3)
          END AS TOT_GL_5      --공급가액_원단위
            
        , SUBSTR(TOT_VAT_AMOUNT, -VAT_LEN, VAT_LEN - 12) AS TOT_VAT_1      --세액_조단위
        , CASE
            WHEN VAT_LEN > 12 THEN SUBSTR(TOT_VAT_AMOUNT, -12, 3)
            WHEN VAT_LEN > 9 THEN SUBSTR(TOT_VAT_AMOUNT, -VAT_LEN, VAT_LEN - 9)
            ELSE ''
          END AS TOT_VAT_2      --세액_십억단위
        , CASE
            WHEN VAT_LEN > 9 THEN SUBSTR(TOT_VAT_AMOUNT, -9, 3)
            WHEN VAT_LEN > 6 THEN SUBSTR(TOT_VAT_AMOUNT, -VAT_LEN, VAT_LEN - 6)
            ELSE ''
          END AS TOT_VAT_3      --세액_백만단위
        , CASE
            WHEN VAT_LEN > 6 THEN SUBSTR(TOT_VAT_AMOUNT, -6, 3)
            WHEN VAT_LEN > 3 THEN SUBSTR(TOT_VAT_AMOUNT, -VAT_LEN, VAT_LEN - 3)
            ELSE ''
          END AS TOT_VAT_4      --세액_천단위
        , CASE
            WHEN VAT_LEN <= 3 THEN TO_CHAR(TOT_VAT_AMOUNT)
            ELSE SUBSTR(TOT_VAT_AMOUNT, -3, 3)
          END AS TOT_VAT_5      --세액_원단위   
    FROM T  ; 
