...
SELECT BM.HEADER_ID
                     ...
                     , BM.HEADER_NAME
                     , BM.ITEM_TYPE
                     , TO_CHAR(&V_THIS_DATE_FR, 'YYYY-MM') AS PERIOD_FR
                     , TO_CHAR(&V_THIS_DATE_TO, 'YYYY-MM') AS PERIOD_TO
                     , BM.SOB_ID
                     , BM.ORG_ID
                  FROM FI_BALANCE_MS BM
                WHERE BM.SOB_ID     = &W_SOB_ID
...
select * FI_BALANCE_MS BB
where             ( SELECT NVL(BB.THIS_L_AMOUNT, 0) + SUM(BB1.THIS_L_AMOUNT * FL.ITEM_SIGN) AS THIS_L_AMOUNT
                FROM FI_BALANCE_MS BB1
                  , FI_FORM_LINE FL
              WHERE BB1.HEADER_ID         = FL.JOIN_FORM_HEADER_ID
                AND FL.FORM_HEADER_ID     = C1.FORM_HEADER_ID
