...
select *
 ...
 where x.asset_code    = 'B000000386'
 ;

select *
  from fi_asset_history_cg x
 where x.asset_id       = 1000
for update 
 ; 
 
SELECT *
  FROM FI_ASSET_DPR_HISTORY_cg X
 WHERE X.ASSET_ID           = 887
ORDER BY X.PERIOD_NAME 
--for update
;

-- 감가상각 자본적 지출금액 및 누계액 미상각잔액 UPDATE -- 
UPDATE FI_ASSET_DPR_HISTORY_CG ADH
   SET ADH.REMARK                 = NULL
 WHERE ADH.ASSET_ID               = 887
;
...
BEGIN
    ...
    IF P_DPR_METHOD_TYPE = '1' THEN --1:정액법
        ...
        LOOP
            ...
            -- 감가상각 반영 -- 
            MERGE INTO FI_ASSET_DPR_HISTORY_CG ADH
