...
-- 이월자료 조회.
SELECT * FROM FI_BALANCE_OVER_GT
;
...
SELECT DECODE(PX1.GL_DATE, &W_GL_DATE_FR - 1, TO_DATE(NULL), PX1.GL_DATE) AS GL_DATE
     ...
     , CASE
        ...
        -- WHEN GROUPING(PX1.PERIOD_NAME) = 1 THEN EAPP_MESSAGE_G.RETURN_TEXT_F(USERENV_G.GET_TERRITORY_S_F, 'EAPP_10050', NULL)                  
     ...
     , PX1.CURRENCY_CODE
     , SUM(PX1.DR_CURRENCY_AMOUNT) AS DR_CURRENCY_AMOUNT
     , SUM(PX1.CR_CURRENCY_AMOUNT) AS CR_CURRENCY_AMOUNT
     , PX1.SLIP_HEADER_ID
  FROM (
    SELECT SX1.ACCOUNT_CODE
         ...
         , SX1.MONTH_DR_AMOUNT AS MONTH_DR_AMOUNT
         , SX1.MONTH_CR_AMOUNT AS MONTH_CR_AMOUNT
         , SX1.SLIP_HEADER_ID
         , SX1.SLIP_LINE_ID
      FROM (
