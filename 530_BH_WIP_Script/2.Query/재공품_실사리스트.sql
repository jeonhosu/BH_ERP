-- 재공품실사리스트
SELECT SSW.WORKCENTER_DESCRIPTION -- 작업장
     ...
     , WOS.JOB_NO                 -- LOT NO
     , NVL((SELECT CASE WHEN WOOJ.OUT_ORDER_ID IS NOT NULL AND WORJ.RECEIPT_ID IS NULL THEN SSW.WORKCENTER_DESCRIPTION
                        WHEN WORJ.INSPECT_REQ_FLAG = 'Y' THEN '수입검사' END
             ...
             WHERE WOOJ.OUT_ORDER_ID = WORJ.OUT_ORDER_ID(+)
               AND WOOJ.OUT_ORDER_ID IN(SELECT MAX(WOOJ.OUT_ORDER_ID)
                                         ...
                                         WHERE WOOJ.JOB_ID                 = WOS.JOB_ID 
                                           AND WOOJ.START_OPERATION_SEQ_NO = WOS.OPERATION_SEQ_NO)
                                        ), '외주창고(발주대기)') AS STORE_LOCATION -- 보관처
