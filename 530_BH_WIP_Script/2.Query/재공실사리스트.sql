SELECT SSW.WORKCENTER_DESCRIPTION -- 작업장
     ...
     , WOS.JOB_NO                 -- LOT NO
     , NVL((SELECT CASE WHEN WORJ.RECEIPT_ID IS NULL THEN '외주창고'
         ...
         WHERE WOOJ.OUT_ORDER_ID = WORJ.OUT_ORDER_ID(+)
           AND WOOJ.OUT_ORDER_ID IN(SELECT MAX(WOOJ.OUT_ORDER_ID) 
                                     ...
                                     WHERE WOOJ.JOB_ID = WOS.JOB_ID 
                                       AND WOOJ.START_OPERATION_SEQ_NO = WOS.OPERATION_SEQ_NO)), SSW.WORKCENTER_DESCRIPTION) AS STORE_LOCATION -- 보관처
