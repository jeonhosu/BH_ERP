CREATE OR REPLACE VIEW INV_PHYSICAL_WC_MAT_LIST_V AS
SELECT IIO.SOB_ID
     , IIO.ORG_ID
     ...
     , IIO.UOM_CODE -- 단위
     , ROUND(SUM(CASE WHEN IWL.PRIMARY_FLAG = 'Y' THEN IIO.ONHAND_QTY ELSE 0 END), 2) AS PRIMARY_QTY -- 정상재고량
     , NULL AS ACTUAL_PRIMARY_QTY -- 실사량(정상)
     , ROUND(SUM(CASE WHEN IWL.OVER_STOCK_FLAG = 'Y' THEN IIO.ONHAND_QTY ELSE 0 END), 2) AS OVER_STOCK_QTY -- 과출재고량
     , NULL AS ACTUAL_OVER_QTY -- 실사량(과출고)
     , ROUND(SUM(IIO.ONHAND_QTY), 2) AS ONHAND_QTY -- 재고량
 ...
 GROUP BY IIO.SOB_ID
        ...
        , IIO.UOM_CODE
 HAVING ROUND(SUM(IIO.ONHAND_QTY), 2) <> 0;
