...
namespace PPMF0703
{
    public class XLPrinting
    {
        ...
        private int mCopy_EndCol = 83;
        private int mCopy_EndRow = 71;
        private int mPrintingLastRow = 68;  //최종 인쇄 라인.
        ...
        {// 그리드의 컬럼에 대한 컬럼인덱스 값 산출
            ...
            // 그리드 or 아답터 위치.
            pGDColumn[0] = pGrid.GetColumnToIndex("OP_TYPE_DESCRIPTION");
            pGDColumn[1] = pGrid.GetColumnToIndex("OPERATION_SEQ_NO");
            pGDColumn[2] = pGrid.GetColumnToIndex("OPERATION_DESCRIPTION");
            pGDColumn[3] = pGrid.GetColumnToIndex("DESCRIPTION");
            pGDColumn[4] = pGrid.GetColumnToIndex("ITEM_CODE");
            pGDColumn[5] = pGrid.GetColumnToIndex("ITEM_DESC");
            pGDColumn[6] = pGrid.GetColumnToIndex("WORKING_SIZE");
            pGDColumn[7] = pGrid.GetColumnToIndex("STORE_LOCATION");
            pGDColumn[8] = pGrid.GetColumnToIndex("JOB_NO");
            pGDColumn[9] = pGrid.GetColumnToIndex("ITEM_UOM_CODE");
            pGDColumn[10] = pGrid.GetColumnToIndex("TOTAL_ONHAND_QTY");
            pGDColumn[11] = pGrid.GetColumnToIndex("MTX_UOM_CODE");
            pGDColumn[12] = pGrid.GetColumnToIndex("FACTOR_VALUE1");
            pGDColumn[13] = pGrid.GetColumnToIndex("MTX_UOM_QTY1");
            pGDColumn[14] = pGrid.GetColumnToIndex("ACTUAL_ONHAND_QTY1");
            pGDColumn[15] = pGrid.GetColumnToIndex("FACTOR_VALUE2");
            pGDColumn[16] = pGrid.GetColumnToIndex("MTX_UOM_QTY2");
            pGDColumn[17] = pGrid.GetColumnToIndex("ACTUAL_ONHAND_QTY2");
        ...
        private void XLHeader1(object pWORKCENTER_DESC, object pOPERATION_DESC)
        {// 헤더 인쇄.
            ...
            { // 원본을 복사해서 타겟 에 복사해 넣음.
                ...
                vXLine = 5;
                vXLColumnIndex = 21;
                IsConvert = IsConvertString(pWORKCENTER_DESC, out vConvertString);
                if (IsConvert == true)
                {
                    vConvertString = string.Format("작업장 : {0}", vConvertString);
        ...
        {// 실제 호출되는 부분.
            ...
            string vOLD_WORKCENTER_CODE = string.Empty;
            string vWORKCENTER_CODE = string.Empty;
            ...
            //작어장 및 공정.
            int mIDX_WORKCENTER_CODE = pGrid.GetColumnToIndex("WORKCENTER_CODE");
            int mIDX_WORKCENTER = pGrid.GetColumnToIndex("WORKCENTER_DESCRIPTION");
            int mIDX_OPERATION = pGrid.GetColumnToIndex("OPERATION_DESCRIPTION");
            ...
            try
            {
                ...
                if (vTotalRow > 0)
                {
                    ...
                    for (int vRow = 0; vRow < vTotalRow; vRow++)
                    {
                        ...
                        vWORKCENTER_CODE = iString.ISNull(pGrid.GetCellValue(vRow, mIDX_WORKCENTER_CODE));
                        if (mPageNumber == 0)
                        {//첫장 인쇄.
                            #region ----- Header Write ----
                            // 헤더 인쇄.
                            HeaderWrite(pPRINTED_PERSON_NAME, pPRINTED_DATE, pPERIOD_NAME);
                            XLHeader1(pGrid.GetCellValue(vRow, mIDX_WORKCENTER), pGrid.GetCellValue(vRow, mIDX_OPERATION));
                        ...
                        else if (vOLD_WORKCENTER_CODE != vWORKCENTER_CODE)
                        {//작업장이 다를 경우.
                            XLHeader1(pGrid.GetCellValue(vRow, mIDX_WORKCENTER), pGrid.GetCellValue(vRow, mIDX_OPERATION));
                        ...
                        vOLD_WORKCENTER_CODE = iString.ISNull(pGrid.GetCellValue(vRow, mIDX_WORKCENTER_CODE));
                        //---------------------------------//
